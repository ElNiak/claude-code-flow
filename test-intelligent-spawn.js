...
async function simulateDetermineOptimalWorkerTypes(objective, showDetails = true) {
  if (showDetails) {
  ...
  try {
    ...
    console.warn('⚠️ Queen analysis failed, using enhanced defaults:', error.message);
...
async function runTests() {
  for (let i = 0; i < testScenarios.length; i++) {
    ...
    const result = await simulateDetermineOptimalWorkerTypes(scenario.objective, true);
    ...
    if (result.analysis.requiredCapabilities.length > 0) {
      reasoning.push(`Components: ${result.analysis.requiredCapabilities.join(', ')}`);
    }
    
    const objectiveLower = scenario.objective.toLowerCase();
    if (result.analysis.complexity === 'high' || result.analysis.complexity === 'very_high') {
    ...
    if (objectiveLower.includes('database') || objectiveLower.includes('analytics')) {
      reasoning.push('Data project: added analyst, architect');
    }
    if (objectiveLower.includes('enterprise') || objectiveLower.includes('security')) {
  ...
  console.log('   claude-flow hive-mind spawn "Build scalable microservices API"');
  console.log('   claude-flow hive-mind spawn "Create React frontend application"');
