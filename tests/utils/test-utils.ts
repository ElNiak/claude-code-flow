...
import { type Spy, stub } from "https://deno.land/std@0.220.0/testing/mock.ts";
...
export class AsyncTestUtils {
	...
	static async waitForAll(
		...
		await AsyncTestUtils.waitFor(async () => {
...
export class MemoryTestUtils {
	...
	static async checkMemoryLeak<T>(
		...
		const baselineMemory = MemoryTestUtils.getAverageMemoryUsage(samples);
		...
		await MemoryTestUtils.forceGC();
		const postMemory = MemoryTestUtils.getAverageMemoryUsage(samples);
		...
		const runBatch = async (batchSize: number) => {
			const promises = Array.from({ length: batchSize }, async () => {
				const { result, duration } =
					await PerformanceTestUtils.measureTime(operation);
				return { result, duration };
			});
...
export class FileSystemTestUtils {
	...
	static async createFixtures(
		...
		const fixtureDir =
			baseDir || (await FileSystemTestUtils.createTempDir("fixtures-"));
...
export class TestDataGenerator {
	...
	static randomArray<T>(generator: () => T, length?: number): T[] {
		const arrayLength = length || TestDataGenerator.randomNumber(1, 10);
	...
	static largeDataset(
		...
		return Array.from({ length: size }, (_, i) => ({
			id: `item-${i}`,
			name: TestDataGenerator.randomString(20),
