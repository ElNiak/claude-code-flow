	...
	return new Promise((resolve, reject) => {
		...
		const fileCheckInterval = setInterval(() => {
			try {
				...
				if (swarmFiles.length > filesCreated.length) {
					...
					console.log(
						`Files created so far: ${filesCreated.length} - ${filesCreated.join(", ")}`,
					);
				}
			} catch (error) {
		...
		claudeProcess.on("close", (code) => {
			...
			if (code === 0) {
				...
				console.warn(
					`Claude CLI failed with code ${code}. stdout: ${stdout.slice(-200)}, stderr: ${stderr.slice(-200)}`,
				);
...
`;
					fs.writeFileSync(
						path.join(mcpTestPath, diagnosticFile),
						diagnosticContent,
						"utf8",
					);
