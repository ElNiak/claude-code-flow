...
async function test() {
	...
	try {
		...
		// Group by type
		const sparcTools = tools.tools.filter(
			(t) =>
				t.name.startsWith("sparc_") &&
				!["sparc_list", "sparc_swarm", "sparc_swarm_status"].includes(t.name),
		);
		const metaTools = tools.tools.filter((t) =>
			["sparc_list", "sparc_swarm", "sparc_swarm_status"].includes(t.name),
		);
		...
		console.log(
			"Result preview:",
			listResult.content[0].text.substring(0, 200) + "...",
		);
