...
import {
	...
	SwarmMemoryManager,
	TaskExecutor,
} from "./dist/cli/commands/swarm-new.js";
...
async function testSwarmImplementation() {
	...
	test("SwarmCoordinator instantiation", () => {
		...
		return (
			coordinator && coordinator.id && coordinator.status === "initializing"
		);
	});
	...
	await testAsync("Agent spawning", async () => {
		...
		return (
			agent && agent.id && agent.type === "coder" && agent.name === "Test Coder"
		);
	});
	...
	await testAsync("Complete workflow integration", async () => {
		...
		return (
			finalStatus.agents.total >= 3 &&
			retrievedData &&
			retrievedData.test === true &&
			executorMetrics &&
			typeof executorMetrics.totalExecutions === "number"
		);
	});
	...
	await testAsync("Error handling and recovery", async () => {
		...
		const objective = coordinator.objectives.find(
			(o) => o.id === "non-existent-objective",
		);
	...
	if (testsPassed === totalTests) {
		console.log(
			"🎉 All tests passed! SwarmCoordinator implementation is working correctly.",
		);
		return true;
	} else {
		console.log(
			`❌ ${totalTests - testsPassed} tests failed. Implementation needs fixes.`,
		);
		return false;
	}
}
