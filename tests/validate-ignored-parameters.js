	...
	testCases.forEach((testCase) => {
		...
		testCase.testParams.forEach((param) => {
			...
			if (analysis.isIgnored) {
				...
				console.log(
					colorize(
						`      ✅ Warning detected: ${analysis.detectedPatterns.join(", ")}`,
						"green",
					),
				);
			}
...
function generateReport(results) {
	...
	console.log(
		colorize(`Total parameters tested: ${results.totalTests}`, "blue"),
	);
	console.log(
		colorize(`Silently ignored: ${results.ignoredParams.length}`, "red"),
	);
	console.log(
		colorize(`Properly warned: ${results.warningParams.length}`, "green"),
	);
	console.log(
		colorize(`Caused errors: ${results.errorParams.length}`, "yellow"),
	);
	...
	if (results.warningParams.length > 0) {
		...
		results.warningParams.forEach((item) => {
			console.log(
				colorize(
					`   ${item.command} ${item.parameter} (${item.patterns.join(", ")})`,
					"green",
				),
			);
		});
	...
	const securityScore = Math.round(
		((results.warningParams.length + results.errorParams.length) /
			results.totalTests) *
			100,
	);
	...
	console.log(
		colorize(
			...
			securityScore > 80 ? "green" : securityScore > 50 ? "yellow" : "red",
		),
	);
...
function saveReport(results) {
	...
	const reportPath = path.join(
		__dirname,
		"..",
		"parameter-validation-report.json",
	);
...
function testSpecificScenarios() {
	...
	const edgeCases = [
		...
		},
		{
			...
			command:
				"init --force --unknown-flag --minimal --fake-value test --dry-run",
		},
		{
		...
		},
		{
			...
			command:
				'init --name "test with $pecial ch@rs" --fake-param "value" --dry-run',
		},
	];
	...
	edgeCases.forEach((testCase) => {
		...
		if (result.success) {
			...
			const hasWarnings =
				result.output.toLowerCase().includes("warning") ||
			...
			if (hasWarnings) {
				...
				console.log(
					colorize("   🚨 No warnings shown (potential issue)", "red"),
				);
			}
		} else {
			console.log(colorize("   ❌ Command failed", "red"));
...
async function main() {
	try {
		console.log(
			colorize("Starting Claude Flow parameter validation analysis...", "cyan"),
		);
