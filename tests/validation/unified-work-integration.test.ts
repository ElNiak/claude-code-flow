...
import type { WorkOptions, TaskAnalysis } from '../../src/unified/work/types.js';
...
describe('Unified Work Command Integration Validation', () => {
  ...
  describe('Task Analysis and Prompt Generation', () => {
    test('should analyze simple development task correctly', async () => {
      ...
      expect(analysis.suggestedTopology).toMatch(/^(mesh|hierarchical|ring|star)$/);
      expect(analysis.suggestedStrategy).toMatch(/^(parallel|sequential|adaptive)$/);
  ...
  describe('Prompt Generation Quality', () => {
    test('should generate valid Claude Code coordination prompt', async () => {
      ...
      try {
        ...
        await command.parseAsync(['node', 'test', 'build a test API', '--dry-run'], { from: 'user' });
    ...
    test('should include security considerations in prompts', async () => {
      ...
      try {
        await command.parseAsync(['node', 'test', 'test security task', '--dry-run'], { from: 'user' });
        await new Promise(resolve => setTimeout(resolve, 100));
  ...
  describe('Input Reformulation and Edge Cases', () => {
    ...
    test('should handle very long task descriptions', async () => {
      const longTask = 'Build a comprehensive enterprise-grade system with ' + 'complex requirements '.repeat(100);
    ...
    test('should handle special characters and unicode in task description', async () => {
      const specialTask = 'Build an API with émojis 🚀 and spéciål châractérs & symbols (@#$%^&*)';
  ...
  describe('Agent Workflow Integration', () => {
    test('should coordinate with existing hive-mind agents', async () => {
      ...
      try {
        await command.parseAsync(['node', 'test', 'test hive integration', '--dry-run'], { from: 'user' });
        await new Promise(resolve => setTimeout(resolve, 100));
    ...
    test('should adapt agent count based on task complexity', async () => {
      ...
      expect(complexAnalysis.suggestedAgents).toBeGreaterThan(simpleAnalysis.suggestedAgents);
    });
  });
  ...
  describe('Performance and Scaling', () => {
    ...
    test('should handle concurrent task analysis requests', async () => {
      ...
      const analyses = await Promise.all(
        tasks.map(task => taskAnalyzer.analyze({
          task,
          params: [],
          context: {
            workingDirectory: process.cwd(),
  ...
  describe('Error Handling and Recovery', () => {
    ...
    test('should recover from intelligence layer failures', async () => {
      ...
      const taskAnalyzerWithoutIntelligence = new TaskAnalyzer({ useIntelligenceLayer: false });
  ...
  describe('Security Validation', () => {
    ...
    test('should sanitize environment variables in prompts', async () => {
      ...
      try {
        ...
        await command.parseAsync(['node', 'test', 'test security', '--dry-run'], { from: 'user' });
        await new Promise(resolve => setTimeout(resolve, 100));
  ...
  describe('Quality Assurance', () => {
    test('should generate prompts with consistent structure', async () => {
      const testCases = [
        ...
        { task: 'Deploy to production', topology: 'ring', strategy: 'sequential' },
        { task: 'Research algorithms', topology: 'hierarchical', strategy: 'adaptive' }
      ];
    ...
    test('should maintain prompt quality metrics', async () => {
      ...
      try {
        await command.parseAsync(['node', 'test', 'comprehensive quality test', '--dry-run', '--verbose'], { from: 'user' });
        await new Promise(resolve => setTimeout(resolve, 100));
        
        // Quality metrics
        const wordCount = promptContent.split(/\s+/).length;
        const lineCount = promptContent.split('\n').length;
        const sectionCount = (promptContent.match(/^[🔧🚨📋⚡🎯💡]/gm) || []).length;
...
class IntegrationTestUtils {
  ...
  static validatePromptStructure(prompt: string): {
    ...
    if (!prompt.includes('pre-task') || !prompt.includes('post-edit') || !prompt.includes('post-task')) {
