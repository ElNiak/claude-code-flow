...
class IntegrationValidationRunner {
  ...
  constructor() {
    this.outputDir = path.join(__dirname, 'validation-reports', `run-${Date.now()}`);
  }
  ...
  async run(): Promise<void> {
    console.log(chalk.blue.bold('🚀 Starting Integration Validation Test Suite'));
  ...
  private displaySuiteResult(result: TestResult): void {
    ...
    if (result.coverage !== undefined) {
      console.log(chalk.blue(`├── 📊 Coverage: ${result.coverage.toFixed(1)}%`));
    }
  ...
  private calculateQualityMetrics(): ValidationReport['qualityMetrics'] {
    ...
    const integrationResult = this.results.find(r => r.suite.includes('Integration'));
    const integrationTests = integrationResult ? integrationResult.passed + integrationResult.failed : 0;
    const integrationPassed = integrationResult ? integrationResult.passed : 0;
    const integrationScore = integrationTests > 0 ? (integrationPassed / integrationTests) * 100 : 0;
    ...
    const promptTests = promptResult ? promptResult.passed + promptResult.failed : 0;
    const promptPassed = promptResult ? promptResult.passed : 0;
    const promptQualityScore = promptTests > 0 ? (promptPassed / promptTests) * 100 : 0;
    ...
    const performanceTests = performanceResult ? performanceResult.passed + performanceResult.failed : 0;
    const performancePassed = performanceResult ? performanceResult.passed : 0;
    const performanceScore = performanceTests > 0 ? (performancePassed / performanceTests) * 100 : 0;

    // Overall Score: weighted average
    const weights = this.testSuites.reduce((acc, suite) => {
  ...
  private analyzeResults(): { recommendations: string[]; issues: string[] } {
    ...
    for (const result of this.results) {
      ...
      if (successRate < 80) {
        issues.push(`${result.suite} has low success rate (${successRate.toFixed(1)}%)`);
        recommendations.push(`Review and fix failing tests in ${result.suite}`);
      }

      if (result.coverage !== undefined && result.coverage < 80) {
        issues.push(`${result.suite} has low coverage (${result.coverage.toFixed(1)}%)`);
    ...
    if (totalTests === 0) {
      ...
      recommendations.push('Ensure test files are properly configured and accessible');
    } else if (totalFailed === 0) {
      recommendations.push('Excellent! All tests passed. Consider adding more edge case tests.');
    } else if (totalFailed > totalTests * 0.1) {
      recommendations.push('High failure rate detected. Review test implementation and system stability.');
    }
  ...
  private generateHtmlReport(report: ValidationReport): string {
    const successRate = report.totalTests > 0 ? (report.totalPassed / report.totalTests) * 100 : 0;
...
<body>
  ...
  private async displaySummary(): Promise<void> {
    ...
    console.log(chalk.gray(`⏱️  Total Duration: ${(Date.now() - this.startTime) / 1000}s`));
