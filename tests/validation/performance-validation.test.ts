...
import type { WorkOptions, TaskInput } from '../../src/unified/work/types.js';
...
describe('Unified Work Command Performance Validation', () => {
  ...
  describe('Task Analysis Performance', () => {
    test('should analyze simple tasks within performance thresholds', async () => {
      ...
      for (const task of simpleTasks) {
        ...
        performanceMetrics.recordTaskAnalysis(task, analysisTime, memoryUsed, 'simple');
      }
    });
    ...
    test('should analyze complex tasks within reasonable thresholds', async () => {
      ...
      for (const task of complexTasks) {
        ...
        performanceMetrics.recordTaskAnalysis(task, analysisTime, memoryUsed, 'complex');
      }
    });
    ...
    test('should handle concurrent task analysis efficiently', async () => {
      ...
      const analyses = await Promise.all(
        concurrentTasks.map(task => taskAnalyzer.analyze({
          task,
          params: [],
          context: {
            workingDirectory: process.cwd(),
      ...
      performanceMetrics.recordConcurrentAnalysis(concurrentTasks.length, totalTime, totalMemory);
    });
    ...
    test('should scale linearly with task complexity', async () => {
      const complexityLevels = [
        ...
        { task: 'Complex task with frontend, backend, database, auth, and deployment', components: 7 },
        { task: 'Very complex task with microservices, event sourcing, CQRS, monitoring, security, CI/CD, and multi-cloud deployment', components: 15 }
      ];
  ...
  describe('Prompt Generation Performance', () => {
    test('should generate prompts within time constraints', async () => {
      const testCases = [
        ...
        { task: 'Complex enterprise system implementation', expectedTimeMs: 500 }
      ];
      ...
      for (const testCase of testCases) {
        ...
        try {
          ...
          await command.parseAsync(['node', 'test', testCase.task, '--dry-run'], { from: 'user' });
          ...
          performanceMetrics.recordPromptGeneration(testCase.task, generationTime);
    ...
    test('should handle prompt generation under load', async () => {
      const loadTestTasks = Array.from({ length: 20 }, (_, i) => `Load test task ${i + 1}`);
      ...
      try {
        ...
        performanceMetrics.recordLoadTest(loadTestTasks.length, totalTime, promptsGenerated);
  ...
  describe('Benchmark Validation', () => {
    test('should meet established performance benchmarks', async () => {
      ...
      // Test each benchmark
      const results = await performanceMetrics.runBenchmarkSuite(taskAnalyzer, workCommand);

      expect(results.simpleTaskAnalysis).toBeLessThan(benchmarks.simpleTaskAnalysis);
      expect(results.complexTaskAnalysis).toBeLessThan(benchmarks.complexTaskAnalysis);
      expect(results.promptGeneration).toBeLessThan(benchmarks.promptGeneration);
      expect(results.concurrentTasksHandled).toBeGreaterThanOrEqual(benchmarks.concurrentTasks);
    ...
    test('should maintain performance regression protection', async () => {
      ...
      try {
        ...
        console.warn('No performance baseline available, skipping regression test');
        return;
      }

      // Run current performance tests
      const currentResults = await performanceMetrics.runBenchmarkSuite(taskAnalyzer, workCommand);

      // Compare with baseline (allow 20% degradation)
      const regressionThreshold = 1.2;

      expect(currentResults.simpleTaskAnalysis).toBeLessThan(baseline.simpleTaskAnalysis * regressionThreshold);
      expect(currentResults.complexTaskAnalysis).toBeLessThan(baseline.complexTaskAnalysis * regressionThreshold);
      expect(currentResults.promptGeneration).toBeLessThan(baseline.promptGeneration * regressionThreshold);
      expect(currentResults.peakMemoryUsage).toBeLessThan(baseline.peakMemoryUsage * regressionThreshold);
...
class PerformanceMetrics {
  ...
  recordTaskAnalysis(task: string, time: number, memory: number, complexity: string): void {
  ...
  recordConcurrentAnalysis(taskCount: number, totalTime: number, totalMemory: number): void {
    this.concurrentTestMetrics.push({ taskCount, totalTime, totalMemory });
  }

  recordScalabilityTest(results: Array<{ components: number; time: number; agents: number }>): void {
    this.scalabilityMetrics.push(...results);
  }

  recordLoadTest(taskCount: number, totalTime: number, promptsGenerated: number): void {
  ...
  async runBenchmarkSuite(taskAnalyzer: TaskAnalyzer, workCommand: WorkCommand): Promise<{
    ...
    const command = workCommand.createCommand();
    await command.parseAsync(['node', 'test', 'benchmark prompt', '--dry-run'], { from: 'user' });
  ...
  generateReport(): void {
    const report = {
      taskAnalysis: {
        ...
        averageTime: this.taskAnalysisMetrics.reduce((sum, m) => sum + m.time, 0) / this.taskAnalysisMetrics.length,
        averageMemory: this.taskAnalysisMetrics.reduce((sum, m) => sum + m.memory, 0) / this.taskAnalysisMetrics.length
      },
      promptGeneration: {
        total: this.promptGenerationMetrics.length,
        averageTime: this.promptGenerationMetrics.reduce((sum, m) => sum + m.time, 0) / this.promptGenerationMetrics.length
      },
