...
describe("Unified Systems Integration Tests", () => {
	...
	beforeEach(() => {
		...
		(SeamlessIntegration as jest.Mock).mockImplementation(
			() => mockSeamlessIntegration,
		);
		(UnifiedExecutionEngine as jest.Mock).mockImplementation(
			() => mockUnifiedExecutionEngine,
		);
	...
	describe("Work Command Integration", () => {
		...
		it("should handle seamless integration failure gracefully", async () => {
			...
			mockSeamlessIntegration.integrateSeamlessly.mockRejectedValue(
		...
		it("should fall back to legacy mode when unified systems fail", async () => {
			...
			mockSeamlessIntegration.integrateSeamlessly.mockRejectedValue(
				new Error("No integration"),
			);
			mockUnifiedExecutionEngine.initialize.mockRejectedValue(
			...
			expect(mockLogger.warn).toHaveBeenCalledWith(
				expect.stringContaining(
					"Unified systems not available, using legacy mode",
				),
			);
		});
	});
	...
	describe("Agent Spawning in reformulateObjective", () => {
		it("should spawn prompt-engineer agent successfully", async () => {
			...
			(globalThis as any).Task = jest
				.fn()
				.mockResolvedValue(JSON.stringify(mockTaskResult));

			const reformulateMethod = (workCommand as any).reformulateObjective;
			const result = await reformulateMethod.call(
				workCommand,
				"original task",
				[],
				{},
		...
		it("should handle Task tool failure with fallback", async () => {
			...
			expect(result.reformulatedTask).toBe(
				"original task (analyzed and optimized for execution)",
			);
		...
		it("should use simple reformulation when all agent spawning fails", async () => {
			...
			expect(result.rationale).toBe(
				"Applied basic reformulation rules for clarity and completeness",
			);
		});
	});
	...
	describe("Execution Plan Generation", () => {
		it("should generate execution steps with SPARC methodology", async () => {
			...
			const steps = await generateStepsMethod.call(
				workCommand,
				mockAnalysis,
				mockCoordination,
			);
	...
	describe("Unified Execution Engine Integration", () => {
		it("should use unified execution engine when available", async () => {
			...
			mockUnifiedExecutionEngine.executeIntelligent.mockResolvedValue(
				mockExecutionResult,
			);
			...
			await executePlanMethod.call(workCommand, mockPlan, {
				strategy: "adaptive",
			});

			expect(
				mockUnifiedExecutionEngine.executeIntelligent,
		...
		it("should fall back to legacy execution when unified engine fails", async () => {
			mockUnifiedExecutionEngine.executeIntelligent.mockRejectedValue(
				new Error("Engine failed"),
			);
			...
			expect(mockLogger.warn).toHaveBeenCalledWith(
				expect.stringContaining(
					"Unified execution failed, falling back to legacy mode",
				),
			);
	...
	describe("System Status and Health", () => {
		it("should provide unified system status", () => {
			...
			mockUnifiedExecutionEngine.getUnifiedStatus.mockReturnValue(
				mockStatus.unifiedEngine,
			);
	...
	describe("Error Handling and Recovery", () => {
		it("should handle import errors gracefully", async () => {
			// Mock import failure
			const originalImport = jest.requireMock(
				"../../src/unified/core/seamless-integration.js",
			);
		...
		it("should validate configuration before execution", async () => {
			...
			const initializeConfigMethod = (workCommand as any)
	...
	describe("Two-Step Workflow Integration", () => {
		it("should execute two-step workflow with reformulation", async () => {
			...
			(workCommand as any).initializeConfiguration = jest
				.fn()
				.mockResolvedValue();
			(workCommand as any).initializeUnifiedSystems = jest
				.fn()
				.mockResolvedValue();
