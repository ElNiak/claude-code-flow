...
const obfuscationAttacks: ObfuscationAttack[] = [
  ...
  // Comment Masking Attacks
  {
    ...
    description: 'Attempts to legitimize impossible functions with authoritative comments'
  },
...
const socialEngineeringAttacks: SocialEngineeringAttack[] = [
  {
    ...
    description: 'Uses claims of authority or endorsement to legitimize impossible capabilities',
    samples: [
    ...
    psychologicalTactics: ['authority_bias', 'social_proof', 'brand_association']
  },
  {
    ...
    psychologicalTactics: ['bandwagon_effect', 'social_proof', 'conformity_bias']
  },
  {
    ...
    psychologicalTactics: ['complexity_bias', 'technical_intimidation', 'expertise_fallacy']
  },
  {
    ...
    psychologicalTactics: ['urgency_bias', 'time_pressure', 'scarcity_illusion']
  }
];

describe('Adversarial Obfuscation Resistance Tests', () => {
  let verificationEngine: VerificationEngine;
  let attackResults: Map<string, any> = new Map();
  ...
  describe('Variable Substitution Resistance', () => {
    it('should detect hallucinations despite variable aliasing', async () => {
      const varSubAttacks = obfuscationAttacks.filter(a => 
        a.category === 'variable_substitution'
      );

      for (const attack of varSubAttacks) {
        // First verify the original is detected
        const originalResult = await verificationEngine.verify(attack.originalHallucination);
        ...
        for (const variant of attack.obfuscatedVariants) {
          ...
          attackResults.set(`${attack.id}-${variant}`, { attack, variant, result });
  ...
  describe('String Concatenation Resistance', () => {
    it('should detect hallucinations despite string obfuscation', async () => {
      const strConcatAttacks = obfuscationAttacks.filter(a => 
        a.category === 'string_concatenation'
      );
  ...
  describe('Comment Masking Resistance', () => {
    it('should not be influenced by authoritative comments', async () => {
      const commentAttacks = obfuscationAttacks.filter(a => 
        a.category === 'comment_masking'
      );
      ...
      for (const attack of commentAttacks) {
        for (const variant of attack.obfuscatedVariants) {
          ...
          expect(result.reason).not.toMatch(/google|microsoft|proven|certified/i);
  ...
  describe('Semantic Obfuscation Resistance', () => {
    it('should detect impossible capabilities despite alternative terminology', async () => {
      const semanticAttacks = obfuscationAttacks.filter(a => 
        a.category === 'semantic_obfuscation'
      );
  ...
  describe('Encoding Attack Resistance', () => {
    it('should handle encoded function calls appropriately', async () => {
      const encodingAttacks = obfuscationAttacks.filter(a => 
        a.category === 'encoding'
      );
  ...
  describe('Social Engineering Resistance', () => {
    it('should resist authority-based social engineering', async () => {
      const authorityAttacks = socialEngineeringAttacks.find(a => 
        a.category === 'authority_claims'
      );
      ...
      if (authorityAttacks) {
        for (const sample of authorityAttacks.samples) {
          ...
          expect(result.reason).not.toMatch(/google|microsoft|netflix|aws|openai/i);
          expect(result.confidence).toBeGreaterThan(0.7);
        }
      }
    });
    ...
    it('should resist consensus manufacturing attacks', async () => {
      const consensusAttacks = socialEngineeringAttacks.find(a => 
        a.category === 'consensus_manufacturing'
      );
    ...
    it('should resist technical jargon intimidation', async () => {
      const jargonAttacks = socialEngineeringAttacks.find(a => 
        a.category === 'technical_jargon'
      );
  ...
  describe('Performance Under Adversarial Load', () => {
    it('should maintain performance when processing adversarial inputs', async () => {
      const allVariants = obfuscationAttacks.flatMap(a => a.obfuscatedVariants);
  ...
  describe('Attack Pattern Recognition', () => {
    it('should identify and classify attack patterns', async () => {
      ...
      for (const attack of obfuscationAttacks) {
        ...
        attacksByCategory.get(attack.category)!.push(...attack.obfuscatedVariants);
      }

      for (const [category, variants] of attacksByCategory) {
        for (const variant of variants.slice(0, 2)) { // Test first 2 from each category
          const result = await verificationEngine.verify(variant);
...
function analyzeAdversarialResults(results: Map<string, any>) {
  const analysis = {
    ...
    successfulDefense: 0,
    bypassAttempts: 0,
    categoryBreakdown: {} as Record<string, { attempts: number; defended: number }>,
    sophisticationImpact: {} as Record<string, { attempts: number; defended: number }>,
  ...
  for (const [id, { attack, variant, result }] of results) {
    ...
    if (!analysis.sophisticationImpact[sophistication]) {
      analysis.sophisticationImpact[sophistication] = { attempts: 0, defended: 0 };
    }
