...
const boundaryTestCases: BoundaryTestCase[] = [
	// Semantic Ambiguity Cases
	{
		...
		code: "optimize()",
		description:
			"Generic optimization function - could be legitimate or hallucinated",
	...
	},
	{
		...
		description:
			"Intelligence claim that could be marketing or genuine capability",
		expectedClassification: "ambiguous",
...
describe("Boundary Condition Tests for Hallucination Detection", () => {
	...
	describe("Semantic Ambiguity Handling", () => {
		...
		it("should differentiate between similar but distinct semantic patterns", async () => {
			...
			const perfectOptimizeResult = results.find(
				(r) => r.pattern === "perfectOptimize()",
			);
			...
			const basicOptimizeResult = results.find(
				(r) => r.pattern === "basicOptimize()",
			);
	...
	describe("Confidence Calibration", () => {
		it("should have well-calibrated confidence scores", async () => {
			...
			const expertCase = boundaryTestCases.find(
				(tc) => tc.difficulty === "expert",
			);
...
function analyzeBoundaryResults(results: Map<string, any>) {
	...
	for (const category in analysis.categoryAccuracy) {
		const totalInCategory = Array.from(results.values()).filter(
			({ testCase }) => testCase.categories.includes(category),
		).length;
		analysis.categoryAccuracy[category] =
