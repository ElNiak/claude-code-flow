...
describe("Task Tool Integration with Hallucination Prevention", () => {
			...
			const validInstructions: TaskInstruction[] = [
				{
					...
					instruction:
						"Research best practices for REST API design and document findings",
				...
				},
				{
					agentType: "coder",
					instruction:
						"Implement user authentication endpoints using JWT tokens",
				...
				},
				{
					agentType: "tester",
					instruction:
						"Write comprehensive unit tests for authentication endpoints",
		...
		it("should flag instructions with impossible capabilities", async () => {
			const problematicInstructions: TaskInstruction[] = [
				{
					agentType: "architect",
					instruction:
						"Design a quantum computing architecture that solves all scalability issues",
				...
				},
				{
					agentType: "ai-coder",
					instruction:
						"Use advanced AI to write perfect bug-free code automatically",
				...
				},
				{
					agentType: "predictor",
					instruction:
						"Predict all future user requirements with 100% accuracy",
			...
			mockVerificationService.validateInstructionRealism
				.mockResolvedValueOnce({
					...
					confidence: 0.95,
					reason:
						"Quantum computing claims are unrealistic for typical applications",
				})
				.mockResolvedValueOnce({
					isRealistic: false,
					confidence: 0.98,
			...
			const result = await taskValidator.validateInstructions(
				problematicInstructions,
			);

			expect(result.validInstructions).toHaveLength(0);
			expect(result.flaggedInstructions).toHaveLength(3);
			expect(result.flaggedInstructions[0].reason).toContain(
				"Quantum computing claims",
			);
			expect(result.flaggedInstructions[1].reason).toContain(
				"perfect automated coding",
			);
			expect(result.flaggedInstructions[2].reason).toContain(
		...
		it("should validate tool availability claims", async () => {
			const instructionsWithTools: TaskInstruction[] = [
				...
				},
				{
					...
					instruction:
						"Use MagicTool and AutoPerfectCode to implement features",
	...
	describe("Agent Spawning Validation", () => {
				...
				agents: [
					{
						...
						instruction:
							"Research modern API design patterns and security best practices",
					...
					},
					{
						...
						instruction:
							"Design scalable architecture based on research findings",
					...
					},
					{
						...
						instruction:
							"Implement the designed architecture with proper error handling",
		...
		it("should flag agent types with impossible capabilities", async () => {
				...
				agents: [
					{
						...
						instruction:
							"Use quantum computing to optimize all code to theoretical perfection",
					...
					},
					{
						...
						instruction:
							"Travel to the future to see optimal implementation and bring it back",
			...
			const result = await taskValidator.validateSpawnRequest(
				unrealisticSpawnRequest,
			);

			expect(result.isValid).toBe(false);
			expect(result.validatedAgents).toHaveLength(0);
			expect(result.flaggedAgents).toHaveLength(2);
			expect(result.flaggedAgents[0].reason).toContain(
	...
	describe("Coordination Hook Validation", () => {
		...
		it("should flag non-existent coordination hooks", async () => {
			...
			const result =
				await taskValidator.validateCoordinationHooks(invalidHooks);
	...
	describe("Batch Agent Instruction Validation", () => {
		it("should efficiently validate large batches of agent instructions", async () => {
			const largeBatch: TaskInstruction[] = Array.from(
				{ length: 20 },
				(_, i) => ({
					agentType: `agent-${i + 1}`,
		...
		it("should handle validation errors gracefully in batch processing", async () => {
			...
			expect(result.errorInstructions[0].error).toContain(
	...
	describe("Real-World Task Validation Scenarios", () => {
		it("should validate complex multi-agent development workflow", async () => {
			const realWorldWorkflow: TaskInstruction[] = [
				{
					...
					instruction:
						"Analyze user requirements and create user stories for e-commerce platform",
				...
				},
				{
					agentType: "architect",
					instruction:
						"Design microservices architecture based on user stories",
			...
			const result =
				await taskValidator.validateInstructions(realWorldWorkflow);
		...
		it("should detect and flag unrealistic workflow claims", async () => {
			const unrealisticWorkflow: TaskInstruction[] = [
				{
					...
					instruction:
						"Use superintelligent AI to design perfect scalable architecture",
				...
				},
				{
					...
					instruction:
						"Implement quantum-enhanced backend with impossible performance",
				...
				},
				{
					...
					instruction:
						"Test all possible user scenarios by reading their minds",
			...
			const result =
				await taskValidator.validateInstructions(unrealisticWorkflow);
