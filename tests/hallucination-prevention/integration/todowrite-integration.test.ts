...
import { TodoItem, TodoWriteRequest } from '../../../src/types/todo-types';
...
describe('TodoWrite Integration with Hallucination Prevention', () => {
  ...
  describe('Todo Content Validation', () => {
    ...
    it('should flag todos with impossible AI capabilities', async () => {
      ...
      mockVerificationEngine.verifyImplementationClaim
        .mockResolvedValueOnce({
          ...
          confidence: 0.95,
          reason: 'Quantum computing claims are not realistic for typical applications'
        })
        .mockResolvedValueOnce({
          isHallucination: true,
          confidence: 0.98,
          reason: 'Telepathic interfaces are not possible with current technology'
        })
        .mockResolvedValueOnce({
          isHallucination: true,
          confidence: 0.92,
      ...
      expect(result.flaggedTodos[0].reason).toContain('Quantum computing claims');
      expect(result.flaggedTodos[1].reason).toContain('Telepathic interfaces');
      expect(result.flaggedTodos[2].reason).toContain('perfect automated code generation');
    });
    ...
    it('should handle mixed valid and invalid todos', async () => {
      ...
      expect(result.flaggedTodos[0].reason).toContain('magical never-failing debugging');
    });
  });
  ...
  describe('Code Snippet Detection in Todos', () => {
    it('should validate code snippets within todo descriptions', async () => {
      ...
      expect(result.flaggedTodos).toHaveLength(1);
      expect(result.flaggedTodos[0].reason).toContain('nonExistentMagicFunction does not exist');
    });
    ...
    it('should handle complex code snippets in todo descriptions', async () => {
      const complexTodos: TodoItem[] = [
        {
          id: '1',
          content: 'Implement: const result = await SwarmCoordinator.initialize({ agents: 5 })',
          status: 'pending',
          priority: 'high'
        }
      ];
      ...
      expect(result.validTodos).toHaveLength(1);
      expect(result.validTodos[0].verificationDetails.evidence).toContain('src/swarm/coordinator.ts');
    });
  });
  ...
  describe('Batch Todo Validation', () => {
    ...
    it('should handle validation errors gracefully in batch processing', async () => {
      ...
      expect(result.errorTodos[0].error).toContain('Verification service error');
    });
  });
  ...
  describe('TodoWrite Tool Integration', () => {
    it('should integrate seamlessly with existing TodoWrite workflow', async () => {
      ...
      const result = await todoValidator.validateTodoWriteRequest(todoWriteRequest);
    ...
    it('should provide alternative suggestions for flagged todos', async () => {
      ...
      const result = await todoValidator.validateTodoWriteRequest(problematicRequest);
  ...
  describe('Performance and Reliability', () => {
    it('should maintain low false positive rate', async () => {
      const legitimateTodos: TodoItem[] = [
        { id: '1', content: 'Add logging to API endpoints', status: 'pending', priority: 'low' },
        { id: '2', content: 'Implement user authentication', status: 'pending', priority: 'high' },
        ...
        { id: '5', content: 'Add error handling middleware', status: 'pending', priority: 'medium' }
      ];
    ...
    it('should maintain high detection rate for obvious hallucinations', async () => {
      const hallucinatedTodos: TodoItem[] = [
        { id: '1', content: 'Enable time travel debugging feature', status: 'pending', priority: 'high' },
        { id: '2', content: 'Implement telepathic API communication', status: 'pending', priority: 'medium' },
        { id: '3', content: 'Add quantum encryption with 100% security', status: 'pending', priority: 'high' },
        { id: '4', content: 'Create AI that never makes mistakes', status: 'pending', priority: 'low' }
      ];
    ...
    it('should handle edge cases with appropriate confidence levels', async () => {
      const edgeCaseTodos: TodoItem[] = [
        { id: '1', content: 'Implement advanced machine learning optimization', status: 'pending', priority: 'medium' }
      ];
      ...
      expect(result.validTodos).toHaveLength(1);
      expect(result.validTodos[0].verificationDetails.requiresManualReview).toBe(true);
