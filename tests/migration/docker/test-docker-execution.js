...
function checkDocker() {
	try {
		...
		console.error(
			`${colors.red}Docker not available: ${error.message}${colors.reset}`,
		);
		return false;
	}
}
...
if (!checkDocker()) {
	console.log(
		`${colors.yellow}Docker tests require Docker to be installed and running${colors.reset}`,
	);
	process.exit(0);
}
...
runTest("Container networking works", () => {
	try {
		...
		exec(
			`docker run -d --name test-server --network test-claude-flow-net ${DOCKER_IMAGE} start --mode server`,
		);
		execSync("sleep 2");
...
runTest("Multi-stage build optimization", () => {
	...
	if (sizeMatch) {
		...
		if (sizeMB > 500) {
			console.log(
				`  Warning: Image size ${sizeMB}MB is larger than recommended`,
			);
		}
	}
});
...
runTest("Health check works", () => {
	try {
		...
		exec(
			`docker run -d --name test-health ${DOCKER_IMAGE} start --mode server`,
		);

		// Wait for health check
		waitForContainer("test-health", 10)
			.then(() => {
