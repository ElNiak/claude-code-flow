...
async function runPerfTest(name, threshold, testFn) {
	...
	try {
		...
		if (duration <= threshold) {
			...
			console.log(
				`${colors.red}✗ FAILED - ${duration}ms (exceeded threshold)${colors.reset}`,
			);
		...
		console.log(
			`${colors.red}✗ FAILED - Error: ${error.message}${colors.reset}`,
		);
		performanceResults.failed++;
		performanceResults.tests.push({
			name,
...
console.log(
	`${colors.yellow}Setting up performance test environment...${colors.reset}`,
);
...
(async () => {
	...
	for (const test of performanceResults.tests) {
		...
		if (test.duration !== undefined) {
			console.log(
				`${test.name.padEnd(20)} ${String(test.duration + "ms").padStart(10)} ${String(
					test.threshold + "ms",
				).padStart(10)}   ${status}`,
			);
		}
	...
	const avgDuration =
		Object.values(performanceResults.metrics).reduce((a, b) => a + b, 0) /
	...
	console.log(
		`${colors.green}Passed: ${performanceResults.passed}${colors.reset}`,
	);
	console.log(
		`${colors.red}Failed: ${performanceResults.failed}${colors.reset}`,
	);
