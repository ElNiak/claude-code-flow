...
try {
	...
	const denoRelated = ["deno", "cliffy"].filter(
		(dep) =>
	...
	if (denoRelated.length > 0) {
		console.log(
			`${colors.red}✗ Found Deno-related dependencies: ${denoRelated.join(", ")}${colors.reset}`,
		);
		hasErrors = true;
	} else {
...
if (hasErrors) {
	...
	console.log(
		"Claude Flow appears to be successfully migrated to pure npm/Node.js!",
	);
	process.exit(0);
}
