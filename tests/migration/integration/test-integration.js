...
const TEST_DIR = path.join(
	os.tmpdir(),
	"claude-flow-integration-" + Date.now(),
);
...
runAsyncTest("ruv-swarm MCP integration", async () => {
	...
	if (await isPortAvailable(port)) {
		...
		const proc = spawn(
			"node",
			[CLAUDE_FLOW_BIN, "mcp", "start", "--port", port.toString()],
			{
				cwd: TEST_DIR,
				detached: true,
				stdio: "ignore",
...
runAsyncTest("Swarm coordination with agents", async () => {
	// Initialize swarm
	const initOutput = exec(
		`node ${CLAUDE_FLOW_BIN} swarm init --topology mesh --max-agents 3`,
	);
	...
	// Spawn test agent
	const spawnOutput = exec(
		`node ${CLAUDE_FLOW_BIN} swarm spawn --type researcher --name "Test Agent"`,
	);
...
runTest("Memory bank persistence", () => {
	// Store test data
	const storeOutput = exec(
		`node ${CLAUDE_FLOW_BIN} memory store --key test-key --value "test-value"`,
	);
...
runTest("Task coordination system", () => {
	...
	const createOutput = exec(
		`node ${CLAUDE_FLOW_BIN} task create --name "Test Task" --priority high`,
	);
