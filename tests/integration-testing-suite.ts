...
class IntegrationTestManager {
	...
	async cleanup(): Promise<void> {
		...
		for (const sessionId of this.testSessions) {
			try {
				await this.executeCommand(
					`npx claude-flow hooks session-end --session-id "${sessionId}" --cleanup true`,
				);
			} catch (error) {
		...
		try {
			await fs.rm(TEST_CONFIG.testWorkingDirectory, {
				recursive: true,
				force: true,
			});
		} catch (error) {
	...
	async measurePerformance<T>(
		...
		try {
			...
			const endTime = performance.now();
			throw new Error(
				`Performance measurement failed after ${endTime - startTime}ms: ${error}`,
			);
		}
	}
	...
	async validateSystemHealth(): Promise<{
		...
		try {
			...
			const runningProcesses = this.testProcesses.filter(
				(proc) => !proc.killed,
			);
...
describe("🔧 Integration Testing Suite", () => {
	...
	describe("🎯 1. Cross-Module Integration Tests", () => {
		test(
			"Unified Work Command + Coordination Manager Integration",
			async () => {
				const sessionId = testManager.generateTestSessionId();

				const { result, duration } = await testManager.measurePerformance(
					async () => {
		...
		test(
			"Agent Management + Memory System Integration",
			async () => {
				const sessionId = testManager.generateTestSessionId();
	...
	describe("🚀 2. End-to-End System Tests", () => {
		test(
			"Complete Unified Work Command Flow",
			async () => {
				const sessionId = testManager.generateTestSessionId();
	...
	describe("⚡ 4. Performance Validation Tests", () => {
		test(
			...
			async () => {
				...
				const { result, duration, memoryUsage } =
					await testManager.measurePerformance(async () => {
						// Test multiple concurrent operations
						const operations = [
							testManager.executeCommand(
								`npx claude-flow work "task 1" --session-id "${sessionId}-1" --agents 2`,
								15000,
							),
							testManager.executeCommand(
								`npx claude-flow work "task 2" --session-id "${sessionId}-2" --agents 2`,
								15000,
							),
							testManager.executeCommand(
								`npx claude-flow work "task 3" --session-id "${sessionId}-3" --agents 2`,
								15000,
							),
						];
				...
				console.log(
					`Performance test completed in ${Math.round(duration)}ms, memory delta: ${Math.round(memoryUsage.heapUsed / 1024 / 1024)}MB`,
				);
			},
			TEST_CONFIG.timeout * 2,
		);
		...
		test(
			...
			async () => {
				...
				console.log(
					`Memory test: Start ${Math.round(startMemory.heapUsed / 1024 / 1024)}MB, Mid ${Math.round(midMemory.heapUsed / 1024 / 1024)}MB, End ${Math.round(endMemory.heapUsed / 1024 / 1024)}MB`,
				);
			},
			TEST_CONFIG.timeout,
		);
		...
		test(
			...
			async () => {
				const sessionIds = Array.from({ length: 5 }, () =>
					testManager.generateTestSessionId(),
				);
				...
				console.log(
					`Concurrency test: ${sessionIds.length} concurrent operations completed in ${Math.round(duration)}ms`,
				);
			},
			TEST_CONFIG.timeout,
		);
	...
	describe("📊 5. Integration Report Generation", () => {
		test("Generate Comprehensive Integration Report", async () => {
			...
			const reportPath = path.join(
				TEST_CONFIG.testWorkingDirectory,
