...
class ConsoleTerminalTester {
	...
	async init() {
		try {
			...
			this.addResult(
				"failed",
				`Failed to initialize console: ${error.message}`,
			);
			throw error;
		}
	}
	...
	async testCommandInput() {
		...
		try {
			...
			if (helpOutput.includes("Available commands:")) {
				...
				this.addResult(
					"failed",
					"Help command did not display expected output",
				);
			}
			...
			if (statusOutput.includes("System Status")) {
				...
				this.addResult(
					"failed",
					"Status command did not display expected output",
				);
			}
	...
	async testCommandHistory() {
		...
		try {
			...
			if (multiUpValue === "status") {
				this.addResult(
					"passed",
					"Multiple UP arrows navigate history correctly",
				);
			} else {
	...
	async testKeyboardShortcuts() {
		...
		try {
			...
			this.addResult(
				"failed",
				`Keyboard shortcuts test failed: ${error.message}`,
			);
		}
	}
	...
	async testVisualFeatures() {
		...
		try {
			...
			const welcomePresent = await this.page.evaluate(() => {
				const output = document.querySelector(".console-output");
				return output
					? output.textContent.includes("Welcome") ||
							output.textContent.includes("Claude")
					: false;
			});
			...
			const hasTimestamp = await this.page.evaluate(() => {
				...
				return (
					lastEntry.querySelector(".timestamp") !== null ||
			...
			const hasColors = await this.page.evaluate(() => {
				const entries = document.querySelectorAll(
					".console-entry, .console-output > div",
				);
		...
		try {
			...
			const statusElements = await this.page.evaluate(() => {
				...
				const uptimeEl = document.querySelector(
					'[id*="uptime"], [class*="uptime"], [data-uptime]',
				);
				...
				const agentEl = document.querySelector(
					'[id*="agent"], [class*="agent"], [data-agents]',
				);
				...
				const msgEl = document.querySelector(
					'[id*="message"], [class*="message"], [data-messages]',
				);
			...
			if (
				updatedUptime !== statusElements.uptimeText &&
				statusElements.uptimeText !== ""
			) {
	...
	async testHeaderButtons() {
		...
		try {
			...
			const clearButton =
				(await this.page.$("#clearConsole")) ||
				(await this.page.$('button[aria-label*="Clear"]'));
			if (clearButton) {
			...
			const fullscreenButton =
				(await this.page.$("#fullscreenToggle")) ||
				(await this.page.$('button[aria-label*="Fullscreen"]'));
			if (fullscreenButton) {
				...
				if (isFullscreen) {
					...
					this.addResult(
						"warnings",
						"Fullscreen functionality may not be working",
					);
				}
			} else {
