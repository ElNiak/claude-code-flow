...
describe('🤖 Agent Listing System Validation', function() {
  this.timeout(60000);
  ...
  after(async function() {
    // Store test results
    const resultsPath = '/Users/elniak/Documents/Project/claude-code-flow/agent-listing-test-results.json';
    await fs.writeFile(resultsPath, JSON.stringify(testResults, null, 2));
    console.log(`📊 Test results saved to: ${resultsPath}`);
  });

  describe('🔧 Implementation Compatibility Tests', function() {

    it('should validate agent-simple.ts list function', async function() {
      console.log('Testing agent-simple.ts implementation...');

      try {
        // Test basic list functionality
        const { stdout } = await execAsync('node -e "import(\'./src/cli/commands/agent-simple.js\').then(m => m.listAgents([], {}))"');
    ...
    it('should validate agent-unified.ts list function', async function() {
      ...
      try {
        ...
        for (const testCase of testCases) {
          try {
            const { stdout } = await execAsync(`node -e "import('./src/cli/commands/agent-unified.js').then(m => m.listAgents(${JSON.stringify(testCase.args)}, ${JSON.stringify(testCase.flags)}))"`);
            console.log(`✅ Test case passed: ${JSON.stringify(testCase.flags)}`);
          } catch (err) {
            console.log(`❌ Test case failed: ${JSON.stringify(testCase.flags)} - ${err.message}`);
            allPassed = false;
          }
        }
    ...
    it('should validate unified-agents.ts handleAgentList function', async function() {
  ...
  describe('📊 Performance Analysis Tests', function() {

    it('should measure list performance with different agent counts', async function() {
      ...
      for (const test of performanceTests) {
        ...
        try {
          ...
          testResults.performance[test.description] = {
            ...
            status: duration < 1000 ? 'optimal' : duration < 3000 ? 'acceptable' : 'slow'
          };

          console.log(`⏱️  ${test.description}: ${duration}ms (${testResults.performance[test.description].status})`);
        } catch (error) {
          testResults.performance[test.description] = {
            agent_count: test.agentCount,
            status: 'failed',
    ...
    it('should test memory usage during agent aggregation', async function() {
      ...
      try {
        ...
        console.log(`💾 Memory delta: ${testResults.performance.memory_usage.delta_mb}MB`);
      } catch (error) {
        testResults.performance.memory_usage = {
          status: 'failed',
          error: error.message
  ...
  describe('🔄 Cross-Pattern Compatibility Tests', function() {

    it('should validate data format consistency across implementations', async function() {
      console.log('🔍 Testing data format consistency...');

      const dataFormats = {
        'agent-simple': {
          expected_fields: ['id', 'name', 'type', 'status', 'health', 'workload'],
          output_type: 'console'
        },
        'agent-unified': {
          expected_fields: ['id', 'name', 'type', 'status', 'health', 'workload'],
      ...
      testResults.functionality.data_format_consistency = {
        ...
        status: consistencyScore === totalChecks ? 'consistent' : 'inconsistent',
        implementations_tested: Object.keys(dataFormats)
      };
    });

    it('should test filtering compatibility across patterns', async function() {
      ...
      for (const test of filterTests) {
        try {
          ...
          console.log(`📊 ${test.description}: ${supportCount}/3 implementations support this filter`);
        } catch (error) {
    ...
    it('should test output format compatibility', async function() {
      ...
      for (const format of outputFormats) {
        try {
          ...
          formatSupport[format] = {
            ...
            'agent-unified': format === 'console' || format === 'json' || format === 'table',
            'unified-agents': format === 'console'
          };

          const supportCount = Object.values(formatSupport[format]).filter(Boolean).length;
          console.log(`📊 ${format} format: ${supportCount}/3 implementations support this`);
        } catch (error) {
  ...
  describe('🛡️ Error Handling and Edge Cases', function() {
    ...
    it('should handle empty agent lists gracefully', async function() {
    ...
    it('should handle invalid filter parameters', async function() {
      ...
      for (const filter of invalidFilters) {
        try {
          ...
          errorHandlingResults[JSON.stringify(filter)] = { error: error.message };
        }
      }
    ...
    it('should test concurrent access scenarios', async function() {
  ...
  describe('📈 Integration and System Tests', function() {

    it('should test integration with swarm coordination', async function() {
    ...
    it('should validate memory persistence across sessions', async function() {
...
function generateTestReport(results) {
  ...
  report.summary.overall_score =
    Math.round((report.summary.passed_tests / report.summary.total_tests) * 100);

  // Generate recommendations
  if (report.summary.overall_score < 80) {
    report.recommendations.push('Consider implementing unified data interface across all implementations');
  }

  if (Object.keys(results.functionality.filter_compatibility || {}).length < 3) {
    report.recommendations.push('Standardize filtering parameters across all agent listing implementations');
  }
