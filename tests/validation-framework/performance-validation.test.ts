...
class RegressionDetector {
	...
	private thresholds = {
		duration: 0.2, // 20% increase is a regression
		memory: 0.3, // 30% increase is a regression
	...
	detectRegression(
		...
		const memoryChange =
			(current.memoryUsage - baseline.memoryUsage) /
			Math.abs(baseline.memoryUsage || 1);
...
class ScalabilityTester {
	...
	private calculateLinearity(
		...
		const sumXY = results.reduce(
			(sum, r) => sum + r.scale * r.metrics.duration,
			0,
		);
		const sumX2 = results.reduce((sum, r) => sum + r.scale * r.scale, 0);
		const sumY2 = results.reduce(
			(sum, r) => sum + r.metrics.duration * r.metrics.duration,
			0,
		);
...
describe("Performance Validation Tests", () => {
	...
	describe("Validation Engine Performance", () => {
		it("should meet validation performance benchmarks", async () => {
			...
			performanceTester.setThreshold(
				"code_validation",
				MAX_VALIDATION_TIME,
				MAX_MEMORY_USAGE,
			);
			...
			performanceTester.registerBenchmark(
				"code_validation",
				validationBenchmark,
			);
			...
			console.log(
				`Validation benchmark: ${result.duration.toFixed(2)}ms, ${(result.memoryUsage / 1024 / 1024).toFixed(2)}MB`,
			);
		});
		...
		it("should scale linearly with input size", async () => {
			...
			console.log(
				`Linearity: ${(result.linearityCoefficient * 100).toFixed(1)}%`,
			);
	...
	describe("Load Testing", () => {
		...
		it("should maintain performance under sustained load", async () => {
			...
			console.log(
				`  Latency ratio: ${(result.maxLatency / result.averageLatency).toFixed(2)}`,
			);
		});
	});
	...
	describe("Regression Detection", () => {
		it("should detect performance regressions", async () => {
			...
			const result = regressionDetector.detectRegression(
			...
			const durationRegression = result.regressions.find(
				(r) => r.metric === "duration",
			);
			...
			result.regressions.forEach((regression) => {
				console.log(
					`  ${regression.metric}: ${(regression.change * 100).toFixed(1)}% (${regression.severity})`,
				);
			});
		});
	...
	describe("Memory Performance", () => {
		it("should not have memory leaks during repeated operations", async () => {
			...
			console.log(
				`Memory growth over ${iterations} iterations: ${(memoryGrowth * 100).toFixed(2)}%`,
			);
			console.log(
				`Initial average: ${(firstHalfAvg / 1024 / 1024).toFixed(2)}MB`,
			);
			console.log(
				`Final average: ${(secondHalfAvg / 1024 / 1024).toFixed(2)}MB`,
			);
		});
		...
		it("should handle memory pressure gracefully", async () => {
			...
			expect(result.passed).toBe(true);
			console.log(
				`Memory pressure test: ${result.duration.toFixed(2)}ms, ${(result.memoryUsage / 1024 / 1024).toFixed(2)}MB`,
			);
		});
	});
	...
	describe("Concurrent Performance", () => {
		it("should maintain performance with multiple concurrent validations", async () => {
			...
			for (const level of concurrencyLevels) {
				...
				console.log(
					`Concurrency ${level}: ${metrics.throughput.toFixed(2)} ops/s, ${metrics.duration.toFixed(2)}ms`,
				);
			}
			...
			expect(mediumConcurrency.metrics.throughput).toBeGreaterThan(
				lowConcurrency.metrics.throughput * 0.8,
			);
		});
	});
});
