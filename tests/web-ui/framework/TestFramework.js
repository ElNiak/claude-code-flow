...
class TestFramework extends EventEmitter {
	...
	async initialize() {
		...
		console.log(
			`📊 Testing ${this.getTotalToolCount()} tools across ${Object.keys(testConfig.testCategories).length} categories`,
		);
	 ...
	 */
	getTotalToolCount() {
		return Object.values(testConfig.testCategories).reduce(
			(total, cat) => total + cat.tools,
			0,
		);
	}
		...
		this.mcpMock = {
			...
			execute: async (toolName, params) => {
				...
				await new Promise((resolve) =>
					setTimeout(resolve, this.mcpMock.latency),
				);
	...
	async runTestSuite(suite) {
		...
		console.log(
			`✅ Suite completed: ${suiteResults.passed} passed, ${suiteResults.failed} failed, ${suiteResults.skipped} skipped`,
		);
	...
	generateReport() {
		...
		const totalPassed = Array.from(this.testResults.values()).reduce(
			(sum, suite) => sum + suite.passed,
			0,
		);
		const totalFailed = Array.from(this.testResults.values()).reduce(
		...
		const report = {
			...
			coverage: {
				tools: {
					...
					percentage:
						(
							(this.coverage.tools.size / this.getTotalToolCount()) *
							100
						).toFixed(2) + "%",
				},
				views: {
					tested: this.coverage.views.size,
					total: Object.values(testConfig.testCategories).flatMap(
						(c) => c.views,
					).length,
					percentage:
						(
							(this.coverage.views.size /
								Object.values(testConfig.testCategories).flatMap((c) => c.views)
									.length) *
							100
						).toFixed(2) + "%",
				},
