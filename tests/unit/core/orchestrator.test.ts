...
import { createMocks, type MockEventBus } from "../../mocks/index.ts";
...
describe("Orchestrator", () => {
	...
	describe("initialization", () => {
		it("should initialize all components", async () => {
			...
			expect(
				mocks.logger
					.hasLog("info")
					.toBe("Orchestrator initialized successfully"),
				true,
			);
		});
	...
	describe("shutdown", () => {
		...
		it("should stop background tasks", async () => {
			...
			expect(mocks.terminalManager.getHealthStatus.calls.length).toBe(
				initialHealthChecks,
			);
		});
	...
	describe("task management", () => {
		...
		it("should assign task to specific agent", async () => {
			...
			expect(mocks.coordinationManager.assignTask.calls[0].args[1]).toBe(
				profile.id,
			);
		});
	...
	describe("health monitoring", () => {
		...
		it("should return degraded status with circuit breaker open", async () => {
			...
			expect(health.components.orchestrator.error).toBe(
	...
	describe("maintenance", () => {
		...
		it("should handle maintenance errors gracefully", async () => {
			...
			expect(
				mocks.logger.hasLog("error").toBe("Error during maintenance"),
				true,
			);
		});
	...
	describe("task queue processing", () => {
		...
		it("should process tasks in priority order", async () => {
			...
			assertSpyCalls(mocks.coordinationManager.assignTask, 1);
			expect(mocks.coordinationManager.assignTask.calls[0].args[0].id).toBe(
				"high",
			);
		});
