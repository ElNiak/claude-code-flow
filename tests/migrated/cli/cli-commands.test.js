...
describe("CLI Commands - Comprehensive Tests", () => {
	let cli: MockCLI;
	let tempDir: string;
	let fakeTime: jest.useFakeTimers;

	beforeEach(async () => {
		setupTestEnv();
		cli = new MockCLI();
		tempDir = await FileSystemTestUtils.createTempDir("cli-test-");
		fakeTime = new jest.useFakeTimers();
	...
	describe("Agent Command", () => {
		...
		it("should handle unknown actions", async () => {
			...
			expect(exitCode).toBe(1);
			expect(output.stderr.join(" ")).toContain("Unknown action: unknown");
		});
	});
	...
	describe("Task Command", () => {
		...
		it("should require command for run action", async () => {
			...
			expect(exitCode).toBe(1);
			expect(output.stderr.join(" ")).toContain("Command is required");
		});
		...
		it("should require task ID for cancel action", async () => {
			...
			expect(exitCode).toBe(1);
			expect(output.stderr.join(" ")).toContain("Task ID is required");
		});
	});
	...
	describe("Global Options and Help", () => {
		...
		it("should handle invalid options", async () => {
			...
			expect(exitCode).toBe(1);
			expect(output.stderr.join(" ")).toContain("Unknown option: invalid-option",
			);
		});
	});
