...
describe("Init Command Performance Tests", () => {
    ...
    const child = spawn('deno',
			...
			testDir = await Deno.makeTempDir({
				prefix: "claude_flow_perf_test_full_",
			});
			Deno.chdir(testDir);
		...
		it("should handle multiple concurrent inits efficiently", async () => {
			...
			for (let i = 0; i < numConcurrent; i++) {
				const dir = await Deno.makeTempDir({
					prefix: `claude_flow_concurrent_${i}_`,
				});
				testDirs.push(dir);
			}
    ...
    const child = spawn('deno',
			...
			console.log(
				`${numConcurrent} concurrent inits completed in ${duration.toFixed(2)}ms`,
			);
		});
	});
	...
	describe("Large project initialization", () => {
		it("should handle initialization in directory with many files", async () => {
			...
			for (let i = 0; i < 10; i++) {
				...
				await fs.promises.writeFile(join(testDir, `dir_${i}`, "file.txt"),
					`Dir ${i} content`,
				);
			}
    ...
    const child = spawn('deno',
			...
			console.log(
				`Init in large project (${numFiles} files) completed in ${duration.toFixed(2)}ms`,
			);
		});
	});
    ...
    const child = spawn('deno',
		...
		it("should handle force overwrite efficiently", async () => {
			...
			await fs.promises.writeFile(join(testDir, "CLAUDE.md"),
				"existing content".repeat(1000),
			);
			await fs.promises.writeFile(join(testDir, "memory-bank.md"),
				"existing memory".repeat(1000),
			);
			await fs.promises.writeFile(join(testDir, "coordination.md"),
				"existing coord".repeat(1000),
			);
    ...
    const child = spawn('deno',
			...
			console.log(
				`Created ${createdDirs} directories in ${duration.toFixed(2)}ms (${dirCreationRate.toFixed(2)} dirs/sec)`,
			);
		});
    ...
    const child = spawn('deno',
			...
			console.log(
				`Created ${createdFiles} files (${totalSize} bytes) in ${duration.toFixed(2)}ms (${(writeRate / 1024).toFixed(2)} KB/sec)`,
			);
		});
	});
});
