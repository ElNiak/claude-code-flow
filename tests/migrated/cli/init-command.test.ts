...
describe("Init Command Unit Tests", () => {
	...
	beforeEach(async () => {
		...
		Deno.env.set("ORIGINAL_CWD", process.cwd());
	...
	describe("Basic initialization", () => {
		...
		it("should create minimal structure with --minimal flag", async () => {
			...
			const claudeMd = await fs.promises.readFile(join(testDir, "CLAUDE.md", 'utf8'));
			const memoryBankMd = await fs.promises.readFile(join(testDir, "memory-bank.md", 'utf8'),
			);
	...
	describe("Error handling", () => {
		it("should handle directory creation errors gracefully", async () => {
			...
			await fs.promises.writeFile(join(testDir, "memory"), "not a directory");
	...
	describe("Flag combinations", () => {
		...
		it("should handle --sparc --force combination", async () => {
			// Create existing files
			await fs.promises.writeFile(join(testDir, "CLAUDE.md"), "old content");
			await fs.promises.writeFile(join(testDir, ".roomodes"), "old roomodes");
