...
describe("Init Command Validation Tests", () => {
	...
	beforeEach(async () => {
		...
		testDir = await Deno.makeTempDir({
			prefix: "claude_flow_validation_test_",
		});
    ...
    const child = spawn('deno',
			...
			if (await exists(workflowPath)) {
				const workflowContent = await fs.promises.readFile(workflowPath, 'utf8');
    ...
    const child = spawn('deno',
			...
			expect(
				await exists(join(testDir, "coordination/memory_bank").toBeDefined()),
			);
			expect(
				await exists(join(testDir, "coordination/subtasks").toBeDefined()),
			);
			expect(
				await exists(join(testDir, "coordination/orchestration").toBeDefined()),
			);
		});
    ...
    const child = spawn('deno',
			...
			const agentsReadme = await fs.promises.readFile(join(testDir, "memory/agents/README.md", 'utf8'),
			);
			...
			const sessionsReadme = await fs.promises.readFile(join(testDir, "memory/sessions/README.md", 'utf8'),
			);
    ...
    const child = spawn('deno',
			...
			const dataContent = await fs.promises.readFile(join(testDir, "memory/claude-flow-data.json", 'utf8'),
			);
			const data = JSON.parse(dataContent);
