...
describe("Init Command Rollback Tests", () => {
	...
	describe("SPARC initialization rollback", () => {
		...
		it("should handle partial SPARC structure creation", async () => {
			...
			await fs.promises.writeFile(join(testDir, ".roo"), "not a directory");
	...
	describe("State consistency after failures", () => {
		it("should maintain clean state after file write failure", async () => {
			...
			await fs.promises.writeFile(join(testDir, "existing.txt"),
				"existing content",
			);
    ...
    const child = spawn('deno',
		...
		it("should handle concurrent access conflicts", async () => {
			// Start first init
			const command1 = await new Promise((resolve, reject) => {
    const child = spawn('deno',
				args: [
					"run",
					"--allow-all",
			...
			const command2 = await new Promise((resolve, reject) => {
    const child = spawn('deno',
				args: [
					"run",
					"--allow-all",
	...
	describe("Recovery mechanisms", () => {
		it("should be able to recover from partial init with --force", async () => {
			...
			await fs.promises.writeFile(join(testDir, "CLAUDE.md"), "partial content");
    ...
    const child = spawn('deno',
			...
			const tempFiles = afterFiles.filter(
				(name) =>
					name.includes("tmp") ||
					name.includes("temp") ||
