	...
	recordEvent(event, data = {}) {
		...
		switch (event) {
			...
			case "resource:acquired":
				...
				this.metrics.resources.free = Math.max(
					0,
					this.metrics.resources.free - 1,
				);
				break;

			case "resource:released":
				this.metrics.resources.free++;
				this.metrics.resources.locked = Math.max(
					0,
	...
	getMetrics() {
		...
		return {
			...
			tasks: {
				...
				successRate:
					this.metrics.tasks.total > 0
						? (
								(this.metrics.tasks.completed / this.metrics.tasks.total) *
								100
							).toFixed(2) + "%"
						: "0%",
				...
				throughput:
					this.metrics.tasks.total > 0
						? (this.metrics.tasks.total / (uptime / 1000)).toFixed(2) +
							" tasks/sec"
						: "0 tasks/sec",
			},
			...
			agents: {
				...this.metrics.agents,
				utilization:
					this.metrics.agents.total > 0
						? (
								(this.metrics.agents.busy / this.metrics.agents.total) *
								100
							).toFixed(2) + "%"
						: "0%",
			},
			...
			resources: {
				...
				utilization:
					this.metrics.resources.total > 0
						? (
								(this.metrics.resources.locked / this.metrics.resources.total) *
								100
							).toFixed(2) + "%"
						: "0%",
			},
			...
			coordination: {
				...
				messageLatency:
					this.metrics.coordination.messagesReceived > 0
			...
			performance: {
				...
				errorRate:
					this.metrics.performance.errors > 0
						? (this.metrics.performance.errors / (uptime / 1000 / 60)).toFixed(
								2,
							) + " errors/min"
						: "0 errors/min",
...
class PerformanceTestSuite {
	...
	async runTests() {
		...
		const tests = [
			...
			{ name: "Resource Contention Test", fn: this.testResourceContention },
			{
				...
				fn: this.testPerformanceCalculations,
			},
		];
	...
	async testBasicMetrics() {
		...
		this.collector.recordEvent("agent:spawned", {
			agentId: "agent-1",
			type: "coordinator",
		});
		this.collector.recordEvent("resource:acquired", {
			resourceId: "db-conn-1",
		});

		await this.delay(50);

		this.collector.recordEvent("task:completed", { taskId: "task-1" });
		this.collector.recordEvent("resource:released", {
			resourceId: "db-conn-1",
		});
		...
		console.log(
			`    ✓ Collected metrics: ${metrics.tasks.total} tasks, ${metrics.agents.total} agents`,
		);
	...
	async testSimpleScenario() {
		...
		for (let i = 0; i < scenario.tasks.length; i++) {
			...
			if (Math.random() > 0.05) {
				...
				this.collector.recordEvent("task:failed", {
					taskId,
					reason: "complexity",
				});
			}
		...
		console.log(
			`    ✓ Simple scenario: ${metrics.tasks.total} tasks in ${scenarioTime.toFixed(2)}ms`,
		);
		console.log(
			`      Success rate: ${metrics.tasks.successRate}, Throughput: ${metrics.tasks.throughput}`,
		);
	...
	async testComplexScenario() {
		...
		console.log(
			`    ✓ Complex scenario: ${metrics.tasks.total} tasks, ${metrics.agents.total} agents in ${scenarioTime.toFixed(2)}ms`,
		);
		console.log(
			`      Success rate: ${metrics.tasks.successRate}, Messages: ${metrics.coordination.messagesSent}, Conflicts: ${metrics.coordination.conflicts}`,
		);
	...
	async testHighLoadScenario() {
		...
		for (let i = 0; i < taskCount; i++) {
			taskPromises.push(
				this.executeTask(`load-task-${i}`, ["high", "medium", "low"][i % 3]),
			);
		}
		...
		console.log(
			`    ✓ High load: ${metrics.tasks.total} tasks, ${metrics.agents.total} agents in ${loadTime.toFixed(2)}ms`,
		);
		console.log(
			`      Throughput: ${metrics.tasks.throughput}, Agent utilization: ${metrics.agents.utilization}`,
		);
	...
	async testResourceContention() {
		...
		console.log(
			`    ✓ Resource contention: ${contentionEvents.length} contention events, ${metrics.coordination.conflicts} conflicts detected`,
		);
	...
	async testPerformanceCalculations() {
		...
		console.log(
			`    ✓ Performance calculations: Memory: ${metrics.performance.memoryUsage.toFixed(2)}MB`,
		);
		console.log(
			`      Avg duration: ${metrics.tasks.avgDuration}, Error rate: ${metrics.performance.errorRate}`,
		);
	...
	generateReport() {
		...
		this.testResults.forEach((result, index) => {
			...
			Object.entries(result).forEach(([key, value]) => {
				if (key !== "test" && key !== "status" && key !== "metrics") {
					console.log(
						`   ${key.charAt(0).toUpperCase() + key.slice(1)}: ${typeof value === "object" ? JSON.stringify(value, null, 2) : value}`,
					);
				}
			});
		...
		console.log(
			`Tasks: ${finalMetrics.tasks.total} total, ${finalMetrics.tasks.completed} completed, success rate: ${finalMetrics.tasks.successRate}`,
		);
		console.log(
			`Agents: ${finalMetrics.agents.total} total, utilization: ${finalMetrics.agents.utilization}`,
		);
		console.log(
			`Resources: ${finalMetrics.resources.total} total, utilization: ${finalMetrics.resources.utilization}`,
		);
		console.log(
			`Coordination: ${finalMetrics.coordination.messagesSent} messages sent, ${finalMetrics.coordination.conflicts} conflicts`,
		);
		console.log(
			`Performance: ${finalMetrics.performance.memoryUsage.toFixed(2)}MB memory, ${finalMetrics.performance.errorRate}`,
		);
		...
		if (passedTests === totalTests) {
			console.log(
				"🎉 All performance metrics features validated successfully!",
			);
			console.log(
				"✅ The Hive Mind performance tracking system is working correctly",
			);
			console.log(
				"✅ Metrics collection, aggregation, and calculations are functioning",
			);
