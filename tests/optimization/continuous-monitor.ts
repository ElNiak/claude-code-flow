...
class ContinuousPerformanceMonitor extends EventEmitter {
	...
	private thresholds: ThresholdConfig[] = [
		// CLI Performance
		{
			metric_name: "cli_init_time",
			warning_threshold: 400,
			critical_threshold: 600,
			comparison_operator: "greater_than",
		},
		{
			metric_name: "cli_response_time",
	...
	async startMonitoring(intervalMs: number = 30000): Promise<void> {
		...
		this.isRunning = true;
		console.log(
			`Starting continuous performance monitoring (interval: ${intervalMs}ms)`,
		);
	...
	private async measureMemoryUsage(): Promise<void> {
		...
		if (previousMemoryMetrics.length >= 2) {
			...
			const newestReading =
				previousMemoryMetrics[previousMemoryMetrics.length - 1];
	...
	private async checkThresholds(): Promise<void> {
		for (const threshold of this.thresholds) {
			...
			// Check thresholds
			const exceedsCritical = this.exceedsThreshold(
				avgValue,
				threshold.critical_threshold,
				threshold.comparison_operator,
			);
			const exceedsWarning = this.exceedsThreshold(
				avgValue,
			...
			if (exceedsCritical) {
				this.recordAlert(
					...
					avgValue,
					threshold.critical_threshold,
					`Critical threshold exceeded for ${threshold.metric_name}`,
				...
				this.recordAlert(
					...
					avgValue,
					threshold.warning_threshold,
					`Warning threshold exceeded for ${threshold.metric_name}`,
					latestMetric.component,
				);
			}
		}
	...
	private recordAlert(
		...
		console.log(
			`[${level.toUpperCase()}] ${message} (Current: ${currentValue.toFixed(2)}, Threshold: ${thresholdValue})`,
		);
	}
	...
	public async generateReport(): Promise<void> {
		...
		const report = {
			...
			optimization_status: {
				...
				database_performance: this.assessOptimizationStatus(
					"db_query_time",
					3.75,
				), // Target: 3.75ms
				agent_performance: this.assessOptimizationStatus(
					"agent_spawn_time",
					50,
				), // Target: 50ms
				memory_efficiency: this.assessOptimizationStatus(
					"memory_usage_mb",
					100,
				), // Target: reasonable usage
			},
		...
		const reportPath = join(
			__dirname,
			"../../tests/results/continuous-monitoring-report.json",
		);
	...
	private generateRecommendations(): string[] {
		...
		if (criticalAlerts.length > 0) {
			recommendations.push(
				"Immediate attention required: Critical performance thresholds exceeded",
			);
		}

		if (warningAlerts.length > 3) {
			recommendations.push(
				"Multiple warning thresholds exceeded - investigate performance trends",
			);
		}
		...
		if (agentMetrics.length > 0) {
			const avgSpawnTime =
				agentMetrics.reduce((sum, m) => sum + m.value, 0) / agentMetrics.length;
			if (avgSpawnTime > 75) {
				recommendations.push(
					"Agent spawn times are above optimal - review batching strategies",
				);
			}
		}
...
if (require.main === module) {
	async function runMonitor() {
		...
		monitor.on("monitoring_started", (data) => {
			console.log(
				`✓ Monitoring started at ${new Date(data.timestamp).toISOString()}`,
			);
		});
