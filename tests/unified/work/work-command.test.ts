...
import type { WorkOptions, TaskAnalysis, WorkPreset } from './types.js';
...
describe('WorkCommand', () => {
  ...
  describe('createCommand', () => {
    it('should create a command with correct name and description', () => {
      ...
      expect(command.getDescription()).toContain('Unified intelligent work command');
    });
  ...
  describe('intelligent analysis', () => {
    ...
    it('should handle complex multi-component tasks', async () => {
      const task = 'build a full-stack application with API, database, authentication, and frontend';
      const params: string[] = [];
      ...
      const mockAnalyze = vi.fn().mockResolvedValue({
        ...
        suggestedTopology: 'hierarchical',
        suggestedStrategy: 'parallel',
        requiredResources: ['code_generation', 'testing', 'database', 'authentication', 'frontend']
      });
  ...
  describe('error handling', () => {
    ...
    it('should handle configuration loading failures', async () => {
      const mockLoadDefaults = vi.fn().mockRejectedValue(new Error('Config failed'));
      mockConfigManager.mockImplementation(() => ({
