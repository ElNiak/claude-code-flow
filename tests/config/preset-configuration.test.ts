...
import { ConfigManager, Config, ConfigError } from '../../src/config/config-manager.js';
...
describe('Preset Configuration System', () => {
  ...
  describe('Research Preset', () => {
    test('should load research preset with correct configuration', async () => {
      ...
      const researchConfig = JSON.parse(await fs.readFile(researchPresetPath, 'utf8'));
  ...
  describe('Deployment Preset', () => {
    test('should load deployment preset with correct configuration', async () => {
      ...
      const deploymentConfig = JSON.parse(await fs.readFile(deploymentPresetPath, 'utf8'));
    ...
    test('should include deployment-specific workflows', async () => {
      ...
      expect(deploymentConfig.workflows.performanceMonitoring.enabled).toBe(true);
    });
  ...
  describe('Preset Validation', () => {
    test('should validate preset structure', async () => {
      ...
      for (const preset of presets) {
        const presetPath = path.join(process.cwd(), `config/presets/${preset}.json`);
  ...
  describe('Preset Loading and Merging', () => {
    ...
    test('should validate merged configuration', async () => {
      ...
      await fs.writeFile(tempConfigPath, JSON.stringify(invalidPreset, null, 2));
      
      await expect(configManager.load(tempConfigPath)).rejects.toThrow(ConfigError);
    });
  });
