...
describe('Agent Capabilities System', () => {
  ...
  beforeEach(async () => {
    const capabilitiesPath = path.join(process.cwd(), 'config/agent-capabilities.json');
  ...
  describe('Agent Type Definitions', () => {
    test('should have all required agent types defined', () => {
      const requiredAgents = [
        ...
        'researcher', 'analyst', 'deployer', 'coordinator'
      ];
      ...
      requiredAgents.forEach(agentType => {
        ...
        expect(agentCapabilities.agentTypes[agentType].description).toBeDefined();
        expect(agentCapabilities.agentTypes[agentType].capabilities).toBeDefined();
      });
    });
    ...
    test('should have resource requirements for each agent', () => {
      Object.values(agentCapabilities.agentTypes).forEach((agent: any) => {
        ...
        expect(agent.resourceRequirements.memory).toMatch(/^(low|medium|high)$/);
        expect(agent.resourceRequirements.cpu).toMatch(/^(low|medium|high)$/);
        expect(agent.resourceRequirements.network).toMatch(/^(low|medium|high)$/);
      });
    });
  ...
  describe('Agent Selection Rules', () => {
    ...
    test('should have project type mappings', () => {
      const projectTypeMappings = agentCapabilities.agentSelectionRules.byProjectType;
    ...
    test('should have workflow phase mappings', () => {
      const workflowPhaseMappings = agentCapabilities.agentSelectionRules.byWorkflowPhase;
  ...
  describe('Adaptive Rules', () => {
    test('should have project size rules', () => {
      ...
      expect(projectSizeRules.small.maxAgents).toBeLessThan(projectSizeRules.medium.maxAgents);
      expect(projectSizeRules.medium.maxAgents).toBeLessThan(projectSizeRules.large.maxAgents);
    });
    ...
    test('should have complexity rules', () => {
      ...
      expect(complexityRules.low.agentTypes.length).toBeLessThan(complexityRules.medium.agentTypes.length);
      expect(complexityRules.medium.agentTypes.length).toBeLessThan(complexityRules.high.agentTypes.length);
    });

    test('should have time constraint rules', () => {
      const timeConstraintRules = agentCapabilities.adaptiveRules.timeConstraints;
  ...
  describe('Capability Consistency', () => {
    ...
    test('should have unique agent names', () => {
      const agentNames = Object.values(agentCapabilities.agentTypes).map((agent: any) => agent.name);
  ...
  describe('Integration with Presets', () => {
    test('should align with development preset agent preferences', async () => {
      const devPresetPath = path.join(process.cwd(), 'config/presets/development.json');
    ...
    test('should align with research preset agent preferences', async () => {
      const researchPresetPath = path.join(process.cwd(), 'config/presets/research.json');
      const researchPreset = JSON.parse(await fs.readFile(researchPresetPath, 'utf8'));
    ...
    test('should align with deployment preset agent preferences', async () => {
      const deploymentPresetPath = path.join(process.cwd(), 'config/presets/deployment.json');
      const deploymentPreset = JSON.parse(await fs.readFile(deploymentPresetPath, 'utf8'));
