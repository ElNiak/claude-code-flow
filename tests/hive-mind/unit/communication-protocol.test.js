...
describe("Communication Protocol", () => {
	...
	describe("Broadcasting", () => {
		it("should broadcast to all agents except sender", () => {
			...
			expect(handlers.agent2).toHaveBeenCalledWith(
				expect.objectContaining({
					content: "Broadcast message",
					type: MESSAGE_TYPES.BROADCAST,
				}),
			);
			expect(handlers.agent3).toHaveBeenCalledWith(
				expect.objectContaining({
					content: "Broadcast message",
					type: MESSAGE_TYPES.BROADCAST,
				}),
	...
	describe("Message History", () => {
		...
		it("should filter history by type", () => {
			...
			const requests = channel.getMessageHistory({
	...
	describe("Request/Response Protocol", () => {
		it("should handle request-response pattern", async () => {
			...
			const result = await protocol.request("agent-1", "agent-2", "calculate", {
				a: 5,
				b: 3,
			});
