...
const AGENT_TYPES = {
	...
	coder: {
		...
		defaultPrompt:
			"You are a software developer focused on writing clean, efficient code.",
	...
	architect: {
		...
		defaultPrompt:
			"You are a system architect focused on design and structure.",
		resourceLimits: { memory: "512MB", cpu: 0.5 },
	},
	tester: {
...
class AgentFactory {
	...
	validateAgentType(type) {
		if (!AGENT_TYPES[type]) {
			throw new Error(
				`Invalid agent type: ${type}. Valid types: ${Object.keys(AGENT_TYPES).join(", ")}`,
			);
		}
	}
	...
	validateResources(type, count = 1) {
		...
		const requiredMemory =
			this.parseMemory(agentConfig.resourceLimits.memory) * count;
		const requiredCpu = agentConfig.resourceLimits.cpu * count;

		const availableMemory =
			this.parseMemory(this.resourceUsage.totalMemory) -
			this.resourceUsage.usedMemory;
		const availableCpu =
			this.resourceUsage.totalCpu - this.resourceUsage.usedCpu;
		...
		if (requiredCpu > availableCpu) {
			throw new Error(
				`Insufficient CPU: required ${requiredCpu}, available ${availableCpu}`,
			);
		}
	...
	createAgent(type, name, options = {}) {
		...
		this.resourceUsage.usedMemory += this.parseMemory(
			agentConfig.resourceLimits.memory,
		);
...
describe("Agent Spawning", () => {
	...
	describe("Agent Type Validation", () => {
		...
		it("should reject unknown agent types", () => {
			expect(() => factory.validateAgentType("invalid")).toThrow(
				/Invalid agent type/,
			);
			expect(() => factory.validateAgentType("")).toThrow(/Invalid agent type/);
			expect(() => factory.validateAgentType(null)).toThrow(
				/Invalid agent type/,
			);
		});
	...
	describe("Resource Management", () => {
		...
		it("should prevent over-allocation of memory", () => {
			...
			expect(() => factory.validateResources("coder", 2)).toThrow(
				/Insufficient memory/,
			);
		});
		...
		it("should prevent over-allocation of CPU", () => {
			...
			expect(() => factory.validateResources("researcher", 1)).toThrow(
				/Insufficient CPU/,
			);
		});
	...
	describe("Single Agent Creation", () => {
		...
		it("should create agent with custom ID", () => {
			const agent = factory.createAgent("coder", "CustomBot", {
				id: "custom-001",
			});
	...
	describe("Batch Spawning", () => {
		...
		it("should spawn mixed agent types", () => {
			...
			expect(
				result.spawned.filter((a) => a.type === "researcher"),
			).toHaveLength(2);
			expect(result.spawned.filter((a) => a.type === "coder")).toHaveLength(3);
			expect(result.spawned.filter((a) => a.type === "analyst")).toHaveLength(
				1,
			);
		});
	...
	describe("Agent Lifecycle", () => {
		it("should update agent properties", () => {
			...
			expect(updated.metadata.lastActive).toBeGreaterThan(
		...
		it("should handle errors gracefully", () => {
			expect(() => factory.updateAgent("non-existent", {})).toThrow(
				/Agent not found/,
			);
			expect(() => factory.removeAgent("non-existent")).toThrow(
				/Agent not found/,
			);
		});
	});
	...
	describe("Agent Capabilities", () => {
		it("should assign type-specific capabilities", () => {
			...
			expect(agents[1].capabilities).toEqual([
				"implement",
				"debug",
				"refactor",
				"test",
			]);
			expect(agents[2].capabilities).toEqual([
				"evaluate",
				"optimize",
				"benchmark",
				"report",
			]);
			expect(agents[3].capabilities).toEqual([
				"design",
				"plan",
				"structure",
				"document",
			]);
			expect(agents[4].capabilities).toEqual([
				"test",
				"validate",
				"verify",
				"qa",
			]);
			expect(agents[5].capabilities).toEqual([
				"orchestrate",
				"delegate",
				"monitor",
				"report",
		...
		it("should allow custom prompts", () => {
			const agent = factory.createAgent("researcher", "R1", {
				prompt:
					"You are a specialized ML researcher focusing on neural networks.",
			});
