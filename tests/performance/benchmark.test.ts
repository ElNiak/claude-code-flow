...
describe("Performance Benchmark Tests", () => {
	...
	describe("System Initialization Performance", () => {
		...
		it("should handle multiple rapid initializations", async () => {
			...
			console.log(
				`Average init time: ${averageTime.toFixed(2)}ms, Max: ${maxTime.toFixed(2)}ms`,
			);
		});
	});
	...
	describe("Agent Management Performance", () => {
		...
		it("should handle agent communication efficiently", async () => {
			// Spawn test agents
			const agent1 = await agentManager.spawnAgent("coordinator", {
				name: "Coordinator",
			});
			const agent2 = await agentManager.spawnAgent("researcher", {
				name: "Researcher",
			});
		...
		it("should scale agent listing efficiently", async () => {
			...
			console.log(
				`Agent listing: avg ${averageTime.toFixed(2)}ms, max ${maxTime.toFixed(2)}ms`,
			);
		});
	});
