...
class ValidationCheckpoint {
	...
	static compareProgress(previousCount, currentCount) {
		...
		if (diff > 0) {
			console.log(
				`✅ PROGRESS: ${diff} error(s) fixed! (${previousCount} → ${currentCount})`,
			);
			return "IMPROVED";
		} else if (diff < 0) {
			console.log(
				`❌ REGRESSION: ${Math.abs(diff)} new error(s)! (${previousCount} → ${currentCount})`,
			);
			return "REGRESSED";
		} else {
	...
	static validateCriticalFiles() {
		...
		criticalFiles.forEach((file) => {
			const result = ValidationCheckpoint.validateFile(file);
			results[file] = result;
	...
	switch (command) {
		...
		case "compare": {
			...
			if (isNaN(prev) || isNaN(curr)) {
				console.log(
					"Usage: node validation-checkpoint.js compare <previousCount> <currentCount>",
				);
				process.exit(1);
			}
		...
		default:
			...
			console.log("Usage:");
			console.log(
				"  node validation-checkpoint.js quick        # Quick error count",
			);
			console.log(
				"  node validation-checkpoint.js file <path>  # Validate specific file",
			);
			console.log(
				"  node validation-checkpoint.js critical     # Validate critical files",
			);
			console.log(
				"  node validation-checkpoint.js compare <prev> <curr>  # Compare progress",
			);
	}
}
