...
function parseUnusedVariables(lintOutput: string): UnusedVariable[] {
	...
	for (const line of lines) {
		...
		const match = line.match(
			/^(.+?):(\d+):(\d+)\s+warning\s+'([^']+)' is (?:defined but never used|assigned a value but never used|never reassigned)/,
		);
		if (match) {
			...
			// Determine type
			let type: "variable" | "import" | "argument" | "destructured" =
				"variable";
	...
	for (const [fileName, variables] of fileGroups) {
		...
		for (const uv of sortedVars) {
			...
			const patterns = [
				// Import statements
				{
					from: `import { ${uv.variable}`,
					to: `import { ${uv.variable} as ${prefixedVar}`,
				},
		...
		if (newContent !== content) {
			...
			console.log(
				`Fixed ${variables.length} unused variables in: ${path.relative(process.cwd(), fullPath)}`,
			);
		}
	}
}
