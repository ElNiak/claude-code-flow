...
class DebugLoggingInjector {
	...
	async run(): Promise<void> {
		...
		console.log(
			`✅ Debug logging injection complete! Processed ${this.processedFiles.size} files.`,
		);
	}
	...
	private async processFile(filePath: string): Promise<void> {
		...
		try {
			...
			if (modifiedContent !== content) {
				...
				console.log(
					`✅ Updated: ${filePath} (${fileInfo.functions.length} functions)`,
				);
			} else {
	...
	private shouldSkipFunction(
		...
		if (
			content.includes("debugLogger.logFunctionEntry") ||
	...
	private injectDebugLogging(fileInfo: FileInfo): string {
		...
		const sortedFunctions = fileInfo.functions.sort(
	...
	private injectFunctionLogging(
		...
		const argsArray =
			func.params.length > 0
				? `[${func.params.map((p) => p.split(":")[0].trim()).join(", ")}]`
				: "[]";
