...
async function basicMigration() {
	...
	if (analysis.hasClaudeFolder) {
		...
		console.log(
			chalk.yellow("   ! No .claude folder found - fresh installation"),
		);
	}
...
async function rollbackExample() {
	...
	if (backups.length > 0) {
		...
		console.log(
			`   Would rollback to: ${latestBackup.timestamp.toLocaleString()}`,
		);
...
async function batchMigrationExample() {
	...
	for (const [index, projectPath] of projects.entries()) {
		...
		try {
			...
			const strategy =
				analysis.customCommands.length > 0 ? "merge" : "selective";
...
async function conflictResolutionExample() {
	...
	if (analysis.conflictingFiles.length > 0) {
		console.log(
			chalk.yellow(
				`   Found ${analysis.conflictingFiles.length} conflicting files:`,
			),
		);
