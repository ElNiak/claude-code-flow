...
const versionFiles = [
	...
	{
		path: "src/cli/simple-commands/status.js",
		pattern: /version: '[^']+'/g,
		replacement: `version: '${version}'`,
	},
	{
		path: "src/cli/simple-commands/config.js",
		pattern: /version: "[^"]+"/g,
		replacement: `version: "${version}"`,
	},
	{
		path: "src/cli/simple-commands/process-ui-enhanced.js",
		pattern: /Claude-Flow Process Management UI v[0-9.]+/,
		replacement: `Claude-Flow Process Management UI v${version}`,
	},
	{
...
const distFiles = ["dist/cli/simple-cli.js", "dist/cli/index.js"];
	...
	if (fs.existsSync(fullPath)) {
		...
		if (versionMatch) {
			if (versionMatch[1] === version) {
				...
				console.log(
					`   ❌ ${distFile}: ${versionMatch[1]} (expected ${version})`,
				);
			}
		} else {
