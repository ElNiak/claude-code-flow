...
async function runTests(
	...
	for (const suite of suites) {
		console.log(
			`📋 Running ${suite.name} (${suite.type}): ${suite.description}`,
		);
  ...
  ./scripts/test-init-command.ts --coverage         # Run with coverage
	...
	if (args.list) {
		...
		for (const suite of TEST_SUITES) {
			console.log(
				`  ${suite.name.padEnd(20)} (${suite.type.padEnd(11)}) - ${suite.description}`,
			);
		}
		return;
	}
	...
	if (args.suite) {
		const requestedSuite = suitesToRun.find(
			(suite) => suite.name === args.suite,
		);
		if (!requestedSuite) {
