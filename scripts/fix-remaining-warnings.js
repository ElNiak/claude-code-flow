...
const SPECIFIC_FIXES = [
  ...
  },
  {
    ...
    fixes: [
      { pattern: /const\s+data\s*=\s*task\.context\?\.data;/g, replacement: 'const _data = task.context?.data;' },
      { pattern: /const\s+benchmark\s*=\s*task\.context\?\.benchmark;/, replacement: 'const _benchmark = task.context?.benchmark;' }
    ]
  },
  {
    file: 'src/cli/agents/architect.ts',
    fixes: [
      { pattern: /\bMicroserviceComponent\b(?=\s*,)/, replacement: '_MicroserviceComponent' },
      { pattern: /\bInfrastructureComponent\b(?=\s*,)/, replacement: '_InfrastructureComponent' }
    ]
  },
  {
    file: 'src/cli/agents/base-agent.ts',
    fixes: [
      { pattern: /import\s+\{\s*generateId\s*\}/, replacement: 'import { generateId as _generateId }' }
    ]
  },
  {
    file: 'src/cli/agents/coder.ts',
    fixes: [
      { pattern: /const\s+code\s*=\s*task\.context\?\.code;/g, replacement: 'const _code = task.context?.code;' },
      { pattern: /const\s+spec\s*=\s*task\.context\?\.spec;/, replacement: 'const _spec = task.context?.spec;' },
      { pattern: /const\s+requirements\s*=\s*task\.context\?\.requirements;/, replacement: 'const _requirements = task.context?.requirements;' }
    ]
  }
];
