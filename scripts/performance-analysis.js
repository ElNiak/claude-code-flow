  ...
  generateRecommendations() {
    ...
    if (this.results.startupTime.average > 500) {
      recs.push({
        ...
        recommendation: 'Consider lazy loading heavy dependencies and command modules'
      });
    }

    // Import recommendations
    for (const [file, analysis] of Object.entries(this.results.importAnalysis)) {
      if (analysis.heavyImports?.length > 0) {
        recs.push({
          category: 'Heavy Imports',
          issue: `${file} imports heavy packages: ${analysis.heavyImports.join(', ')}`,
          recommendation: 'Lazy load these packages only when their commands are used'
        });
      }
    ...
    for (const [cmd, timing] of Object.entries(this.results.commandExecutionTime)) {
  ...
  async generateReport() {
    ...
    const report = {
      ...
      summary: {
        ...
        criticalIssues: this.results.recommendations.filter(r =>
          r.category === 'Startup Time' || r.category === 'Memory Usage'
        ).length
      },
...
`;
  ...
  async run() {
    ...
    // Display summary
    console.log('\n📊 Summary:');
    console.log(`  Average startup time: ${report.summary.averageStartupTime.toFixed(2)}ms`);
