...
async function fixLintingIssues() {
  ...
  for (const file of files) {
    ...
    if (newContent.includes('import { getErrorMessage } from')) {
      ...
      const usesGetErrorMessage = newContent.includes('getErrorMessage(') || 
                                 newContent.includes('getErrorMessage ') ||
                                 (newContent.match(/getErrorMessage/g) || []).length > 1;
      
      if (!usesGetErrorMessage) {
        // Remove the entire import line
        newContent = newContent.replace(/^import \{ getErrorMessage \} from ['"]\.\.[^'"]*error-handler(?:\.js)?['"];\s*\n/gm, '');
        fileChanges++;
      }
    }
    ...
    newContent = newContent.replace(
      ...
      (match, paramName, paramType, offset, string) => {
        ...
        if (functionStart !== -1 && functionEnd !== -1) {
          ...
          const isUsed = functionBody.includes(paramName) && 
                        functionBody.indexOf(paramName) !== functionBody.lastIndexOf(paramName);
  ...
  try {
    ...
    console.log('Still have some linting issues, but major ones should be fixed.');
  }
}
