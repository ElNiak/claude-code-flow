...
class TestRunner {
	...
	generateReport() {
		...
		console.log(
			`   Success Rate: ${chalk.cyan(((passed / total) * 100).toFixed(1))}%`,
		);
		console.log(
			`   Total Time: ${chalk.yellow((totalTime / 1000).toFixed(2))}s`,
		);
		...
		results
			...
			.forEach((result) => {
				console.log(
					`   • ${chalk.green(result.name)}: ${(result.duration / 1000).toFixed(2)}s`,
				);
			});
