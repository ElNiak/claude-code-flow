...
const testSuites = {
	config: {
		...
		patterns: ["tests/config/**/*.test.ts", "tests/config/**/*.test.js"],
		timeout: 60000,
		critical: true,
	},
	unifiedCoordination: {
	...
	memoryIntegration: {
		...
		patterns: ["tests/unified-coordination/memory-integration.test.ts"],
		timeout: 90000,
		critical: true,
	},
	mcpHooksIntegration: {
		name: "MCP and Hooks Integration Tests",
		patterns: ["tests/unified-coordination/mcp-hooks-integration.test.ts"],
		timeout: 90000,
		critical: true,
	},
	performanceBenchmarks: {
		name: "Performance Benchmark Tests",
		patterns: ["tests/unified-coordination/performance-benchmarks.test.ts"],
		timeout: 180000,
		critical: false,
	},
	migrationValidation: {
		name: "Migration Validation Tests",
		patterns: ["tests/unified-coordination/migration-validation.test.ts"],
		timeout: 120000,
		critical: true,
	},
	endToEndWorkflows: {
		name: "End-to-End Workflow Tests",
		patterns: ["tests/unified-coordination/end-to-end-workflows.test.ts"],
		timeout: 300000,
		critical: false,
	},
	existing: {
...
function printResults(results) {
	...
	results.forEach((result) => {
		...
		printColored(
			`${status} ${result.name}: ${result.tests} tests, ${result.duration}ms`,
			color,
		);
...
async function runJest(patterns, options = {}) {
	return new Promise((resolve) => {
		const jestArgs = [
			"--passWithNoTests",
			"--testTimeout",
			(options.timeout || 60000).toString(),
			"--maxWorkers",
			testOptions.maxWorkers,
		];
...
async function validateSetup() {
	...
	if (missingFiles.length > 0) {
		...
		printColored(
			"\nPlease ensure all configuration files are properly set up.",
			"yellow",
		);
		return false;
	}
...
function printUsage() {
	...
	printColored(
		"Usage: npm run test:unified [options] [--suite=<suite-name>]\n",
		"bright",
	);
...
async function main() {
	...
	printColored(
		`\n🚀 Running ${Object.keys(suitesToRun).length} test suite(s)...`,
		"blue",
	);
	...
	if (testOptions.coverage) {
		printColored(
			"\n📊 Coverage report generated in coverage/unified-tests/",
			"blue",
		);
	}
	...
	if (criticalFailures.length > 0) {
		printColored(
			`\n❌ ${criticalFailures.length} critical test suite(s) failed`,
			"red",
		);
		process.exit(1);
	} else if (hasFailures) {
