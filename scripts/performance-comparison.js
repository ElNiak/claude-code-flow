...
class PerformanceComparison {
  ...
  calculateImprovements() {
    ...
    const origAvgStartup = origStartup.reduce((a, b) => a + b, 0) / origStartup.length;
    const optAvgStartup = optStartup.reduce((a, b) => a + b, 0) / optStartup.length;

    this.results.improvements.startup = {
      original: origAvgStartup,
      optimized: optAvgStartup,
      improvement: ((origAvgStartup - optAvgStartup) / origAvgStartup * 100).toFixed(2) + '%',
    ...
    for (const cmd of Object.keys(this.results.original.commands || {})) {
      ...
      if (orig && opt) {
        this.results.improvements.commands[cmd] = {
          ...
          improvement: ((orig.average - opt.average) / orig.average * 100).toFixed(2) + '%',
    ...
    if (this.results.original.memory && this.results.optimized.memory) {
      this.results.improvements.memory = {
        heapUsed: {
          ...
          improvement: (
            (this.results.original.memory.heapUsed - this.results.optimized.memory.heapUsed) /
            this.results.original.memory.heapUsed * 100
          ).toFixed(2) + '%',
        },
        rss: {
          ...
          improvement: (
            (this.results.original.memory.rss - this.results.optimized.memory.rss) /
            this.results.original.memory.rss * 100
          ).toFixed(2) + '%',
        },
      };
    }
  ...
  async generateReport() {
    ...
    const report = {
      ...
      summary: {
        startupImprovement: this.results.improvements.startup?.improvement || 'N/A',
        startupSpeedup: this.results.improvements.startup?.speedup || 'N/A',
        avgCommandImprovement: this.calculateAverageImprovement(),
        memoryReduction: this.results.improvements.memory?.heapUsed?.improvement || 'N/A',
      },
...
`;
    ...
    for (const [cmd, data] of Object.entries(this.results.improvements.commands || {})) {
...
`;
  ...
  async run() {
    ...
    try {
      ...
      console.error('⚠️ Optimized CLI not found. Running comparison with simulated optimizations...');
      ...
      for (const [cmd, data] of Object.entries(this.results.original.commands)) {
    ...
    console.log(`📄 Markdown report saved to: performance-comparison-report.md`);
    ...
    console.log(`  Average command improvement: ${report.summary.avgCommandImprovement}`);
