  ...
  private async fixUnusedVariablesInFile(filePath: string): Promise<FixResult> {
    ...
    newContent = newContent.replace(unusedImportPattern, (match, imports) => {
      const importList = imports.split(',').map((imp: string) => {
        ...
        if (trimmed && !trimmed.startsWith('_') && !trimmed.startsWith('type ')) {
          return `_${trimmed}`;
        }
        return trimmed;
      }).join(', ');
    ...
    const unusedParamPattern = /\b(function|async\s+function|\w+\s*:\s*\([^)]*\)|=>\s*\([^)]*\)|\([^)]*\)\s*=>\s*\{|\([^)]*\)\s*:\s*[^=]+\s*=>\s*\{)/g;
    
    // Fix unused variables by prefixing with underscore
    const unusedVarPattern = /\b(const|let|var)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*[=:]/g;
    newContent = newContent.replace(unusedVarPattern, (match, keyword, varName) => {
