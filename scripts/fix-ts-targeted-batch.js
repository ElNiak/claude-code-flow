...
async function fixSpecificIssues() {
	console.log(
		"🎯 Applying targeted fixes for remaining high-impact errors...\n",
	);
	...
	for (const file of chalkFiles) {
		try {
			...
			if (
				content.includes("chalk.") &&
				!content.includes("import chalk from 'chalk'")
			) {
	...
	const { stdout: commandFiles } = await execAsync(
		'find src/cli/commands -name "*.ts" -type f',
	);
...
`;
	...
	try {
		...
		if (
			!content.includes("ComponentStatus") &&
			content.includes("ComponentStatus")
		) {
			const lines = content.split("\n");
...
`;
			...
			if (
				content.includes("type TaskType") ||
				content.includes("enum TaskType")
			) {
				updated = updated.replace(
