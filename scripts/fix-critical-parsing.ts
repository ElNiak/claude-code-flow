...
class ParsingErrorFixer {
		...
		for (const file of files) {
			try {
				...
				if (result.fixes > 0) {
					...
					console.log(
						`  ✅ ${file.replace(Deno.cwd(), ".")}: ${result.fixes} fixes`,
					);
				}
			} catch (error) {
	...
	private async fixParsingErrorsInFile(filePath: string): Promise<FixResult> {
		...
		if (recordEntriesMatches) {
			...
			issues.push(
				`Fixed ${recordEntriesMatches.length} Record<string, any>.entries() calls`,
			);
		}
		...
		if (recordKeysMatches) {
			...
			issues.push(
				`Fixed ${recordKeysMatches.length} Record<string, any>.keys() calls`,
			);
		}
		...
		if (recordValuesMatches) {
			...
			issues.push(
				`Fixed ${recordValuesMatches.length} Record<string, any>.values() calls`,
			);
		}
		...
		if (malformedPropMatches) {
			...
			issues.push(
				`Fixed ${malformedPropMatches.length} malformed object properties`,
			);
		}
		...
		if (brokenImportMatches) {
			...
			issues.push(
				`Fixed ${brokenImportMatches.length} broken import statements`,
			);
		}
