...
class MCPValidator {
	static validateSwarmInit(params) {
		...
		if (params.maxAgents !== undefined) {
			if (
				typeof params.maxAgents !== "number" ||
				params.maxAgents < 1 ||
		...
		if (
			params.strategy &&
			!["balanced", "specialized", "adaptive", "parallel"].includes(
				params.strategy,
			)
		) {
			errors.push(
...
export class RobustMCPTools {
	...
	async initialize(ruvSwarmInstance = null) {
		return await this.errorHandler.wrapOperation(
			async () => {
				...
				if (
					ruvSwarmInstance &&
					!this.validateRuvSwarmInstance(ruvSwarmInstance)
				) {
	...
	async swarm_init(params = {}) {
		return await this.executeValidatedOperation(
			"swarm_init",
			params,
		...
		return await this.executeValidatedOperation(
			...
			async (validatedParams) => {
				...
				if (!resourceCheck.suitable) {
					console.warn(
						"⚠️ System resources may be insufficient for benchmarking:",
						resourceCheck.warnings,
					);
				}
	...
	async preOperationChecks(operationName) {
		...
		if (!this.baseMCP.ruvSwarm) {
			throw new MCPCommunicationError(
				"MCP tools not properly initialized",
				operationName,
			);
		}
		...
		if (!health.healthy && health.severity === "critical") {
			throw new MCPCommunicationError(
				`System health critical, cannot execute ${operationName}`,
				operationName,
				false,
			);
		}
	...
	async verifyNeuralCapabilities() {
		try {
			const features = await this.baseMCP.features_detect({
				category: "neural_networks",
			});
			if (!features.available) {
				throw new MCPCommunicationError(
					"Neural network capabilities not available",
					"neural_train",
					false,
				);
			}
			...
			throw new MCPCommunicationError(
				`Failed to verify neural capabilities: ${error.message}`,
				"neural_train",
				true,
			);
		}
	...
	async verifyAgentForNeuralTraining(agentId) {
		try {
			...
			if (agent.status === "error" || agent.status === "offline") {
				throw new MCPCommunicationError(
					`Agent ${agentId} is not available (status: ${agent.status})`,
					"neural_train",
					true,
				);
			}
			...
			throw new MCPCommunicationError(
				`Failed to verify agent ${agentId}: ${error.message}`,
				"neural_train",
				true,
			);
		}
	...
	async verifyTaskExists(taskId) {
		try {
			...
			throw new MCPCommunicationError(
				`Failed to verify task ${taskId}: ${error.message}`,
				"task_results",
				true,
			);
		}
	...
	async checkAgentAvailability(params) {
		try {
			...
			if (availableAgents.length === 0) {
				throw new MCPCommunicationError(
					"No idle agents available for task orchestration",
					"task_orchestrate",
					false,
				);
			}
			...
			if (availableAgents.length < maxAgents) {
				console.warn(
					`⚠️ Requested ${maxAgents} agents but only ${availableAgents.length} available`,
				);
			}
		} catch (error) {
			...
			throw new MCPCommunicationError(
				`Failed to check agent availability: ${error.message}`,
				"task_orchestrate",
				true,
			);
		}
	...
	validateNeuralTrainResult(result) {
		...
		if (
			typeof result.final_accuracy !== "number" ||
			result.final_accuracy < 0 ||
			result.final_accuracy > 1
		) {
	...
	setupEventHandlers() {
		this.errorHandler.on("error", (errorRecord) => {
			console.error(
				`🚨 MCP Error: ${errorRecord.operation} - ${errorRecord.error.message}`,
			);
		});

		this.errorHandler.on("criticalError", (errorRecord) => {
			console.error(
				`💥 Critical MCP Error: ${errorRecord.operation} - Manual intervention may be required`,
			);
		});
	...
	async agent_list(params = {}) {
		return await this.executeValidatedOperation(
			...
			params,
			async (p) =>
				await this.withTimeout(this.baseMCP.agent_list(p), 5000, "agent_list"),
		);
	}
