...
import {
	...
	ErrorSeverity,
} from "./error-handling-manager.js";
...
export class EnhancedSwarmPersistence {
	constructor(dbPath = null, options = {}) {
		...
		this.backupPath =
			options.backupPath ||
			path.join(path.dirname(dbPath || "./data/ruv-swarm.db"), "backups");
	...
	async initializePersistence() {
		try {
			await this.errorHandler.wrapOperation(
				async () => {
	...
	enableInMemoryMode() {
		...
		this.persistence = null;
		console.warn(
			"⚠️ Operating in in-memory mode - data will not persist across restarts",
		);
	}

	// Enhanced swarm operations with error handling
	async createSwarm(swarm) {
		return await this.errorHandler.wrapOperation(
			async () => {
	...
	async updateAgentStatus(agentId, status) {
		return await this.errorHandler.wrapOperation(
			async () => {
				...
				const transactionId = this.recordTransaction("update_agent_status", {
					agentId,
					status,
				});
	...
	async storeMemory(agentId, key, value, ttlSecs = null) {
		return await this.errorHandler.wrapOperation(
			async () => {
				...
				const transactionId = this.recordTransaction("store_memory", {
					agentId,
					key,
					ttlSecs,
				});
				...
				try {
					const result = this.persistence.storeMemory(
						agentId,
						key,
						value,
						ttlSecs,
		...
		return await this.errorHandler.wrapOperation(
			async () => {
				...
				if (memory && this.isCorruptedMemoryData(memory)) {
					console.warn(
						`Corrupted memory data detected for agent ${agentId}, key ${key}`,
					);
	...
	validateSwarmData(swarm) {
		...
		if (!swarm.id || typeof swarm.id !== "string") {
			throw new Error(
				"Invalid swarm data: id is required and must be a string",
			);
		}

		if (!swarm.name || typeof swarm.name !== "string") {
			throw new Error(
				"Invalid swarm data: name is required and must be a string",
			);
		}

		if (!swarm.topology || typeof swarm.topology !== "string") {
			throw new Error(
				"Invalid swarm data: topology is required and must be a string",
			);
		}

		if (typeof swarm.maxAgents !== "number" || swarm.maxAgents < 1) {
			throw new Error(
				"Invalid swarm data: maxAgents must be a positive number",
			);
		}
	}
	...
	validateAgentData(agent) {
		...
		if (!agent.id || typeof agent.id !== "string") {
			throw new Error(
				"Invalid agent data: id is required and must be a string",
			);
		}

		if (!agent.swarmId || typeof agent.swarmId !== "string") {
			throw new Error(
				"Invalid agent data: swarmId is required and must be a string",
			);
		}

		if (!agent.name || typeof agent.name !== "string") {
			throw new Error(
				"Invalid agent data: name is required and must be a string",
			);
		}

		if (!agent.type || typeof agent.type !== "string") {
			throw new Error(
				"Invalid agent data: type is required and must be a string",
			);
		}
	}
	...
	validateMemoryData(agentId, key, value) {
		if (!agentId || typeof agentId !== "string") {
			throw new Error(
				"Invalid memory data: agentId is required and must be a string",
			);
		}

		if (!key || typeof key !== "string") {
			throw new Error(
				"Invalid memory data: key is required and must be a string",
			);
		}
		...
		try {
			...
			throw new Error(
				"Invalid memory data: value contains circular references or is not serializable",
			);
		}
	}
	...
	commitTransaction(transactionId) {
		const transaction = this.transactionLog.find(
	...
	rollbackTransaction(transactionId) {
		...
		if (transaction) {
			...
			console.warn(
				`Transaction ${transactionId} rolled back: ${transaction.operation}`,
			);
		}
	}
	...
	async cleanupCorruptedMemory(agentId, key) {
		try {
			...
			console.log(
				`Cleaned up corrupted memory for agent ${agentId}, key ${key}`,
			);
		} catch (error) {
...
class DatabaseCorruptionDetector {
	async checkDatabaseIntegrity(dbPath) {
		try {
			...
			if (stats.size < 1024) {
				// Less than 1KB might indicate corruption
				console.warn(
					`Database file ${dbPath} is suspiciously small (${stats.size} bytes)`,
				);
			}
