...
import {
	...
	ErrorSeverity,
} from "../src/error-handling-manager.js";
	...
	() => {
		...
		describe("ErrorHandlingManager Core Functionality", () => {
			...
			test("should categorize error severity correctly", () => {
				...
				const validationRecord = errorHandler.createErrorRecord(
					validationError,
					{
		...
		describe("Enhanced Persistence Error Handling", () => {
			...
			test("should handle database connection failures gracefully", async () => {
				...
				const invalidPersistence = new EnhancedSwarmPersistence(
			...
			test("should validate swarm data before operations", async () => {
				...
				await expect(persistence.createSwarm(invalidSwarm)).rejects.toThrow(
					"Invalid swarm data",
				);
			});
			...
			test("should handle transaction rollbacks", async () => {
				...
				if (persistence.persistence) {
					...
					await expect(persistence.createSwarm(validSwarm)).rejects.toThrow(
						"Database error",
					);
		...
		describe("Enhanced WASM Loader Error Handling", () => {
			...
			test("should handle WASM initialization timeouts", async () => {
				...
				await expect(wasmLoader.initialize("progressive")).rejects.toThrow(
					"Timeout",
				);
			...
			test("should validate module names", async () => {
				await expect(wasmLoader.loadModule("invalid-module")).rejects.toThrow(
					"Unknown module",
				);

				await expect(wasmLoader.loadModule("")).rejects.toThrow(
			...
			test("should handle module validation failures", async () => {
				...
				const isValid = await wasmLoader.validateLoadedModule(
					"core",
					invalidModule,
				);
			...
			test("should perform integrity checks", async () => {
				...
				await expect(
					wasmLoader.performIntegrityCheck("test", validModule),
				).resolves.toBe(true);
				...
				await expect(
					wasmLoader.performIntegrityCheck("test", invalidModule),
		...
		describe("Robust MCP Tools Error Handling", () => {
			...
			test("should validate agent_spawn parameters", async () => {
				...
				await expect(mcpTools.agent_spawn(invalidParams)).rejects.toThrow(
					"Parameter validation failed",
				);
			});
			...
			test("should validate neural_train parameters", async () => {
				...
				await expect(mcpTools.neural_train(invalidParams)).rejects.toThrow(
					"Parameter validation failed",
				);
			});
			...
			test("should handle operation timeouts", async () => {
				...
				await expect(mcpTools.swarm_init({})).rejects.toThrow("Timeout");
			});
			...
			test("should verify neural capabilities", async () => {
				...
				await expect(
					mcpTools.neural_train({ agentId: "test" }),
			...
			test("should check agent availability for tasks", async () => {
				...
				await expect(
					mcpTools.task_orchestrate({ task: "test task" }),
			...
			test("should verify task existence for results", async () => {
				...
				await expect(
					mcpTools.task_results({ taskId: "nonexistent" }),
		...
		describe("Error Recovery Validation", () => {
			test("should provide correct recovery suggestions", () => {
				...
				const timeoutSuggestions =
					mcpTools.baseMCP.generateRecoverySuggestions(timeoutError);
				expect(timeoutSuggestions).toContain("Increase task timeout duration");

				const memorySuggestions =
					mcpTools.baseMCP.generateRecoverySuggestions(memoryError);
				expect(memorySuggestions).toContain(
					"Reduce memory usage in task execution",
				);

				const agentSuggestions =
					mcpTools.baseMCP.generateRecoverySuggestions(agentError);
				expect(agentSuggestions).toContain(
					"Check agent availability and status",
				);

				const networkSuggestions =
					mcpTools.baseMCP.generateRecoverySuggestions(networkError);
			...
			test("should calculate efficiency scores correctly", () => {
				...
				const efficiency =
					mcpTools.baseMCP.calculateEfficiencyScore(perfectResults);
				...
				const poorEfficiency =
					mcpTools.baseMCP.calculateEfficiencyScore(poorResults);
