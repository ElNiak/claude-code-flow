...
export const taskCommand = new Command()
	...
	.action(async (options: any) => {
		console.log(
			chalk.yellow("Task listing requires a running Claude-Flow instance")
		);
	})
	.command("status")
	...
	.action(async (taskId: string, options: any) => {
		console.log(
			chalk.yellow(`Task status requires a running Claude-Flow instance`)
		);
	})
	.command("cancel")
	...
	.action(async (workflowFile: string, options: any) => {
		try {
			...
			console.error(
				chalk.red("Failed to load workflow:"),
