...
export const statusCommand = new Command("status")
	...
	.action(async (options) => {
		try {
			...
			console.log(
				formatInfo(`Status: ${getStatusEmoji(status.health)} ${status.health}`)
			);
			...
			Object.entries(status.agentsByType).forEach(([type, count]) => {
				const active = status.agents.filter(
					(a) => a.type === type && a.status === "active"
				).length;
				const idle = status.agents.filter(
					(a) => a.type === type && a.status === "idle"
				).length;
				const busy = status.agents.filter(
					(a) => a.type === type && a.status === "busy"
				).length;
			...
			if (options.memory) {
				...
				Object.entries(status.memoryStats.byNamespace).forEach(
					([ns, stats]) => {
						memTable.push([
							ns,
							stats.entries,
				...
				console.log(
					formatInfo(
						`Total Memory Usage: ${formatBytes(status.memoryStats.totalSize)}`
					)
				);
				console.log(
					formatInfo(`Total Entries: ${status.memoryStats.totalEntries}`)
				);
			}
			...
			if (options.performance) {
				...
				console.log(
					formatInfo(
						`Avg Task Completion: ${status.performance.avgTaskCompletion}ms`
					)
				);
				console.log(
					formatInfo(
						`Message Throughput: ${status.performance.messageThroughput}/min`
					)
				);
				console.log(
					formatInfo(
						`Consensus Success Rate: ${status.performance.consensusSuccessRate}%`
					)
				);
				console.log(
					formatInfo(`Memory Hit Rate: ${status.performance.memoryHitRate}%`)
				);
				console.log(
					formatInfo(
						`Agent Utilization: ${status.performance.agentUtilization}%`
					)
				);
			}
			...
			console.log(
				formatInfo(`Total Messages: ${status.communicationStats.totalMessages}`)
			);
			console.log(
				formatInfo(`Avg Latency: ${status.communicationStats.avgLatency}ms`)
			);
			console.log(
				formatInfo(
					`Active Channels: ${status.communicationStats.activeChannels}`
				)
			);
