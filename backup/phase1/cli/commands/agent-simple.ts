...
export const agentCommands = {
	...
	async list(args: string[], options: Record<string, any> = {}): Promise<void> {
		try {
			...
			for (const agent of agents) {
				const healthEmoji =
					agent.health > 0.8 ? "🟢" : agent.health > 0.5 ? "🟡" : "🔴";
				const statusEmoji =
					agent.status === "idle"
						? "⭕"
						: agent.status === "busy"
				...
				if (options.detailed) {
					...
					console.log(
						`   Memory: ${Math.round(agent.metrics.memoryUsage / 1024 / 1024)}MB`
					);
				}
			...
			console.log(
				`   Average Health: ${Math.round(stats.averageHealth * 100)}%`
			);
		} catch (error) {
			console.error(
	...
	async info(
		...
		try {
			...
			console.log(
				`  Average Execution Time: ${agent.metrics.averageExecutionTime}ms`
			);
			console.log(
				`  Memory Usage: ${Math.round(agent.metrics.memoryUsage / 1024 / 1024)}MB`
			);
			...
			if (health) {
				console.log("\\n🏥 Health Details:");
				console.log(
					`  Responsiveness: ${Math.round(health.components.responsiveness * 100)}%`
				);
				console.log(
					`  Performance: ${Math.round(health.components.performance * 100)}%`
				);
				console.log(
					`  Reliability: ${Math.round(health.components.reliability * 100)}%`
				);
				console.log(
					`  Resource Usage: ${Math.round(health.components.resourceUsage * 100)}%`
				);
			...
			console.log("\\n🛠️ Capabilities:");
			console.log(
				`  Code Generation: ${agent.capabilities.codeGeneration ? "✅" : "❌"}`
			);
			console.log(
				`  Code Review: ${agent.capabilities.codeReview ? "✅" : "❌"}`
			);
			console.log(`  Testing: ${agent.capabilities.testing ? "✅" : "❌"}`);
			console.log(`  Research: ${agent.capabilities.research ? "✅" : "❌"}`);
			console.log(
				`  Web Search: ${agent.capabilities.webSearch ? "✅" : "❌"}`
			);
	...
	async terminate(
		...
		try {
			...
			if (agent.metrics) {
				...
				console.log(
					`  Total Uptime: ${Math.round(agent.metrics.totalUptime / 1000)}s`
				);
			}
		} catch (error) {
			console.error(
		...
		try {
			...
			if (unhealthyAgents.length > 0) {
				...
				unhealthyAgents.forEach((agent) => {
					...
					console.log(
						`  🔴 ${agent.name}: ${healthPercent}% (${agent.status})`
					);
				});
			} else {
			...
			console.log(
				`  Memory: ${Math.round(stats.resourceUtilization.memory / 1024 / 1024)}MB`
			);
			console.log(
				`  Disk: ${Math.round(stats.resourceUtilization.disk / 1024 / 1024)}MB`
			);
		} catch (error) {
			console.error(
