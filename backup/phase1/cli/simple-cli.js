...
import { getCommandHelp, getMainHelp } from "./help-text.js";
import {
	args,
	cwd,
	errors,
	...
	switch (command) {
		...
		case "environment":
			if (enhancedFlags._environment) {
				...
				if (
					enhancedFlags.appliedDefaults &&
					enhancedFlags.appliedDefaults.length > 0
				) {
					console.log(`\n✅ Auto-applied:`);
		...
		config: async (key) => {
			try {
				const config = JSON.parse(
					await readTextFile("claude-flow.config.json")
				);
				if (key) {
