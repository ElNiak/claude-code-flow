...
export class ConfigValidator {
		...
		try {
			...
			if (error instanceof Deno.errors.NotFound) {
				result.warnings.push(
					".roomodes file not found - SPARC features may not be available"
				);
			} else {
				result.success = false;
		...
		try {
			...
			for (const command of importantCommands) {
				if (!content.includes(command)) {
					result.warnings.push(
						`Missing important command reference: ${command}`
					);
				}
			}
	...
	async validateCoordinationConfig() {
		...
		try {
			...
			for (const section of requiredSections) {
				if (!content.includes(section)) {
					result.warnings.push(
						`Missing recommended section in coordination.md: ${section}`
					);
				}
			}
		...
		try {
			...
			if (content.includes("#!/usr/bin/env")) {
				...
				if (!content.includes("claude-flow") && !content.includes("deno run")) {
					result.warnings.push(
						"Executable script may not be properly configured"
					);
				}
			} else {
	...
	validateRoomodesStructure(config) {
		...
		for (const field of requiredFields) {
			if (!(field in config)) {
				result.warnings.push(
					`Missing recommended field in .roomodes: ${field}`
				);
			}
		}
			...
			if (typeof config.modes !== "object" || config.modes === null) {
				...
				for (const [modeName, modeConfig] of Object.entries(config.modes)) {
					...
					if (!modeValidation.valid) {
						result.warnings.push(
							...modeValidation.errors.map((err) => `Mode ${modeName}: ${err}`)
						);
					}
				}
			}
