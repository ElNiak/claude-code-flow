...
export class RollbackSystem {
	...
	async performFullRollback(backupId = null) {
		...
		try {
			...
			const rollbackResult =
				await this.rollbackExecutor.executeFullRollback(targetBackup);
	...
	async performPartialRollback(phase, checkpointId = null) {
		...
		try {
			...
			// Find checkpoint
			const checkpoint =
				checkpointId || (await this.findLatestCheckpoint(phase));
			if (!checkpoint) {
				result.success = false;
			...
			const rollbackResult = await this.rollbackExecutor.executePartialRollback(
				phase,
				checkpoint
			);
			result.success = rollbackResult.success;
	...
	async performAutoRecovery(failureType, context = {}) {
		...
		try {
			...
			const recoveryResult = await this.recoveryManager.performRecovery(
				failureType,
				context
			);
	...
	async cleanupOldBackups(keepCount = 5) {
		...
		try {
			const cleanupResult =
				await this.backupManager.cleanupOldBackups(keepCount);
			...
			if (cleanupResult.success) {
				console.log(
					`🗑️  Cleaned up ${cleanupResult.cleaned.length} old backups`
				);
			}
		} catch (error) {
			result.success = false;
	...
	async findLatestPreInitBackup() {
		try {
			const rollbackPoints = await this.stateTracker.getRollbackPoints();
			const preInitPoints = rollbackPoints.filter(
				(point) => point.type === "pre-init"
			);

			if (preInitPoints.length > 0) {
				return preInitPoints.sort((a, b) => b.timestamp - a.timestamp)[0]
					.backupId;
			}
...
export class AtomicOperation {
	...
	async commit() {
		...
		if (this.checkpointId) {
			await this.rollbackSystem.stateTracker.updateCheckpoint(
				this.checkpointId,
				{
					status: "committed",
					completed: Date.now(),
