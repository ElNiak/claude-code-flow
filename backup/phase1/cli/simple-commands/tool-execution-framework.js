...
export class ToolExecutionFramework {
	...
	async executeTool(toolName, parameters = {}, options = {}) {
		try {
			...
			if (this.currentExecutions >= this.maxConcurrentExecutions) {
				...
				this.ui.addLog(
					"info",
					`Tool ${toolName} queued (${this.executionQueue.length} in queue)`
				);
			} else {
	...
	async executeToolDirect(execution) {
		...
		try {
			...
			this.ui.addLog(
				"success",
				`${execution.toolName} completed in ${execution.duration}ms`
			);
	...
	async executeToolsBatch(toolExecutions, options = {}) {
		...
		this.ui.addLog(
			"info",
			`Starting batch execution: ${toolExecutions.length} tools`
		);

		try {
			if (options.parallel) {
				// Execute in parallel
				const promises = toolExecutions.map(
					({ toolName, parameters, toolOptions }) =>
		...
		try {
			for (let i = 0; i < workflow.steps.length; i++) {
				...
				if (execution.status === "failed" && step.required !== false) {
					throw new Error(
						`Required step ${step.toolName} failed: ${execution.error}`
					);
				}
			...
			return {
				...
				summary: {
					...
					completedSteps: results.filter((r) => r.status === "completed")
						.length,
	...
	formatResult(toolName, result) {
		const formatter =
			this.resultFormatters.get(toolName) ||
			this.resultFormatters.get("default");
