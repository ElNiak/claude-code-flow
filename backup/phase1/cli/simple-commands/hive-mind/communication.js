...
export class SwarmCommunication extends EventEmitter {
	...
	async consensus(proposal, validators = []) {
		...
		const consensusReached =
			winner && winner[1] / validators.length >= this.config.consensusQuorum;
	...
	_handleGossipMessage(envelope) {
		...
		if (gossipData.hops < 3) {
			...
			const selected = this._selectRandomAgents(
				agents,
				this.config.gossipFanout
			);
	...
	_handleConsensusMessage(envelope) {
		...
		switch (phase) {
			...
			case "vote":
				// Collect vote
				this.emit(
					`vote:${consensusId}:${envelope.from}`,
					envelope.message.vote
				);
				break;
	...
	getStatistics() {
		const avgLatency =
			this.state.metrics.latency.length > 0
				? this.state.metrics.latency.reduce((a, b) => a + b, 0) /
					this.state.metrics.latency.length
				: 0;

		return {
			agents: {
				total: this.state.agents.size,
				online: Array.from(this.state.agents.values()).filter(
					(a) => a.status === "online"
				).length,
				offline: Array.from(this.state.agents.values()).filter(
					(a) => a.status === "offline"
				).length,
			},
			messages: {
			...
			performance: {
				...
				successRate:
					this.state.metrics.sent > 0
						? (
								((this.state.metrics.sent - this.state.metrics.failed) /
									this.state.metrics.sent) *
								100
							).toFixed(2)
						: 100,
