...
class GitHubAPIClient {
	...
	async authenticate(token = null) {
		...
		if (!this.token) {
			printError(
				"GitHub token not found. Set GITHUB_TOKEN environment variable or provide token."
			);
			return false;
		}
	...
	async handlePushEvent(eventData) {
		printInfo(
			`Push event: ${eventData.commits.length} commits to ${eventData.ref}`
		);
	...
	async handleWorkflowRunEvent(eventData) {
		...
		printInfo(
			`Workflow run ${action}: ${workflowRun.name} - ${workflowRun.conclusion}`
		);
