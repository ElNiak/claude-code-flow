...
`;
		...
		const child = spawn(
			"node",
			[path.join(rootDir, "src/cli/simple-cli.js"), ...cmdArgs],
			{
	...
	return new Promise((resolve, reject) => {
		server.on("error", (err: any) => {
			if (err.code === "EADDRINUSE") {
				...
				console.log(
					`💡 Try a different port: claude-flow start --ui --port ${port + 1}`
				);
				console.log(
					`💡 Or stop the process using port ${port}: lsof -ti:${port} | xargs kill -9`
				);
