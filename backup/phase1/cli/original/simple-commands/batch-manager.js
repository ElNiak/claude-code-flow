...
import { ENVIRONMENT_CONFIGS, PROJECT_TEMPLATES } from "./init/batch-init.js";
		...
		if (config.projectConfigs) {
			for (const [projectName, projectConfig] of Object.entries(
				...
				if (
					...
					!PROJECT_TEMPLATES[projectConfig.template]
				) {
					issues.push(
						`Project ${projectName}: Unknown template ${projectConfig.template}`
					);
				}

				if (
					projectConfig.environment &&
					!ENVIRONMENT_CONFIGS[projectConfig.environment]
				) {
					issues.push(
						`Project ${projectName}: Unknown environment ${projectConfig.environment}`
					);
				}
			}
		}
...
function listTemplates() {
	...
	for (const [key, template] of Object.entries(PROJECT_TEMPLATES)) {
		...
		console.log(
			`   Extra Directories: ${template.extraDirs ? template.extraDirs.length : 0}`
		);
		console.log(
