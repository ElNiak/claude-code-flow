...
export class MCPToolWrapper {
	...
	async executeTool(toolName, params = {}) {
		...
		for (let attempt = 1; attempt <= this.config.retryCount; attempt++) {
			try {
				...
				lastError = error;
				console.error(
					`Attempt ${attempt} failed for ${toolName}:`,
					error.message
				);
				...
				for (let i = 0; i < group.length; i += concurrencyLimit) {
					...
					for (let j = 0; j < batchResults.length; j++) {
						...
						if (result.status === "fulfilled") {
							...
							console.warn(
								`Tool execution failed: ${batch[j].tool}`,
								result.reason
							);
							groupResults.push({
	...
	_groupToolsByPriority(toolCalls) {
		...
		toolCalls.forEach((call) => {
			...
			if (["swarm_init", "swarm_destroy", "memory_backup"].includes(tool)) {
				...
				["agent_spawn", "memory_usage", "neural_train"].includes(tool)
			) {
	...
	_trackBatchPerformance(toolCount, executionTime, concurrency) {
		...
		this.batchStats.avgConcurrency =
			(this.batchStats.avgConcurrency * (this.batchStats.totalBatches - 1) +
				concurrency) /
			this.batchStats.totalBatches;
		this.batchStats.avgToolsPerBatch =
			this.batchStats.totalTools / this.batchStats.totalBatches;
		this.batchStats.avgTimePerTool =
			this.batchStats.totalTime / this.batchStats.totalTools;
	}
	...
	async _executeToolInternal(toolName, params) {
		...
		console.log(
			`Executing MCP tool: mcp__claude-flow__${toolName} with params:`,
			params
		);
	...
	getStatistics() {
		...
		return {
			...
			performance: {
				totalCalls: Array.from(this.toolStats.values()).reduce(
	...
	_calculateOverallSuccessRate() {
		const total = Array.from(this.toolStats.values()).reduce(
			(sum, stat) => sum + stat.calls,
			0
		);
	...
	_calculateAvgLatency() {
		const stats = Array.from(this.toolStats.values()).filter(
			(stat) => stat.calls > 0
		);
	...
	_trackSpawnPerformance(agentCount, spawnTime) {
		...
		this.spawnStats.avgTimePerAgent =
			this.spawnStats.totalTime / this.spawnStats.totalAgents;
