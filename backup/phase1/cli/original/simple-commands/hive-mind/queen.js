...
export class QueenCoordinator extends EventEmitter {
	...
	_countComplexityKeywords(text) {
		...
		return complexKeywords.filter((keyword) => lowerText.includes(keyword))
			.length;
	}
	...
	async _divideAndConquerStrategy(analysis, workers) {
		...
		phases.push({
			...
			workers: workers.filter((w) =>
				["researcher", "architect"].includes(w.type)
			),
			parallel: true,
		});
		...
		phases.push({
			...
			workers: workers.filter((w) =>
				["optimizer", "documenter"].includes(w.type)
			),
			parallel: true,
		});
	...
	async _adaptiveLearningStrategy(analysis, workers) {
		const learningPhases = [
			...
			},
			{
				...
				workers: workers.filter((w) =>
					["analyst", "researcher"].includes(w.type)
				),
				parallel: false,
				learning: true,
			},
	...
	_calculateQueenVote(topic, options, workerVotes) {
		console.log(
			chalk.blue(`👑 [QUEEN DEBUG] Queen vote calculation triggered:`)
		);
		...
		if (this.config.type === "strategic") {
			console.log(
				chalk.yellow(`🎯 [QUEEN DEBUG] Using STRATEGIC voting logic`)
			);
		...
		// Adaptive queen learns from past decisions
			...
			console.log(
				chalk.red(`❓ [QUEEN DEBUG] Unknown queen type, using default`)
			);
	...
	_strategicVote(topic, options) {
		console.log(
			chalk.cyan(`🎯 [STRATEGIC DEBUG] Strategic voting algorithm activated`)
		);
		...
		console.log(
			chalk.gray(`Strategic keywords: [${strategicKeywords.join(", ")}]`)
		);
		...
		for (const option of options) {
			...
			if (matchedKeywords.length > 0) {
				console.log(
					chalk.green(
						`✅ Strategic match found: "${option}" (keywords: ${matchedKeywords.join(", ")})`
					)
				);
				return option;
			} else {
		...
		console.log(
			chalk.yellow(
				`⚠️ No strategic options found, defaulting to first option: "${options[0]}"`
			)
		);
		return options[0];
	}
	...
	_tacticalVote(topic, options, workerVotes) {
		console.log(
			chalk.cyan(`⚡ [TACTICAL DEBUG] Tactical voting algorithm activated`)
		);
		...
		if (sorted.length > 0 && sorted[0][1] > consensusThreshold) {
			console.log(
				chalk.green(
					`✅ Strong consensus found: "${sorted[0][0]}" (${sorted[0][1]} votes, above threshold)`
				)
			);
			return sorted[0][0];
		}
		...
		console.log(
			chalk.gray(
				`No strong consensus, checking tactical keywords: [${tacticalKeywords.join(", ")}]`
			)
		);

		for (const option of options) {
			const optionLower = option.toLowerCase();
			const matchedKeywords = tacticalKeywords.filter((keyword) =>
				optionLower.includes(keyword)
			);
			...
			if (matchedKeywords.length > 0) {
				console.log(
					chalk.green(
						`✅ Tactical match found: "${option}" (keywords: ${matchedKeywords.join(", ")})`
					)
				);
				return option;
			} else {
		...
		console.log(
			chalk.yellow(
				`⚠️ No tactical preferences, defaulting to first option: "${options[0]}"`
			)
		);
		return options[0];
	}
	...
	_adaptiveVote(topic, options, workerVotes) {
		...
		const similarDecisions = Array.from(
			this.state.learningData.entries()
		).filter(
	...
	async executePhases(executionPlan, hiveMindCore) {
		console.log(
			chalk.blue(
				`👑 [QUEEN] Executing ${executionPlan.strategy} strategy with ${executionPlan.phases.length} phases`
			)
		);
		...
		for (let i = 0; i < executionPlan.phases.length; i++) {
			...
			console.log(
				chalk.yellow(`📋 [QUEEN] Starting phase ${i + 1}: ${phase.name}`)
			);

			// 🤝 CONSENSUS INTEGRATION: Check if phase requires consensus
			if (phase.requiresConsensus) {
				console.log(
					chalk.blue(`🤝 [QUEEN] Phase requires consensus: "${phase.name}"`)
				);
				...
				try {
					...
					console.log(
						chalk.green(
							`✅ [QUEEN] Consensus reached for "${phase.name}": "${consensus.result}" (${Math.round(consensus.confidence * 100)}% confidence)`
						)
					);
				} catch (error) {
					console.warn(
						chalk.yellow(
							`⚠️ [QUEEN] Consensus failed for phase "${phase.name}", using default approach: ${error.message}`
						)
					);
			...
			console.log(
				chalk.green(`✅ [QUEEN] Completed phase ${i + 1}: ${phase.name}`)
			);
		}

		console.log(
			chalk.green(
				`🎉 [QUEEN] All phases completed for ${executionPlan.strategy} strategy`
			)
		);
		return results;
	}
	...
	_generatePhaseOptions(phase) {
		...
		if (
			phaseName.includes("optimization") ||
			phaseName.includes("performance")
		) {
			return [
	...
	async _executePhase(phase, hiveMindCore) {
		...
		try {
			...
			console.error(
				chalk.red(`❌ [QUEEN] Phase execution failed: ${phase.name}`),
				error
			);
			return {
				phase: phase.name,
