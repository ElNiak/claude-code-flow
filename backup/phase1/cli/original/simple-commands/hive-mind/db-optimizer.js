...
function addMemoryOptimization(db) {
  ...
  `)
	...
	if (tables.length === 0) {
		console.log(
			"collective_memory table does not exist, skipping memory optimization"
		);
		return;
	}
	...
	try {
      ...
      `)
			...
			if (hasMemoryTable) {
				...
				cutoffDate.setDate(
					cutoffDate.getDate() - (options.memoryRetentionDays || 30)
				);
          ...
          `)
					...
					console.log(
						chalk.green(`✓ Removed ${result.changes} old memory entries`)
					);
				} catch (error) {
					console.warn(
						chalk.yellow(`⚠ Could not clean memory entries: ${error.message}`)
					);
				}
			} else {
				console.log(
					chalk.yellow(
						"⚠ collective_memory table not found, skipping memory cleanup"
					)
				);
			}
		}
      ...
      `)
			...
			if (hasCompletedAt && hasCompletedAt.count > 0) {
				...
				archiveCutoff.setDate(
					archiveCutoff.getDate() - (options.taskRetentionDays || 7)
				);
        ...
        `)
			...
			console.log(
				chalk.green(`✓ Archived ${archived.changes} completed tasks`)
			);
		}
