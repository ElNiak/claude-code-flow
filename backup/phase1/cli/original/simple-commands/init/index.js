...
import { createAtomicOperation, RollbackSystem } from "./rollback/index.js";
...
import { runFullValidation, ValidationSystem } from "./validation/index.js";
	...
	for (const server of servers) {
		try {
			if (!dryRun) {
				...
				console.log(
					`  [DRY RUN] Would add ${server.name} - ${server.description}`
				);
			}
		} catch (err) {
...
export async function initCommand(subArgs, flags) {
	...
	const useEnhanced =
		subArgs.includes("--enhanced") || subArgs.includes("--safe");
	...
	// Parse init options
	const initForce =
		subArgs.includes("--force") || subArgs.includes("-f") || flags.force;
	const initMinimal =
		subArgs.includes("--minimal") || subArgs.includes("-m") || flags.minimal;
	const initSparc =
		subArgs.includes("--sparc") || subArgs.includes("-s") || flags.sparc;
	const initDryRun =
		subArgs.includes("--dry-run") || subArgs.includes("-d") || flags.dryRun;
	...
	try {
		...
		if (!initDryRun) {
			...
			console.log(
				"  [DRY RUN] Would create memory/claude-flow-data.json (persistence database)"
			);
		}
		...
		if (!initDryRun) {
			...
			console.log(
				"  [DRY RUN] Would create local claude-flow executable wrapper"
			);
		}
		...
		if (initSparc) {
			...
			if (initDryRun) {
				...
				console.log(
					"  [DRY RUN] Would create SPARC environment with all modes"
				);
				console.log(
				...
				try {
					...
					if (createSparcResult.success) {
						...
						printWarning(
							"create-sparc failed, creating basic SPARC structure manually..."
						);
					...
					printWarning(
						"create-sparc not available, creating basic SPARC structure manually..."
					);
				...
				if (sparcInitialized) {
					try {
						if (initOptimized) {
							await createOptimizedClaudeSlashCommands(
								workingDir,
								selectedModes
							);
						} else {
						...
						printWarning(
							`Could not create Claude Code slash commands: ${err.message}`
						);
					}
				}
			}
		...
		if (initDryRun) {
			...
			if (initOptimized) {
				console.log(
					"  • Batchtools optimization: Enabled for parallel processing"
				);
				console.log(
					"  • Performance enhancements: Smart batching, concurrent operations"
				);
			}
			console.log(
			...
			printSuccess(
				"🎉 Claude Code integration files initialized successfully!"
			);
			...
			if (initSparc) {
				...
				console.log(
					"7. Try TDD workflow with './claude-flow sparc tdd \"your task\"'"
				);

				if (initOptimized) {
					console.log(
						"8. Use batchtools commands: /batchtools, /performance for optimization"
					);
					console.log("9. Enable parallel processing with --parallel flags");
					console.log(
						"10. Monitor performance with './claude-flow performance monitor'"
					);
				}
			}
...
async function handleBatchInit(subArgs, flags) {
	try {
		...
		printError(
			"No batch operation specified. Use --batch-init <projects> or --config <file>"
		);
	} catch (err) {
...
async function enhancedInitCommand(subArgs, flags) {
	console.log(
		"🛡️  Starting enhanced initialization with validation and rollback..."
	);
	...
	try {
		...
		const initOptions = {
			...
			minimal:
				args.includes("--minimal") || args.includes("-m") || options.minimal,
		...
		await performInitializationWithCheckpoints(
			rollbackSystem,
			options,
			workingDir,
			dryRun
		...
		if (healthChecks.warnings.length > 0) {
			...
			healthChecks.warnings.forEach((warning) =>
				console.warn(`  ⚠️  ${warning}`)
			);
		}
	...
	try {
		...
		if (rollbackPoints.rollbackPoints.length === 0) {
			...
			rollbackPoints.rollbackPoints.forEach((point, index) => {
				const date = new Date(point.timestamp).toLocaleString();
				console.log(
					`  ${index + 1}. ${point.type} - ${date} (${point.backupId || "No backup"})`
				);
			});
		}
		...
		if (rollbackPoints.checkpoints.length > 0) {
			...
			rollbackPoints.checkpoints.slice(-5).forEach((checkpoint, index) => {
				...
				console.log(
					`  ${index + 1}. ${checkpoint.phase} - ${date} (${checkpoint.status})`
				);
			});
		}
	} catch (error) {
...
async function performInitializationWithCheckpoints(
	...
	const phases = [
		{
			name: "file-creation",
			action: () => createInitialFiles(options, workingDir, dryRun),
		},
		{
			name: "directory-structure",
			action: () => createDirectoryStructure(workingDir, dryRun),
		},
		{
			name: "memory-setup",
			action: () => setupMemorySystem(workingDir, dryRun),
		},
		{
			name: "coordination-setup",
			action: () => setupCoordinationSystem(workingDir, dryRun),
		},
		{
			name: "executable-creation",
			action: () => createLocalExecutable(workingDir, dryRun),
		},
	];
...
`;
				...
				console.log(
					`[DRY RUN] Would create ${commands.length} ${category} command docs`
				);
			}
		}
		...
		if (!dryRun) {
			...
			try {
				...
				const { FallbackMemoryStore } = await import(
					"../../../memory/fallback-store.js"
				);
