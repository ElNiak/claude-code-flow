...
export class SparcPseudocode extends SparcPhase {
	...
	async execute() {
		...
		try {
			...
			// Analyze complexity
			result.complexityAnalysis = await this.analyzeComplexity(
				result.pseudocode
			);
		...
		for (let i = 0; i < nodes.length - 1; i++) {
			...
			if (current.type === "decision") {
				...
				const errorNode = nodes.find(
					(n) => n.id.includes("error") || n.id.includes("handle")
				);
				if (errorNode) {
					edges.push({
						from: current.id,
	...
	estimateComplexity(steps) {
		const decisionSteps = steps.filter(
			(step) =>
				step.includes("IF") || step.includes("WHILE") || step.includes("FOR")
		);
		...
		return {
			...
			lines: steps.length,
			level:
				decisionSteps.length > 5
					? "high"
					: decisionSteps.length > 2
						? "medium"
						: "low",
		};
	}
	...
	async defineDataStructures(specification) {
		...
		for (const requirement of requirements) {
			...
			if (
				reqLower.includes("map") ||
				reqLower.includes("dictionary") ||
				reqLower.includes("key")
			) {
				dataStructures.push({
					name: "Hash Map",
			...
			if (
				reqLower.includes("queue") ||
				reqLower.includes("fifo") ||
				reqLower.includes("buffer")
			) {
				dataStructures.push({
					name: "Queue",
			...
			if (
				reqLower.includes("stack") ||
				reqLower.includes("lifo") ||
				reqLower.includes("undo")
			) {
				dataStructures.push({
					name: "Stack",
	...
	async analyzeComplexity(pseudocode) {
		...
		const averageComplexity =
			functionCount > 0 ? totalComplexity / functionCount : 1;
		...
		if (averageComplexity > 10) {
			...
			analysis.recommendations.push(
				"Monitor function complexity during implementation"
			);
		} else {
