...
export class CompatibleUI {
	...
	private render(): void {
		...
		if (stats.errorProcesses > 0) {
			console.log(
				chalk.red(`⚠️  ${stats.errorProcesses} processes with errors`)
			);
		}
	...
	private showStatus(): void {
		...
		console.log(
			chalk.white("Stopped:"),
			chalk.gray(
				stats.totalProcesses - stats.runningProcesses - stats.errorProcesses
			)
		);
		...
		this.processes.forEach((process, index) => {
			...
			if (process.metrics) {
				if (process.metrics.cpu !== undefined) {
					console.log(
						chalk.gray(`    CPU: ${process.metrics.cpu.toFixed(1)}%`)
					);
				}
				if (process.metrics.memory !== undefined) {
					console.log(
						chalk.gray(`    Memory: ${process.metrics.memory.toFixed(0)} MB`)
					);
				}
			}
	...
	private getSystemStats(): UISystemStats {
		return {
			...
			runningProcesses: this.processes.filter((p) => p.status === "running")
				.length,
			errorProcesses: this.processes.filter(
	...
	private async handleExit(): Promise<void> {
		const runningProcesses = this.processes.filter(
			(p) => p.status === "running"
		);
		...
		if (runningProcesses.length > 0) {
			...
			console.log(
				chalk.yellow(
					`⚠️  ${runningProcesses.length} processes are still running.`
				)
			);
...
export async function launchUI(): Promise<void> {
	...
	console.log(
		chalk.gray("Note: Using compatible UI mode for broader terminal support")
	);
	console.log();
