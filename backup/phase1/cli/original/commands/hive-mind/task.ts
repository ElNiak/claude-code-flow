...
import type { TaskPriority, TaskStrategy } from "../../../hive-mind/types.js";
import {
	formatError,
	formatInfo,
	formatSuccess,
...
export const taskCommand = new Command("task")
	...
	.option(
		"-d, --dependencies <ids>",
		"Comma-separated list of dependent task IDs"
	)
	...
	.action(async (description, _options) => {
		try {
			...
			if (_options.interactive || !description) {
				...
				_options.requireConsensus =
					answers.requireConsensus || _options.requireConsensus;
			...
			console.log(
				formatInfo(
					`Priority: ${getPriorityBadge(task.priority)} ${task.priority}`
				)
			);
			...
			if (task.assignedAgents.length > 0) {
				console.log(
					formatInfo(`Assigned to: ${task.assignedAgents.join(", ")}`)
				);
			}
			...
			if (_options.watch) {
				...
				console.log(
					"\n" +
						chalk.gray(
							`Track progress: ruv-swarm hive-mind task --watch ${task.id}`
						)
				);
			}
		} catch (error) {
...
async function watchTaskProgress(hiveMind: HiveMind, taskId: string) {
	...
	const interval = setInterval(async () => {
		try {
			...
			bar.stop();
			console.error(
				formatError("Error watching task: " + (error as Error).message)
			);
		}
	}, 1000);
