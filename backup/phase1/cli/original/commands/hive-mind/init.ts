...
import { formatError, formatInfo, formatSuccess } from "../../formatter.js";
...
export const initCommand = new Command("init")
	...
	.action(async (options) => {
		...
		try {
			...
			console.log(
				formatInfo(`Consensus Threshold: ${config.consensusThreshold * 100}%`)
			);
			...
			console.log(
				formatInfo("1. Spawn agents: ruv-swarm hive-mind spawn <type>")
			);
			console.log(
				formatInfo('2. Submit task: ruv-swarm hive-mind task "Your task"')
			);
