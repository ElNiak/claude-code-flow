...
import { existsSync as _existsSync, promises as fs } from "node:fs";
...
async function listSessions(options: any): Promise<void> {
	try {
		...
		console.error(
			chalk.red("Failed to list sessions:"),
...
async function restoreSession(sessionId: string, options: any): Promise<void> {
	try {
		...
		console.log(`${chalk.white("Name:")} ${session.name}`);
		console.log(
			`${chalk.white("Description:")} ${session.description || "None"}`
		);
		console.log(`${chalk.white("Agents:")} ${session.state.agents.length}`);
		console.log(`${chalk.white("Tasks:")} ${session.state.tasks.length}`);
		console.log(
			`${chalk.white("Created:")} ${session.createdAt.toLocaleString()}`
		);
		...
		console.error(
			chalk.red("Failed to restore session:"),
			(error as Error).message
		);
	}
}
...
async function exportSession(
	...
	try {
		...
		console.error(
			chalk.red("Failed to export session:"),
			(error as Error).message
		);
	}
}
...
async function importSession(inputFile: string, options: any): Promise<void> {
	try {
		...
		console.log(
			`${chalk.white("Action:")} ${options.overwrite ? "Overwritten" : "Created"}`
		);
	} catch (error) {
		console.error(
...
async function showSessionInfo(sessionId: string): Promise<void> {
	try {
		...
		console.log(
			`${chalk.white("Updated:")} ${session.updatedAt.toLocaleString()}`
		);
		console.log();
		...
		console.log(`${chalk.white("Tasks:")} ${session.state.tasks.length}`);
		console.log(
			`${chalk.white("Memory Entries:")} ${session.state.memory.length}`
		);
		console.log();
		...
		try {
			...
			console.log(
				`${chalk.white("Modified:")} ${fileInfo.mtime?.toLocaleString() || "Unknown"}`
			);
		} catch {
		...
		console.error(
			chalk.red("Failed to show session info:"),
			(error as Error).message
		);
	}
}
	...
	try {
		...
		for (const session of toDelete) {
			try {
				...
				console.error(
					chalk.red(`Failed to delete ${session.name}:`),
					(error as Error).message
				);
			}
		}
		...
		console.error(
			chalk.red("Failed to clean sessions:"),
			(error as Error).message
		);
	}
}
