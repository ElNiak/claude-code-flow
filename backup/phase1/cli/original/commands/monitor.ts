...
class Dashboard {
	...
	private renderHeader(data: MonitorData): void {
		...
		console.log(
			chalk.cyan.bold("Claude-Flow Live Monitor") + chalk.gray(` - ${time}`)
		);
	...
	private generateMockEvents(): any[] {
		...
		const eventTypes = [
			...
			{
				type: "agent_spawned",
				message: "New implementer agent spawned",
				level: "info" as const,
			},
			{
				type: "task_assigned",
	...
	private async exportMonitoringData(): Promise<void> {
		try {
			...
			console.log(
				chalk.green(`✓ Monitoring data exported to ${this.options.export}`)
			);
		} catch (error) {
			console.error(
				chalk.red("Failed to export data:"),
...
async function startMonitorDashboard(options: any): Promise<void> {
	...
	if (options.export) {
		...
		try {
			...
			console.error(
				chalk.red(`Cannot write to export file: ${options.export}`)
			);
			return;
		}
	}
