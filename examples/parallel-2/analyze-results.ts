...
async function analyzeResults() {
	try {
		...
		console.log(
			`   Successful: ${results.summary.successfulAgents} (${Math.round((results.summary.successfulAgents / results.summary.totalAgents) * 100)}%)`,
		);
		console.log(
			`   Failed: ${results.summary.failedAgents} (${Math.round((results.summary.failedAgents / results.summary.totalAgents) * 100)}%)`,
		);
		console.log(
			`   Total Duration: ${(results.summary.totalDuration / 1000).toFixed(2)}s`,
		);
		console.log(
			`   Average Duration: ${(results.summary.averageDuration / 1000).toFixed(2)}s per agent`,
		);

		// Performance analysis
		const sortedByDuration = [...results.results].sort(
		...
		sortedByDuration.slice(0, 3).forEach((result, index) => {
			console.log(
				`   ${index + 1}. ${result.agent}: ${(result.duration / 1000).toFixed(2)}s`,
			);
		});
		...
		if (failures.length > 0) {
			...
			failures.forEach((failure) => {
				console.log(
					`   - ${failure.agent}: ${failure.error?.split("\n")[0] || "Unknown error"}`,
				);
			});
		}
		...
		if (successes.length > 0) {
			...
			successes.forEach((success) => {
				...
				console.log(
					`   - ${success.agent}: Generated ${outputLines} lines of output`,
				);
			});
		}

		// Parallel efficiency
		const sequentialTime = results.results.reduce(
			(sum, r) => sum + r.duration,
			0,
		);
		const parallelEfficiency = (
			(sequentialTime / results.summary.totalDuration) *
			100
		).toFixed(1);

		console.log("\n🚀 Parallel Execution Efficiency:");
		console.log(
			`   Sequential time (if run one by one): ${(sequentialTime / 1000).toFixed(2)}s`,
		);
		console.log(
			`   Actual parallel time: ${(results.summary.totalDuration / 1000).toFixed(2)}s`,
		);
		console.log(`   Efficiency gain: ${parallelEfficiency}%`);
		console.log(
			`   Time saved: ${((sequentialTime - results.summary.totalDuration) / 1000).toFixed(2)}s`,
		);
