...
class AgentTestRunner {
	async runSequentialTests() {
		...
		results.push(
			await coordinator.orchestrateSwarmTask("Build authentication system", 3),
		);
		...
		results.push(
			await specialist.provideMachineLearningExpertise(
	...
	async runComparison() {
		...
		console.log("\n💡 KEY INSIGHTS:");
		console.log(
			"- Parallel execution dramatically reduces total execution time",
		);
		...
		console.log(
			"- Suitable for complex, multi-faceted software engineering tasks",
		);
	}
...
if (require.main === module) {
	...
	if (args.length === 0 || args[0] === "compare") {
		...
		console.log("Usage:");
		console.log(
			"  ts-node run-all-tests.ts [compare]    - Run comparison (default)",
		);
		console.log(
			"  ts-node run-all-tests.ts sequential   - Run sequential tests only",
		);
		console.log(
			"  ts-node run-all-tests.ts parallel     - Run parallel tests only",
		);
		console.log(
			"  ts-node run-all-tests.ts agent <type> - Run specific agent demo",
		);
		console.log("\nAgent types: coordinator, researcher, developer, analyzer,");
		console.log(
			"             reviewer, tester, documenter, monitor, specialist",
		);
	}
}
