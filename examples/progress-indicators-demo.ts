...
async function demoBasicProgress() {
  ...
  progress.defineSteps([
    { id: 'setup', name: 'Setup Environment', description: 'Setting up build environment', weight: 0.1 },
    { id: 'compile', name: 'Compile Source', description: 'Compiling TypeScript to JavaScript', weight: 0.4 },
    { id: 'bundle', name: 'Bundle Assets', description: 'Bundling application assets', weight: 0.3 },
    { id: 'optimize', name: 'Optimize Output', description: 'Optimizing build output', weight: 0.2 }
  ]);
...
async function demoParallelProgress() {
  ...
  const progress = createProgressIndicator('analysis', 'Multi-Component Analysis');
  
  progress.defineSteps([
    { id: 'frontend', name: 'Frontend Analysis', description: 'Analyzing React components', weight: 0.3 },
    { id: 'backend', name: 'Backend Analysis', description: 'Analyzing API endpoints', weight: 0.4 },
    { id: 'database', name: 'Database Analysis', description: 'Analyzing data models', weight: 0.3 }
  ]);
...
async function demoRealTimeUpdates() {
  ...
  progress.defineSteps([
    ...
    { id: 'deploy', name: 'Deploy & Configure', description: 'Deploying and configuring service', weight: 0.3 },
 ...
 */
async function simulateRealWork(progress: any, stepId: string, duration: number = 500): Promise<void> {
  ...
  while (performance.now() - startTime < duration) {
    ...
    const data = new Array(1000).fill(0).map((_, i) => Math.sin(i * Math.PI / 180));
  ...
  progress.completeStep(stepId, { iterations, duration: performance.now() - startTime });
}
 ...
 */
async function main() {
  console.log('🚀 Progress Indicators Demo - Real-time Progress Without Delays\n');
