...
import {
	copyPrompts,
	copyPromptsEnhanced,
} from "../src/swarm/prompt-copier-enhanced.ts";
...
async function demonstratePromptManager(
	...
	try {
		...
		report.sources.forEach((source) => {
			console.log(
				`     - ${source.path}: ${source.exists ? "exists" : "missing"}`,
			);
...
async function demonstrateConflictResolution(
	...
	try {
		...
		for (const strategy of strategies) {
			...
			console.log(
				`   Copied: ${result.copiedFiles}, Skipped: ${result.skippedFiles}`,
			);

			if (result.backupLocation) {
				console.log(
					`   Backup created: ${path.basename(result.backupLocation)}`,
				);
			}

			// Check final content
			try {
				const finalContent = await Deno.readTextFile(
					path.join(strategyDestDir, "architect.md"),
				);
				...
				if (strategy === "skip") {
					console.log(
						`   Content: ${isModified ? "Modified (skipped)" : "Original"}`,
					);
				} else if (strategy === "merge") {
					console.log(
						`   Content: ${isOriginal && isModified ? "Merged" : "Single version"}`,
					);
				} else {
					console.log(
						`   Content: ${isOriginal ? "Original (overwritten)" : "Modified"}`,
					);
				}
			} catch {
