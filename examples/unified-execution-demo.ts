...
import type { WorkflowDefinition } from "../src/cli/commands/workflow.js";
import type { WorkflowTask } from "../src/task/engine.js";
import {
	UnifiedExecutionEngine,
	type UnifiedExecutionOptions,
} from "../src/unified/execution/unified-execution-engine.js";
...
async function demonstrateUnifiedExecution() {
	...
	console.log(
		`Efficiency: ${(result1.performance.efficiency * 100).toFixed(1)}%`,
	);
	console.log("");
	...
	const complexWorkflow: WorkflowDefinition = {
		...
		tasks: [
			...
			},
			{
				...
				type: "implementation",
				description:
					"Implement REST API with authentication and business logic",
	...
	console.log(
		`Tasks Completed: ${result2.workflowExecution?.tasksCompleted || 0}`,
	);
	console.log(`Decomposed Tasks: ${result2.taskExecutions?.length || 0}`);
	console.log(
		`Decomposition Quality: ${(result2.performance.decompositionQuality * 100).toFixed(1)}%`,
	);
	console.log(
		`Parallelization: ${(result2.performance.parallelization * 100).toFixed(1)}%`,
	);
	console.log("");

	// Demo 3: Individual task (routes to task execution)
	console.log(
		"📋 Demo 3: Individual Task → Task Execution with Resource Management",
	);
	console.log("─".repeat(50));
	...
	console.log(
		`Resource Utilization: ${(result3.performance.resourceUtilization * 100).toFixed(1)}%`,
	);
	console.log(
		`Efficiency: ${(result3.performance.efficiency * 100).toFixed(1)}%`,
	);
	console.log("");
	...
	console.log(
		`Adaptations Applied: ${result4.presetResult?.adaptations.join(", ") || "None"}`,
	);
	console.log(
		`Learning Patterns: ${result4.learningData.patterns.join(", ") || "None"}`,
	);
	console.log("");
	...
	console.log(
		`├── Workflows: ${status.workflows.total} total, ${status.workflows.active} active`,
	);
	console.log(
		`├── Tasks: ${status.tasks.total} total, ${status.tasks.running} running, ${status.tasks.completed} completed`,
	);
	console.log(
		`├── Presets: ${status.presets.available} available, ${status.presets.executed} executed`,
	);
	...
	console.log(
		`    ├── Parallelization: ${(status.performance.parallelization * 100).toFixed(1)}%`,
	);
	console.log(
	...
	console.log(
		`├── Optimizations Found: ${status.learning.optimizations.length}`,
	);
	...
	if (status.learning.patterns.length > 0) {
		...
		status.learning.patterns
			.slice(0, 3)
			.forEach((pattern: string, index: number) => {
	...
	if (status.learning.optimizations.length > 0) {
		...
		status.learning.optimizations
			.slice(0, 3)
			.forEach((opt: string, index: number) => {
	...
	console.log(
		"✓ Intelligent input routing (string → preset, workflow → workflow+tasks, task → task)",
	);
