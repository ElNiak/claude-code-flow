...
class TestAutoStrategy:
    ...
    async def test_auto_strategy_execution(self):
        ...
        strategy._delegate_to_strategy = AsyncMock(
            return_value=Result(
                task_id=task.id,
                agent_id="auto-agent",
...
class TestDevelopmentStrategy:
    ...
    async def test_development_strategy_execution(self):
        ...
        assert result.task_id == task.id
        assert (
            "implementation" in str(result.output).lower()
            or "code" in str(result.output).lower()
        )
