...
class TestSwarmStrategies:
    ...
    def test_swarm_strategy_execution(self, test_case: SwarmTestCase):
        ...
        for coord_mode in test_case.coordination_modes:
            ...
            if expected_keywords:
                keyword_found = any(
                    keyword.lower() in output_text for keyword in expected_keywords
                )
                assert keyword_found, f"Expected behaviors not found for {test_case.strategy} with {coord_mode}"
        ...
        if durations:
            ...
            assert (
                max_duration <= test_case.performance_thresholds["max_duration"]
            ), f"{test_case.strategy} exceeded max duration threshold"
            assert (
                avg_duration <= test_case.performance_thresholds["avg_duration"]
    ...
    def test_concurrent_swarm_execution(self):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for strategy, future in futures:
                ...
                assert (
                    result.success or result.return_code == 0
    ...
    def _generate_compatibility_matrix(self, results: List[Dict]):
        ...
        with open(matrix_path, "w") as f:
            json.dump(
                {"timestamp": datetime.now().isoformat(), "matrix": matrix}, f, indent=2
            )
        ...
        report = {
            ...
            "summary": {
                ...
                "successful_scenarios": sum(
                    1 for s in scenarios if s.get("result", {}).get("success", False)
                ),
    ...
    def benchmark_results(self):
        ...
        if self.results:
            ...
            summary = {
                ...
                "coordination_modes_tested": len(
                    set(r.coordination_mode for r in self.results)
                ),
