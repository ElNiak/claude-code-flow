...
class TestSPARCModes:
    ...
    def execute_sparc_command(
        self, mode: str, prompt: str, special_flags: List[str] = None
    ) -> SPARCTestResult:
    ...
    def test_sparc_mode_execution(self, test_case: SPARCTestCase):
        ...
        for prompt in test_case.test_prompts:
            ...
            ), f"Command execution failed for {test_case.mode}"

            # Check for expected output keywords (at least one should be present)
            output_text = (result.stdout + result.stderr).lower()
            keyword_found = any(
                keyword in output_text for keyword in test_case.expected_outputs
            )
            assert (
                keyword_found
            ), f"Expected keywords not found in {test_case.mode} output"
    ...
    def test_sparc_mode_performance_comparison(self):
        ...
        for test_case in self.SPARC_MODES:
            result = self.execute_sparc_command(
                mode=test_case.mode, prompt=standard_prompt
            )
        ...
        report = {
            ...
            "summary": {
                ...
                "successful_modes": sum(
                    1 for v in performance_data.values() if v["success"]
                ),
                "average_duration": statistics.mean(
                    v["duration"] for v in performance_data.values()
                ),
                "fastest_mode": min(
                    performance_data.items(), key=lambda x: x[1]["duration"]
                )[0],
                "slowest_mode": max(
                    performance_data.items(), key=lambda x: x[1]["duration"]
                )[0],
