...
class BenchmarkRunner:
    ...
    def run_integration_tests(self, test_type: str = "all") -> Dict[str, Any]:
        ...
        test_files = []
        if test_type == "all":
            test_files = ["test_sparc_modes.py", "test_swarm_strategies.py"]
        ...
        for test_file in test_files:
            ...
            result = subprocess.run(
                [
                    ...
                    "--tb=short",
                    "-k",
                    "not stress and not advanced",  # Skip stress tests by default
                    "--json-report",
        ...
        for suite_name, suite_data in self.results["test_suites"].items():
            ...
            if isinstance(suite_data, dict):
                ...
                if "tests_run" in suite_data:
                    ...
                    report_lines.append(
                        f"- Passed: {suite_data.get('tests_passed', 0)}"
                    )
                report_lines.append("")
...
def main():
    ...
    parser.add_argument(
        "--integration-type",
        choices=["all", "sparc", "swarm"],
        default="all",
        help="Type of integration tests",
    )
