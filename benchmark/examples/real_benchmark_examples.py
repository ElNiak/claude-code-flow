...
async def example_sparc_mode_comparison():
    ...
    for mode in modes_to_test:
        ...
        config = BenchmarkConfig(name=f"sparc-{mode}-test", task_timeout=45)
        ...
        if "resource_usage" in result:
            ...
            print(
                f"  Memory: {result['resource_usage'].get('peak_memory_mb', 0):.1f} MB"
            )
...
async def example_swarm_strategies():
    ...
    for config_def in test_configs:
        ...
        results.append({"config": config_def["description"], "result": result})
...
async def example_comprehensive_mode_testing():
    ...
    for strategy, mode in swarm_configs:
        ...
        task = Task(objective=objective, strategy=strategy, mode=mode)
    ...
    return {"sparc_modes": sparc_results, "swarm_strategies": swarm_results}
...
async def example_resource_monitoring():
    ...
    config = BenchmarkConfig(
        ...
        monitoring=True,
        task_timeout=60,
        resource_limits={"max_memory_mb": 512, "max_cpu_percent": 75},
    )
    ...
    result = await engine.run_benchmark(
        "Analyze a large codebase for complexity metrics"
    )
    ...
    return {"system_summary": summary, "benchmark_result": result}
