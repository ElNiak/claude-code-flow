...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("performance_monitor.log"), logging.StreamHandler()],
)
...
class SwarmMetricsCollector:
    ...
    def start_collection(self, interval_seconds: float = 10.0, session_id: str = None):
        ...
        self._collection_thread = threading.Thread(
            target=self._collect_metrics_loop, args=(interval_seconds,), daemon=True
        )
...
class PerformanceMonitor:
    ...
    def _on_metrics_collected(self, metrics: MetricSnapshot):
        ...
        # Log summary
        logger.info(
            f"Metrics collected - Memory: {metrics.memory_usage_mb:.1f}MB, "
    ...
    def _check_alerts(self, metrics: MetricSnapshot):
        ...
        for alert in self.alerts:
            if alert.metric_name in metrics_dict:
                ...
                if isinstance(current_value, (int, float)) and alert.should_trigger(
                    current_value
                ):
                    # Store alert
    ...
    def get_performance_dashboard_data(self, hours: int = 24) -> Dict[str, Any]:
        ...
        init_times = [
            m.swarm_init_time for m in recent_metrics if m.swarm_init_time > 0
        ]
        coordination_latencies = [
            m.agent_coordination_latency
            for m in recent_metrics
            if m.agent_coordination_latency > 0
        ]
        ...
        dashboard_data = {
            "summary": {
                ...
                "last_update": recent_metrics[0].timestamp.isoformat()
                if recent_metrics
                else None,
            },
            "swarm_performance": {
                ...
                "avg_coordination_latency": statistics.mean(coordination_latencies)
                if coordination_latencies
                else 0,
                "max_coordination_latency": max(coordination_latencies)
                if coordination_latencies
                else 0,
            },
            "resource_usage": {
                ...
                if len(memory_usage) > 1 and memory_usage[-1] > memory_usage[0]
                else "stable",
            },
            "time_series": [
                m.to_dict() for m in recent_metrics[-100:]
            ],  # Last 100 samples for charts
            "baselines": {
                "swarm_init_time": self.db.get_baseline("swarm_init_time"),
                "agent_coordination_latency": self.db.get_baseline(
    ...
    def generate_regression_report(self) -> Dict[str, Any]:
        ...
        recent_avg_init = statistics.mean(
            [m.swarm_init_time for m in recent_metrics if m.swarm_init_time > 0]
        )
        historical_avg_init = statistics.mean(
            [m.swarm_init_time for m in older_metrics if m.swarm_init_time > 0]
        )

        recent_avg_memory = statistics.mean([m.memory_usage_mb for m in recent_metrics])
        historical_avg_memory = statistics.mean(
            [m.memory_usage_mb for m in older_metrics]
        )

        # Calculate regression percentages
        init_regression = (
            ((recent_avg_init - historical_avg_init) / historical_avg_init) * 100
            if historical_avg_init > 0
            else 0
        )
        memory_regression = (
            ((recent_avg_memory - historical_avg_memory) / historical_avg_memory) * 100
        ...
        return {
            ...
            "recommendations": self._generate_regression_recommendations(
                init_regression, memory_regression
            ),
        }

    def _generate_regression_recommendations(
        self, init_regression: float, memory_regression: float
    ) -> List[str]:
        ...
        if init_regression > 10:
            recommendations.append(
                "Swarm initialization time has regressed by >10% - investigate startup optimizations"
            )

        if memory_regression > 15:
            recommendations.append(
                "Memory usage has increased by >15% - check for memory leaks"
            )

        if init_regression < -10:
            recommendations.append(
                "Swarm initialization time has improved significantly - consider documenting optimizations"
            )
...
async def run_continuous_monitoring(
    duration_minutes: int = 60, interval_seconds: float = 10.0
):
...
def main():
    ...
    parser.add_argument(
        "--interval", type=float, default=10.0, help="Collection interval in seconds"
    )
    parser.add_argument(
        "--dashboard", action="store_true", help="Generate dashboard data only"
    )
    parser.add_argument(
        "--regression", action="store_true", help="Generate regression report only"
    )
