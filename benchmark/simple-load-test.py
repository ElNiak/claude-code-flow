...
class SimpleLoadTester:
    ...
    def generate_summary(self, results: Dict[str, Any]):
        ...
        basic_success = sum(
            1 for r in results["basic_commands"].values() if r.get("success", False)
        )
        ...
        # Agent spawning
        agent_success = sum(
            1 for r in results["agent_spawning"].values() if r.get("success", False)
        )
        ...
        # Memory operations
        memory_success = sum(
            1 for r in results["memory_operations"].values() if r.get("success", False)
        )
        ...
        total_passed = (
            basic_success + (1 if swarm_success else 0) + agent_success + memory_success
        )
        overall_success_rate = total_passed / total_tests if total_tests > 0 else 0

        print("\\n🎯 OVERALL RESULT:")
        print(
            f"Tests Passed: {total_passed}/{total_tests} ({overall_success_rate:.1%})"
        )
