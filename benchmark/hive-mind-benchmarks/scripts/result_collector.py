...
class ResultDatabase:
        ...
        for result in results:
            ...
            """,
                (
                    ...
                    result.error_count,
                    result.consensus_decisions,
                    result.agent_spawn_time,
                    result.collective_memory_ops,
                    result.success,
    ...
    def query_results(
        self, filters: Dict[str, Any] = None, time_range: Tuple[str, str] = None
    ) -> pd.DataFrame:
    ...
    def calculate_aggregated_metrics(
        ...
        for group_values, group_df in df.groupby(group_by):
            ...
            p95_latency = (
                latency_values.quantile(0.95) if not latency_values.empty else 0
            )
            p99_latency = (
                latency_values.quantile(0.99) if not latency_values.empty else 0
            )
...
class AdvancedResultCollector:
    ...
    def __init__(
        self, database: ResultDatabase = None, output_dir: str = "collected-results"
    ):
    ...
    def generate_performance_analysis(self, time_window: str = "7d") -> Dict[str, Any]:
        ...
        for topology in df["topology"].unique():
            ...
            topology_analysis[topology] = {
                ...
                "success_rate": (
                    len(topology_df[topology_df["success"] == 1]) / len(topology_df)
                )
                * 100,
        ...
        for coordination in df["coordination"].unique():
            ...
            coordination_analysis[coordination] = {
                ...
                "success_rate": (
                    len(coord_df[coord_df["success"] == 1]) / len(coord_df)
                )
                * 100,
        ...
        for min_agents, max_agents in agent_ranges:
            range_df = df[
                (df["agent_count"] >= min_agents) & (df["agent_count"] <= max_agents)
            ]
    ...
    def generate_visualizations(
        self, output_subdir: str = "visualizations"
    ) -> Dict[str, str]:
        ...
        if not successful_df.empty:
            scatter = plt.scatter(
                successful_df["agent_count"],
                successful_df["memory_usage_mb"],
                c=successful_df["coordination_latency"],
                cmap="viridis",
                alpha=0.6,
            )
        ...
        success_by_config = (
            df.groupby(["topology", "coordination"])["success"]
            .agg(["mean", "count"])
            .reset_index()
        )
        success_by_config = success_by_config[
            success_by_config["count"] >= 3
        ]  # At least 3 tests

        if not success_by_config.empty:
            pivot_table = success_by_config.pivot(
                index="topology", columns="coordination", values="mean"
            )
            sns.heatmap(
                pivot_table,
                annot=True,
        ...
        if "created_at" in df.columns:
            ...
            axes[0, 0].plot(
                ...
                df_sorted["coordination_latency"],
                "b-",
                alpha=0.7,
            )
            ...
            axes[0, 1].plot(
                df_sorted["created_at"], df_sorted["memory_usage_mb"], "g-", alpha=0.7
            )
            ...
            axes[1, 1].hist(
                df_sorted["agent_count"], bins=20, alpha=0.7, color="purple"
            )
    ...
    def generate_html_report(
        self, analysis: Dict[str, Any], visualizations: Dict[str, str] = None
    ) -> str:
...
<body>
    ...
    def export_aggregated_data(self, format: str = "csv") -> str:
        ...
        if format.lower() == "csv":
            ...
            csv_file = (
                self.output_dir
                / f"aggregated_metrics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            )
            ...
            json_file = (
                self.output_dir
                / f"aggregated_metrics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
...
def main():
    ...
    parser.add_argument(
        "--analyze", action="store_true", help="Generate performance analysis"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--export", choices=["csv", "json"], help="Export aggregated data"
    )
    parser.add_argument(
        "--time-window",
        default="7d",
