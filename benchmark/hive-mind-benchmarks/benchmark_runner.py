...
class HiveMindBenchmarkRunner:
    ...
    def create_benchmark_configs(self) -> List[BenchmarkConfig]:
        ...
        scales = [(5, "simple"), (20, "medium"), (100, "complex"), (1000, "enterprise")]
            ...
            for coordination in coordinations:
                for memory_type in memory_types:
                    for agent_count, complexity in scales:
                        if agent_count <= 100 or (
                            topology == "hierarchical" and coordination == "queen"
                        ):
        ...
        configs.extend(
            [
                BenchmarkConfig(
                    "optimal_small", "hierarchical", "queen", "sqlite", 5, "simple", 30
                ),
                BenchmarkConfig(
                    "optimal_medium",
                    "hierarchical",
                ...
                BenchmarkConfig(
                    "democracy_test", "mesh", "consensus", "sqlite", 50, "medium", 90
                ),
                BenchmarkConfig(
                    "hybrid_adaptive", "star", "hybrid", "memory", 30, "medium", 45
                ),
            ]
        )
    ...
    def measure_system_metrics(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {"memory_mb": 0.0, "cpu_percent": 0.0, "open_files": 0, "threads": 1}
    ...
    def run_single_benchmark(self, config: BenchmarkConfig) -> BenchmarkResult:
        ...
        try:
            ...
            init_result = self.run_hive_mind_command(
                ["hive-mind", "init", "--test-mode"]
            )
            ...
            spawn_result = self.run_hive_mind_command(
                spawn_cmd, timeout=config.duration_seconds
            )
            ...
            task_completion_rate = (
                1.0 if spawn_result["success"] and task_result["success"] else 0.0
            )
            error_count = sum(
                1
                for r in [init_result, spawn_result, status_result, task_result]
                if not r["success"]
            )
            ...
            return BenchmarkResult(
                ...
                consensus_decisions=config.agent_count
                if config.coordination == "consensus"
                else 1,
    ...
    def run_parallel_benchmarks(
        self, configs: List[BenchmarkConfig], max_workers: int = 3
    ...
    def analyze_results(self, results: List[BenchmarkResult]) -> Dict[str, Any]:
        ...
        optimal_small = min(
            [r for r in successful_results if r.config.agent_count <= 10],
            key=lambda r: r.coordination_latency,
            default=None,
        )
        optimal_medium = min(
            [r for r in successful_results if 10 < r.config.agent_count <= 50],
            key=lambda r: r.coordination_latency,
            default=None,
        )
        optimal_large = min(
            [r for r in successful_results if r.config.agent_count > 50],
            key=lambda r: r.coordination_latency,
            default=None,
        )
        ...
        return {
            "summary": {
                ...
                "avg_initialization_time": avg(
                    [r for r in successful_results], "initialization_time"
                ),
                "avg_coordination_latency": avg(
                    [r for r in successful_results], "coordination_latency"
                ),
                "avg_memory_usage": avg(
                    [r for r in successful_results], "memory_usage_mb"
                ),
            },
            ...
            "performance_targets": {
                "initialization_time_met": len(
                    [r for r in successful_results if r.initialization_time < 5.0]
                ),
                "coordination_latency_met": len(
                    [r for r in successful_results if r.coordination_latency < 200]
                ),
                "memory_usage_met": len(
                    [r for r in successful_results if r.memory_usage_mb < 100]
                ),
            },
        }
    ...
    def save_results(self, results: List[BenchmarkResult], analysis: Dict[str, Any]):
        ...
        # Save analysis
        analysis_file = (
            self.output_dir / f"hive_mind_benchmark_analysis_{timestamp}.json"
        )
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--quick":
        ...
        configs = [
            BenchmarkConfig(
                "quick_hierarchical", "hierarchical", "queen", "sqlite", 5, "simple", 10
            ),
            BenchmarkConfig(
                "quick_mesh", "mesh", "consensus", "memory", 10, "medium", 15
            ),
