    ...
    def _collect_process_metrics(self) -> List[ProcessMetrics]:
        ...
        if self._main_process:
            try:
                ...
                processes = [self._main_process] + self._main_process.children(
                    recursive=True
                )
    ...
    def _aggregate_metrics(self) -> PerformanceMetrics:
        ...
        if len(self._metrics_buffer) >= 2:
            ...
            if "network_io" in first_metrics and "network_io" in last_metrics:
                network_bytes_sent = (
                    last_metrics["network_io"]["bytes_sent"]
                    - first_metrics["network_io"]["bytes_sent"]
                )
                network_bytes_recv = (
                    last_metrics["network_io"]["bytes_recv"]
                    - first_metrics["network_io"]["bytes_recv"]
                )

            if "disk_io" in first_metrics and "disk_io" in last_metrics:
                disk_bytes_read = (
                    last_metrics["disk_io"]["read_bytes"]
                    - first_metrics["disk_io"]["read_bytes"]
                )
                disk_bytes_write = (
                    last_metrics["disk_io"]["write_bytes"]
                    - first_metrics["disk_io"]["write_bytes"]
                )
            ...
            "intervals": [
                {
                    ...
                    "total_cpu_percent": sum(
                        pm.cpu_percent for pm in interval.process_metrics
                    ),
                    "total_memory_mb": sum(
                        pm.memory_mb for pm in interval.process_metrics
                    ),
                }
...
class AsyncPerformanceCollector(PerformanceCollector):
    ...
    async def start_collection_async(
    ...
    async def collect_for_duration(
        self, duration: float, process: Optional[subprocess.Popen] = None
    ) -> PerformanceMetrics:
