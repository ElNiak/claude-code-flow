...
class MetricsAggregator:
        ...
        for collector in self._performance_collectors.values():
            ...
            if detailed and "intervals" in detailed:
                for interval in detailed["intervals"]:
                    total_cpu_time += (
                        interval.get("total_cpu_percent", 0) * 0.001
                    )  # Convert to seconds
    ...
    def _aggregate_process_metrics(self, metrics: AggregatedMetrics) -> None:
        ...
        if metrics.total_processes > 0:
            metrics.average_output_size = (
                metrics.total_output_size // metrics.total_processes
            )

        # Calculate output complexity (lines per second)
        if metrics.wall_clock_time > 0:
            metrics.output_complexity_score = (
                metrics.total_output_size / metrics.wall_clock_time
            )
    ...
    def _calculate_system_metrics(self, metrics: AggregatedMetrics) -> None:
        ...
        if metrics.total_cpu_seconds > 0 and metrics.wall_clock_time > 0:
            ...
            metrics.average_cpu_percent = (
                metrics.total_cpu_seconds / metrics.wall_clock_time / num_cores
            ) * 100
    ...
    def aggregate_benchmark_results(self, results: List[Result]) -> BenchmarkMetrics:
        ...
        benchmark_metrics.completed_tasks = len(
            [r for r in results if r.status == ResultStatus.SUCCESS]
        )
        benchmark_metrics.failed_tasks = len(
            [
                r
                for r in results
        ...
        if aggregated.wall_clock_time > 0:
            ...
            benchmark_metrics.resource_efficiency = min(
                1.0, aggregated.average_cpu_percent / 100.0
            )

            # Coordination efficiency: overhead of coordination
            benchmark_metrics.coordination_efficiency = 1.0 - (
                aggregated.coordination_overhead_percent / 100.0
            )
