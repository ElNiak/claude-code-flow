...
class ProcessTracker:
    ...
    async def execute_command_async(
        ...
        return await loop.run_in_executor(
            None, self.execute_command, command, timeout, env, cwd
        )

    def _update_command_metrics(
        self, command: List[str], result: ProcessExecutionResult
    ) -> None:
        ...
        if key not in self._command_metrics:
            self._command_metrics[key] = CommandMetrics(
                command_type=command_type, args=args
            )
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        peak_memory_values = [
            r.resource_usage.peak_memory_mb for r in self._execution_history
        ]
