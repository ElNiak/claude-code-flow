...
class ResourceMonitor:
    ...
    def _calculate_aggregate_usage(self) -> ResourceUsage:
        ...
        if len(self._history) >= 2:
            io_read_bytes = (
                self._history[-1].io_read_bytes - self._history[0].io_read_bytes
            )
            io_write_bytes = (
                self._history[-1].io_write_bytes - self._history[0].io_write_bytes
            )
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_mb": {
                ...
                "stdev": statistics.stdev(memory_values)
...
class SystemResourceMonitor:
    ...
    def measure_delta(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_percent_delta": current["memory_percent"]
            - self._baseline["memory_percent"],
            "disk_read_mb": (
                current["disk_read_bytes"] - self._baseline["disk_read_bytes"]
            )
            / 1024
            / 1024,
            "disk_write_mb": (
                current["disk_write_bytes"] - self._baseline["disk_write_bytes"]
            )
            / 1024
            / 1024,
            "network_sent_mb": (
                current["network_sent_bytes"] - self._baseline["network_sent_bytes"]
            )
            / 1024
            / 1024,
            "network_recv_mb": (
                current["network_recv_bytes"] - self._baseline["network_recv_bytes"]
            )
            / 1024
            / 1024,
        }
