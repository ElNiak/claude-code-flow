...
class TaskScheduler:
    ...
    def schedule_tasks(
        ...
        if self.algorithm == SchedulingAlgorithm.ROUND_ROBIN:
            ...
            assignments = self._schedule_capability_based(
                sorted_tasks, available_agents
            )
    ...
    def _schedule_capability_based(
        self, tasks: List[Task], agents: List[Agent]
    ) -> Dict[Agent, List[Task]]:
        ...
        if unassigned_tasks:
            remaining_assignments = self._schedule_least_loaded(
                unassigned_tasks, agents
            )
    ...
    def _schedule_dynamic(
        ...
        for task in tasks:
            ...
            for agent in agents:
                ...
                if required_caps:
                    cap_score = len(agent_caps.intersection(required_caps)) / len(
                        required_caps
                    )
                else:
                ...
                workload_score = 1.0 - (
                    self.agent_workload[agent.id] / (avg_workload + 1)
                )
                ...
                total_score = 0.4 * cap_score + 0.3 * workload_score + 0.3 * perf_score
    ...
    def steal_work(self, idle_agent: Agent) -> Optional[Task]:
        ...
        while self.work_steal_queue:
            ...
            if task.status == TaskStatus.PENDING:
                ...
                logger.info(
                    f"Agent {idle_agent.id} stole task {task.id} from agent {original_agent_id}"
                )
                return task
    ...
    def _enable_work_stealing(self, assignments: Dict[Agent, List[Task]]):
        ...
        avg_workload = sum(len(tasks) for tasks in assignments.values()) / len(
            assignments
        )
    ...
    def _update_metrics(
        self, assignments: Dict[Agent, List[Task]], start_time: datetime
    ):
        """Update scheduling metrics."""
        self.metrics.total_scheduled += sum(
            len(tasks) for tasks in assignments.values()
        )
