...
class ClaudeFlowExecutor:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized ClaudeFlowExecutor with path: {self.claude_flow_path}"
        )
    ...
    def _extract_metrics(self, output: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if "agents" in line_lower and any(char.isdigit() for char in line):
                try:
                    agent_count = int(
                        "".join(filter(str.isdigit, line.split("agents")[0]))
                    )
    ...
    async def execute_async(
        self, command_type: str, config: Union[SwarmConfig, SparcConfig]
    ) -> ExecutionResult:
        """
