...
class RealBenchmarkEngine(BenchmarkEngine):
    ...
    async def run_benchmark(self, objective: str) -> Dict[str, Any]:
        ...
        try:
            ...
            benchmark.metrics = self.metrics_aggregator.aggregate_benchmark_results(
                benchmark.results
            )
            ...
            metrics_path = (
                Path(self.config.output_directory) / f"metrics_{benchmark.id}.json"
            )
            ...
            process_report_path = (
                ...
                / f"process_report_{benchmark.id}.json"
            )
    ...
    async def _execute_task_with_metrics(self, task: Task) -> Result:
        ...
        execution_result = await self.process_tracker.execute_command_async(
            command, timeout=task.timeout, env={"CLAUDE_FLOW_BENCHMARK": "true"}
        )
        ...
        result = Result(
            ...
            errors=execution_result.stderr.splitlines()
        ...
        result.quality_metrics.accuracy_score = 1.0 - (
            execution_result.error_count / max(1, execution_result.output_size)
        )
    ...
    def _parse_output(self, stdout: str) -> Dict[str, Any]:
        ...
        output = {"raw_output": stdout, "lines": stdout.splitlines(), "sections": {}}
