            ...
            try:
                with self.resource_lock:
                    ...
                    if self.current_usage.cpu_percent > self.limits.max_cpu_percent:
                        ...
                        logger.warning(
                            f"CPU usage {self.current_usage.cpu_percent:.1f}% exceeds limit {self.limits.max_cpu_percent}%"
                        )

                    if self.current_usage.memory_mb > self.limits.max_memory_mb:
                        self.violation_count += 1
                        logger.warning(
                            f"Memory usage {self.current_usage.memory_mb:.1f}MB exceeds limit {self.limits.max_memory_mb}MB"
                        )

                    # Update peak values
                    self.current_usage.peak_memory_mb = max(
                        self.current_usage.peak_memory_mb, self.current_usage.memory_mb
                    )
    ...
    def check_resources(self) -> bool:
        ...
        with self.resource_lock:
            return (
                ...
                and self.current_usage.memory_mb <= self.limits.max_memory_mb
...
class ParallelExecutor:
    ...
    def _initialize_executors(self):
        ...
        if self.mode in [ExecutionMode.PROCESS, ExecutionMode.HYBRID]:
            ...
            max_processes = min(
                self.limits.max_concurrent_tasks // 2, multiprocessing.cpu_count()
            )
    ...
    async def start(self):
        ...
        logger.info(
            f"Started ParallelExecutor with {self.limits.max_concurrent_tasks} workers"
        )
    ...
    async def submit_task(self, task: Task, priority: int = 1) -> str:
        ...
        if self.task_queue.full():
            raise RuntimeError(
                f"Task queue is full (max: {self.limits.max_queue_size})"
            )
    ...
    async def _execute_in_process(self, task: Task) -> Result:
        ...
        return await loop.run_in_executor(
            self.process_executor, _execute_task_in_process, task
        )
            ...
            try:
                async with self._lock:
                    ...
                    self.metrics.peak_cpu_usage = max(
                        self.metrics.peak_cpu_usage, usage.cpu_percent
                    )
                    self.metrics.peak_memory_usage = max(
                        self.metrics.peak_memory_usage, usage.memory_mb
                    )
...
class BatchExecutor:
    ...
    def __init__(
        ...
        self.executor = parallel_executor or ParallelExecutor(
            mode=ExecutionMode.HYBRID, config=config
        )
        self.config = config or BenchmarkConfig()
        self.results: Dict[str, List[Result]] = {}

    async def run_benchmarks(
        self, objectives: List[str], priorities: Optional[List[int]] = None
    ) -> Dict[str, Any]:
    ...
    async def run_benchmark_suite(self, suite_config: Dict[str, Any]) -> Dict[str, Any]:
