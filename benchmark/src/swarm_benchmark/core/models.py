...
class Agent:
    ...
    def update_performance(self, metrics: PerformanceMetrics) -> None:
        ...
        execution_times = [
            m.execution_time for m in self.performance_history if m.execution_time > 0
        ]
        if execution_times:
...
class BenchmarkMetrics:
    ...
    def update_from_results(self, results: List[Result]) -> None:
        ...
        execution_times = [
            r.performance_metrics.execution_time
            for r in results
            if r.performance_metrics.execution_time > 0
        ]
        if execution_times:
            self.average_execution_time = sum(execution_times) / len(execution_times)
        ...
        quality_scores = [
            ...
            for r in results
            if r.quality_metrics.overall_quality > 0
        ]
        if quality_scores:
        ...
        memory_usage = [
            r.resource_usage.peak_memory_mb
            for r in results
            if r.resource_usage.peak_memory_mb > 0
        ]
        if memory_usage:
