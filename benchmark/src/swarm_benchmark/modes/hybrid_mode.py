...
class HybridMode(BaseCoordinationMode):
    ...
    async def coordinate(self, agents: List[Agent], tasks: List[Task]) -> List[Result]:
        ...
        for strategy_name, (
            strategy_agents,
            strategy_tasks,
        ) in strategy_partitions.items():
            if strategy_tasks:
                strategy_mode = self.coordination_strategies[strategy_name]
                strategy_results = await strategy_mode.coordinate(
                    strategy_agents, strategy_tasks
                )
                all_results.extend(strategy_results)

                # Track strategy usage
                self.strategy_usage[strategy_name] = self.strategy_usage.get(
                    strategy_name, 0
                ) + len(strategy_tasks)
    ...
    def _partition_tasks_by_strategy(
    ...
    def _estimate_task_complexity(self, task: Task) -> float:
        ...
        complexity_factors.append(
            strategy_complexity.get(task.strategy.value.lower(), 0.5)
        )
    ...
    def _distribute_agents_to_strategies(
        self, agents: List[Agent], partitions: Dict[str, tuple]
    ) -> None:
    ...
    def _calculate_adaptation_score(self) -> float:
        ...
        used_strategies = len(
            [count for count in self.strategy_usage.values() if count > 0]
        )
        ...
        if total_usage == 0:
            ...
            variance = sum(
                (count - avg_usage) ** 2 for count in self.strategy_usage.values()
            )
            normalized_variance = variance / (avg_usage**2) if avg_usage > 0 else 0
            balance_score = max(
                0.0, 1.0 - normalized_variance / 4.0
            )  # Normalize to 0-1
