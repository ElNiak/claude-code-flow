...
@click.option(
    ...
    type=int,
    default=300,
    help="Individual task timeout in seconds (default: 300)",
)
@click.option(
    "--max-retries", type=int, default=3, help="Maximum retries per task (default: 3)"
)
...
@click.option(
    "--real-metrics", is_flag=True, help="Use real metrics collection (default: False)"
)
@click.pass_context
def run(
    ctx,
    ...
    try:
        ...
        if result:
            ...
            if "metrics" in result and real_metrics:
                ...
                click.echo(
                    f"  • Wall clock time: {metrics.get('wall_clock_time', 0):.2f}s"
                )
                click.echo(
                    f"  • Tasks per second: {metrics.get('tasks_per_second', 0):.2f}"
                )
                ...
                click.echo(
                    f"  • Average CPU: {metrics.get('average_cpu_percent', 0):.1f}%"
                )
                click.echo(
                    f"  • Total output: {metrics.get('total_output_lines', 0)} lines"
                )
        else:
...
@click.option(
    "--all-modes", is_flag=True, help="Test all SPARC modes and swarm strategies"
)
@click.option("--max-agents", type=int, default=5, help="Maximum agents (default: 5)")
@click.option(
    "--timeout", type=int, default=60, help="Timeout in minutes (default: 60)"
...
def real(
    ...
    try:
        result = asyncio.run(
            _run_real_benchmark(objective, config, sparc_mode, all_modes)
        )

        if result:
            click.echo("✅ Real benchmark completed successfully!")
            click.echo(f"📊 Results saved to: {output_dir}")
            if ctx.obj.get("verbose"):
                click.echo(f"📋 Summary: {result.get('summary', 'N/A')}")
        else:
...
async def _run_benchmark(
    objective: str, config: BenchmarkConfig, use_real_metrics: bool = False
) -> Optional[dict]:
