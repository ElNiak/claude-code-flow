...
class SQLiteManager:
    ...
    async def _ensure_database(self) -> None:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_benchmarks_strategy ON benchmarks (strategy)"
            )
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_benchmarks_mode ON benchmarks (mode)"
            )
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_tasks_benchmark_id ON tasks (benchmark_id)"
            )
            await db.execute(
                "CREATE INDEX IF NOT EXISTS idx_results_benchmark_id ON results (benchmark_id)"
            )
            await db.execute(
    ...
    async def _insert_benchmark(
        self, db: aiosqlite.Connection, benchmark: Benchmark
    ) -> None:
    ...
    async def _insert_task(
        self, db: aiosqlite.Connection, task: Task, benchmark_id: str
    ) -> None:
    ...
    async def _insert_result(
        self, db: aiosqlite.Connection, result: Result, benchmark_id: str
    ) -> None:
        ...
        """,
            (
                ...
                json.dumps(
                    self._performance_metrics_to_dict(result.performance_metrics)
                ),
