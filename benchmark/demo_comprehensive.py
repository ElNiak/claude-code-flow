...
async def run_comprehensive_demo():
        ...
        try:
            ...
            if result["status"] == "success":
                ...
                if result["results"]:
                    ...
                    cpu_usage = first_result.get("resource_usage", {}).get(
                        "cpu_percent", 0
                    )
                    memory_usage = first_result.get("resource_usage", {}).get(
                        "memory_mb", 0
                    )
    ...
    if successful_tests:
        print("\n🏆 Performance Metrics:")
        avg_duration = sum(r.get("duration", 0) for r in successful_tests) / len(
            successful_tests
        )
        avg_execution = sum(r.get("execution_time", 0) for r in successful_tests) / len(
            successful_tests
        )
        ...
        for strategy, durations in strategy_performance.items():
            ...
            print(
                f"   {strategy.capitalize()}: {avg_duration:.2f}s avg ({len(durations)} tests)"
            )
        ...
        for mode, durations in mode_performance.items():
            avg_duration = sum(durations) / len(durations)
            print(
                f"   {mode.capitalize()}: {avg_duration:.2f}s avg ({len(durations)} tests)"
            )
        ...
        json.dump(
            {
                ...
                "summary_statistics": {
                    ...
                    "strategy_coverage": len(
                        set(r["strategy"] for r in results_summary)
                    ),
    ...
    print("📁 Individual benchmark reports saved to: ./demo_reports/")
