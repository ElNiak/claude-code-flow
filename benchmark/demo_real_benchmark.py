...
async def demo_basic_benchmark():
    ...
    sparc_config = SparcConfig(
        mode="coder", prompt="Create a fibonacci function in Python"
    )
...
async def demo_parallel_benchmark():
    ...
    for task in tasks:
        future = await parallel_executor.submit_task(
            task["func"], priority=task["priority"], task_name=task["name"]
        )
    ...
    for name, future in futures:
        try:
            result = await future
            print(
                f"✅ {name}: Success={result.success}, Time={result.execution_time:.2f}s"
            )
...
async def demo_comprehensive_benchmark():
    ...
    results = await orchestrator.run_parallel_benchmarks(
        benchmarks, max_parallel=3, monitor_resources=True
    )
    ...
    for suite_name, suite_results in results.items():
        ...
        print(
            f"    - Success Rate: {success_count}/{total_count} ({success_count/total_count:.1%})"
        )
        ...
        for result in suite_results:
            ...
            print(
                f"    {status} {result.get('task_name', 'Unknown')}: {result.get('execution_time', 0):.2f}s"
            )
