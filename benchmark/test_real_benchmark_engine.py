...
from swarm_benchmark.core.real_benchmark_engine import (
    RealBenchmarkEngine,
    SystemMonitor,
)
...
class RealBenchmarkTester:
    ...
    async def test_sparc_modes(self):
        ...
        config = BenchmarkConfig(
            name="sparc-test", task_timeout=45, output_formats=["json"]
        )
        ...
        for mode in test_modes:
            ...
            try:
                ...
                result = await engine._execute_sparc_mode(
                    mode, "Create a simple calculator function"
                )
    ...
    async def test_parallel_execution(self):
        ...
        config = BenchmarkConfig(
            name="parallel-test", parallel=True, max_agents=3, task_timeout=30
        )
        ...
        tasks = [
            Task(
                objective="Create a function to add numbers", strategy=StrategyType.AUTO
            ),
            Task(
                objective="Create a function to multiply numbers",
                strategy=StrategyType.AUTO,
            ),
            Task(
                objective="Create a function to divide numbers",
                strategy=StrategyType.AUTO,
            ),
        ]
    ...
    async def test_resource_monitoring(self):
        ...
        config = BenchmarkConfig(name="resource-test", monitoring=True, task_timeout=30)
        ...
        try:
            ...
            print(
                f"   CPU Usage: {system_summary.get('avg_cpu', 0):.1f}% (avg), {system_summary.get('max_cpu', 0):.1f}% (max)"
            )
            print(
                f"   Memory Usage: {system_summary.get('avg_memory', 0):.1f}% (avg), {system_summary.get('max_memory', 0):.1f}% (max)"
            )
            ...
            if result.get("results") and len(result["results"]) > 0:
                ...
                print(
                    f"   Process CPU: {resource_usage.get('average_cpu_percent', 0):.1f}%"
                )
                print(
                    f"   Process Memory: {resource_usage.get('peak_memory_mb', 0):.1f} MB"
                )
