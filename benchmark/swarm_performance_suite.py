...
class SwarmPerformanceBenchmark:
    ...
    async def run_full_suite(self) -> Dict[str, Any]:
        ...
        report = await self._generate_performance_report(
            category_results, suite_start, suite_end
        )
    ...
    async def _test_startup_performance(self) -> List[SwarmBenchmarkResult]:
        ...
        result = await self._benchmark_operation(
            "swarm_init_basic",
            lambda: self._run_claude_flow_command(
                ["swarm", "init", "--mode", "mesh", "--agents", "3"]
            ),
        ...
        result = await self._benchmark_operation(
            "swarm_init_large",
            lambda: self._run_claude_flow_command(
                ["swarm", "init", "--mode", "hierarchical", "--agents", "8"]
            ),
            target_time=self.targets.swarm_init_time_seconds
            * 2,  # Allow more time for larger swarms
        )
        tests.append(result)
    ...
    async def _test_memory_performance(self) -> List[SwarmBenchmarkResult]:
        ...
        result = await self._benchmark_operation(
            ...
            target_time=self.targets.persistence_overhead_percent
            / 100.0,  # Convert to multiplier
        )
        tests.append(result)
    ...
    async def _test_scalability(self) -> List[SwarmBenchmarkResult]:
        ...
        for count in agent_counts:
            result = await self._benchmark_operation(
                ...
                target_time=self.targets.swarm_init_time_seconds
                * (1 + count / 10),  # Linear scaling expectation
            )
            tests.append(result)
    ...
    async def _benchmark_memory_operation(
        self, test_name: str, operation, target_growth_mb: float
    ) -> SwarmBenchmarkResult:
        ...
        try:
            ...
            target_met = (
                memory_metrics["memory_growth_mb"] <= target_growth_mb
                and not memory_metrics["leak_detected"]
            )
    ...
    def _run_claude_flow_command(
        self, args: List[str], clear_cache: bool = False
    ) -> str:
    ...
    def _measure_agent_spawn_time(self) -> float:
        ...
        start = time.time()
        output = self._run_claude_flow_command(
            ["swarm", "spawn", "researcher", "--name", "test-agent"]
        )
        end = time.time()
        return end - start
    ...
    def _measure_coordination_overhead(self) -> float:
        ...
        start = time.time()
        output = self._run_claude_flow_command(
            ["swarm", "task", "simple test task", "--agents", "3"]
        )
        end = time.time()
        return end - start
    ...
    def _run_scalability_test(self, agent_count: int) -> float:
        ...
        start = time.time()
        output = self._run_claude_flow_command(
            ["swarm", "init", "--agents", str(agent_count)]
        )
        end = time.time()
        return end - start
    ...
    async def _generate_performance_report(
        self,
        category_results: Dict[str, List[SwarmBenchmarkResult]],
    ...
    def _generate_recommendations(
        ...
        slow_startup_tests = [
            r for r in results if "init" in r.test_name and not r.target_met
        ]
        if slow_startup_tests:
            recommendations.append(
                "Consider optimizing swarm initialization by implementing connection pooling"
            )

        memory_issues = [
            r for r in results if "memory" in r.test_name and not r.target_met
        ]
        if memory_issues:
            recommendations.append(
                "Memory usage exceeded targets - investigate memory leaks in agent cleanup"
            )

        coordination_issues = [
            r for r in results if "coordination" in r.test_name and not r.target_met
        ]
        if coordination_issues:
            recommendations.append(
                "Agent coordination latency high - consider implementing async message passing"
            )

        scalability_issues = [
            r for r in results if "scalability" in r.test_name and not r.target_met
        ]
        if scalability_issues:
            recommendations.append(
                "Scalability limits reached - implement load balancing and resource pooling"
            )

        if not recommendations:
            recommendations.append(
                "All performance targets met - consider tightening targets for continued optimization"
            )
        ...
        for result in memory_tests:
            if "memory_metrics" in result.metrics:
                memory_values.append(
                    result.metrics["memory_metrics"].get("peak_memory_mb", 0)
                )
    ...
    async def _identify_bottlenecks(
        ...
        for result in results:
            if result.success and not result.target_met:
                if (
                    "target_seconds" in result.metrics
                    and result.duration_seconds > result.metrics["target_seconds"] * 2
                ):
                    bottlenecks.append(
                        {
                            "test": result.test_name,
                            ...
                            "slowdown_factor": result.duration_seconds
                            / result.metrics["target_seconds"],
                        }
                    )
    ...
    async def _save_results(self, report: Dict[str, Any]):
        ...
        with open(csv_file, "w") as f:
            ...
            for category, results in report["category_results"].items():
                for result in results:
                    f.write(
                        f"{result['test_name']},{result['duration_seconds']},{result['target_met']},{result['success']},{category}\n"
                    )
...
async def main():
    ...
    if args.quick:
        ...
        print(
            f"Quick check completed: {len([r for r in result if r.target_met])}/{len(result)} targets met"
        )
    else:
        # Run full suite
