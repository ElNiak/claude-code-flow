...
class CIPerformanceGate:
    ...
    def _load_config(self, config_file: Optional[str]) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "optional_tests": ["scalability_agents_8", "neural_pattern_processing"],
            "ci_environment": {
                "timeout_minutes": 15,
    ...
    async def run_performance_gate(self) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            required_failures = [
                r for r in required_results if not r.get("target_met", False)
            ]
            if required_failures:
                gate_passed = False
                print(
                    f"❌ {len(required_failures)} required tests failed performance targets"
                )
            else:
    ...
    def _generate_ci_recommendations(
        ...
        if failed_required:
            recommendations.append(
                f"❌ {len(failed_required)} required performance tests failed - consider optimization before merge"
            )
        ...
        failed_optional = [
            r for r in optional_results if not r.get("target_met", False)
        ]
        if failed_optional:
            recommendations.append(
                f"⚠️ {len(failed_optional)} optional tests failed - monitor for trends"
            )
    ...
    def _generate_junit_xml(self, gate_results: Dict[str, Any], output_file: Path):
        ...
        for category, results in all_results.items():
            for result in results:
                ...
                xml_content.append(
                    f'  <testcase name="{test_name}" classname="PerformanceTest" time="{duration}">'
                )
...
async def main():
    ...
    parser.add_argument(
        "--create-config", action="store_true", help="Create CI configuration file"
    )
    ...
    try:
        ...
        gate_passed, results = await asyncio.wait_for(
            gate.run_performance_gate(), timeout=args.timeout * 60
        )
