...
def test_real_claude_flow():
    ...
    result = subprocess.run(
        [str(claude_flow_path), "--version"], capture_output=True, text=True
    )
    duration = time.time() - start

    print("   Command: claude-flow --version")
    ...
    start = time.time()
    result = subprocess.run(
        [str(claude_flow_path), "sparc", "list"], capture_output=True, text=True
    )
    duration = time.time() - start

    print("   Command: claude-flow sparc list")
    print(f"   Success: {result.returncode == 0}")
    print(f"   Duration: {duration:.3f}s")
    ...
    print("   Command: claude-flow sparc coder 'Write hello world' --dry-run")
    print(f"   Success: {result.returncode == 0}")
    print(f"   Duration: {duration:.3f}s")
    ...
    print(
        "   Command: claude-flow swarm 'Build a calculator' --strategy development --mode centralized --dry-run"
    )
    print(f"   Success: {result.returncode == 0}")
    print(f"   Duration: {duration:.3f}s")
