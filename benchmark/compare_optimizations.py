...
from benchmark.src.swarm_benchmark.core.optimized_benchmark_engine import (
    OptimizedBenchmarkEngine,
)


async def run_comparison(
    objective: str, strategy: StrategyType, mode: CoordinationMode
):
    ...
    if "performance_metrics" in optimized_result:
        ...
        if "executor" in metrics:
            print(
                f"  - Cache hit rate: {metrics['executor'].get('cacheHitRate', 0):.2%}"
            )
            print(
                f"  - Active executions: {metrics['executor'].get('activeExecutions', 0)}"
            )
        if "cache" in metrics:
...
async def main():
    ...
    test_cases = [
        ...
        (
            "Create test suite for authentication module",
    ...
    print(f"{'='*60}")
    print(
        f"{'Strategy':<15} {'Mode':<15} {'Standard':<10} {'Optimized':<10} {'Improvement':<12} {'Speedup':<8}"
    )
    print(f"{'-'*80}")
    ...
    for result in results:
        print(
            f"{result['strategy']:<15} {result['mode']:<15} "
