...
class PerformanceTestOrchestrator:
        ...
        try:
            ...
            return {
                ...
                "critical_regressions": [
                    k for k, v in regressions.items() if v["regression_percent"] > 25
                ],
                "success": True,
            }
    ...
    def _generate_summary(
        self, test_results: Dict[str, Any], end_time: datetime
    ) -> Dict[str, Any]:
        ...
        if critical_regressions:
            critical_issues.append(
                f"{len(critical_regressions)} critical performance regressions"
            )
        ...
        return {
            ...
            if not critical_issues and successful_tests == total_tests
            else "failed",
    ...
    def _calculate_overall_score(self, results: Dict[str, Any]) -> float:
        ...
        if "results" in benchmark_results and benchmark_results.get("mode") == "full":
            scores.append(
                ...
                .get("summary", {})
                .get("performance_score", 50)
            )
        elif "passed_count" in benchmark_results and "test_count" in benchmark_results:
            scores.append(
                (benchmark_results["passed_count"] / benchmark_results["test_count"])
                * 100
            )
        ...
        if regression_results.get("success", False):
            critical_regressions = len(
                regression_results.get("critical_regressions", [])
            )
    ...
    async def _save_comprehensive_results(
        ...
        results_file = (
            output_dir / f"comprehensive_performance_results_{timestamp}.json"
        )
    ...
    def _print_final_summary(self, test_results: Dict[str, Any]):
        ...
        print(
            f"⏱️  Total Duration: {summary.get('total_duration_minutes', 0):.1f} minutes"
        )
        print(f"📊 Performance Score: {summary.get('performance_score', 0):.1f}/100")
        print(
            f"🧪 Tests: {summary.get('successful_tests', 0)}/{summary.get('total_tests', 0)} successful"
        )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive Performance Test Runner"
    )
    ...
    parser.add_argument(
        "--benchmark-only", action="store_true", help="Run benchmark suite only"
    )
    parser.add_argument(
        "--monitor-only", action="store_true", help="Run continuous monitoring only"
    )
    parser.add_argument(
        "--output-dir", default="performance_results", help="Output directory"
    )
    parser.add_argument(
        "--duration", type=int, default=10, help="Monitoring duration in minutes"
    )
