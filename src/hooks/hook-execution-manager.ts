...
import { CircuitBreaker, CircuitBreakerManager } from '../utils/circuit-breaker.js';
...
export class HookExecutionQueue extends EventEmitter {
     ...
     */
    async enqueue(hookType: HookType, args: string[], priority: 'low' | 'medium' | 'high' = 'medium'): Promise<any> {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.emit('hook_queued', { taskId, hookType, queueLength: this.queue.length });
    ...
    private async executeHookTask(task: HookTask): Promise<void> {
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                const result = await this.executeHookWithTimeout(task.hookType, task.args);
     ...
     */
    private async executeHookWithTimeout(hookType: HookType, args: string[]): Promise<{ output: string; exitCode: number }> {
        const timeout = HOOK_TIMEOUTS[hookType];

        return Promise.race([
            this.executeHookProcess(hookType, args),
            new Promise<never>((_, reject) =>
                setTimeout(() => reject(new Error(`Hook ${hookType} timed out after ${timeout}ms`)), timeout)
            )
        ]);
    }
     ...
     */
    private async executeHookProcess(hookType: HookType, args: string[]): Promise<{ output: string; exitCode: number }> {
        return new Promise((resolve, reject) => {
            const childProcess = spawn('npx', ['claude-flow', 'hooks', hookType, ...args], {
            ...
            childProcess.on('close', (code: number | null) => {
                if (code === 0) {
                    ...
                    reject(new Error(`Hook ${hookType} failed with exit code ${code}: ${errorOutput}`));
                }
            });
     ...
     */
    private recordExecutionResult(hookType: HookType, result: HookExecutionResult): void {
    ...
    getExecutionStats(): Record<string, any> {
        ...
        this.executionStats.forEach((results, hookType) => {
            ...
            stats[hookType] = {
                ...
                minDuration: results.length > 0 ? Math.min(...results.map(r => r.duration)) : 0,
                maxDuration: results.length > 0 ? Math.max(...results.map(r => r.duration)) : 0
            };
        });
     ...
     */
    getQueueStatus(): { length: number; executing: boolean; priorities: Record<string, number> } {
...
export class HookProcessPool extends EventEmitter {
     ...
     */
    async executeHook(hookType: HookType, args: string[]): Promise<{ output: string; exitCode: number }> {
     ...
     */
    private async executeOnProcess(process: ChildProcess, hookType: HookType, args: string[]): Promise<{ output: string; exitCode: number }> {
        return new Promise((resolve, reject) => {
            const timeout = HOOK_TIMEOUTS[hookType];
            let output = '';
            let errorOutput = '';
...
export async function executeHookSafely(hookType: HookType, args: string[], priority: 'low' | 'medium' | 'high' = 'medium'): Promise<any> {
