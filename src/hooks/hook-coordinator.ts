...
export class HookCoordinator extends EventEmitter {
	...
	constructor(
		options: { lockDirectory?: string; maxConcurrentHooks?: number } = {}
	) {
		super();

		this.lockDirectory =
			options.lockDirectory ||
			path.join(process.cwd(), ".claude-flow", "locks");
	...
	async coordinateHook(
		...
		const priority =
			options.priority || this.getDependency(hookType)?.priority || "medium";
	...
	private async acquireCoordinationLock(
		...
		for (let attempt = 1; attempt <= maxRetries; attempt++) {
			try {
				...
				if (coordLockExists) {
					// Read existing lock
					const existingLockData = await fs.readFile(
						this.coordLockFile,
						"utf8"
					);
				...
				// Create new lock
				await fs.writeFile(this.coordLockFile, JSON.stringify(lock), {
					flag: "wx",
				});
	...
	private isHookRunning(hookType: HookType): boolean {
		return Array.from(this.pendingExecutions.values()).some(
	...
	private async cleanupExpiredLocks(): Promise<void> {
		const now = Date.now();
		const expiredLocks = Array.from(this.activeLocks.entries()).filter(
	...
	private cleanupStaleExecutions(): void {
		...
		const staleExecutions = Array.from(this.pendingExecutions.entries()).filter(
