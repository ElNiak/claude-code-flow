...
import { HookExecutionQueue, HookProcessPool, HookType } from './hook-execution-manager.js';
...
export class HookIntegrationTests {
    ...
    private async testTimeoutEnforcement(): Promise<void> {
        ...
        try {
            ...
            const timeoutPromise = this.queue.enqueue('pre-task', ['--description', 'Long running task'], 'high');
            ...
            if (duration < 6000) { // Should complete within 5 seconds + buffer
                this.results.push({
                    name: testName,
                    passed: true,
                    duration,
    ...
    private async testPriorityOrdering(): Promise<void> {
        ...
        try {
            ...
            const promises = [
                ...
                this.queue.enqueue('notify', ['--message', 'Medium priority'], 'medium')
            ];
    ...
    private async testDeadlockPrevention(): Promise<void> {
        ...
        try {
            ...
            const promises = [
                this.coordinator.coordinateHook('pre-edit', ['--file', 'file1.ts'], { priority: 'high' }),
                this.coordinator.coordinateHook('post-edit', ['--file', 'file1.ts'], { priority: 'high' }),
                this.coordinator.coordinateHook('pre-task', ['--description', 'Task 1'], { priority: 'high' })
            ];
    ...
    private async testDependencyResolution(): Promise<void> {
        ...
        try {
            ...
            const promises = [
                this.coordinator.coordinateHook('pre-task', ['--description', 'Parent task'], { priority: 'high' }),
                this.coordinator.coordinateHook('pre-edit', ['--file', 'dependent.ts'], { priority: 'medium' }),
                this.coordinator.coordinateHook('post-edit', ['--file', 'dependent.ts'], { priority: 'medium' })
            ];
    ...
    private async testCircularDependencyDetection(): Promise<void> {
        ...
        try {
            ...
            await this.coordinator.coordinateHook('pre-task', ['--description', 'Test task'], { priority: 'high' });
    ...
    private async testHookStampedePrevention(): Promise<void> {
        ...
        try {
            ...
            for (let i = 0; i < 20; i++) {
                promises.push(
                    this.coordinator.coordinateHook('pre-task', [`--description`, `Stampede task ${i}`], { priority: 'high' })
                );
            }
    ...
    private async testErrorRecovery(): Promise<void> {
        ...
        try {
            ...
            try {
                await this.coordinator.coordinateHook('pre-task', ['--invalid-arg'], { priority: 'high' });
            } catch (error) {
                // Expected to fail
            }
     ...
     */
    private generateTestReport(): { passed: number; failed: number; results: TestResult[] } {
        ...
        console.log(`Average Time: ${(totalTime / this.results.length).toFixed(2)}ms`);
     ...
     * Run load test
     */
    async runLoadTest(duration: number = 30000, concurrency: number = 10): Promise<LoadTestResult> {
        ...
        console.log(`  Success Rate: ${((loadTestResult.successful / loadTestResult.totalRequests) * 100).toFixed(1)}%`);
     ...
     * Load test worker
     */
    private async loadTestWorker(workerId: number, endTime: number, results: any): Promise<void> {
        while (Date.now() < endTime) {
            const startTime = performance.now();
            try {
                await this.wrapper.notify(`Load test worker ${workerId} message`, { telemetry: false });
