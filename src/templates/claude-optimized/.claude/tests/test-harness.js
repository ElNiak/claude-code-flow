...
class TestHarness {
	...
	generateLargeProject(fileCount) {
		...
		for (let i = 0; i < fileCount; i++) {
			project[`src/module${i}.js`] =
				`export const module${i} = () => { return ${i}; };`;
			project[`test/module${i}.test.js`] =
				`describe("Module ${i}", () => { /* tests */ });`;
		}
		return project;
	}
	...
	generateComplexProject() {
		return {
			...
			"src/services/user.service.ts":
				"export class UserService { /* implementation */ }",
			"src/controllers/auth.controller.ts":
				"export class AuthController { /* implementation */ }",
			...
			"config/default.json":
				'{ "port": 3000, "database": "mongodb://localhost" }',
