...
if (!fs.existsSync(TARGET_PROJECT)) {
	console.error(
		`Error: Target project directory does not exist: ${TARGET_PROJECT}`
	);
	process.exit(1);
}
...
const hasProjectFile = projectFiles.some((file) =>
	fs.existsSync(path.join(TARGET_PROJECT, file))
);
...
if (!fs.existsSync(TARGET_DIR)) {
	...
	console.log(
		"⚠️  .claude directory already exists - files will be overwritten"
	);
}
