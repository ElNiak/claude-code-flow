...
for (const [category, info] of Object.entries(manifest.categories)) {
	const actualCount = manifest.files.filter(
...
if (passedTests === totalTests) {
	log(
		"\n🎉 Template validation passed! All files are properly installed.",
		"green"
	);
...
log(
	`  Command files: ${manifest.files.filter((f) => f.category === "command").length}`
);
log(
	`  SPARC mode files: ${manifest.files.filter((f) => f.category === "sparc-mode").length}`
);
log(
	`  Test files: ${manifest.files.filter((f) => f.category === "test").length}`
);
