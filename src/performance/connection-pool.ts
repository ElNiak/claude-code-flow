...
export class ConnectionPool extends EventEmitter {
  ...
  private handleConnectionFailure(connection: Connection): void {
    ...
    if (this.config.enableAutoReconnect && connection.retryCount < this.config.maxRetries) {
      setTimeout(() => {
  ...
  release(connection: Connection): void {
    ...
    connection.avgResponseTime =
      (connection.avgResponseTime * (connection.requestCount - 1) + responseTime) / connection.requestCount;
  ...
  private processPendingRequests(): void {
    if (this.pendingRequests.size === 0 || this.availableConnections.size === 0) {
      return;
    }
    ...
    if (activeConnections.length > 0) {
      await new Promise<void>((resolve) => {
        const checkInterval = setInterval(() => {
          const stillActive = Array.from(this.connections.values())
            .filter(conn => !conn.isIdle);
