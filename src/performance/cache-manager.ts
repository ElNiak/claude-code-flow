...
export class AdvancedCacheManager extends EventEmitter {
  ...
  getStats(): CacheStats {
    ...
    const compressionRatio = this.stats.compressionSaved / (this.stats.compressionSaved + this.cache.size) || 0;
...
export class SymbolCache extends AdvancedCacheManager {
  ...
  async cacheSymbol(filePath: string, symbols: any[], ast?: any): Promise<void> {
...
export function createCacheManager(type: 'general' | 'symbols' | 'ast', config: Partial<CacheConfig> = {}): AdvancedCacheManager {
