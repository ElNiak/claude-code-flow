...
export async function createMonitoringSystem(
  ...
  const { defaultMonitoringConfig, mergeMonitoringConfig } = await import("./monitoring-config.js");
  const { MonitoringSystemIntegration } = await import("./monitoring-integration.js");

  const finalConfig = mergeMonitoringConfig(defaultMonitoringConfig, config);

  const monitoringSystem = new MonitoringSystemIntegration(
    logger,
    eventBus,
    {
      ...
      reportingConfig: {
        ...
        format: (finalConfig.reporting.formats[0]?.type === "csv" ? "json" : finalConfig.reporting.formats[0]?.type) || "json",
      ...
      exportConfig: {
        ...
        format: (finalConfig.export.formats[0]?.type === "parquet" ? "json" : finalConfig.export.formats[0]?.type) || "json",
        destinations: finalConfig.export.destinations.map(d => d.config?.path || "./logs/monitoring-export"),
...
export async function createProductionMonitoringSystem(
  ...
  const productionConfig = {
    ...
    dashboard: {
      ...
      authentication: { enabled: config.enableAuth || false, type: "basic" as const, config: {} }
    },
    diagnostics: {
      enabled: true,
    ...
    export: {
      ...
      destinations: [{ type: "file" as const, config: { path: "./logs/monitoring-export" } }],
      schedule: {
