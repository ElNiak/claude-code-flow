...
class PerformanceMonitor:
    ...
    def _log_metrics(self, metrics: Dict[str, Any]):
        ...
        if summary_parts:
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] " + " | ".join(summary_parts)
            )
    ...
    def _check_alerts(self, metrics: Dict[str, Any]):
        ...
        if "connection_pool" in metrics:
            ...
            if metrics["connection_pool"]["waiting_queue"] > 5:
                alerts.append(
                    f"Connection queue backed up: {metrics['connection_pool']['waiting_queue']} waiting"
                )
    ...
    def _calculate_summary(self) -> Dict[str, Any]:
        ...
        if any("executor" in m for m in self.metrics_history):
            exec_metrics = [
                m["executor"] for m in self.metrics_history if "executor" in m
            ]
            summary["executor"] = {
                "total_tasks": exec_metrics[-1]["total_executed"]
                if exec_metrics
                else 0,
                "avg_success_rate": sum(m["success_rate"] for m in exec_metrics)
                / len(exec_metrics)
                if exec_metrics
                else 0,
                "avg_cache_hit_rate": sum(m["cache_hit_rate"] for m in exec_metrics)
                / len(exec_metrics)
                if exec_metrics
                else 0,
                "avg_execution_time": sum(m["avg_execution_time"] for m in exec_metrics)
                / len(exec_metrics)
                if exec_metrics
                else 0,
            }
        ...
        if any("connection_pool" in m for m in self.metrics_history):
            pool_metrics = [
                ...
                if "connection_pool" in m
            ]
            summary["connection_pool"] = {
                "avg_utilization": sum(m["utilization"] for m in pool_metrics)
                / len(pool_metrics)
                if pool_metrics
                else 0,
                "max_connections": max(m["total_connections"] for m in pool_metrics)
                if pool_metrics
                else 0,
                "total_uses": pool_metrics[-1]["total_use_count"]
                if pool_metrics
                else 0,
            }
        ...
        if any("system" in m and m["system"] for m in self.metrics_history):
            ...
            summary["system"] = {
                "avg_cpu_percent": sum(m.get("cpu_percent", 0) for m in sys_metrics)
                / len(sys_metrics)
                if sys_metrics
                else 0,
                "max_memory_mb": max(m.get("memory_mb", 0) for m in sys_metrics)
                if sys_metrics
                else 0,
                "avg_memory_mb": sum(m.get("memory_mb", 0) for m in sys_metrics)
                / len(sys_metrics)
                if sys_metrics
                else 0,
            }
