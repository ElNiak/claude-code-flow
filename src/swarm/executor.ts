...
export class TaskExecutor extends EventEmitter {
	...
	constructor(config: Partial<ExecutionConfig> = {}) {
		...
		this.logger = new Logger(
			{
				level: this.getValidLogLevel(this.config.logLevel),
				format: "text",
				destination: "console",
			},
		...
		return new Promise((resolve, reject) => {
			const timeout = setTimeout(() => {
				...
				session.stop("Timeout").then(() => {
					reject(
						new Error(`Execution timed out after ${session.config.timeoutMs}ms`)
					);
				});
	...
	protected async collectResourceUsage(
		sessionId: string
