	...
	private async processChunkWithWorker(chunk: FileInfo[]): Promise<void> {
		return new Promise((resolve, reject) => {
			...
			const tryAssignWork = () => {
				...
				const availableWorkerIndex = pool.workers.findIndex(
					(_, index) => !pool.busy.has(index)
				);
				...
				const workerData = {
					files: chunk.map((file) => ({
						...
						permissions: this.options.preservePermissions
							? file.permissions
							: undefined,
		...
		for (const file of this.fileQueue) {
			...
			try {
				...
				if (destStats.size !== sourceStats.size) {
					throw new Error(
						`Size mismatch: ${destStats.size} != ${sourceStats.size}`
					);
				}
				...
				if (workerResult?.hash) {
					...
					if (sourceHash !== workerResult.hash) {
						throw new Error(
							`Hash mismatch: ${sourceHash} != ${workerResult.hash}`
						);
					}
				}
			} catch (error) {
...
export async function copyPromptsEnhanced(
	options: CopyOptions
): Promise<CopyResult> {
...
// Re-export types and functions from prompt-copier for compatibility
export type { CopyOptions, CopyResult, FileInfo } from "./prompt-copier.js";
export { copyPrompts } from "./prompt-copier.js";
