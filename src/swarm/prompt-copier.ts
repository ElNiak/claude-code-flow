...
export class PromptCopier extends EventEmitter {
	...
	async copy(): Promise<CopyResult> {
		...
		try {
			...
			const result: CopyResult = {
				...
				skippedFiles:
					this.fileQueue.length - this.copiedFiles.size - this.errors.length,
				errors: this.errors,
				duration,
			};
	...
	private async ensureDestinationDirectories(): Promise<void> {
		...
		for (const file of this.fileQueue) {
			const destDir = path.dirname(
				path.join(this.options.destination, file.relativePath)
			);
	...
	private async createBackupManifest(): Promise<string> {
		...
		const manifest = {
			...
			backups: Array.from(this.backupMap.entries()).map(
				([original, backup]) => ({
					original,
					backup,
				})
