...
export class AutoStrategy extends BaseStrategy {
	 ...
	 */
	override async decomposeObjective(
	...
	override async selectAgentForTask(
		...
		this.updateAgentPerformanceHistory(
			selectedAgent.id.id,
			scoredAgents[0].score
		);
		...
		return new Promise((resolve) => {
			setTimeout(() => {
				...
				for (const [factor, weight] of Object.entries(
					this.mlHeuristics.complexityFactors
				)) {
	...
	private async generateTasksWithBatching(
		...
		if (objective.strategy === "development") {
			tasks.push(
				...(await this.generateDevelopmentTasks(objective, complexity))
			);
	...
	private createParallelImplementationTasks(
		...
		const canParallelize = this.canParallelizeImplementation(
	...
	private async generateAutoTasks(
		...
		const optimalStructure = this.determineOptimalTaskStructure(
			patterns,
			taskTypes,
			complexity
		);
		...
		if (optimalStructure.requiresImplementation) {
			const implTasks = this.createOptimalImplementationTasks(
				objective,
				optimalStructure,
				baseId
			);
	...
	private determineOptimalTaskStructure(
		...
		return {
			...
			requiresImplementation:
				taskTypes.includes("development") || taskTypes.includes("coding"),
	...
	private createOptimalImplementationTasks(
		...
		return [
			this.createTaskDefinition({
				...
				estimatedDuration: Math.max(
					15 * 60 * 1000,
					structure.complexity * 8 * 60 * 1000
				),
				capabilities: ["code-generation", "file-system"],
	...
	private selectOptimalStrategy(
		...
		if (objective.description.toLowerCase().includes("analyze"))
			return "analysis";
