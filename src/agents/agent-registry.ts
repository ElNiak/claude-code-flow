...
export class AgentRegistry extends EventEmitter {
	...
	async updateAgent(
		...
		entry.tags = [
			...
			entry.agent.status,
			...entry.tags.filter(
				(t) => t !== entry.agent.type && t !== entry.agent.status
			),
		];
	...
	async getAgentEntry(agentId: string): Promise<AgentRegistryEntry | null> {
		...
		if (memoryEntry && memoryEntry.value) {
			...
			const registryEntry: AgentRegistryEntry =
				memoryEntry.value as AgentRegistryEntry;
	...
	async queryAgents(query: AgentQuery = {}): Promise<AgentState[]> {
		...
		if (query.createdAfter) {
			...
			const matchingEntries = entries.filter(
				(entry) => entry.createdAt >= query.createdAfter!
			);
			agents = matchingEntries.map((entry) => entry.agent);
		}
		...
		if (query.lastActiveAfter) {
			agents = agents.filter(
				(agent) => agent.metrics.lastActivity >= query.lastActiveAfter!
			);
		}
