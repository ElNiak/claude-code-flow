...
export class MessageBus extends EventEmitter {
	...
	async broadcastMessage(
		...
		let receivers = channel.participants.filter(
			(p) => (typeof p === "string" ? p : p.id) !== senderId
		);
	...
	private handleAgentDisconnected(agentId: AgentId | { id: string }): void {
		...
		for (const channel of this.channels.values()) {
			channel.participants = channel.participants.filter(
				(p) => (typeof p === "string" ? p : p.id) !== agentId.id
			);
		}
		...
		for (const [subId, subscription] of this.subscriptions) {
			if (
				(typeof subscription.subscriber === "string"
					...
					: subscription.subscriber.id) === agentId.id
			) {
...
class RetryManager extends EventEmitter {
	...
	private calculateDelay(attempts: number): number {
		...
		return Math.min(
			baseDelay * this.config.backoffMultiplier ** (attempts - 1),
