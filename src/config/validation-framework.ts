...
export class ConfigurationValidator {
	...
	private validateStructure(
		...
		if (!config._version) {
			...
			errors.push(
				this.createError("version", "Invalid version format", "INVALID_VERSION")
			);
		}
				...
				rules: [
					{
						...
						message:
							"Strictness must be one of: lenient, moderate, strict, pedantic",
					},
				],
			},
			"codeQuality.strictness.global": {
				...
				rules: [
					{
						...
						message:
							"Global strictness must be one of: minimal, relaxed, moderate, strict, pedantic",
					},
				],
			},
			"development.agents.defaultCount": {
				...
				rules: [
					{
						...
						message:
							"Security severity must be one of: low, medium, high, critical",
					},
				],
			},
		};
...
export class ValidationReporter {
	...
	static generateReport(result: ValidationResult): ValidationReport {
		return {
			...
			recommendations: ValidationReporter.generateRecommendations(result),
			nextSteps: ValidationReporter.generateNextSteps(result),
		};
	}
...
export class ConfigurationQualityAnalyzer {
	...
	static analyzeCompleteness(
		...
		return {
			...
			recommendations:
				ConfigurationQualityAnalyzer.generateCompletenessRecommendations(
					configuredSections,
					sections
				),
		};
	...
	static analyzeComplexity(
		...
		return {
			score: complexity,
			level,
			factors: ConfigurationQualityAnalyzer.identifyComplexityFactors(config),
			recommendations:
				ConfigurationQualityAnalyzer.generateComplexityRecommendations(level),
		};
	}
