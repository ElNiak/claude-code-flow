...
export class RuvSwarmConfigManager {
	...
	constructor(
		...
		this.configPath =
			configPath || join(process.cwd(), ".claude", "ruv-swarm-config.json");
	...
	updateIntegrationConfig(
		updates: Partial<RuvSwarmConfig["integration"]>
	): void {
		this.updateConfig({
	...
	validateConfig(): { valid: boolean; errors: string[] } {
		...
		if (
			this.config.memory.compressionLevel < 0 ||
			this.config.memory.compressionLevel > 9
		) {
		...
		if (
			this.config.neural.learningRate <= 0 ||
			this.config.neural.learningRate > 1
		) {
		...
		if (alertThresholds.memory < 0 || alertThresholds.memory > 100) {
			errors.push(
				"monitoring.alertThresholds.memory must be between 0 and 100"
			);
		}

		if (
			alertThresholds.taskFailureRate < 0 ||
			alertThresholds.taskFailureRate > 100
		) {
			errors.push(
				"monitoring.alertThresholds.taskFailureRate must be between 0 and 100"
			);
		}
	...
	getCommandArgs(): string[] {
		...
		if (this.config.memory.enablePersistence) {
			...
			args.push(
				"--compression-level",
				String(this.config.memory.compressionLevel)
			);
		...
		if (this.config.neural.enableTraining) {
			...
			args.push(
				"--training-iterations",
				String(this.config.neural.trainingIterations)
			);
		}
