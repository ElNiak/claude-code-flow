...
import { Config, ConfigManager, ConfigError } from './config-manager.js';
...
export class PresetManager {
  ...
  private async loadAgentCapabilities(): Promise<void> {
    try {
      ...
      throw new ConfigError(`Failed to load agent capabilities: ${(error as Error).message}`);
    }
  }
  ...
  async getAvailablePresets(): Promise<string[]> {
    try {
      ...
      throw new ConfigError(`Failed to read presets directory: ${(error as Error).message}`);
    }
  }
  ...
  async loadPreset(presetName: string): Promise<PresetConfig> {
    try {
      ...
      throw new ConfigError(`Failed to load preset '${presetName}': ${(error as Error).message}`);
    }
  }
   ...
   */
  async createAdaptiveConfig(projectAnalysis: ProjectAnalysis): Promise<PresetConfig> {
    ...
    } else if (projectAnalysis.type === 'data_analysis' || projectAnalysis.type === 'research') {
      basePreset = await this.loadPreset('research');
    } else if (projectAnalysis.type === 'infrastructure' || projectAnalysis.type === 'deployment') {
  ...
  private adaptConfigurationForProject(baseConfig: PresetConfig, analysis: ProjectAnalysis): PresetConfig {
    ...
    if (analysis.fileTypes && this.agentCapabilities) {
      ...
      analysis.fileTypes.forEach(fileType => {
        const agents = this.agentCapabilities!.agentSelectionRules.byFileType[fileType];
        if (agents) {
    ...
    if (timeRules) {
      ...
      if (timeRule && adaptedConfig.performance) {
        adaptedConfig.performance.parallelExecution = timeRule.parallelism === 'maximum';
        adaptedConfig.performance.intelligentBatching = timeRule.quality !== 'fast';
      }
    }
   ...
   */
  private async getProjectFiles(projectPath: string, maxDepth = 3): Promise<string[]> {
    const files: string[] = [];
   ...
   */
  private inferProjectComplexity(files: string[], fileTypes: string[]): 'low' | 'medium' | 'high' {
    const complexityIndicators = {
      languages: fileTypes.length,
      testFiles: files.filter(f => f.includes('test') || f.includes('spec')).length,
      configFiles: files.filter(f => f.includes('config') || f.includes('package.json')).length,
      buildFiles: files.filter(f => f.includes('webpack') || f.includes('rollup') || f.includes('babel')).length
    };
  ...
  private isDataIntensiveProject(fileTypes: string[]): boolean {
    ...
    return fileTypes.some(type => dataFileTypes.includes(type.replace('*', '')));
  }

  /**
   * Get recommended configuration for a project
   */
  async getRecommendedConfiguration(projectPath: string): Promise<PresetConfig> {
  ...
  async saveAsCustomPreset(config: PresetConfig, name: string): Promise<void> {
    const customPresetsDir = path.join(process.cwd(), 'config', 'custom-presets');
