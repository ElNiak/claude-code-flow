...
export class ConfigManager {
	...
	validate(config: Config): void {
		...
		if (
			config.orchestrator.taskQueueSize < 1 ||
			config.orchestrator.taskQueueSize > 10000
		) {
			throw new ConfigError(
				"orchestrator.taskQueueSize must be between 1 and 10000"
			);
		}
		...
		if (!["auto", "vscode", "native"].includes(config.terminal.type)) {
			throw new ConfigError(
				"terminal.type must be one of: auto, vscode, native"
			);
		}
		...
		if (!["sqlite", "markdown", "hybrid"].includes(config.memory.backend)) {
			throw new ConfigError(
				"memory.backend must be one of: sqlite, markdown, hybrid"
			);
		}
		...
		if (
			config.coordination.maxRetries < 0 ||
			config.coordination.maxRetries > 100
		) {
			throw new ConfigError(
				"coordination.maxRetries must be between 0 and 100"
			);
		}
		...
		if (!["stdio", "http", "websocket"].includes(config.mcp.transport)) {
			throw new ConfigError(
				"mcp.transport must be one of: stdio, http, websocket"
			);
		}
		...
		if (!["console", "file"].includes(config.logging.destination)) {
			throw new ConfigError(
				"logging.destination must be one of: console, file"
			);
		}

		// ruv-swarm validation,
		if (
			!["mesh", "hierarchical", "ring", "star"].includes(
				config.ruvSwarm.defaultTopology
			)
		) {
			throw new ConfigError(
		...
		if (
			!["balanced", "specialized", "adaptive"].includes(
				config.ruvSwarm.defaultStrategy
			)
		) {
			throw new ConfigError(
