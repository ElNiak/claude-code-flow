...
import type {
  ...
  WizardStep,
  ValidationRule} from "./user-configuration-system.js";
  ...
  config: {
        ...
        customRules: [
          {
            ...
            pattern: "class.*{[\\s\\S]*?(?:get|set|private|public|protected)[\\s\\S]*?(?:get|set|private|public|protected)[\\s\\S]*?}",
            severity: "warning",
            autoFix: false},
          {
            principle: "O",
            description: "Open/Closed Principle - Software entities should be open for extension, but closed for modification",
            pattern: "extends.*{[\\s\\S]*?(?:override|super)[\\s\\S]*?}",
            severity: "warning",
            autoFix: false},
          {
            principle: "L",
            description: "Liskov Substitution Principle - Objects should be replaceable with instances of their subtypes",
            pattern: "instanceof.*?\\?.*?:",
            severity: "warning",
            autoFix: false},
          {
            principle: "I",
            description: "Interface Segregation Principle - Many client-specific interfaces are better than one general-purpose interface",
            pattern: "interface.*{[\\s\\S]*?(?:[^;]{20}[\\s\\S]*?){3}",
            severity: "warning",
            autoFix: false},
          {
            principle: "D",
            description: "Dependency Inversion Principle - Depend on abstractions, not concretions",
        ...
        _rules: [
          {
            ...
            pattern: "class\\s+\\w+\\s*{[^}]*(?:function|method)[^}]*(?:function|method)[^}]*}",
            severity: "warning",
    ...
    _userExperience: {
      ...
      _learning: {
        ...
        _personalizedRecommendations: true}}} as any,
        ...
        options: [
          {
            value: "lenient",
            label: "Lenient",
            description: "Basic SOLID principle checks with minimal enforcement",
            recommended: false},
          {
            value: "moderate",
            label: "Moderate",
            ...
            recommended: true},
          {
            value: "strict",
            label: "Strict",
            description: "Comprehensive SOLID enforcement with detailed analysis",
            recommended: false}],
        validation: [
          {
            type: "required",
...
export const _legacySupportPreset: PresetTemplate = {
  _meta: {
    name: "legacy-support",
    version: "1.0.0",
    description: "Maintain compatibility with legacy systems and older codebases",
    author: "Claude Flow Team",
            ...
            description: "Support modern browsers only"}]},
      {
        ...
        options: [
            ...
            description: "Keep existing code as-is, minimal changes"},
          {
            ...
            description: "Incrementally modernize while maintaining compatibility"},
          {
            value: "aggressive",
            label: "Aggressive Modernization",
            description: "Modernize aggressively with polyfills for compatibility"}]}]}};
...
export const _apiDevelopmentPreset: PresetTemplate = {
  _meta: {
    ...
    version: "1.0.0",
    description: "Comprehensive REST API development with authentication, testing, and documentation",
    author: "Claude Flow Team",
...
export const _qualityAssurancePreset: PresetTemplate = {
  _meta: {
    ...
    version: "1.0.0",
    description: "Comprehensive quality assurance with testing, code analysis, and validation",
    author: "Claude Flow Team",
    category: "testing",
        ...
        options: [
          {
            ...
            description: "Many unit tests, some integration tests, few E2E tests",
            recommended: true},
          {
            value: "trophy",
...
export const _validatePresetTemplate = (template: PresetTemplate): any => {
  ...
  // Validate metadata,
  if (!template._meta.name) {_errors.push("Preset must have a name");}
  if (!template._meta.version) {_errors.push("Preset must have a version");}
  if (!template._meta.description) {_errors.push("Preset must have a description");}
