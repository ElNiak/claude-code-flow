...
import type {
  ...
  MigrationResult,
  MigrationChange,
  ValidationResult} from "./user-configuration-system.js";
...
export class MigrationAnalyzer {
      ...
      switch (step.type) {
        case "add":
          if (step.to) {
            ...
            impact.affectedSections.push(this.extractSection(step.to));
          }
          break;
        case "remove":
          if (step.from) {
            impact.removedFeatures.push(step.from);
            impact.affectedSections.push(this.extractSection(step.from));
          }
          break;
        case "rename":
        case "move":
  ...
  static generateMigrationPreview(
    ...
    for (const step of migration.migrations) {
      const _change = this.simulateStep(_simulatedConfig, step);
      if (_change) {
