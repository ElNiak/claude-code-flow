...
export class CliFeatureAdapter {
   ...
   */
  async resetFeatures(): Promise<{ success: boolean; message: string; resetCount: number }> {
   ...
   */
  async updateFeatureConfig(featureName: string, config: Partial<FeatureConfig>): Promise<{ success: boolean; message: string }> {
    const feature = this.features.get(featureName);
    if (!feature) {
      return {
        success: false,
