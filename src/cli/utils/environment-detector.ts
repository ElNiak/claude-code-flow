...
export function detectExecutionEnvironment(
	...
	env.isGitBash =
		...
		process.env.MSYSTEM?.startsWith("MINGW") ||
		false;
...
function generateRecommendations(env: ExecutionEnvironment): void {
	...
	if (env.isVSCode || env.isVSCodeInsiders) {
		...
		env.warnings.push(
			"VS Code integrated terminal detected - interactive features may be limited"
		);
	}
	...
	if (env.isCI) {
		...
		env.warnings.push(
			"CI environment detected - running in non-interactive mode"
		);
	}
	...
	if (env.isDocker && !env.isInteractive) {
		...
		env.recommendedFlags.push("--non-interactive");
		env.warnings.push(
			"Docker container without TTY - interactive features disabled"
		);
	}
	...
	if (env.isGitBash) {
		env.warnings.push(
			"Git Bash detected - some interactive features may not work correctly"
		);
	}
	...
	if (env.isWSL) {
		...
		env.warnings.push(
			"WSL detected - raw mode may cause hangs, using non-interactive mode"
		);
		if (!env.supportsRawMode) {
			env.warnings.push(
				"WSL subprocess context detected - interactive features disabled"
			);
		}
	}
	...
	if (!env.supportsRawMode && env.isInteractive) {
		env.recommendedFlags.push("--compatible-ui");
		env.warnings.push(
			"Terminal does not support raw mode - using compatible UI"
		);
	}
}
...
export function applySmartDefaults<T extends Record<string, any>>(
	...
	if (
		(environment.isVSCode ||
			environment.isCI ||
			!environment.supportsRawMode) &&
	...
	if (options.verbose && appliedDefaults.length > 0) {
		console.log(
			chalk.gray(`ℹ️  Auto-applied flags: ${appliedDefaults.join(" ")}`)
		);
	}
