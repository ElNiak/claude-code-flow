	...
	switch (command) {
		...
		case "-h":
			if (parsedArgs.length > 0) {
				...
				console.log(
					`Help for command: ${parsedArgs[0]} - Command registry not available`
				);
			} else {
			...
			switch (terminalCmd) {
				...
				case "create": {
					...
					const terminalConfig = {
						...
						workingDirectory:
							wdIndex >= 0 ? subArgs[wdIndex + 1] : process.cwd(),
				...
				case "batch-exec": {
					...
					const batchSession = subArgs.find(
				...
				case "cleanup": {
					...
					const idleTime = subArgs.find((arg: string) =>
				...
				case "share": {
					...
					const accessLevel = subArgs.find((arg: string) =>
				...
				case "batch-create": {
					...
					const configFile = subArgs.find((arg: string) =>
						arg.includes("--config")
					);
		...
		case "deploy": {
			...
			switch (deployCmd) {
				case "ha-cluster": {
					...
					const regions = subArgs.find((arg: string) =>
						arg.includes("--regions")
					);
					const replicationFactor = subArgs.find((arg: string) =>
...
async function startRepl() {
	...
	const replCommands: Record<string, (...args: any[]) => Promise<void> | void> =
		{
			help: () => {
				console.log(`
...
`);
			...
			status: () => {
				...
				console.log(
					`  Terminals: ${replState.context.terminals.length} active`
				);
				console.log(
			...
			config: async (key: string) => {
				try {
					const config = JSON.parse(
						await fs.readFile("claude-flow.config.json", "utf-8")
					);
					if (key) {
	...
	async function handleTaskCommand(args: string[], state: any) {
		...
		switch (subCmd) {
			...
			case "status": {
				...
				const statusTask = state.context.tasks.find(
					(t: any) => t.id === statusId
				);
				if (statusTask) {
...
async function createSparcStructureManually() {
	try {
		...
		try {
			...
			roomodesContent = await fs.readFile(".roomodes", "utf-8");
		...
		console.log(
			`  ❌ Failed to create SPARC structure: ${(err as Error).message}`
		);
	}
}
