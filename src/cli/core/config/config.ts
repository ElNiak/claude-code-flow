...
import { ConfigError, ValidationError } from "../cli/shared/errors/errors.js";
import type { Config } from "../cli/shared/types/utils-types.js";
import { deepMerge, safeParseJSON } from "../cli/shared/utils/helpers.js";
...
export class ConfigManager {
	...
	private async loadFromFile(path: string): Promise<Partial<Config>> {
		try {
			...
			if (!config) {
				throw new ConfigError(
					`Invalid ${format.toUpperCase()} in configuration file: ${path}`,
				);
			}
	...
	private validateWithDependencies(config: Config): void {
		...
		if (
			...
			warnings.push(
				"High agent-to-terminal ratio may cause resource contention",
			);
		}

		if (
			config.memory.cacheSizeMB > 1000 &&
			config.memory.backend === "sqlite"
		) {
			warnings.push(
				"Large cache size with SQLite backend may impact performance",
			);
		}

		if (config.mcp.transport === "http" && !config.mcp.tlsEnabled) {
			warnings.push(
				"HTTP transport without TLS is not recommended for production",
			);
		}
	...
	private validatePath(path: string, value: any, config?: Config): void {
		...
		if (rule.values && !rule.values.includes(value)) {
			throw new ValidationError(
				`${path} must be one of: ${rule.values.join(", ")}`,
			);
		}
