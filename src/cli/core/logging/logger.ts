...
import type { LoggingConfig } from "../cli/shared/types/utils-types.js";
import { formatBytes } from "../cli/shared/utils/helpers.js";
	...
	static getInstance(config?: LoggingConfig): Logger {
		if (!Logger.instance) {
			if (!config) {
				...
				const isTestEnv = env.CLAUDE_FLOW_ENV === "test";
				if (isTestEnv) {
