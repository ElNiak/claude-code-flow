...
async function swarmInitCommand(subArgs: string[], flags: Record<string, any>) {
	const options = flags;
	...
	try {
		...
		if (swarmResult.success) {
			...
			console.log(
				`  🔗 Channels: ${swarmResult.communicationChannels || "Established"}`,
			);
			console.log(
				`  📈 Performance: ${swarmResult.expectedPerformance || "Optimized"}`,
			);
			...
			console.log(
				`  3. Monitor swarm: claude-flow monitoring swarm-monitor --swarm-id ${swarmId}`,
			);
		} else {
			printError(
				`Swarm initialization failed: ${swarmResult.error || "Unknown error"}`,
			);
		}
	} catch (err) {
...
async function agentSpawnCommand(
	subArgs: string[],
	flags: Record<string, any>,
) {
	const options = flags;
	...
	if (!validTypes.includes(agentType)) {
		printWarning(
			`⚠️  Unknown agent type '${agentType}'. Using 'general' instead.`,
		);
	}
...
async function taskOrchestrateCommand(
	subArgs: string[],
	flags: Record<string, any>,
) {
	const options = flags;
