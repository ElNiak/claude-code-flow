...
export class ResearchStrategy extends BaseStrategy {
	...
	async decomposeObjective(
		...
		const researchParams = this.extractResearchParameters(
...
Ensure the report is well-structured and actionable.`,
	...
	private async executeOptimizedWebSearch(
		task: TaskDefinition,
		agent: any,
	): Promise<any> {
		...
		const rankedResults =
			await this.rankResultsByCredibility(successfulResults);
	...
	private async executeOptimizedDataExtraction(
		task: TaskDefinition,
		agent: any,
	): Promise<any> {
		...
		try {
			...
			const extractionPromises = this.createParallelExtractionTasks(
				task,
				agent,
			);
			...
			return {
				...
				deduplicationRate:
					1 - deduplicatedData.length / extractedData.flat().length,
			};
		} finally {
	...
	private async executeOptimizedClustering(
		task: TaskDefinition,
		agent: any,
	): Promise<any> {
		...
		return {
			...
			averageClusterSize:
				clusters.reduce((sum, c) => sum + c.results.length, 0) /
				clusters.length,
			coherenceScore:
				clusters.reduce((sum, c) => sum + c.coherenceScore, 0) /
				clusters.length,
		};
	}

	private async executeGenericResearchTask(
		task: TaskDefinition,
		agent: any,
	): Promise<any> {
	...
	private async rankResultsByCredibility(
		results: ResearchResult[],
	): Promise<ResearchResult[]> {
		...
		while (processed.size < tasks.length) {
			...
			const batch = {
				...
				estimatedDuration: Math.max(
					...batchTasks.map((t) => t.constraints.timeoutAfter || 0),
				),
				requiredResources: {
	...
	override getMetrics() {
		...
		return {
			...
			cacheHitRate:
				this.researchMetrics.cacheHits /
				(this.researchMetrics.cacheHits + this.researchMetrics.cacheMisses ||
					1),
	...
	async optimizeTaskSchedule(
		...
		const otherTasks = tasks.filter(
			(t) => !["research", "analysis"].includes(t.type as string),
		);
