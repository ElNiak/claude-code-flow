...
import type { Message } from "../../../shared/types/message-types.js";
import { generateId } from "../../../shared/utils/helpers.js";
import { debugLogger } from "../../../utils/utils-debug-logger.js";
import type { AdvancedTaskScheduler } from "../../work/core/advanced-scheduler.js";
...
export class SwarmCoordinator extends EventEmitter {
	...
	constructor(config: Partial<SwarmConfig> = {}) {
		...
		this.memoryManager = new _MemoryManager(
			{
				backend: "sqlite",
				cacheSizeMB: 50,
				syncInterval: 60000,
	...
	private async handleTaskCompleted(
		...
		await this.memoryManager.store({
			...
			context: {
				resultType: "task-completion",
				taskType: task.type,
				agentId: agent?.id,
			},
			timestamp: new Date(),
			tags: ["task-result", "completion", task.type],
			version: 1,
		});
	...
	private async syncMemoryState(): Promise<void> {
		...
		try {
			...
			await this.memoryManager.store({
				...
				context: {
					stateType: "swarm-sync",
					objectiveCount: state.objectives.length,
					taskCount: state.tasks.length,
					agentCount: state.agents.length,
				},
				timestamp: new Date(),
				tags: ["swarm-state", "sync", "coordination"],
				version: 1,
			});
		} catch (error) {
