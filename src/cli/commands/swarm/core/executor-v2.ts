...
import { generateId } from "../cli/shared/utils/helpers.js";
import {
	applySmartDefaults,
...
export class TaskExecutorV2 extends TaskExecutor {
	...
	override async executeClaudeTask(
		...
		try {
			...
			if (
				this.isInteractiveError(error) &&
				enhancedOptions.retryOnInteractiveError
			) {
				this.logger.warn(
					"Interactive error detected, retrying with non-interactive mode",
					{
	...
	private async executeClaudeWithTimeoutV2(
		...
		if (options.promptDefaults) {
			(env as any).CLAUDE_PROMPT_DEFAULTS = JSON.stringify(
					...
					async (code: number | null, signal: string | null) => {
						...
						try {
							...
							if (isTimeout) {
								...
								exitCode !== 0 &&
								this.isInteractiveErrorMessage(errorBuffer)
							) {
								reject(
									new Error(`Interactive mode error: ${errorBuffer.trim()}`),
								);
							} else {
								resolve(result);
							}
	...
	private buildClaudeCommandV2(
		...
		if (
			...
			this.environment.recommendedFlags.includes(
