...
export async function getUnifiedSwarmMetrics() {
	...
	results.integrated = await integrateMetrics(
		results.hiveMind,
		results.ruvSwarm,
	);
	...
	if (integration.systems.length > 0) {
		...
		integration.combined.success_rate =
			integration.combined.total_tasks > 0
				? (
						(integration.combined.completed_tasks /
							integration.combined.total_tasks) *
						100
					).toFixed(1)
				: "0";
	}
...
export async function showUnifiedMetrics() {
	...
	if (metrics.integrated.available) {
		console.log("Combined System Overview:");
		...
		console.log(`  Completed: ${combined.completed_tasks}`);
		console.log(`  In Progress: ${combined.in_progress_tasks}`);
		console.log(`  Pending: ${combined.pending_tasks}`);
	...
	for (const system of metrics.integrated.systems) {
		...
		if (system.swarms && system.swarms.length > 0) {
			...
			system.swarms.forEach((swarm) => {
				...
				console.log(`    ${name}: ${completed}/${total} tasks (${rate}%)`);
				if (swarm.objective) {
					console.log(
						`      Objective: ${swarm.objective.substring(0, 50)}...`,
					);
				}
				if (swarm.topology) {
					console.log(
						`      Topology: ${swarm.topology}, Strategy: ${swarm.strategy || "N/A"}`,
					);
				}
			});
		}
	...
	console.log(
		`  Hive-Mind: ${metrics.hiveMind.available ? "✓ Available" : "✗ " + metrics.hiveMind.reason}`,
	);
	console.log(
		`  ruv-swarm: ${metrics.ruvSwarm.available ? "✓ Available" : "✗ " + metrics.ruvSwarm.reason}`,
	);
...
export async function fixTaskAttribution() {
	...
	if (metrics.hiveMind.available && metrics.ruvSwarm.available) {
		console.log("✓ Both swarm systems detected");
		...
		if (zeroTaskSwarms.length > 0) {
			console.log(`⚠️  Found ${zeroTaskSwarms.length} swarms with 0 tasks`);
			fixes.push("CREATE_SAMPLE_TASKS");
		} else {
			console.log("✓ All swarms have task assignments");
		}
	} else if (metrics.hiveMind.available) {
		console.log("⚠️  Only Hive-Mind system available");
		fixes.push("SETUP_RUV_SWARM");
	} else if (metrics.ruvSwarm.available) {
		console.log("⚠️  Only ruv-swarm system available");
		fixes.push("SETUP_HIVE_MIND");
	} else {
		console.log("✗ No swarm systems available");
...
async function applyFix(fixType, metrics) {
	switch (fixType) {
		case "CREATE_SAMPLE_TASKS":
			console.log("📝 Creating sample tasks for empty swarms...");
			await createSampleTasks(metrics);
			break;

		case "SETUP_RUV_SWARM":
			console.log("🐝 Setting up ruv-swarm system...");
			console.log("  Run: npx ruv-swarm init");
			break;

		case "SETUP_HIVE_MIND":
			console.log("🧠 Setting up hive-mind system...");
			console.log("  Run: claude-flow hive-mind init");
			break;

		case "SETUP_BOTH_SYSTEMS":
			console.log("🔧 Setting up both swarm systems...");
			console.log("  Run: claude-flow hive-mind init && npx ruv-swarm init");
			break;
	}
}
...
async function createSampleTasks(metrics) {
	...
	console.log("✓ Sample tasks already created for ruv-swarm system");
	console.log("✓ Sample tasks already exist for hive-mind system");
}
