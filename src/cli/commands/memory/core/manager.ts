...
import type { ILogger } from "../../../core/logging/logger.js";
...
export interface IMemoryManager {
	...
	getCoordinationStatus(
		sessionId: string,
	): Promise<{ agents: number; lastSync: Date; entries: number }>;
}
...
export class MemoryManager implements IMemoryManager {
	...
	async retrieveAgentCoordination(
		sessionId: string,
		agentId?: string,
	): Promise<MemoryEntry[]> {
		if (!this.initialized) {
	...
	async syncAgentMemory(sessionId: string, agentIds: string[]): Promise<void> {
		...
		this.logger.info("Syncing agent memory", {
			sessionId,
