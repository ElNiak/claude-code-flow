...
async function autoAgentCommand(subArgs: string[], flags: Record<string, any>) {
	const options = flags;
	const complexity =
		options["task-complexity"] || options.complexity || "medium";
...
async function smartSpawnCommand(
	subArgs: string[],
	flags: Record<string, any>,
) {
	const options = flags;
	...
	const maxAgents = parseInt(
		options["max-agents"] || options.maxAgents || "10",
	);
	...
	if (
		requirement.includes("enterprise") ||
		requirement.includes("production")
	) {
		recommendedAgents.push(
...
async function workflowSelectCommand(
	subArgs: string[],
	flags: Record<string, any>,
) {
	const options = flags;
	...
	const selectedWorkflow =
		(workflows as Record<string, any>)[projectType] || workflows["general"];
	...
	console.log(`\n📋 WORKFLOW PHASES:`);
	selectedWorkflow.phases.forEach((phase: string, index: number) => {
		console.log(
	...
	Object.entries(selectedWorkflow.agents).forEach(([type, count]) => {
		const agentCount = typeof count === "number" ? count : 1;
		console.log(`  • ${type}: ${agentCount} agent${agentCount > 1 ? "s" : ""}`);
	});
