...
class AutomatedValidation {
	...
	async executeWithRetry(command, retries = this.options.maxRetries) {
		for (let attempt = 1; attempt <= retries; attempt++) {
			try {
				...
				this.log(
					`Command failed (attempt ${attempt}/${retries}): ${command}`,
					"warning",
				);
		...
		if (outdatedResult.success && outdatedResult.stdout.trim()) {
			try {
				...
				validations.push({
					...
					details: {
						outdated,
						majorOutdated: majorOutdated.map(([name]) => name),
					},
				});
			} catch (parseError) {
				validations.push({
	...
	async validateCLIFunctionality() {
		...
		for (const { cmd, name, expected } of basicCommands) {
			...
			const result = await this.executeWithRetry(
				`node dist/cli/simple-cli.js ${cmd}`,
			);

			const passed =
				result.success && (expected ? expected.test(result.stdout) : true);
		...
		for (const category of commandCategories) {
			...
			const result = await this.executeWithRetry(
				`node dist/cli/simple-cli.js ${category} --help`,
			);
		...
		if (passed === total) {
			this.log(
				`CLI functionality validation: ${passed}/${total} checks passed`,
				"success",
			);
		} else {
			this.log(
	...
	async validatePerformance() {
		...
		for (let i = 0; i < 5; i++) {
			...
			const result = await this.executeWithRetry(
				"node dist/cli/simple-cli.js --version",
			);
    ...
    `;
		...
		if (passed === total) {
			this.log(
				`Performance validation: ${passed}/${total} checks passed`,
				"success",
			);
		} else {
			this.log(
	...
	async validateSystemIntegration() {
		...
		if (passed === total) {
			this.log(
				`System integration validation: ${passed}/${total} checks passed`,
				"success",
			);
		} else {
			this.log(
	...
	generateSummary() {
		...
		for (const [category, validations] of this.results.validations) {
			...
			criticalFailures += validations.filter(
				(v) => !v.passed && criticalChecks.includes(v.name),
			).length;
		}
		...
		this.results.summary = {
			...
			overallStatus:
				criticalFailures === 0 && passedValidations === totalValidations
					? "PASS"
					: criticalFailures > 0
						? "CRITICAL_FAIL"
						: "PARTIAL_PASS",
	...
	displayResults() {
		...
		console.log(
			`   Passed: ${chalk.green(summary.passedValidations)} (${summary.successRate}%)`,
		);
