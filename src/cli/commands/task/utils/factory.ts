// ABOUTME: Command factory functions for task management system
// ABOUTME: Creates command handlers with proper error handling and logging integration
...
export function createTaskWorkflowCommand(context: TaskCommandContext) {
	return {
		...
		execute: async (
			action: "create" | "execute" | "list" | "get",
			...args: any[]
		) => {
			try {
				switch (action) {
					case "create": {
						...
						const createdWorkflow =
							await context.taskEngine.createWorkflow(workflowData);
