...
export class ProcessUI {
	...
	private render(): void {
		...
		console.log(chalk.gray("─".repeat(60)));
		console.log(
			chalk.gray("Commands: [1-9] Select process [a] Start All [z] Stop All")
		);
	...
	private async startProcess(processId: string): Promise<void> {
		try {
			...
			console.log(
				chalk.red(`✗ Failed to start ${processId}: ${(error as Error).message}`)
			);
		}
		await this.waitForKey();
	}
	...
	private async stopProcess(processId: string): Promise<void> {
		try {
			...
			console.log(
				chalk.red(`✗ Failed to stop ${processId}: ${(error as Error).message}`)
			);
		}
		await this.waitForKey();
	}
	...
	private async startAll(): Promise<void> {
		try {
			...
			console.log(
				chalk.red(`✗ Failed to start all: ${(error as Error).message}`)
			);
		}
		await this.waitForKey();
		this.render();
	...
	private async stopAll(): Promise<void> {
		try {
			...
			console.log(
				chalk.red(`✗ Failed to stop all: ${(error as Error).message}`)
			);
		}
		await this.waitForKey();
		this.render();
