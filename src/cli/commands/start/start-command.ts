...
import { formatDuration as _formatDuration } from "../../formatter.js";
	...
	.action(async (options: StartOptions) => {
		...
		try {
			...
			if (!options.force && (await isSystemRunning())) {
				...
				if (!shouldContinue) {
					console.log(
						chalk.gray('Use --force to override or "claude-flow stop" first')
					);
					return;
				}
			...
			if (options.ui) {
				...
				try {
					...
					throw new Error(
						"Web UI server not available - ClaudeCodeWebServer not found"
					);
					...
					console.log(
						chalk.yellow("Web UI not available, falling back to Terminal UI")
					);
			...
			// Daemon mode,
				...
				console.log(
					chalk.gray('Use "claude-flow status" to check system status')
				);
				console.log(
					chalk.gray('Use "claude-flow monitor" for real-time monitoring')
				);
			...
			console.error(
				chalk.red.bold("Failed to start:"),
...
async function checkNetworkConnectivity(): Promise<void> {
	...
	try {
		...
		console.log(
			chalk.yellow("  ⚠ Network connectivity check skipped (offline mode?)")
		);
	}
}
...
function setupSystemEventHandlers(
	...
	processManager.on("processError", (event: any) => {
		console.error(
			chalk.red(`Process error in ${event.processId}:`),
			event.error
		);
		if (event.processId === "orchestrator") {
			console.error(
				chalk.red.bold("Critical process failed, initiating recovery...")
			);
...
async function startWithProgress(
	...
	for (let i = 0; i < processes.length; i++) {
		...
		try {
			...
			console.log(
				chalk.red(
					`${progress} ✗ ${processId} failed: ${(error as Error).message}`
				)
			);
...
async function waitForSystemReady(
	...
	console.log(
		chalk.yellow(
			"⚠ System startup completed but some processes may not be fully ready"
		)
	);
}
...
function setupVerboseLogging(monitor: SystemMonitor): void {
	...
	eventBus.on("process:error", (data: any) => {
		console.log(
			chalk.red(`[VERBOSE] Process error: ${data.processId} - ${data.error}`)
		);
	});
}
