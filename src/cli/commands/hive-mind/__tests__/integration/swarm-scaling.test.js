...
class SwarmOrchestrator {
	...
	async distributeTasks(taskCount) {
		...
		const distribution = this.getOptimalDistribution(
			tasks.length,
			agents.length,
		);
		let taskIndex = 0;
...
describe("Swarm Scaling Integration Tests", () => {
	...
	describe("Large Scale (50-100 agents)", () => {
		...
		it("should distribute 500 tasks among 75 agents", async () => {
			...
			expect(Math.round(distribution.avgTasksPerAgent)).toBeGreaterThanOrEqual(
				6,
			);
	...
	describe("Performance Metrics", () => {
		it("should track scaling performance", async () => {
			...
			const scaleMetrics = report.metrics.filter(
				(m) => m.metric_type === "scale_time",
			);
			...
			for (let i = 1; i < memoryMetrics.length; i++) {
				expect(memoryMetrics[i].avg_value).toBeGreaterThanOrEqual(
					memoryMetrics[i - 1].avg_value,
				);
			}
		});
		...
		it("should provide scaling recommendations", () => {
			...
			if (spawnTimes.length > 0) {
				const avgSpawnTimePerAgent =
					spawnTimes.reduce((sum, st) => sum + st.time / st.count, 0) /
					spawnTimes.length;
