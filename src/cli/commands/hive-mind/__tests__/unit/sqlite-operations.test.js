...
describe("SQLite Operations", () => {
	...
	describe("Agent Operations", () => {
		...
		it("should list agents by type", () => {
			...
			const researchers = db
				.prepare("SELECT * FROM agents WHERE type = ?")
				.all("researcher");
	...
	describe("Task Operations", () => {
		...
		it("should assign task to agent", () => {
			// Create task
			db.prepare("INSERT INTO tasks (id, description) VALUES (?, ?)").run(
				"task-001",
				"Write tests",
			);
		...
		it("should handle task dependencies", () => {
			...
			db.prepare(
				"INSERT INTO tasks (id, description, dependencies) VALUES (?, ?, ?)",
	...
	describe("Message Operations", () => {
		...
		it("should broadcast message to all", () => {
			const stmt = db.prepare(
				"INSERT INTO messages (from_agent, to_agent, type, content) VALUES (?, ?, ?, ?)",
			);
			const result = stmt.run(
				"agent-001",
				null,
				"broadcast",
				"Team update: Starting sprint",
			);
	...
	describe("Consensus Operations", () => {
		...
		it("should create consensus vote", () => {
			...
			const stmt = db.prepare(
				"INSERT INTO consensus (id, topic, initiator, participants) VALUES (?, ?, ?, ?)",
			);
			const result = stmt.run(
				"vote-001",
				"Use TypeScript?",
			...
			const vote = db
				.prepare("SELECT * FROM consensus WHERE id = ?")
				.get("vote-001");
		...
		it("should close consensus with result", () => {
			...
			const stmt = db.prepare(
				"UPDATE consensus SET status = ?, result = ?, closed_at = ? WHERE id = ?",
			);
	...
	describe("Memory Operations", () => {
		...
		it("should update access count on retrieval", () => {
			// Store memory
			db.prepare("INSERT INTO memory (key, value) VALUES (?, ?)").run(
				"config",
				'{"port": 3000}',
			);
		...
		it("should efficiently query agents by type using index", () => {
			...
			for (let i = 0; i < 100; i++) {
				insert.run(
					`agent-${i}`,
					`Bot${i}`,
					i % 3 === 0 ? "researcher" : "coder",
				);
			}

			// Query with EXPLAIN to verify index usage
			const explain = db
				.prepare("EXPLAIN QUERY PLAN SELECT * FROM agents WHERE type = ?")
				.all("researcher");
