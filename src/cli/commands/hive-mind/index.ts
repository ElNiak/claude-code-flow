...
interface HiveMindArgs extends Array<string> {
	[index: number]: string;
}
...
import { Communication } from "./core/communication.js";
import { HiveMindCore } from "./core/core.js";
import {
	cleanMemories,
	exportMemoryBackup,
	hiveMindWizard,
	...
	spawnSwarmWizard,
	storeMemoryWizard,
} from "./core/hiveMindWizard.js";
// Import MCP tool wrappers
import { MCPToolWrapper } from "./core/mcp-wrapper.js";
import { Memory } from "./core/memory.js";
import { Queen } from "./core/queen.js";
// Import database type definitions
import {
	type ConsensusCountResult,
	type CountResult,
	type DbAgent,
	type DbConsensusDecision,
	type DbSwarm,
	getErrorMessage,
	isCountResult,
	isDbAgent,
	isDbConsensusDecision,
	isDbSwarm,
	isTaskStatsResult,
	type MemoryCountResult,
...
async function spawnSwarm(
	...
	try {
		...
		try {
			...
			const validateHiveMindParams = (flags: HiveMindFlags) => {
				...
				if (
					...
					maxWorkers !== null &&
					(typeof maxWorkers === "number" ? maxWorkers > 0 : maxWorkers !== "")
				) {
					parsedMaxWorkers =
						typeof maxWorkers === "number"
							? maxWorkers
							: parseInt(String(maxWorkers), 10);
					if (
			...
			throw new Error(
				`Failed to initialize HiveMindCore: ${getErrorMessage(error)}`,
			);
		}

		spinner.text = "Setting up database connection...";
		// Initialize database connection
		const dbDir = path.join(process.cwd(), ".hive-mind");
		...
		try {
			...
			console.warn(
				"Database issue detected, recreating...",
			...
			if (existsSync(dbPath)) {
				try {
					...
					console.warn(
						"Could not remove corrupted database:",
						getErrorMessage(e),
					);
				}
			}
	...
	`);
			...
			throw new Error(
				`Failed to create database schema: ${getErrorMessage(error)}`,
			);
		}
	  ...
	  `).run(swarmId, hiveMind.config.name, objective, hiveMind.config.queenType);
			...
			throw new Error(
				`Failed to create swarm record: ${getErrorMessage(error)}`,
			);
		}
		...
		const queen = new Queen({
			swarmId,
			mode:
				(hiveMind.config.queenType as "centralized" | "distributed") ||
				"centralized",
	...
	`).run(...Object.values(queenAgent));
		...
		for (let i = 0; i < hiveMind.config.maxWorkers; i++) {
			const workerType = workerTypes[i % workerTypes.length] as string;
			console.log(
				chalk.gray(
	  ...
	  `).run(...Object.values(worker));
		...
		const memory = new Memory(swarmId, {
			maxMemorySize: (flags.memorySize || 100) * 1024 * 1024, // Convert to bytes
		});
		...
		const communication = new Communication(swarmId);
		...
		if (
			getErrorMessage(error).includes("sha3") ||
			getErrorMessage(error).includes("SHA3")
		) {
			...
			console.error("\n🚨 Detailed error:");
			console.error((error as Error).stack || getErrorMessage(error));
		}
 ...
 */
async function determineOptimalWorkerTypes(
	flags: HiveMindFlags,
	objective: string,
	queen: any,
): Promise<string[]> {
	...
	if (flags.workerTypes) {
		const userTypes = flags.workerTypes.split(",").map((t: string) => t.trim());
		console.log(
	...
	try {
		...
		const finalTypes: string[] = Array.from(recommendedTypes).filter(
			(type): type is string => allWorkerTypes.includes(type as string),
		);
 ...
 */
function getAgentCapabilities(type: string) {
 ...
 */
async function showStatus(flags: HiveMindFlags) {
	try {
	...
	`)
			.all();
		const swarms = (swarmsResult as unknown[]).filter(isDbSwarm);
	  ...
	  `)
				.all(swarm.id);
			const agents = (agentsResult as unknown[]).filter(isDbAgent);
	  ...
	  `)
				.get(swarm.id);
			const taskStats =
				taskStatsResult && isTaskStatsResult(taskStatsResult)
					? taskStatsResult
	  ...
	  `)
				.get(swarm.id);
			const memoryCount =
				memoryCountResult && isCountResult(memoryCountResult)
					? memoryCountResult
					: { count: 0 };
	  ...
	  `)
			...
			const consensusCount =
				consensusCountResult && isCountResult(consensusCountResult)
					? consensusCountResult
					: { count: 0 };
 ...
 */
export async function showConsensus(flags: HiveMindFlags) {
	try {
	...
	`)
			.all();
		const decisions: DbConsensusDecision[] = (
			decisionsResult as unknown[]
		).filter(isDbConsensusDecision);
			...
			if (decision.votes) {
				...
				if (
					...
					if (votes.details && Array.isArray(votes.details)) {
						console.log("  - details:");
						votes.details.forEach((detail: any, index: number) => {
 ...
 */
async function manualConsensus(flags: HiveMindFlags) {
	try {
		...
		if (flags.options) {
			options = flags.options.split(",").map((opt: string) => opt.trim());
		}
		...
		const hiveMindCore = new HiveMindCore({
			...
			maxWorkers: flags.agents ? parseInt(String(flags.agents)) || 5 : 5,
		...
		const workerCount = Math.min(
			flags.agents ? parseInt(String(flags.agents)) || 5 : 5,
			workerTypes.length,
		);
		...
		try {
			...
			console.log(
				chalk.cyan("Decision:"),
				chalk.bold((consensus as any).result || consensus.status),
			);
			console.log(
				chalk.cyan("Confidence:"),
				(consensus as any).confidence
					? `${Math.round((consensus as any).confidence * 100)}%`
					: "Unknown",
			);
			...
			if (hiveMindCore.state.swarmId) {
				await mcpWrapper.storeMemory(
					...
					`manual-consensus-${Date.now()}`,
					{
						topic,
						options,
						result: (consensus as any).result || consensus.status,
						confidence: (consensus as any).confidence || 0,
			...
			console.error(
				chalk.red("Consensus error:"),
				getErrorMessage(consensusError),
			);
		}
		...
		console.error(chalk.red("Manual consensus error:"), getErrorMessage(error));
		process.exit(1);
	}
}
 ...
 */
export async function showMetrics(flags: HiveMindFlags) {
	try {
		const dbPath = path.join(process.cwd(), ".hive-mind", "hive.db");
		const db = new Database(dbPath);
	...
	`)
		...
		if (taskBreakdown.length > 0) {
			...
			taskBreakdown.forEach((status: TaskBreakdown) => {
				const percentage =
					overallStats.total_tasks > 0
	  ...
	  `)
		...
		`)
			...
			console.warn("Could not get agent performance:", getErrorMessage(error));
		}

		if (agentPerf.length > 0) {
			console.log("\n" + chalk.cyan("Top Performing Agents:"));
			agentPerf.forEach((agent: AgentPerf, index: number) => {
				const successRate =
	...
	`)
		...
		if (swarmPerf.length > 0) {
			...
			swarmPerf.forEach((swarm: SwarmPerf) => {
				const successRate =
					swarm.task_count > 0
		...
		let avgTaskTime: { avg_minutes: number | null } = { avg_minutes: null };
		try {
			// Check if completed_at exists
			const hasCompletedAt = db
				.prepare(`
	  ...
	  `)
		...
		`)
			...
			console.warn(
				"Could not calculate average task time:",
				getErrorMessage(error),
			);
		}

		// Get agent type performance
		let agentTypePerf: any[] = [];
		try {
			// Check if completed_at exists
			const hasCompletedAt = db
				.prepare(`
	  ...
	  `)
		...
		`)
			...
			console.warn(
				"Could not get agent type performance:",
				getErrorMessage(error),
			);
		}

		if (
			avgTaskTime.avg_minutes !== null &&
			avgTaskTime.avg_minutes !== undefined
		) {
 ...
 */
export async function hiveMindCommand(
	args: HiveMindArgs,
	flags: HiveMindFlags,
): Promise<void> {
...
async function listMemories() {
	try {
		...
		console.error(chalk.red("Error listing memories:"), getErrorMessage(error));
		console.log(
...
async function searchMemories() {
	try {
		...
		console.error(
			chalk.red("Error searching memories:"),
			getErrorMessage(error),
		);
	}
}
...
async function storeMemoryWizard() {
	try {
		...
		console.error(chalk.red("Error storing memory:"), getErrorMessage(error));
	}
}
...
async function showMemoryStats() {
	try {
		...
		console.error(
			chalk.red("Error getting memory stats:"),
			getErrorMessage(error),
		);
	}
}
...
async function cleanMemories() {
	try {
		...
		const oldMemories = memories.filter((memory: any) => {
		...
		console.error(
			chalk.red("Error cleaning memories:"),
			getErrorMessage(error),
		);
	}
}
...
async function exportMemoryBackup() {
	try {
		...
		console.error(
			chalk.red("Error exporting memory backup:"),
			getErrorMessage(error),
		);
	}
}
 ...
 */
function getWorkerTypeInstructions(workerType: string): string {
...
async function getMcpWrapper() {
	const { MCPToolWrapper } = await import("./core/mcp-wrapper.js");
