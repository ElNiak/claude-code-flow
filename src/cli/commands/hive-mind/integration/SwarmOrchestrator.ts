...
import type { Agent } from "../../../../hive-mind/core/Agent.js";
import { DatabaseManager } from "../../../../hive-mind/core/DatabaseManager.js";
import type { HiveMind } from "../../../../hive-mind/core/HiveMind.js";
import {
...
export class SwarmOrchestrator extends EventEmitter {
	...
	private async createExecutionPlan(task: Task): Promise<ExecutionPlan> {
		...
		const phaseAssignments = await Promise.all(
			phases.map((phase: string) =>
				this.createPhaseAssignments(task, phase, analysis),
			),
		);
	...
	private async executePhase(
		...
		const agentAssignments = await this.assignAgentsToPhase(
			task,
			phase,
			assignments,
		);
		...
		switch (phase) {
			...
			case "parallel-execution": {
				...
				const executionCount = Math.min(
					analysis.resourceRequirements.maxAgents,
					3,
				);
	...
	private async evaluateCheckpoint(
		...
		if (score < checkpoint.failureThreshold) {
			throw new Error(
				`Checkpoint failed at phase ${checkpoint.phase}: score ${score}`,
			);
		}
	 ...
	 */
	private async notifyAgentTaskCancelled(
		agentId: string,
		taskId: string,
	): Promise<void> {
	 ...
	 */
	async createGradioApp(config?: any): Promise<any> {
	 ...
	 */
	async createPythonRestAPI(config?: any): Promise<any> {
