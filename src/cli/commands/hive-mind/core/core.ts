...
export class HiveMindCore extends EventEmitter {
	constructor(config = {}) {
		...
		console.log(
			chalk.gray(`🔍 [DEBUG] HiveMindCore constructor called with config:`),
		);
		...
		console.log(
			chalk.green(`  - consensusAlgorithm: ${this.config.consensusAlgorithm}`),
		);
	...
	_initializePerformanceMonitoring() {
		...
		setInterval(() => {
			...
			if (parseFloat(stats.asyncQueue.utilization) > 90) {
				console.warn(
					"High async queue utilization:",
					stats.asyncQueue.utilization + "%",
				);
			}

			if (parseFloat(stats.asyncQueue.successRate) < 95) {
				console.warn(
					"Low async operation success rate:",
					stats.asyncQueue.successRate + "%",
				);
			}
		}, 60000); // Every minute
	}
	...
	async initialize() {
		try {
			...
			const [swarmInit, memoryInit, neuralInit] =
				await this.mcpWrapper.initializeSwarm({
	...
	_determineTopology() {
		...
		if (objective.includes("research") || objective.includes("analysis")) {
			...
			objective.includes("monitor") ||
			objective.includes("maintain")
		) {
			return "ring"; // Circular for continuous monitoring
		} else if (
			objective.includes("coordinate") ||
			objective.includes("orchestrate")
		) {
	...
	async spawnQueen(queenData) {
		...
		console.log(
			chalk.blue(
				`👑 [DEBUG] Creating queen with type: ${this.config.queenType}`,
			),
		);
		...
		console.log(
			chalk.green(
				`✅ [DEBUG] Queen created successfully with type: ${this.state.queen.type}`,
			),
		);
	...
	async spawnWorkers(workerTypes) {
		...
		try {
			...
			const allResults = await Promise.all(
				chunks.map((chunk) =>
					this.mcpWrapper.spawnAgents(chunk, this.state.swarmId),
				),
			);
	...
	async createTask(description, priority = 5, metadata = {}) {
		...
		if (this._shouldTriggerConsensus(task)) {
			console.log(
				chalk.blue(
					`🤝 [CONSENSUS] High-priority task requires consensus: "${task.description}"`,
				),
			);
			...
			try {
				...
				console.log(
					chalk.green(
						`✅ [CONSENSUS] Team decided on: "${consensus.result}" (confidence: ${Math.round(consensus.confidence * 100)}%)`,
					),
				);
				...
				console.warn(
					chalk.yellow(
						`⚠️ [CONSENSUS] Failed to build consensus, proceeding normally: ${error.message}`,
					),
				);
			}
		}
	...
	_shouldTriggerConsensus(task) {
		...
		if (
			architecturalKeywords.some((keyword) => description.includes(keyword))
		) {
			return true;
		}
		...
		if (
			resourceKeywords.some((keyword) => description.includes(keyword)) &&
			task.priority >= 6
		) {
			return true;
		}
	...
	async _findBestWorkerAsync(task) {
		...
		const cachedMatch = await this.mcpWrapper.retrieveMemory(
			this.state.swarmId,
			cacheKey,
		);
		...
		if (cachedMatch && cachedMatch.timestamp > Date.now() - 300000) {
			// 5 min cache
			const cachedWorker = availableWorkers.find(
				(w) => w.type === cachedMatch.workerType,
			);
		...
		const priorityMap = {
			...
			tester: {
				keywords: ["test", "validate", "check", "verify", "quality", "qa"],
				weight: 1.0,
			},
			architect: {
			...
			reviewer: {
				keywords: ["review", "feedback", "improve", "refactor", "audit"],
				weight: 1.0,
			},
			optimizer: {
			...
			documenter: {
				keywords: ["document", "explain", "write", "describe", "manual"],
				weight: 0.9,
			},
		};
		...
		const workerScores = availableWorkers.map((worker) => {
			...
			const performanceScore = worker.performance
				? worker.performance.successRate * 0.5 +
					(1 / (worker.performance.avgTaskTime + 1)) * 0.5
				: 0.5;
	...
	_findBestWorker(task) {
		...
		for (const worker of availableWorkers) {
			...
			const performanceBonus = worker.performance
				? worker.performance.successRate * 0.5
				: 0;
		...
		try {
			...
			if (!worker.performance.avgTaskTime) {
				...
				worker.performance.avgTaskTime =
					(worker.performance.avgTaskTime * (worker.tasksCompleted - 1) +
			...
			worker.performance.successRate =
				(worker.performance.successRate * worker.tasksCompleted) /
	...
	async buildConsensus(topic, options) {
		console.log(
			chalk.blue(
				`🤝 [CONSENSUS] Creating decision with algorithm: ${this.config.consensusAlgorithm}`,
			),
		);
		...
		console.log(
			chalk.blue(
				`🗳️  [CONSENSUS] Collecting intelligent votes from ${workers.length} workers...`,
			),
		);
		...
		const decisionForStorage = {
			...decision,
			votes:
				decision.votes instanceof Map
					? Object.fromEntries(decision.votes)
					: decision.votes,
		};
	...
	_calculateConsensus(decision) {
		...
		switch (decision.algorithm) {
			...
			case "weighted": {
				...
				const weightedSorted = Object.entries(voteCount).sort(
					(a, b) => b[1] - a[1],
				);
			...
			case "byzantine": {
				...
				const byzantineSorted = Object.entries(voteCount).sort(
					(a, b) => b[1] - a[1],
				);
	...
	async _getQueenVote(topic, options, workerVotes) {
		...
		if (this.state.queen) {
			try {
				const decision = await this.state.queen.makeDecision(
					topic,
					options,
					workerVotes,
				);
				...
				console.warn(
					chalk.yellow(
						`⚠️  [CONSENSUS] Queen decision failed, using fallback: ${error.message}`,
					),
				);
			}
		}
		...
		if (
			sorted.length > 0 &&
			sorted[0][1] > Object.keys(workerVotes).length * 0.6
		) {
			return sorted[0][0];
		}
	...
	async _updatePerformanceMetrics() {
		...
		const completionRate =
			this.state.metrics.tasksCompleted / this.state.metrics.tasksCreated;
		const avgTasksPerWorker =
			this.state.metrics.tasksCompleted / this.state.workers.size;
	...
	_calculateWorkerEfficiency(workers) {
		...
		const efficiencies = workers.map(
			(worker) => worker.performance?.successRate || 1.0,
		);
		return (
			(efficiencies.reduce((sum, eff) => sum + eff, 0) / workers.length) *
			100
		).toFixed(2);
	}
	...
	_calculateThroughput(tasks) {
		...
		return timeSpanMinutes > 0
			? (completedTasks.length / timeSpanMinutes).toFixed(2)
			: 0;
	}
