...
export class HiveCommunicationProtocol extends EventEmitter {
	...
	getStatistics() {
		...
		for (const channel of this.channels.values()) {
			...
			for (const message of channel.messages) {
				...
				const priorityCount =
					stats.messagesByPriority.get(message.priority) || 0;
	...
	exportLog(): any {
		const log = {
			...
			knowledge: Array.from(this.knowledgeBase.entries()).map(
				([key, value]) => ({
					key,
					...value,
				}),
