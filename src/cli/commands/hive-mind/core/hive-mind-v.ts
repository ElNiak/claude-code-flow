...
export class HiveMind extends EventEmitter {
	...
	async autoSpawnAgents(): Promise<Agent[]> {
		const topologyConfigs: Record<
			SwarmTopology,
			Array<{ type: AgentType; count: number }>
		> = {
			hierarchical: [
	...
	async spawnAgent(options: AgentSpawnOptions): Promise<Agent> {
		...
		const agent = new Agent({
			...
			capabilities:
				...
				(this.getDefaultCapabilities(options.type) as AgentCapability[]),
		});
	...
	async getFullStatus(): Promise<SwarmStatus> {
		...
		const memoryStats = {
			...
			byNamespace: {} as Record<
				string,
				{ entries: number; size: number; avgTTL: number }
			>,
			hotKeys: [] as string[],
		};
		...
		return {
			...
			tasks: tasks.map((t) => ({
				...
				assignedAgent: t.assigned_agents
					? JSON.parse(t.assigned_agents)[0]
					: null,
			})),
			taskStats,
			memoryStats,
	...
	async getStats() {
		...
		return {
			...
			availableCapacity: Math.round(
				(1 - agents.filter((a) => a.status === "busy").length / agents.length) *
					100,
			),
		};
	}
	...
	async retryTask(taskId: string): Promise<Task> {
		...
		const newTask = await this.submitTask({
			...
			requiredCapabilities: JSON.parse(
				originalTask.required_capabilities || "[]",
			),
			metadata: {
