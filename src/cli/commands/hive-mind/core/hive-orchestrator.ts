...
export class HiveOrchestrator extends EventEmitter {
	...
	async decomposeObjective(objective: string): Promise<HiveTask[]> {
		...
		const needsImplementation =
			needsDesign || objective.toLowerCase().includes("implement");
		...
		if (needsDesign) {
			...
			if (needsImplementation) {
				const implTask = this.createTask(
					"implementation",
	 ...
	 */
	async proposeTaskAssignment(
		taskId: string,
		agentId: string,
	): Promise<HiveDecision> {
	...
	private evaluateDecision(decision: HiveDecision) {
		...
		decision.result =
			approvalRate >= this.consensusThreshold ? "approved" : "rejected";
	...
	getPerformanceMetrics() {
		...
		const avgExecutionTime =
			completed.length > 0
				? completed.reduce(
						(sum, t) =>
							sum + (t.metrics?.endTime || 0) - (t.metrics?.startTime || 0),
						0,
					) / completed.length
				: 0;
		...
		return {
			...
			consensusRate:
				decisions.length > 0 ? approvedDecisions.length / decisions.length : 0,
