...
import type { MCPContext, MCPTool } from "../cli/shared/types/utils-types.js";
import { execAsync } from "../cli/shared/utils/helpers.js";
...
export function createRuvSwarmTools(logger: ILogger): MCPTool[] {
	return [
		// === SWARM LIFECYCLE TOOLS ===
		{
			...
			description:
				"Initialize a new ruv-swarm with specified topology and configuration",
			inputSchema: {
				type: "object",
				properties: {
					topology: {
			...
			handler: async (input: any, context?: RuvSwarmToolContext) => {
				...
				return await executeRuvSwarmCommand(
					"swarm init",
					args,
					context,
					logger,
