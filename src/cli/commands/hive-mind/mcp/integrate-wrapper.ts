...
export class MCPIntegration {
	...
	async connectToClaudeCode(): Promise<void> {
		try {
			...
			const claudeCodeProcess = spawn(
				"npx",
				["-y", "@anthropic/claude-code", "mcp"],
				{
...
export function injectClaudeCodeClient(
	wrapper: ClaudeCodeMCPWrapper,
	client: Client,
): void {
	// Override the forwardToClaudeCode method
	(wrapper as any).forwardToClaudeCode = async (
		toolName: string,
		args: any,
	) => {
		try {
