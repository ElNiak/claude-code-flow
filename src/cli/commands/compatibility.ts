...
Would you like to use the new command instead? (Recommended)
      ...
      console.log(chalk.yellow(`💡 Next time, try: claude-flow ${newCommand} for enhanced functionality`));
    }
  };
}
...
export const backwardCompatibilityCommands: Command[] = [
  ...
  {
    name: "swarm-legacy",
    description: "🔄 Legacy swarm command (use 'work' or 'intrinsic' for unified system)",
    aliases: ["swarm-old"],
    action: async (ctx: CommandContext) => {
  ...
  💾 Persistent coordination across sessions
      ...
      switch (subcommand) {
        ...
        case "spawn":
          ...
          console.log("Recommended: claude-flow agent spawn [type] --intrinsic");
          break;
        case "status":
        ...
        default:
          ...
          console.log("Run 'claude-flow help' to see available unified commands");
      }
    }
  },
...
Your command will work, but consider these enhancements:
  ...
  Enhanced: claude-flow agent status --detailed --session-id [ID]
      ...
      info("🔄 Executing with basic coordination (consider using enhanced features)");
