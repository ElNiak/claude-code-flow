...
export async function githubCommand(
	...
	if (!objective) {
		...
		if ((GITHUB_MODES as Record<string, any>)[mode]) {
			console.log(`\nExamples for ${mode}:`);
			for (const example of (GITHUB_MODES as Record<string, any>)[mode]
				.examples) {
				console.log(`  ${example}`);
			}
		} else {
	...
	try {
		...
		try {
			...
			printWarning(
				"⚠️  Claude CLI not found. GitHub automation requires Claude.",
			);
...
GITHUB MODE: ${mode}
DESCRIPTION: ${(GITHUB_MODES as Record<string, any>)[mode].description}
...
Begin execution now. Create all necessary GitHub workflow files and configuration.`;
		...
		printError(`❌ GitHub automation failed: ${(error as Error).message}`);
...
// Check if this file is being run directly (Node.js compatibility)
if (
		...
		for (let i = 0; i < process.argv.slice(2).length; i++) {
			...
			if (arg.startsWith("--")) {
				...
				if (nextArg && !nextArg.startsWith("--")) {
					(flags as Record<string, any>)[flagName] = nextArg;
					i++; // Skip the next argument
				} else {
					(flags as Record<string, any>)[flagName] = true;
				}
			} else {
				args.push(arg);
			}
