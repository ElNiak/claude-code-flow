...
import { printInfo } from "../../core/utils.js";
...
export class PerformanceMonitor {
	...
	calculateAverages() {
		if (this.metrics.memoryReadings.length > 0) {
			...
			this.metrics.averageMemoryMB =
				totalMemory / this.metrics.memoryReadings.length;
		}
	}
	...
	getMetrics() {
		...
		return {
			...
			memoryEfficiency:
				this.metrics.peakMemoryMB < this.maxMemoryMB * 0.8 ? "good" : "warning",
		};
	}
	...
	displayRealTimeStats() {
		...
		const elapsed = this.metrics.startTime
			? (currentTime - this.metrics.startTime) / 1000
			: 0;
	...
	static createDefaultCallbacks() {
		return {
			...
			onMemoryError: (current, max) => {
				console.error(
					`❌ Memory usage critical: ${current.toFixed(1)}MB / ${max}MB`,
				);
...
export class BatchOptimizer {
	...
	static estimateCompletionTime(projectCount, options = {}) {
		...
		return {
			...
			savingsPercent: (
				((totalSequentialTime - parallelTime) / totalSequentialTime) *
				100
			).toFixed(1),
		};
	}
	...
	static generateRecommendations(projectCount, options = {}) {
		...
		if (options.sparc && projectCount > 5) {
			recommendations.push(
				"SPARC initialization adds overhead - monitor memory usage",
			);
		}

		if (options.template === "microservice" && projectCount > 3) {
			recommendations.push(
				"Microservice template is complex - consider lower concurrency",
			);
		}
