...
import { printWarning } from "../../../core/utils.js";
...
export class PreInitValidator {
	...
	async checkPermissions() {
		...
		try {
			...
			result.errors.push(
				`Insufficient permissions in ${this.workingDir}: ${error.message}`,
			);
		}
					...
					if (parts.length >= 4) {
						...
						if (availableMB < 100) {
							...
							result.warnings.push(
								`Low disk space: ${availableMB.toFixed(2)}MB available`,
							);
						}
					}
				}
		...
		for (const dep of dependencies) {
			try {
				...
				if (dep.required) {
					result.success = false;
					result.errors.push(
						`Required dependency '${dep.name}' is not available`,
					);
				} else {
					result.warnings.push(
						`Optional dependency '${dep.name}' is not available`,
					);
				}
			}
		}
	...
	async checkEnvironment() {
		...
		for (const envVar of envVars) {
			...
			if (value) {
				...
				if (envVar.required) {
					result.success = false;
					result.errors.push(
						`Required environment variable ${envVar.name} is not set`,
					);
				}
			}
		}
		...
		try {
			...
			if (!success) {
				result.warnings.push(
					"Not in a git repository - version control recommended",
				);
			}
		} catch {
