...
async function autoRegisterMCP() {
	return new Promise((resolve) => {
		...
		claudeCheck.on("close", (code) => {
			if (code === 0) {
				console.log(
					"Claude Code detected! Auto-registering all supported MCP servers...",
				);
				...
				console.log(
					"Claude Code not detected. Manual MCP setup will be required.",
				);
		...
		setTimeout(() => {
			console.log(
				"Claude Code detection timed out. Continuing installation...",
			);
			resolve();
		}, 5000);
	});
	...
	const mcpServers = [
		{
			name: "claude-flow",
			description:
				"Advanced swarm coordination, neural optimization & intelligent task orchestration",
			command: isGlobal
		...
		},
		{
			...
			command: ["mcp", "add", "perplexity", "npx", "server-perplexity-ask"],
			priority: "medium",
		},
		{
	...
	const registerServer = (serverIndex) => {
		...
		register.on("close", (regCode) => {
			...
			if (regCode === 0) {
				...
				console.log(
					`   ❌ ${server.name} registration failed (exit code: ${regCode})`,
				);
			}
