...
describe("MCP Lifecycle Manager", () => {
	...
	beforeEach(() => {
		mockServerFactory = jest.fn(
			() => new MCPServer(mockMCPConfig, mockEventBus, mockLogger),
		);
		...
		it("should track tool metrics", async () => {
			...
			expect(
				Array.isArray(metrics)
					? metrics[0].totalInvocations
					: metrics.totalInvocations,
			).toBe(2);
			expect(
				Array.isArray(metrics)
					? metrics[0].successfulInvocations
...
describe("MCP Orchestration Integration", () => {
	...
	describe("Integration Management", () => {
		it("should start integration successfully", async () => {
			...
			const orchestratorStatus = status.find(
				(s) => s.component === "orchestrator",
			);
		...
		it("should register orchestrator tools when enabled", async () => {
			...
			const tools = (server as any).toolRegistry.listTools();
			const orchestratorTools = tools.filter((t: any) =>
				t.name.startsWith("orchestrator/"),
			);
