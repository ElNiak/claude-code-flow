...
import type { MCPConfig, MCPRequest } from "../../shared/types/utils-types.js";
...
export class ConnectionStateManager {
	...
	recordEvent(event: Omit<ConnectionEvent, "timestamp">): void {
		...
		if (this.connectionHistory.length > this.config.maxHistorySize) {
			this.connectionHistory = this.connectionHistory.slice(
	...
	removePendingRequest(requestId: string): void {
		...
		this.currentState.pendingRequests =
			this.currentState.pendingRequests.filter((req) => req.id !== requestId);
	}
	...
	getReconnectionTime(sessionId: string): number | null {
		...
		return (
			reconnectEvent.timestamp.getTime() - disconnectEvent.timestamp.getTime()
		);
	}
			...
			case "disconnect": {
				...
				if (duration !== null) {
					...
					const totalDuration =
						...
						duration;
					this.metrics.averageSessionDuration =
						totalDuration / this.metrics.totalDisconnections;
				}
				break;
			}
			...
			case "reconnect": {
				...
				if (reconnectTime !== null) {
					...
					const totalTime =
						...
						reconnectTime;
					this.metrics.averageReconnectionTime =
						totalTime / this.metrics.totalReconnections;
				}
				break;
			}
		}
