...
export class MCPProtocolManager {
	...
	constructor(
		...
		this.serverCapabilities =
			serverCapabilities || this.getDefaultCapabilities();
	...
	async negotiateProtocol(
		...
		try {
			...
			if (this.isVersionSupported(clientParams.protocolVersion)) {
				const clientVersionInfo = this.getVersionInfo(
					clientParams.protocolVersion,
				);
	...
	checkCompatibility(clientVersion: MCPProtocolVersion): CompatibilityResult {
		...
		if (!clientVersionInfo) {
			result.errors.push(
				`Unknown protocol version: ${this.versionToString(clientVersion)}`,
			);
		...
		const missingFeatures = serverFeatures.filter(
			(feature) => !clientFeatures.includes(feature),
		);
	...
	private negotiateCapabilities(
		...
		if (clientCapabilities.prompts && serverCapabilities.prompts) {
			result.prompts = {
				listChanged:
					clientCapabilities.prompts.listChanged &&
	...
	private filterCapabilitiesByVersion(
		...
		if (
			versionInfo.supportedFeatures.includes("logging") &&
		...
		if (
			versionInfo.supportedFeatures.includes("resources") &&
		...
		if (
			versionInfo.supportedFeatures.includes("prompts") &&
