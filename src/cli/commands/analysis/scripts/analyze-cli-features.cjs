...
function extractCommands(content) {
	...
	const cmdPattern1 =
		/(?:command|cmd)\s*:\s*['"]([^'"]+)['"]\s*,\s*description\s*:\s*['"]([^'"]+)['"]/g;
	let match;
	...
	const cmdPattern3 =
		/\.command\s*\(\s*['"]([^'"]+)['"]\s*\)\s*\.description\s*\(\s*['"]([^'"]+)['"]\s*\)/g;
...
if (require.main === module) {
	...
	// Save matrix
	fs.writeFileSync("CLI_FEATURE_MATRIX.json", JSON.stringify(matrix, null, 2));
	...
	console.log(
		`   Average Lines/File: ${inventory.summary.averageLinesPerFile}`,
	);
	console.log("\n✅ Analysis complete!");
