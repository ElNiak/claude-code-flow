...
if (import.meta.url === `file://${process.argv[1]}`) {
	...
	if (args.length === 0) {
		console.log(
			"Usage: node analyze-file-dependencies.js <file1> [file2] [file3] ...",
		);
		console.log("       node analyze-file-dependencies.js --batch <pattern>");
		console.log(
			"       node analyze-file-dependencies.js --report <output-file>",
		);
		process.exit(1);
	}
	...
	async function runCLI() {
		if (args[0] === "--batch") {
			...
			console.log(
				`🔍 Analyzing ${files.length} files matching pattern: ${pattern}`,
			);
