...
// Core components
export { createWorkCommand, WorkCommand } from "./work-command.js";

import { ConfigManager } from "./config-manager.js";
import { PresetManager } from "./preset-manager.js";
import { TaskAnalyzer } from "./task-analyzer.js";
import type { ValidationResult, WorkOptions } from "./types.js";
// Import all components for default export
import { createWorkCommand, WorkCommand } from "./work-command.js";
...
export const COMPLEXITY_INDICATORS = {
	...
	low: ["simple", "basic", "quick", "small", "minimal", "prototype"],
};
...
export const USAGE_EXAMPLES = [
	{
		...
		description:
			"Automatically detects development task, spawns 4 agents in hierarchical topology",
		expectedResult:
			"Complete API implementation with auth, tests, and documentation",
	},
	{
		command: 'work "research machine learning frameworks" --preset research',
		description:
			"Uses research preset with 3 agents in mesh topology for comprehensive analysis",
	...
	},
	{
		...
		description:
			"Custom deployment with specific agent count and sequential execution",
...
export const validateWorkOptions = (options: WorkOptions): ValidationResult => {
	...
	if (
		options.topology &&
		!["mesh", "hierarchical", "ring", "star", "auto"].includes(options.topology)
	) {
		errors.push(
			"Invalid topology. Must be: mesh, hierarchical, ring, star, or auto",
		);
	}

	if (
		options.strategy &&
		!["parallel", "sequential", "adaptive"].includes(options.strategy)
	) {
