...
export class ConfigManager {
	...
	async loadDefaults(): Promise<void> {
		...
		this.config = this.mergeConfigs(
			this.config,
			this.convertEnvironmentConfig(this.environmentConfig),
		);
	...
	updateFromOptions(options: WorkOptions): void {
		...
		if (options.topology && options.topology !== "auto") {
			optionsConfig.coordination = {
				...this.config.coordination,
				defaultTopology: options.topology as any,
			};
		}
	 ...
	 */
	updateSection<K extends keyof WorkConfig>(
		section: K,
		updates: Partial<WorkConfig[K]>,
	): void {
	...
	/**
	 * Convert environment config to partial work config
	 */
	private convertEnvironmentConfig(
		envConfig: EnvironmentConfig,
	): Partial<WorkConfig> {
		const workConfig: Partial<WorkConfig> = {};

		if (envConfig.defaultAgents !== undefined) {
			workConfig.coordination = {
				...this.config.coordination,
				defaultAgents: envConfig.defaultAgents,
			};
		}
		if (envConfig.defaultTopology !== undefined) {
			workConfig.coordination = {
				...(workConfig.coordination || this.config.coordination),
				defaultTopology: envConfig.defaultTopology,
			};
		}
		if (envConfig.defaultStrategy !== undefined) {
			workConfig.coordination = {
				...(workConfig.coordination || this.config.coordination),
				defaultStrategy: envConfig.defaultStrategy,
			};
		}
		...
		if (envConfig.debug !== undefined) {
			workConfig.logging = {
				...(workConfig.logging || this.config.logging),
				debug: envConfig.debug,
			};
		}
		if (envConfig.enableMemory !== undefined) {
			workConfig.features = {
				...this.config.features,
				enableMemory: envConfig.enableMemory,
			};
		}
		if (envConfig.enableHooks !== undefined) {
			workConfig.features = {
				...(workConfig.features || this.config.features),
				enableHooks: envConfig.enableHooks,
			};
		}
	...
	private validateAgainstSchema(config: any, schema: any, path = ""): string[] {
		...
		for (const [key, rulesAny] of Object.entries(schema)) {
			const currentPath = path ? `${path}.${key}` : key;
			const value = config[key];
			const rules = rulesAny as any;
			...
			if (value !== undefined && value !== null) {
				...
				if (rules.enum && !rules.enum.includes(value)) {
					errors.push(
						`${currentPath} must be one of: ${rules.enum.join(", ")}`,
					);
				}
