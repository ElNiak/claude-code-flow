...
import { delay } from "../cli/shared/utils/helpers.js";
...
export class TaskScheduler {
	...
	async assignTask(task: Task, agentId: string): Promise<void> {
		this.logger.info("🔍 DEADLOCK DEBUG: Scheduler assignTask called", {
			taskId: task.id,
			agentId,
		});
		...
		if (task.dependencies.length > 0) {
			this.logger.info("🔍 DEADLOCK DEBUG: Checking task dependencies", {
				taskId: task.id,
			...
			if (unmetDependencies.length > 0) {
				this.logger.warn("🚨 DEADLOCK DEBUG: Task has unmet dependencies", {
					taskId: task.id,
					unmetDependencies,
				});
		...
		this.logger.info("🔍 DEADLOCK DEBUG: Creating scheduled task", {
			taskId: task.id,
			agentId,
		});
		...
		this.logger.info("🔍 DEADLOCK DEBUG: Updated agent tasks", {
			agentId,
			taskCount: this.agentTasks.get(agentId)!.size,
		});
		...
		this.logger.info("🔍 DEADLOCK DEBUG: Starting task execution", {
			taskId: task.id,
		});
		this.startTask(task.id);
		this.logger.info("🔍 DEADLOCK DEBUG: Scheduler assignTask completed", {
			taskId: task.id,
			agentId,
		});
	}
	...
	async failTask(taskId: string, error: Error): Promise<void> {
		...
		if (scheduled.attempts < this.config.maxRetries) {
			...
			const retryDelay = this.config.retryDelay * 2 ** (scheduled.attempts - 1);
