...
export function validateHookParams(
	...
	switch (hookType) {
		case "pre-task":
			if (
				params.complexity &&
				!["low", "medium", "high"].includes(params.complexity)
			) {
		...
		case "post-edit":
			...
			if (hookType === "pre-edit" && params.operation) {
				if (!["read", "write", "edit", "delete"].includes(params.operation)) {
					result.errors.push(
						"--operation must be one of: read, write, edit, delete"
					);
					result.valid = false;
				}
			}
		...
		case "notification":
			...
			if (
				params.level &&
				!["info", "warning", "error"].includes(params.level)
			) {
	...
	if (
		...
		result.warnings.push(
			"--load-previous without --session-id may load unexpected data"
		);
	}
 ...
 */
export function sanitizeHookParams(
	params: Record<string, any>
): Record<string, any> {
	...
	for (const [key, value] of Object.entries(params)) {
		...
		// Sanitize file paths,
		if (
			["file", "saveTo", "target"].includes(key) &&
			typeof value === "string"
		) {
