...
function showMainHelp(): void {
	...
	console.log(
		chalk.white("Claude-Flow is an advanced AI agent orchestration system.")
	);
	console.log(
		chalk.white(
	...
	console.log(
		chalk.gray("  claude-flow help getting-started    # Beginner tutorial")
	);
	console.log(
		chalk.gray("  claude-flow help --interactive      # Interactive help mode")
	);
	console.log(
		chalk.gray("  claude-flow help <topic>            # Specific topic help")
	);
	console.log();
	...
	for (const [category, description] of Object.entries(categories)) {
		...
		for (const topic of topics) {
			console.log(
				chalk.gray(`    ${topic.name.padEnd(20)} ${topic.description}`)
			);
		}
	}
	...
	console.log();
	console.log(
		chalk.gray('Use "claude-flow help <topic>" for detailed information.')
	);
...
async function showTopicHelp(topicName: string, options: any): Promise<void> {
	...
	if (!topic) {
		...
		if (similar.length > 0) {
			...
			console.log(
				chalk.gray('Use "claude-flow help --all" to see all topics.')
			);
		}
		return;
	}
	...
	if (!options.examples && !options.tutorial) {
		...
		if (topic.examples) {
			...
			if (topic.examples.length > 3) {
				console.log(
					chalk.gray(`    ... and ${topic.examples.length - 3} more`)
				);
			}
			console.log();
