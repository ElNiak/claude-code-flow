...
export class HookContextManager {
	static setContext(hookType, depth = 1) {
		...
		process.env[HOOK_SAFETY_CONFIG.ENV_VARS.SESSION_ID] =
...
export class HookCommandValidator {
	...
	static validateCommand(command, hookType) {
		...
		if (hookType === "Stop" && HookCommandValidator.isClaudeCommand(command)) {
			errors.push({
		...
		if (context.type && HookCommandValidator.isClaudeCommand(command)) {
		...
		if (HookCommandValidator.isDangerousPattern(command, hookType)) {
			warnings.push({
...
export class HookCircuitBreaker {
	...
	static checkExecution(hookType) {
		...
		if (
			hookType === "Stop" &&
			executionCount > HOOK_SAFETY_CONFIG.MAX_STOP_HOOK_EXECUTIONS
		) {
			throw new Error(
		...
		if (hookType === "Stop" && executionCount > 1) {
			printWarning(
				`⚠️  Stop hook execution #${executionCount} detected. Monitor for recursion.`,
			);
		}
	...
	static getStatus() {
		return {
			...
			executions: Array.from(executionTracker.executions.entries()).map(
				([key, count]) => {
...
export class HookConfigValidator {
	...
	static validateClaudeCodeConfig(configPath = null) {
		...
		try {
			...
			const validation = HookConfigValidator.validateHooksConfig(
				config.hooks || {},
			);
			...
			for (const hookGroup of hooksConfig.Stop) {
				for (const hook of hookGroup.hooks || []) {
					if (hook.type === "command" && hook.command) {
						const result = HookCommandValidator.validateCommand(
							hook.command,
							"Stop",
						);
...
export class SafeHookExecutor {
	...
	static async executeHookCommand(command, hookType, options = {}) {
		try {
			...
			// Command validation
			const validation = HookCommandValidator.validateCommand(
				command,
				hookType,
			);
			...
			const result = await SafeHookExecutor.executeCommand(command, options);
