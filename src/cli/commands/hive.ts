...
process.on("SIGINT", async () => {
	...
	await gracefulShutdown(ExitCode.SUCCESS);
});
...
export async function hiveAction(ctx: CommandContext) {
	...
	try {
		...
		globalMemory = new SwarmMemoryManager({
		...
		// Use global references for easier cleanup
		...
		if (!options.background) {
			...
			// Background mode - just clean up globals
			globalCoordinator = null;
			globalMemory = null;
			success(`✅ Hive Mind ${hiveId} started in background mode`);
		}
	} catch (err) {
		const errorMessage = err instanceof Error ? err.message : String(err);
		error(`Hive Mind error: ${errorMessage}`);
...
async function spawnHiveAgents(
	...
	for (
		let i = 0;
		i < Math.min(options.maxAgents - 1, agentConfigs.length);
		i++
	) {
		...
		case "mesh":
			return [
				...
				},
				{
					type: "scout",
					role: "explorer",
					capabilities: ["discovery", "research", "innovation"],
				},
				{
					type: "guardian",
					role: "validator",
		...
		case "star":
			return [
				...
				},
				{
					type: "scout",
					role: "sensor",
					capabilities: ["monitoring", "detection", "alerting"],
				},
				{
					type: "architect",
					role: "planner",
					capabilities: ["planning", "design", "strategy"],
				},
			];
		default:
			return [];
...
async function executeHive(
	...
	const assignments = await assignTasksWithVoting(
		coordinator,
		memory,
		tasks,
		agents,
	...
	await executeTasksWithMonitoring(
		coordinator,
		memory,
		assignments,
		agents,
	...
	await aggregateResultsWithQuality(
		coordinator,
		memory,
		objectiveId,
		agents,
...
function calculateBidScore(agent: HiveAgent, task: any): number {
	...
	if (
		...
		agent.capabilities.includes("documentation")
	)
		score += 2;
...
async function aggregateResultsWithQuality(
	...
	for (const agent of agents) {
		...
		const executions = await memory.search(pattern);
