...
export async function sparcDev(taskDescription, options = {}) {
	...
	try {
		...
		console.log(
			`📊 Test Coverage: ${result.refinement?.testResults?.coverage?.toFixed(1)}%`,
		);
		console.log(
			`🎯 Code Quality: ${result.refinement?.codeQuality?.overall?.toFixed(1)}/100`,
		);
		console.log(
			`⚡ Performance: ${result.refinement?.performance?.responseTime?.average}ms`,
		);
...
export async function sparcApi(taskDescription, options = {}) {
	...
	try {
		...
		console.log(
			`🔌 API Endpoints: ${result.architecture?.apiDesign?.endpoints?.length || 0}`,
		);
		console.log(
			`🔒 Security Score: ${result.refinement?.security?.score || 0}/100`,
		);
...
export async function sparcUi(taskDescription, options = {}) {
	...
	try {
		...
		console.log(
			`🎨 Components: ${result.architecture?.components?.length || 0}`,
		);
		console.log(
...
export async function sparcResearch(taskDescription, options = {}) {
	...
	try {
		...
		console.log(
			`📚 Sources Analyzed: ${result.specification?.userStories?.length || 0}`,
		);
		console.log(
			`📊 Analysis Workflows: ${result.pseudocode?.algorithms?.length || 0}`,
		);
		console.log(
...
export async function sparcData(taskDescription, options = {}) {
	...
	try {
		...
		console.log(
			`📊 Data Models: ${result.architecture?.dataModel?.entities?.length || 0}`,
		);
		console.log(
			`🔄 ETL Pipelines: ${result.architecture?.integrationPoints?.length || 0}`,
		);
		console.log(
...
export async function sparcSecurity(taskDescription, options = {}) {
	...
	try {
		...
		console.log(
			`🛡️ Vulnerabilities: ${result.refinement?.security?.vulnerabilities?.length || 0}`,
		);
		console.log(
...
export async function sparcPerformance(taskDescription, options = {}) {
	...
	try {
		...
		console.log(
			`📈 Optimizations: ${result.refinement?.optimizations?.length || 0} applied`,
		);
...
export function getSparcCommandHelp(command) {
	const helpTexts = {
		...
		refactor:
			"Code refactoring and optimization while preserving functionality",
...
export function showSparcCommandsHelp() {
	...
	console.log(
		'  claude-flow sparc-api "create REST API for e-commerce" --swarm',
	);
	console.log(
		'  claude-flow sparc-test "comprehensive testing for payment module"',
	);
	console.log(
		'  claude-flow sparc-security "security audit for web application"',
	);
}
