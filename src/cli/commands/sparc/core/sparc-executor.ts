...
export class SparcTaskExecutor {
	...
	private async executeSparcPhase(
		...
		switch (agent.type) {
			...
			case "coordinator":
				if (
					task.name.includes("Architecture") ||
					objective.includes("design")
				) {
	...
	private determineAppType(description: string): string {
		...
		if (desc.includes("pandas") || desc.includes("data"))
			return "data-pipeline";
		if (desc.includes("machine learning") || desc.includes("ml"))
			return "ml-app";
		if (desc.includes("cli") || desc.includes("command")) return "cli-tool";
		if (desc.includes("scraper") || desc.includes("scraping"))
			return "web-scraper";
		...
		const stories: Record<string, any[]> = {
			"rest-api": [
				{
					id: "US001",
					story:
						"As a developer, I want to create resources via POST endpoints",
					priority: "high",
				},
				{
					id: "US002",
					story:
						"As a developer, I want to retrieve resources via GET endpoints",
					priority: "high",
				},
				{
					id: "US003",
					story:
						"As a developer, I want to update resources via PUT/PATCH endpoints",
					priority: "medium",
				},
				{
					id: "US004",
					story:
						"As a developer, I want to delete resources via DELETE endpoints",
					priority: "medium",
				},
				{
					id: "US005",
					story:
						"As a developer, I want API documentation to understand endpoints",
					priority: "high",
				},
			],
			"python-web": [
			...
			"data-pipeline": [
				...
				},
				{
					id: "US003",
					story:
						"As a data analyst, I want to export results in various formats",
					priority: "medium",
				},
			],
		};
	...
	private async generateFailingTests(
		appType: string,
		language: string,
	): Promise<any> {
		...
		const tests = {
			...
			integration: this.generateIntegrationTestCases(
				appType,
				language,
				testFramework,
			),
	...
	private async refactorImplementation(
		implementation: any,
		tests: any,
	): Promise<any> {
		return {
	...
	private async generateProjectFiles(
		...
		for (const [filename, content] of Object.entries(files)) {
			await fs.writeFile(
				path.join(targetDir, filename as string),
				content as string,
			);
		}
	}
