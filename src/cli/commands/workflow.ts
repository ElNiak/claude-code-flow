...
async function runWorkflow(workflowFile: string, options: any): Promise<void> {
	try {
		...
		console.error(
			chalk.red("Workflow execution failed:"),
...
async function validateWorkflow(
	...
	try {
		...
		console.error(
			chalk.red("✗ Workflow validation failed:"),
			(error as Error).message
		);
		process.exit(1);
	}
...
async function listWorkflows(options: any): Promise<void> {
	try {
		...
		console.error(
			chalk.red("Failed to list workflows:"),
			(error as Error).message
		);
	}
}
...
async function showWorkflowStatus(
	...
	try {
		...
		console.error(
			chalk.red("Failed to get workflow status:"),
			(error as Error).message
		);
	}
}
...
async function stopWorkflow(workflowId: string, options: any): Promise<void> {
	try {
		...
		if (execution.status !== "running") {
			console.log(
				chalk.yellow(`Workflow is not running (status: ${execution.status})`)
			);
			return;
		}
		...
		console.error(
			chalk.red("Failed to stop workflow:"),
			(error as Error).message
		);
	}
}
...
async function generateTemplate(
	...
	if (!template) {
		...
		console.log(
			chalk.gray("Available templates:"),
...
async function watchWorkflowStatus(workflowId: string): Promise<void> {
	...
	while (true) {
		try {
			...
			console.error(
				chalk.red("Error watching workflow:"),
				(error as Error).message
			);
			break;
		}
...
function displayWorkflowStatus(execution: WorkflowExecution): void {
	...
	console.log(
		`${chalk.white("Started:")} ${execution.startedAt.toLocaleString()}`
	);
	console.log(`${chalk.white("Duration:")} ${duration}`);
