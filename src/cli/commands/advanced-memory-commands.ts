...
export async function advancedMemoryCommand(
	...
	try {
		...
		logger.error("Advanced memory command error", {
			error: message,
			subcommand,
			subArgs,
			flags,
...
function showAdvancedMemoryHelp(): void {
	...
	console.log(
		"  memory export <file> [options]      - Export memory data in multiple formats"
	);
	console.log(
	...
	console.log(
		"  memory store <key> <value> [opts]   - Store data with advanced options"
	);
	console.log(
		"  memory get <key> [options]          - Retrieve data with caching"
	);
	console.log(
		"  memory delete <key> [options]       - Delete specific entries"
	);
	console.log(
		"  memory list [options]               - List entries with filtering"
	);
	...
	console.log(
		"  memory config [options]             - View/update configuration"
	);
	console.log("\nFeatures:");
...
async function queryCommand(
	...
	if (!search) {
		...
		console.log(
			"  --access-level <level>  Filter by access level (private|shared|public)"
		);
		...
		console.log(
			"  --aggregate-by <field>  Generate aggregations (namespace|type|owner|tags)"
		);
	...
	try {
		...
		const queryOptions: QueryOptions = {
			...
			type: flags.type,
			tags: flags.tags
				? flags.tags.split(",").map((t: string) => t.trim())
				: undefined,
			owner: flags.owner,
			...
			createdAfter: flags["created-after"]
				? new Date(flags["created-after"])
				: undefined,
			createdBefore: flags["created-before"]
				? new Date(flags["created-before"])
				: undefined,
			updatedAfter: flags["updated-after"]
				? new Date(flags["updated-after"])
				: undefined,
			updatedBefore: flags["updated-before"]
				? new Date(flags["updated-before"])
				: undefined,
			sizeGreaterThan: flags["size-gt"]
				? parseInt(flags["size-gt"])
				: undefined,
		...
		printSuccess(
			`Found ${result.total} entries in ${formatDuration(duration)}`
		);
		...
		switch (format) {
			...
			default: // table,
				...
				result.entries.forEach((entry: any, i: number) => {
					...
					console.log(
						`   Last Accessed: ${entry.lastAccessedAt.toLocaleString()}`
					);
		...
		if (result.aggregations) {
			...
			for (const [key, value] of Object.entries(result.aggregations)) {
				console.log(`${key}:`);
				for (const [subKey, stats] of Object.entries(
					value as Record<string, any>
				)) {
					console.log(
						`  ${subKey}: ${(stats as any).count} entries, ${formatBytes((stats as any).totalSize)}`
					);
				}
				console.log();
			}
		...
		if (result.total > result.entries.length) {
			const showing =
				(flags.offset ? parseInt(flags.offset) : 0) + result.entries.length;
...
async function exportCommand(
	...
	if (!file) {
		...
		console.log(
			"  --filter-query <json>   Advanced filtering (JSON query options)"
		);
		return;
	}
...
async function importCommand(
	...
	if (!file) {
		...
		console.log("Options:");
		console.log(
			"  --format <format>           Import format (json|csv|xml|yaml)"
		);
		console.log(
			"  --namespace <namespace>     Target namespace for imported data"
		);
		console.log(
...
async function statsCommand(
	...
	try {
		...
		console.log(
			`   Total Entries: ${stats.overview.totalEntries.toLocaleString()}`
		);
		...
		if (Object.keys(stats.distribution.byNamespace).length > 0) {
			...
			for (const [namespace, data] of Object.entries(
				stats.distribution.byNamespace
			)) {
				console.log(
					`     ${namespace}: ${(data as any).count} entries, ${formatBytes((data as any).size)}`
				);
			}
		}
		...
		if (Object.keys(stats.distribution.byType).length > 0) {
			...
			for (const [type, data] of Object.entries(stats.distribution.byType)) {
				console.log(
					`     ${type}: ${(data as any).count} entries, ${formatBytes((data as any).size)}`
				);
			}
		}
		console.log();
		...
		console.log(
			`   Average Query Time: ${formatDuration(stats.performance.averageQueryTime)}`
		);
		console.log(
			`   Average Write Time: ${formatDuration(stats.performance.averageWriteTime)}`
		);
		console.log(
			`   Cache Hit Ratio: ${(stats.performance.cacheHitRatio * 100).toFixed(1)}%`
		);
		console.log(
			`   Index Efficiency: ${(stats.performance.indexEfficiency * 100).toFixed(1)}%`
		);
		console.log();

		// Health,
		console.log("🏥 Health:");
		const healthStatus = stats.health.recommendedCleanup
			? "Needs Attention"
			: "Healthy";
		...
		if (stats.optimization.suggestions.length > 0) {
			...
			console.log(
				`   Cleanup: ${formatBytes(stats.optimization.potentialSavings.cleanup)}`
			);
			console.log(
	...
	try {
		...
		const cleanupOptions: CleanupOptions = {
			...
			archiveOld: flags["archive-old"]
				? {
						enabled: true,
						olderThan: flags["archive-older-than"]
							? parseInt(flags["archive-older-than"])
							: 365,
		...
		if (
			flags["dry-run"] &&
			(result.entriesRemoved > 0 || result.entriesArchived > 0)
		) {
...
async function storeCommand(
	...
	if (!key || !value) {
		...
		console.log(
			"  --access-level <level>  Access level (private|shared|public, default: shared)"
		);
	...
	try {
		...
		if (flags.format === "json") {
			...
			if (typeof entry.value === "string" && entry.value.length > 500) {
				...
				console.log(
					`(showing first 500 characters of ${entry.value.length} total)`
				);
			} else {
...
async function deleteCommand(
	...
	try {
		...
		if (!flags.confirm) {
			console.log(
				`About to delete entry: ${key} (namespace: ${entry.namespace})`
			);
