...
class PerformanceBaselineAnalyzer {
	...
	async establishBaseline() {
		...
		try {
			const { stdout } = await execAsync(
				"node dist/cli/simple-cli.js --version 2>/dev/null",
			);
	...
	async measureStartupPerformance() {
		...
		console.log(
			`  ✅ Startup baseline: ${this.results.metrics.startup.statistics.mean.toFixed(2)}ms (avg)`,
		);
	}
	...
	generateRecommendations() {
		...
		if (startupMean > 150) {
			recs.push({
				...
				recommendation:
					"Consider lazy loading heavy dependencies and reducing initial import cost",
				threshold: "< 150ms for good UX",
			});
		}

		// Command performance recommendations
		for (const [cmdName, cmdData] of Object.entries(
			this.results.metrics.commands,
		)) {
			if (cmdData.statistics.mean > 200) {
				recs.push({
					...
					recommendation:
						"Optimize command execution path and reduce blocking operations",
		...
		recs.push({
			...
			recommendation:
				"Implement dynamic imports, command caching, and build optimizations",
...
`;
		...
		for (const [name, data] of Object.entries(this.results.metrics.commands)) {
			const status =
				data.statistics.mean < 150
					? "✅"
					: data.statistics.mean < 250
						? "⚠️"
						: "❌";
...
`;
		...
		for (const [file, data] of Object.entries(this.results.metrics.fileSize)) {
			if (!data.error) {
				const status =
					data.kilobytes < 50 ? "✅" : data.kilobytes < 150 ? "⚠️" : "❌";
...
`;
	...
	async run() {
		...
		console.log("\n📊 Summary:");
		console.log(
			`  Startup time: ${this.results.metrics.startup.statistics.mean.toFixed(2)}ms (avg)`,
		);
		console.log(
			`  Commands tested: ${Object.keys(this.results.metrics.commands).length}`,
		);
		console.log(`  Recommendations: ${this.results.recommendations.length}`);
		console.log(
			`  High priority issues: ${this.results.recommendations.filter((r) => r.severity === "high").length}`,
		);
