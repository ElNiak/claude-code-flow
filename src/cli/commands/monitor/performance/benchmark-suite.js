...
class BenchmarkSuite {
	...
	async singleRun(command, args = []) {
		...
		return new Promise((resolve) => {
			const fullCommand =
				`node ${this.cliPath} ${command} ${args.join(" ")}`.trim();
	...
	analyzeMetrics(measurements) {
		...
		return {
			...
			throughput: {
				opsPerSec:
					1000 / (durations.reduce((a, b) => a + b, 0) / durations.length),
				totalTime: durations.reduce((a, b) => a + b, 0),
			},
			stability: {
				coefficient:
					this.standardDeviation(durations) /
	...
	async runBenchmarkCategory(category) {
		...
		for (const benchmark of category.benchmarks) {
			this.log(
				`  Running: ${benchmark.name} - ${benchmark.description}`,
				"info",
			);
			...
			if (!result.error) {
				this.log(
					`    ✅ Average: ${result.duration.mean.toFixed(2)}ms, P95: ${result.duration.p95.toFixed(2)}ms`,
					"success",
				);
			} else {
	...
	generateSummary() {
		...
		for (const [categoryName, benchmarks] of this.results.benchmarks) {
			...
			if (categoryMetrics.count > 0) {
				categoryStats.set(categoryName, {
					averageDuration:
						categoryMetrics.durations.reduce((a, b) => a + b, 0) /
						categoryMetrics.durations.length,
					averageMemory:
						categoryMetrics.memoryUsages.reduce((a, b) => a + b, 0) /
						categoryMetrics.memoryUsages.length,
		...
		this.results.summary = {
			...
			successRate:
				totalBenchmarks > 0
					? ((successfulBenchmarks / totalBenchmarks) * 100).toFixed(2)
					: 0,
			overall:
				overallMetrics.durations.length > 0
					? {
							averageDuration:
								overallMetrics.durations.reduce((a, b) => a + b, 0) /
								overallMetrics.durations.length,
							fastestCommand: Math.min(...overallMetrics.durations),
							slowestCommand: Math.max(...overallMetrics.durations),
							averageMemoryUsage:
								overallMetrics.memoryUsages.reduce((a, b) => a + b, 0) /
								overallMetrics.memoryUsages.length,
						}
					: null,
	...
	async compareWithBaseline() {
		...
		try {
			...
			for (const [categoryName, benchmarks] of this.results.benchmarks) {
				...
				for (const [benchmarkName, current] of benchmarks) {
					const baselineBenchmark =
						baseline.benchmarks?.[categoryName]?.[benchmarkName];

					if (baselineBenchmark && !current.error && !baselineBenchmark.error) {
						const durationChange =
							((current.duration.mean - baselineBenchmark.duration.mean) /
								baselineBenchmark.duration.mean) *
							100;
						const memoryChange =
							((current.memory.mean - baselineBenchmark.memory.mean) /
								baselineBenchmark.memory.mean) *
							100;
						...
						categoryComparisons.set(benchmarkName, {
							...
							p95: {
								...
								change:
									((current.duration.p95 - baselineBenchmark.duration.p95) /
										baselineBenchmark.duration.p95) *
									100,
							},
						});
					}
			...
			this.results.comparisons = Object.fromEntries(
				comparisons.entries().map(([k, v]) => [k, Object.fromEntries(v)]),
			);
	...
	async exportJSON() {
		const jsonData = {
			...
			benchmarks: Object.fromEntries(
				Array.from(this.results.benchmarks.entries()).map(([k, v]) => [
					k,
...
`;
		...
		if (this.results.comparisons) {
			...
			for (const [categoryName, comparisons] of Object.entries(
...
`;
		...
		for (const [categoryName, stats] of Object.entries(
			this.results.summary.categories,
		)) {
...
`;
	...
	async run() {
		...
		this.log(
			`Environment: Node ${this.results.environment.nodeVersion} on ${this.results.environment.platform}`,
			"debug",
		);
	...
	displaySummary() {
		...
		console.log(
			`   Successful: ${chalk.green(summary.successfulBenchmarks)} (${summary.successRate}%)`,
		);
		...
		if (summary.overall) {
			console.log(`\n⚡ Performance:`);
			console.log(
				`   Average Time: ${chalk.cyan(summary.overall.averageDuration.toFixed(2))}ms`,
			);
			console.log(
				`   Fastest Command: ${chalk.green(summary.overall.fastestCommand.toFixed(2))}ms`,
			);
			console.log(
				`   Slowest Command: ${chalk.yellow(summary.overall.slowestCommand.toFixed(2))}ms`,
			);
			console.log(
				`   Average Memory: ${chalk.blue(summary.overall.averageMemoryUsage.toFixed(2))}MB`,
			);
		}
			...
			const performanceColor =
				stats.averageDuration < 1000
					...
					: stats.averageDuration < 2000
						...
						: chalk.red;
			console.log(
				`   ${category}: ${performanceColor(stats.averageDuration.toFixed(2))}ms avg (${stats.benchmarkCount} benchmarks)`,
			);
		}
...
function parseArgs() {
	...
	const options = {
		...
		baseline:
			args.find((arg) => arg.startsWith("--baseline="))?.split("=")[1] || null,
		outputFormat:
			args.find((arg) => arg.startsWith("--output="))?.split("=")[1] || "both",
