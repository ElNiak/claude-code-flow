	...
	detectRegressions() {
		...
		if (Math.abs(startupChange) > 5) {
			...
			const item = {
				...
				severity:
					Math.abs(startupChange) > 20
						? "high"
						: Math.abs(startupChange) > 10
							? "medium"
							: "low",
			};
	...
	async generateRegressionReport() {
		const report = {
			...
			summary: {
				...
				highSeverityRegressions: this.regressions.filter(
					(r) => r.severity === "high",
				).length,
				overallStatus:
					this.regressions.length === 0
						? "PASS"
						: this.regressions.filter((r) => r.severity === "high").length > 0
							? "FAIL"
							: "WARNING",
			},
	...
	generateRecommendations() {
		...
		if (this.regressions.length === 0) {
			recs.push({
				...
				recommendation:
					"No performance regressions detected. Continue monitoring.",
				priority: "low",
			});
		} else {
			// High severity regressions
			const highSeverity = this.regressions.filter(
				(r) => r.severity === "high",
			);
			...
			const startupRegressions = this.regressions.filter(
				(r) => r.metric === "Startup Time",
			);
			...
			// Command regressions
			const commandRegressions = this.regressions.filter((r) =>
				r.metric.startsWith("Command:"),
			);
			if (commandRegressions.length > 0) {
				recs.push({
					category: "Command Performance",
					recommendation:
						"Profile affected commands and optimize execution paths",
					priority: "medium",
