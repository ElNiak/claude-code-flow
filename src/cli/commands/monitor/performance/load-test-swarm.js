...
import { SystemIntegration } from "../dist/cli/commands/monitor/core/system-integration.js";
import { SwarmCoordinator } from "../dist/cli/commands/swarm/core/swarm-coordinator.js";
...
class SwarmLoadTester {
	...
	async initialize() {
		...
		try {
			...
			this.swarmCoordinator =
				this.systemIntegration.getComponent("swarmCoordinator");
	...
	async spawnAgents() {
		this.log(
			`Phase 2: Spawning ${this.config.agentsPerSwarm} agents per swarm`,
		);
	...
	async runForDuration() {
		...
		while (Date.now() < endTime && this.isRunning) {
			try {
				...
				this.log(
					`Error during load test execution: ${error.message}`,
					"warning",
				);
				this.metrics.errors.push({
					message: error.message,
	...
	generateReport() {
		...
		console.log(
			`   Swarms Created: ${chalk.green(this.metrics.swarmsCreated)}`,
		);
		console.log(
			`   Agents Spawned: ${chalk.green(this.metrics.agentsSpawned)}`,
		);
		console.log(`   Tasks Created: ${chalk.green(this.metrics.tasksCreated)}`);
		console.log(`   Total Operations: ${chalk.cyan(totalOperations)}`);
		console.log(
			`   Throughput: ${chalk.yellow(throughput.toFixed(2))} ops/sec`,
		);

		console.log(`\n🚀 Performance Metrics:`);
		console.log(
			`   Average Response Time: ${chalk.cyan(avgResponseTime.toFixed(2))}ms`,
		);
		console.log(
			`   Min Response Time: ${chalk.green(minResponseTime.toFixed(2))}ms`,
		);
		console.log(
			`   Max Response Time: ${chalk.yellow(maxResponseTime.toFixed(2))}ms`,
		);
		console.log(
			`   Total Requests: ${chalk.blue(this.metrics.responseTimes.length)}`,
		);
		...
		if (avgResponseTime < 100) {
			...
			console.log(
				`   Response Time: ${chalk.yellow("ACCEPTABLE")} (500-1000ms)`,
			);
		} else {
		...
		if (throughput > 50) {
			...
			console.log(
				`   Throughput: ${chalk.yellow("ACCEPTABLE")} (10-20 ops/sec)`,
			);
		} else {
		...
		if (errorRate === 0) {
			...
			console.log(
				`   Reliability: ${chalk.yellow("ACCEPTABLE")} (1-5% errors)`,
			);
		} else {
