...
import type { EventBus } from "../cli/core/events/event-bus.js";
...
import { SystemIntegration } from "./core/system-integration.js";
...
export class DiagnosticManager {
	...
	constructor(
		...
		this.healthCheckManager =
			healthCheckManager || new HealthCheckManager(eventBus, logger);
	...
	private async analyzeComponent(
		...
		try {
			...
			const healthHistory =
				this.healthCheckManager.getHealthHistory(componentName);
			const recentChecks = healthHistory.slice(-10); // Last 10 checks,
			const failureRate =
				recentChecks.filter((check) => !check.healthy).length /
				recentChecks.length;
			...
			if (config.includePerformanceMetrics !== false) {
				const performanceIssues =
					this.analyzeComponentPerformance(componentName);
			...
			if (
				...
				"getMetrics" in component &&
				typeof component.getMetrics === "function"
			) {
				try {
					...
					this.logger.warn(
						`Failed to get metrics for ${componentName}: ${_getErrorMessage(error)}`,
					);
				}
			}
	...
	private async analyzePerformance(
		...
		const responseTimeHistory =
			this.performanceHistory.get("responseTime") || [];
		const averageResponseTime =
			responseTimeHistory.length > 0
				? responseTimeHistory.reduce((sum, time) => sum + time, 0) /
	...
	private generateRecommendations(
		...
		if (health.overall === "unhealthy") {
			recommendations.push(
				"System health is compromised - immediate attention required",
			);
		} else if (health.overall === "warning") {
			recommendations.push(
				"System showing warning signs - proactive maintenance recommended",
			);
		}
		...
		if (highFailureComponents.length > 0) {
			recommendations.push(
				"Some components have high failure rates - check logs and dependencies",
			);
		}
	...
	private calculateSeverity(
		...
		const criticalIssues = components.reduce(
			(count, component) =>
				count +
				component.issues.filter((issue) => issue.severity === "critical")
					.length,
			0,
		);
		...
		const highIssues = components.reduce(
			(count, component) =>
				count +
				component.issues.filter((issue) => issue.severity === "high").length,
			0,
		);
...
`;
	...
	private setupEventHandlers(): void {
		...
		this.eventBus.on("system:error", (error: any) => {
			const component =
				error && typeof error === "object" && error.component
					? error.component
					: "system";
	...
	async quickDiagnostic(): Promise<{
		...
		const issues = components.reduce(
			(count, comp) => count + comp.issues.length,
			0,
		);
