...
import { CoordinationMetricsCollector } from "../src/cli/commands/monitor/core/metrics.js";
...
export class PerformanceMetricsTestSuite {
	constructor() {
		...
		this.metricsCollector = new CoordinationMetricsCollector(
			this.logger,
			this.eventBus,
			5000,
		);
	...
	async testBasicMetricsCollection() {
		...
		this.metricsCollector.recordMetric("test.metric", 150, {
			type: "advanced",
		});
	...
	async testTaskLifecycleMetrics() {
		...
		if (metrics.taskMetrics.totalTasks < 5) {
			throw new Error(
				`Expected at least 5 tasks, got ${metrics.taskMetrics.totalTasks}`,
			);
		}

		console.log(
			`    ✓ Task metrics: ${metrics.taskMetrics.totalTasks} total, ${metrics.taskMetrics.completedTasks} completed`,
		);
	...
	async testAgentMetrics() {
		...
		if (metrics.agentMetrics.totalAgents < 5) {
			throw new Error(
				`Expected at least 5 agents, got ${metrics.agentMetrics.totalAgents}`,
			);
		}

		console.log(
			`    ✓ Agent metrics: ${metrics.agentMetrics.totalAgents} total, utilization: ${metrics.agentMetrics.agentUtilization.toFixed(1)}%`,
		);
	...
	async testResourceMetrics() {
		...
		for (let i = 0; i < 3; i++) {
			this.eventBus.emit("system:resource:released", {
		...
		// Simulate deadlock
		this.eventBus.emit("system:deadlock:detected", {
			resources: ["db-conn-1", "file-lock-1"],
		});
		...
		if (metrics.resourceMetrics.totalResources < 2) {
			throw new Error(
				`Expected resource metrics, got ${metrics.resourceMetrics.totalResources}`,
			);
		}

		console.log(
			`    ✓ Resource metrics: ${metrics.resourceMetrics.totalResources} total, ${metrics.resourceMetrics.lockedResources} locked, ${metrics.resourceMetrics.deadlockCount} deadlocks`,
		);
	...
	async testCoordinationMetrics() {
		...
		if (metrics.coordinationMetrics.messagesSent < 20) {
			throw new Error(
				`Expected 20 messages sent, got ${metrics.coordinationMetrics.messagesSent}`,
			);
		}

		console.log(
			`    ✓ Coordination metrics: ${metrics.coordinationMetrics.messagesSent} sent, ${metrics.coordinationMetrics.messagesReceived} received, ${metrics.coordinationMetrics.conflictsResolved} conflicts resolved`,
		);
	...
	async testPerformanceCalculations() {
		...
		console.log(
			`    ✓ Performance calculations: Memory: ${metrics.performanceMetrics.memoryUsage.toFixed(2)}MB, Error rate: ${metrics.performanceMetrics.errorRate}`,
		);
	...
	async testRealisticSwarmScenario() {
		...
		for (const agentType of scenario.agents) {
			for (let i = 0; i < agentType.count; i++) {
				...
				this.eventBus.emit("system:agent:spawned", {
					agentId,
					type: agentType.type,
				});
				await this.delay(100);
		...
		for (let i = 0; i < scenario.tasks.length; i++) {
			...
			if (i % 3 === 0) {
				...
				this.eventBus.emit("system:resource:acquired", {
					resourceId: `shared-resource-${i}`,
				});
				await this.delay(200);
				this.eventBus.emit("system:resource:released", {
					resourceId: `shared-resource-${i}`,
			...
			if (Math.random() > 0.1) {
				...
				this.eventBus.emit("system:task:failed", {
					taskId,
					reason: "complexity",
				});
				// Retry failed task
				...
				this.eventBus.emit("system:task:started", {
					taskId: `${taskId}-retry`,
				});
				await this.delay(200);
				this.eventBus.emit("system:task:completed", {
					taskId: `${taskId}-retry`,
				});
			}
		}
		...
		console.log(
			`    ✓ Realistic scenario completed in ${executionTime.toFixed(2)}ms`,
		);
		console.log(
			`      Tasks: ${metrics.taskMetrics.totalTasks} total, ${metrics.taskMetrics.completedTasks} completed`,
		);
		console.log(
			`      Agents: ${metrics.agentMetrics.totalAgents} total, utilization: ${metrics.agentMetrics.agentUtilization.toFixed(1)}%`,
		);
	...
	async testHighLoadScenario() {
		...
		for (let i = 0; i < 50; i++) {
			agentPromises.push(
				this.simulateAgentSpawn(
					`load-agent-${i}`,
					["coordinator", "coder", "analyst", "tester"][i % 4],
				),
			);
		}
		...
		console.log(
			`    ✓ High load scenario completed in ${loadTime.toFixed(2)}ms`,
		);
		console.log(
			`      Peak load: ${metrics.taskMetrics.totalTasks} tasks, ${metrics.agentMetrics.totalAgents} agents`,
		);
		...
		this.testResults.push({
			...
			taskThroughput:
				(metrics.taskMetrics.totalTasks / (loadTime / 1000)).toFixed(2) +
				" tasks/sec",
			agentThroughput:
				(metrics.agentMetrics.totalAgents / (loadTime / 1000)).toFixed(2) +
				" agents/sec",
		});
	}
	...
	async testMetricsAggregation() {
		...
		console.log(
			`    ✓ History: ${history.length} samples, Top metrics: ${topMetrics.length}`,
		);
	...
	generateFinalReport() {
		...
		console.log("-".repeat(50));
		console.log(
			`Tasks: ${finalMetrics.taskMetrics.totalTasks} total, ${finalMetrics.taskMetrics.completedTasks} completed`,
		);
		console.log(
			`Agents: ${finalMetrics.agentMetrics.totalAgents} total, ${finalMetrics.agentMetrics.agentUtilization.toFixed(1)}% utilization`,
		);
		console.log(
			`Resources: ${finalMetrics.resourceMetrics.totalResources} total, ${finalMetrics.resourceMetrics.resourceUtilization.toFixed(1)}% utilization`,
		);
		console.log(
			`Messages: ${finalMetrics.coordinationMetrics.messagesSent} sent, ${finalMetrics.coordinationMetrics.messagesReceived} received`,
		);
		console.log(
			`Performance: ${finalMetrics.performanceMetrics.memoryUsage.toFixed(2)}MB memory, ${finalMetrics.performanceMetrics.errorRate} errors`,
		);
