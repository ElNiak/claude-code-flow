	...
	async checkTemplateIntegrity() {
		...
		try {
			...
			if (result.templates.corrupted.length > 0) {
				result.success = false;
				result.errors.push(
					`Corrupted templates: ${result.templates.corrupted.join(", ")}`
				);
			}

			if (result.templates.missing.length > 0) {
				result.warnings.push(
					`Missing templates: ${result.templates.missing.join(", ")}`
				);
			}
		} catch (error) {
			result.success = false;
	...
	async checkConfigConsistency() {
		...
		try {
			...
			if (!roomodesCheck.consistent) {
				result.warnings.push(
					"Inconsistency between .roomodes and available commands"
				);
			}
			...
			if (!claudeCheck.consistent) {
				result.warnings.push(
					"Inconsistency between CLAUDE.md and actual setup"
				);
			}
			...
			result.success = false;
			result.errors.push(
				`Configuration consistency check failed: ${error.message}`
			);
		}
				...
				if (content.includes(command)) {
					...
					if (parts[0] === "claude-flow") {
						...
						try {
							...
							result.consistent = false;
							result.issues.push(
								`Command ${command} mentioned but executable not found`
							);
						}
					}
				}
