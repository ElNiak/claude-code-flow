...
class SessionManager implements ISessionManager {
	...
	async terminateSession(sessionId: string): Promise<void> {
		this.logger.info("🔍 DEADLOCK DEBUG: Starting session termination", {
			sessionId,
		});
		...
		try {
			...
			this.logger.info(
				"🔍 DEADLOCK DEBUG: Updating session status to terminated",
				{ sessionId }
			);
			...
			this.logger.info("🔍 DEADLOCK DEBUG: Starting terminal termination", {
				sessionId,
			...
			await Promise.race([
				...
				this.logger.error("🚨 DEADLOCK DEBUG: Terminal termination failed", {
					sessionId,
					terminalId: session.terminalId,
			...
			const terminalDuration = Date.now() - terminalStart;
			this.logger.info("🔍 DEADLOCK DEBUG: Terminal termination completed", {
				sessionId,
				terminalId: session.terminalId,
				duration: terminalDuration,
			});
			...
			this.logger.info("🔍 DEADLOCK DEBUG: Starting memory bank close", {
				sessionId,
				memoryBankId: session.memoryBankId,
			});
			...
			await Promise.race([
				...
				this.logger.error("🚨 DEADLOCK DEBUG: Memory bank close failed", {
					sessionId,
					memoryBankId: session.memoryBankId,
			...
			const memoryDuration = Date.now() - memoryStart;
			this.logger.info("🔍 DEADLOCK DEBUG: Memory bank close completed", {
				sessionId,
				memoryBankId: session.memoryBankId,
				duration: memoryDuration,
			});
...
export class Orchestrator implements IOrchestrator {
	...
	async initialize(): Promise<void> {
		...
		try {
			...
			await this.initializeComponent("MCP Server", () =>
	...
	async assignTask(task: Task): Promise<void> {
		this.logger.info("🔍 DEADLOCK DEBUG: Starting task assignment", {
			taskId: task.id,
		...
		this.logger.info("🔍 DEADLOCK DEBUG: Storing task in history", {
			taskId: task.id,
		});
		...
		try {
			this.logger.info(
				"🔍 DEADLOCK DEBUG: Entering task assignment circuit breaker",
				{ taskId: task.id }
			);
			await this.taskAssignmentCircuitBreaker.execute(async () => {
				...
				if (!task.assignedAgent) {
					this.logger.info(
						"🔍 DEADLOCK DEBUG: No agent assigned, adding to queue",
						{ taskId: task.id, queueLength: this.taskQueue.length }
					);
					...
					this.logger.info(
						"🔍 DEADLOCK DEBUG: Processing task queue immediately",
						{ taskId: task.id }
					);
				...
				this.logger.info("🔍 DEADLOCK DEBUG: Assigning to specific agent", {
					taskId: task.id,
				...
				this.logger.info(
					"🔍 DEADLOCK DEBUG: Calling coordination manager assign task",
				...
				this.logger.info(
					"🔍 DEADLOCK DEBUG: Coordination manager assign task completed",
					{
						taskId: task.id,
						agentId: task.assignedAgent,
	...
	async getHealthStatus(): Promise<HealthStatus> {
		...
		try {
			return await this.healthCheckCircuitBreaker.execute(async () => {
				this.logger.info(
					"🔍 DEADLOCK DEBUG: Inside health check circuit breaker",
					{}
				);
				...
				this.logger.info(
					"🔍 DEADLOCK DEBUG: Starting parallel component health checks",
					{}
				);
				...
				this.logger.info(
					"🔍 DEADLOCK DEBUG: Component health checks completed",
	...
	private startHealthChecks(): void {
		this.logger.info("🔍 DEADLOCK DEBUG: Starting health checks", {
			interval: this.config.orchestrator.healthCheckInterval,
		});
		this.healthCheckInterval = setInterval(async () => {
			const healthStart = Date.now();
			this.logger.info("🔍 DEADLOCK DEBUG: Health check starting", {
				timestamp: new Date().toISOString(),
			});
			try {
				const health = await this.getHealthStatus();
				const healthDuration = Date.now() - healthStart;
				this.logger.info("🔍 DEADLOCK DEBUG: Health check completed", {
				...
				if (health.status === "unhealthy") {
					this.logger.warn(
						"🚨 DEADLOCK DEBUG: System health check failed",
						health
					);

					// Attempt recovery for unhealthy components,
					this.logger.info(
						"🔍 DEADLOCK DEBUG: Starting recovery for unhealthy components",
						{}
					);
				...
				const healthDuration = Date.now() - healthStart;
				this.logger.error("🚨 DEADLOCK DEBUG: Health check error", {
					duration: healthDuration,
					error: error,
				});
			}
	...
	private async processTaskQueue(): Promise<void> {
		this.logger.info("🔍 DEADLOCK DEBUG: Starting task queue processing", {
		...
		this.logger.info("🔍 DEADLOCK DEBUG: Available agents retrieved", {
			availableAgentCount: availableAgents.length,
		});

		while (this.taskQueue.length > 0 && availableAgents.length > 0) {
			const task = this.taskQueue.shift()!;
			this.logger.info("🔍 DEADLOCK DEBUG: Processing task from queue", {
				taskId: task.id,
			});
			const agent = this.selectAgentForTask(task, availableAgents);

			if (agent) {
				this.logger.info("🔍 DEADLOCK DEBUG: Agent selected for task", {
					taskId: task.id,
					agentId: agent.id,
				});
				...
				try {
					this.logger.info(
						"🔍 DEADLOCK DEBUG: Assigning task to agent via coordination manager",
						{ taskId: task.id, agentId: agent.id }
					);
					const assignStart = Date.now();
					await this.coordinationManager.assignTask(task, agent.id);
					const assignDuration = Date.now() - assignStart;
					this.logger.info("🔍 DEADLOCK DEBUG: Task assignment completed", {
						taskId: task.id,
						agentId: agent.id,
						duration: assignDuration,
					});
