...
export class TaskEngine extends EventEmitter {
		...
		if (filter) {
			filteredTasks = filteredTasks.filter((task) => {
				...
				if (
					...
					!filter.assignedAgent.includes(task.assignedAgent || "")
				)
					return false;
				if (filter.priority) {
					if (
						filter.priority.min !== undefined &&
						task.priority < filter.priority.min
					)
						return false;
					if (
						filter.priority.max !== undefined &&
						task.priority > filter.priority.max
					)
						return false;
				}
			...
			filteredTasks.sort((a, b) => {
				...
				switch (sort.field) {
					...
					case "estimatedDuration":
						return (
							direction *
							((a.estimatedDurationMs || 0) - (b.estimatedDurationMs || 0))
						);
					default:
						return 0;
				}
		...
		for (const dependent of dependents) {
			if (dependent.status === "pending" || dependent.status === "queued") {
				await this.cancelTask(
					dependent.id,
					`Dependency ${taskId} was cancelled`
				);
			}
		}
	}
	...
	private async rollbackTask(task: WorkflowTask): Promise<void> {
		...
		const targetIndex = task.checkpoints.findIndex(
			(cp) => cp.id === targetCheckpoint.id
		);
	...
	private handleTaskFailed(data: { taskId: string; error: Error }): void {
		...
		if (
			task.retryPolicy &&
			(task.metadata.retryCount || 0) < task.retryPolicy.maxAttempts
		) {
			...
			setTimeout(
				...
				task.retryPolicy!.backoffMs *
					task.retryPolicy!.backoffMultiplier ** currentRetryCount
			);
		}
	}
