...
import type { TaskEngine, TaskExecution, WorkflowTask } from "./engine.js";
import type { CoordinationContext, MemoryEntry, TodoItem } from "./types.js";
...
export class TaskCoordinator extends EventEmitter {
	...
	async readTodos(
		...
		if (sessionId) {
			...
			todos = todos.filter((todo) =>
				sessionTodos.some((st) => st.id === todo.id)
			);
		}
		...
		if (filter) {
			...
			if (filter.assignedAgent) {
				todos = todos.filter(
					(todo) => todo.assignedAgent === filter.assignedAgent
				);
			}
			if (filter.tags) {
				todos = todos.filter((todo) =>
			...
			if (filter.batchOptimized !== undefined) {
				todos = todos.filter(
					(todo) => todo.batchOptimized === filter.batchOptimized
				);
			}
		}
	...
	async launchParallelAgents(
		...
		for (const task of tasks) {
			try {
				const agentId = await this.launchAgent(
					task,
					coordinationContext,
					batchId
				);
	...
	async coordinateBatchOperations(
		...
		for (const [type, ops] of Array.from(groupedOps.entries())) {
			promises.push(
				this.executeBatchOperationType(type, ops, batchId, results)
			);
		}
	...
	private async handleTaskStarted(data: {
		taskId: string;
		agentId: string;
	}): Promise<void> {
	...
	private async handleTaskCancelled(data: {
		taskId: string;
		reason: string;
	}): Promise<void> {
