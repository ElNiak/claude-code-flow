...
export class DatabaseManager extends EventEmitter {
	...
	async getAgentPerformance(agentId: string): Promise<any> {
		...
		return {
			successRate:
				agent.success_count / (agent.success_count + agent.error_count) || 0,
	...
	async submitConsensusVote(
		...
		const status =
			currentRatio >= (proposal as any).required_threshold
				? "achieved"
				: "pending";
    ...
    `)
		...
		return {
			...
			...(taskStats as any),
			agentUtilization:
				(agentStats as any).agentCount > 0
	...
	async updateConsensusStatus(id: string, status: string): Promise<void> {
		this.db
			.prepare(
				"UPDATE consensus SET status = ?, completed_at = CURRENT_TIMESTAMP WHERE id = ?"
			)
			.run(status, id);
	}
	...
	async getRecentConsensusProposals(limit: number): Promise<any[]> {
		return this.db
			.prepare("SELECT * FROM consensus ORDER BY created_at DESC LIMIT ?")
			.all(limit);
	}
