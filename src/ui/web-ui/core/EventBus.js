...
export class EventBus {
			...
			for (const handlerInfo of handlers) {
				if (handlerInfo.regex.test(event)) {
					try {
						...
						console.error(
							`📡 EventBus: Error in wildcard handler for '${pattern}':`,
							_error
						);
	...
	async emitAsync(event, data = null) {
		...
		for (const [pattern, handlers] of this.wildcardHandlers) {
			for (const handlerInfo of handlers) {
				if (handlerInfo.regex.test(event)) {
					const promise = this.executeHandlerAsync(
						handlerInfo,
						data,
						eventInfo
					);
		...
		if (failures.length > 0) {
			console.error(
				`📡 EventBus: ${failures.length} handlers failed for '${event}'`
			);
	...
	removeAllListeners(event = null) {
		...
		if (this.isLogging) {
			console.debug(
				`📡 EventBus: Removed all listeners${event ? ` for '${event}'` : ""}`
			);
		}
	}
	...
	getStats() {
		return {
			...
			totalHandlers:
				...
				Array.from(this.onceEvents.values()).reduce(
					(sum, handlers) => sum + handlers.length,
					0
				) +
