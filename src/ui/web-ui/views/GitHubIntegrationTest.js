...
export const GitHubViewMCPIntegration = {
	...
	testIntegration: async () => {
		...
		const analysisResult =
			await GitHubViewMCPIntegration.tools.github_repo_analyze.handler({
		...
		// Test PR management
		const prResult =
			await GitHubViewMCPIntegration.tools.github_pr_manage.handler({
				repo: "ruvnet/claude-flow",
				action: "review",
				pr_number: 123,
			});
		...
		// Test metrics
		const metricsResult =
			await GitHubViewMCPIntegration.tools.github_metrics.handler({
				repo: "ruvnet/claude-flow",
			});
