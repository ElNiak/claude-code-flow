...
export class NeuralPanelTest {
	...
	async testToolsTab() {
		...
		if (toolsTab) {
			...
			const executeButtons = document.querySelectorAll(
				'.execute-btn, [data-action="execute"]'
			);
			const configButtons = document.querySelectorAll(
				'.config-btn, [data-action="configure"]'
			);
	...
	async testToolCard(card, index) {
		...
		const toolName =
			card.querySelector(".tool-name, h3, h4")?.textContent ||
			`Tool ${index + 1}`;
		...
		if (execBtn) {
			...
			const response = document.querySelector(
				".tool-response, .modal, .dialog"
			);
			if (response) {
				console.log(`    ✅ Response shown for ${toolName}`);
				// Close if needed
				const closeBtn = response.querySelector(
		...
		const configBtn = card.querySelector(
			'.config-btn, [data-action="configure"]'
		);
		if (configBtn) {
			configBtn.click();
	...
	findControl(type) {
		const selectors = {
			refresh:
				'[data-action="refresh"], .refresh-btn, button[aria-label*="Refresh"]',
			export:
				'[data-action="export"], .export-btn, button[aria-label*="Export"]',
	...
	calculateOverallStatus() {
		...
		return {
			...
			status:
				passed === total ? "PASS" : passed > total / 2 ? "PARTIAL" : "FAIL",
		};
	}
