...
class DAATools {
	...
	createDAAPanel() {
		...
		const consoleContainer =
			document.querySelector(".console-container") || document.body;
	...
	performOptimization(optimization) {
		...
		const totalGain =
			improvements.reduce((sum, imp) => sum + imp.improvement, 0) /
			improvements.length;
	...
	updateResourceGraphs() {
		...
		resources.forEach((resource, index) => {
			const utilization =
				resource.capacity > 0 ? resource.allocated / resource.capacity : 0;
	...
	updateOptimizationDashboard() {
		const optimizationStatus = document.getElementById(
			"daa-optimization-status"
		);
	...
	switchTab(tabName) {
		...
		document
			.querySelector(`.daa-tab[data-tab="${tabName}"]`)
	...
	handleAllocateResource() {
		...
		const capacity =
			parseInt(document.getElementById("daa-resource-capacity").value) || 100;
