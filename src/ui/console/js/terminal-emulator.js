...
export class TerminalEmulator {
	...
	showWelcomeMessage() {
		...
		const existingWelcome =
			this.outputElement.querySelector(".welcome-message");
		if (existingWelcome) {
...
╚═══════════════════════════════════════════════════════════╝</div>
	...
	setupScrollBehavior() {
		...
		this.outputElement.addEventListener("scroll", () => {
			...
			const maxScrollTop =
				this.outputElement.scrollHeight - this.outputElement.clientHeight;
	...
	exportHistory() {
		const entries = Array.from(
			this.outputElement.querySelectorAll(".output-entry")
		);

		return entries.map((entry) => {
			const timestamp =
				entry.querySelector(".output-timestamp")?.textContent || "";
