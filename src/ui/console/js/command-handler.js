...
export class CommandHandler {
	...
	async processCommand(command) {
		if (this.isProcessing) {
			this.terminal.writeWarning(
				"Another command is still processing. Please wait..."
			);
			return;
		}
	...
	async showHelp(args) {
		...
		Object.keys(this.builtinCommands).forEach((cmd) => {
			this.terminal.writeLine(
				`  ${cmd.padEnd(12)} - ${this.getCommandDescription(cmd)}`
			);
		});
		...
		this.terminal.writeLine(
			"  claude-flow/execute  - Execute Claude Flow commands"
		);
...
Examples:
  ...
  swarm list`,
		...
		try {
			...
			if (health.healthy) {
				...
				this.terminal.writeError(
					`Server is unhealthy: ${health.error || "Unknown error"}`
				);
			}
	...
	async executeClaudeFlow(args) {
		...
		try {
			...
			this.terminal.writeSuccess(
				`Claude Flow ${subcommand} executed successfully`
			);
	...
	async initializeProject(args) {
		...
		this.terminal.writeWarning(
			"Project initialization not yet implemented in web console"
		);
	}
	...
	async manageConfig(args) {
		this.terminal.writeInfo(
			"Use the Settings panel (⚙️ button) to manage configuration"
		);
	}
	...
	async executeSparcMode(mode, _args) {
		...
		try {
			...
			this.terminal.writeError(
				`SPARC ${mode} execution failed: ${error.message}`
			);
		}
	}
	...
	async executeRemoteCommand(command, _args) {
		if (!this.wsClient.isConnected) {
			this.terminal.writeError(
				'Not connected to server. Use "connect" command first.'
			);
			return;
		}
	 ...
	 */
	hasCommand(command) {
		return Object.hasOwn(this.allCommands, command.toLowerCase());
	}
}
