...
class ClaudeCodeConsole {
		...
		this.wsClient.on("disconnected", (info) => {
			...
			if (info && info.code !== 1000) {
				this.terminal.writeError(
					`Connection lost: ${info.reason || "Unknown reason"}`
				);
			}
		});
		...
		switch (method) {
			case "claude-flow/started":
				this.terminal.writeSuccess(
					`Claude Flow started in ${params.mode} mode`
				);
				break;
			...
			default:
				this.terminal.writeInfo(
					`Claude Flow: ${method} - ${JSON.stringify(params)}`
				);
		}
	}
			...
			const type =
				params.level === "error"
					...
					: params.level === "warn"
						...
						: "info";
			this.terminal.write(
				`[${params.level.toUpperCase()}] ${params.message}`,
				type
			);
		}
	}
	...
	setupEventListeners() {
		...
		window.addEventListener("unhandledrejection", (event) => {
			...
			if (this.terminal) {
				this.terminal.writeError(
					`Unhandled error: ${event.reason.message || event.reason}`
				);
			}
		});
		...
		window.addEventListener("error", (event) => {
			...
			if (this.terminal) {
				this.terminal.writeError(
					`Application error: ${event.error.message || event.error}`
				);
			}
		});
	}
