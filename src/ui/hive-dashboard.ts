...
import type { HiveOrchestrator } from "../coordination/hive-orchestrator.js";
import type { HiveCommunicationProtocol } from "../coordination/hive-protocol.js";
...
export class HiveDashboard {
	...
	static formatConsoleOutput(data: HiveDashboardData): string {
		...
		for (const agent of data.agents) {
			const statusIcon = HiveDashboard.getStatusIcon(agent.status);
			const workloadBar = HiveDashboard.createProgressBar(agent.workload);
			output.push(`${statusIcon} ${agent.name} (${agent.type})`);
			output.push(
				`   Status: ${agent.status} | Workload: ${workloadBar} ${agent.workload}%`
			);
			if (agent.currentTask) {
				output.push(`   Task: ${agent.currentTask}`);
			}
			output.push(
				`   Votes: ${agent.votes} | Contributions: ${agent.contributions}`
			);
			output.push("");
		}
		...
		for (const task of data.tasks) {
			const progressBar = HiveDashboard.createProgressBar(task.progress);
			const statusIcon = HiveDashboard.getTaskStatusIcon(task.status);
		...
		output.push(
			`Average Consensus: ${(data.consensus.averageConsensus * 100).toFixed(1)}%`
		);
		output.push("");
		...
		output.push(
			`Quality Score: ${(data.performance.qualityScore * 100).toFixed(1)}%`
		);
		output.push(
			`Avg Execution Time: ${(data.performance.avgExecutionTime / 1000).toFixed(1)}s`
		);
		output.push("");
		...
		output.push(
			`Message Rate: ${data.communication.messageRate.toFixed(1)}/min`
		);
		output.push(
			`Knowledge Shared: ${data.communication.knowledgeShared} entries`
		);
