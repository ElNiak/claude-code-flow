...
describe("Migration System", () => {
	...
	describe("MigrationAnalyzer", () => {
		...
		it("should detect existing .claude folder and commands", async () => {
			...
			await fs.writeFile(
				path.join(commandsPath, "sparc.md"),
				"# SPARC Command"
			);
			await fs.writeFile(
				path.join(commandsPath, "custom-command.md"),
				"# Custom Command"
			);
		...
		it("should detect optimized prompts", async () => {
			...
			await fs.writeFile(
				path.join(claudePath, "BATCHTOOLS_GUIDE.md"),
				"# Guide"
			);
			await fs.writeFile(
				path.join(claudePath, "BATCHTOOLS_BEST_PRACTICES.md"),
				"# Practices"
			);
	...
	describe("RollbackManager", () => {
		...
		it("should list backups chronologically", async () => {
			...
			expect(backups[0].timestamp.getTime()).toBeGreaterThan(
	...
	describe("MigrationValidator", () => {
		...
		it("should validate successful migration", async () => {
			...
			await fs.writeFile(
				path.join(commandsPath, "claude-flow-help.md"),
				"# Help"
			);
			await fs.writeFile(
				path.join(claudePath, "BATCHTOOLS_GUIDE.md"),
		...
		it("should detect missing required files", async () => {
			...
			expect(result.errors.length).toBeGreaterThan(0);
			expect(
				result.errors.some((e) => e.includes("Required file missing"))
			).toBe(true);
		});
	...
	describe("Integration Tests", () => {
		...
		it("should recover from failed migration", async () => {
			...
			const brokenExists = await fs.pathExists(
				path.join(claudePath, "broken.md")
			);
	...
	describe("Edge Cases", () => {
		...
		it("should handle invalid JSON configurations", async () => {
			...
			expect(
				analysis.migrationRisks.some((r) =>
					r.description.includes("Invalid .roomodes")
				)
			).toBe(true);
		});
		...
		it("should handle missing permissions", async () => {
			...
			expect(result.warnings.some((w) => w.includes("not be writable"))).toBe(
				true
			);
		});
