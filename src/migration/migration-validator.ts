...
export class MigrationValidator {
	...
	private async validateCommandFiles(
		projectPath: string,
		result: ValidationResult
	): Promise<void> {
		const check: ValidationCheck = {
		...
		try {
			...
			if (!hasInstructions) {
				result.warnings.push(
					`Command ${command} may have insufficient content`
				);
			}
		...
		if (await fs.pathExists(packageJsonPath)) {
			try {
				...
				if (conflictingScripts.length > 0) {
					result.warnings.push(
						`Potential script conflicts: ${conflictingScripts.join(", ")}`
					);
				}
			} catch (error) {
