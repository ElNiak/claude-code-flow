...
export class MigrationAnalyzer {
			...
			try {
				...
				analysis.customConfigurations[".roomodes"] = {
					...
					customModes: Object.keys(roomodes).filter(
						(mode) =>
							!["architect", "code", "tdd", "debug", "docs-writer"].includes(
								mode
							)
					),
				};
	...
	printAnalysis(analysis: MigrationAnalysis, detailed: boolean = false): void {
		...
		console.log(
			`${chalk.bold("Timestamp:")} ${analysis.timestamp.toISOString()}`
		);
		...
		if (analysis.migrationRisks.length > 0) {
			...
			analysis.migrationRisks.forEach((risk) => {
				...
				console.log(
					`  ${icon} ${chalk.bold(risk.level.toUpperCase())}: ${risk.description}`
				);
				if (risk.mitigation) {
					console.log(
						`     ${chalk.gray("→")} ${chalk.italic(risk.mitigation)}`
					);
				}
			});
		}
		...
		if (detailed) {
			...
			if (Object.keys(analysis.customConfigurations).length > 0) {
				console.log(chalk.bold("\n⚙️  Configurations:"));
				Object.entries(analysis.customConfigurations).forEach(
					([file, config]) => {
