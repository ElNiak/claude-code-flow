import { BaseCommandComponent, ComponentOptions, ComponentResult, ExecutionContext, ValidationResult, ComponentMetadata } from '../core/command-component.js';
  ...
  validate(options: ComponentOptions): ValidationResult {
    ...
    if (options.topology) {
      ...
      if (!validTopologies.includes(options.topology)) {
        errors.push(`Invalid topology: ${options.topology}. Must be one of: ${validTopologies.join(', ')}`);
      }
    }

    // Validate max agents
    if (options.maxAgents) {
      if (typeof options.maxAgents !== 'number' || options.maxAgents < 1 || options.maxAgents > 50) {
