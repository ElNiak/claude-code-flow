...
export const arrayUtils = {
  ...
  filter: <T>(items: T[], predicate: (item: T) => boolean): T[] => items.filter(predicate),
  map: <T, U>(items: T[], transform: (item: T) => U): U[] => items.map(transform),
  find: <T>(items: T[], predicate: (item: T) => boolean): T | undefined => items.find(predicate)
};