  ...
  return ((...args: Parameters<T>) => {
    ...
    try {
      ...
      if (result && typeof result.then === "function") {
        return result
          .then((value: any) => {
            debugLogger.logFunctionExit(callId, moduleName, functionName, value);
            return value;
          })
          .catch((error: Error) => {
            debugLogger.logFunctionError(callId, moduleName, functionName, error);
            throw error;
          });
      }
 ...
 */
function shouldInstrumentMethod(methodName: string, config: InstrumentationConfig): boolean {
  ...
  if ((methodName.startsWith("get") || methodName.startsWith("set")) && 
      !config.includeGetters && !config.includeSetters) {
    return false;
  }
  
  // Check exclude patterns
  if (config.excludePatterns.some(pattern => 
    pattern === methodName || methodName.includes(pattern)
  )) {
    return false;
  }
 ...
 */
function instrumentMethod(obj: any, methodName: string, moduleName: string): void {
  ...
  obj[methodName] = function (...args: unknown[]) {
    ...
    try {
      ...
      debugLogger.logFunctionError(callId, moduleName, methodName, error as Error);
      throw error;
    }
  };
...
export function withDebugLogging<T extends(...args: any[]) => any>(
  ...
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = function (...args: unknown[]) {
      const callId = debugLogger.logFunctionEntry(moduleName, functionName || propertyKey, args);
      ...
      try {
        ...
        if (result && typeof result.then === "function") {
          return result
            .then((value: unknown) => {
              debugLogger.logFunctionExit(callId, moduleName, functionName || propertyKey, value);
              return value;
            })
            .catch((error: Error) => {
              debugLogger.logFunctionError(callId, moduleName, functionName || propertyKey, error);
              throw error;
            });
        }
        
        debugLogger.logFunctionExit(callId, moduleName, functionName || propertyKey, result);
        return result;
      } catch (error) {
        debugLogger.logFunctionError(callId, moduleName, functionName || propertyKey, error as Error);
        throw error;
      }
    };
