...
export class EmergencyMemoryManager {
  ...
  private startGCMonitoring(): void {
    ...
    this.gcInterval = setInterval(() => {
      ...
      if (heapPercent > EMERGENCY_LIMITS.gcThreshold) {
        console.warn(`🚨 Emergency GC triggered at ${(heapPercent * 100).toFixed(1)}% heap usage`);
  ...
  private startMemoryMonitoring(): void {
    ...
    this.memoryCheckInterval = setInterval(() => {
      ...
      if (heapPercent > EMERGENCY_LIMITS.emergencyThreshold) {
        console.error(`🆘 EMERGENCY MEMORY CLEANUP at ${(heapPercent * 100).toFixed(1)}% heap usage`);
  ...
  private executeEmergencyCleanup(): void {
    ...
    const afterCleanup = process.memoryUsage().heapUsed / process.memoryUsage().heapTotal;
    if (afterCleanup > 0.9) {
      console.error('🚨 CRITICAL: Memory still high after cleanup. System may need restart.');
    }
  }
...
if (initialPercent > 0.7) {
  console.log(`🚨 High initial heap usage detected: ${(initialPercent * 100).toFixed(1)}%`);
