...
export class MemoryCircuitBreaker {
  ...
  constructor(config?: Partial<CircuitBreakerConfig>) {
    ...
    console.log(`   Max Heap: ${(this.config.maxHeapUsageBytes / 1024 / 1024 / 1024).toFixed(1)}GB`);
    console.log(`   Check Interval: ${this.config.checkInterval}ms`);
    console.log(`   Warning Threshold: ${(this.config.warningThreshold * 100).toFixed(0)}%`);
  }

  static getInstance(config?: Partial<CircuitBreakerConfig>): MemoryCircuitBreaker {
  ...
  private startMonitoring(): void {
    ...
    this.checkInterval = setInterval(() => {
      ...
      else if (heapUsed > this.config.maxHeapUsageBytes * this.config.warningThreshold) {
        this.emitWarning(heapUsed);
      }

      // Also check for approaching total heap limit
      if (heapUsed / heapTotal > 0.9) {
        console.warn(`⚠️ High heap usage: ${(heapUsed / 1024 / 1024 / 1024).toFixed(2)}GB / ${(heapTotal / 1024 / 1024 / 1024).toFixed(2)}GB (${((heapUsed / heapTotal) * 100).toFixed(1)}%)`);
      }
  ...
  private emitWarning(heapUsed: number): void {
    ...
    console.warn(`⚠️ MEMORY WARNING: ${heapGB.toFixed(2)}GB / ${maxGB.toFixed(1)}GB (${percentage.toFixed(1)}%)`);
  ...
  private triggerCircuitBreaker(heapUsed: number): void {
    ...
    console.error(`   Percentage: ${((heapUsed / this.config.maxHeapUsageBytes) * 100).toFixed(1)}%`);
  ...
  private gracefulShutdown(): void {
    console.log(`🔄 Initiating graceful shutdown in ${this.config.shutdownDelay}ms...`);
  ...
  getStatus(): any {
    ...
    return {
      ...
      heapUsagePercent: (memUsage.heapUsed / this.config.maxHeapUsageBytes) * 100,
