...
export class MemoryProtectedSpawner {
	...
	private buildMemoryProtectedEnv(
		...
		const { agentId, agentType, taskId, description, enableMemoryProtection } =
			agentInfo;
	...
	private logMemoryProtection(env: NodeJS.ProcessEnv): void {
		if (env.MEMORY_PROTECTED_AGENT === "true") {
			...
			console.log(
				`   Circuit Breaker: ${env.MEMORY_CIRCUIT_BREAKER === "true" ? "ENABLED" : "DISABLED"}`
			);
		} else {
	...
	private setupProcessMonitoring(
		...
		const monitorInterval = setInterval(() => {
			...
			console.log(
				`📊 Agent ${agentId} (PID: ${childProcess.pid}) - Memory monitoring active`
			);
	...
	private cleanupAllProcesses(): void {
		console.log(
			`🧹 Cleaning up ${this.spawnedProcesses.size} spawned processes...`
		);
...
export async function spawnMemoryProtectedAgent(
