...
export class DebugLogger {
  ...
  private callStack: Map<string, { startTime: number; depth: number }> = new Map();

  constructor() {
    this.config = {
      enabled: this.getEnvironmentConfig(),
      level: (process.env.CLAUDE_FLOW_DEBUG_LEVEL as any) || "DEBUG",
      modules: this.parseModuleList(process.env.CLAUDE_FLOW_DEBUG_MODULES || "*"),
      functions: this.parseModuleList(process.env.CLAUDE_FLOW_DEBUG_FUNCTIONS || "*"),
      maxArgLength: parseInt(process.env.CLAUDE_FLOW_DEBUG_MAX_ARG_LENGTH || "1000"),
  ...
  private shouldLog(module: string, functionName: string): boolean {
    ...
    const moduleMatch = this.config.modules.includes("*") || 
                       this.config.modules.some(m => module.includes(m));
    
    const functionMatch = this.config.functions.includes("*") || 
   ...
   */
  logFunctionEntry(module: string, functionName: string, args: unknown[]): string | null {
    if (!this.shouldLog(module, functionName)) {return null;}
    ...
    const argsStr = sanitizedArgs.map(arg => 
   ...
   */
  logFunctionExit(callId: string | null, module: string, functionName: string, returnValue?: unknown): void {
    if (!callId || !this.shouldLog(module, functionName)) {return;}

    const callInfo = this.callStack.get(callId);
    if (!callInfo) {return;}

    const executionTime = this.config.measureTime ? Date.now() - callInfo.startTime: undefined;
    ...
    if (this.config.includeReturn && returnValue !== undefined) {
      const returnStr = typeof returnValue === "string" ? `"${returnValue}"` : 
   ...
   * Log function error
   */
  logFunctionError(callId: string | null, module: string, functionName: string, error: Error): void {
    if (!callId || !this.shouldLog(module, functionName)) {return;}
  ...
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = function (...args: unknown[]) {
      ...
      try {
        ...
        debugLogger.logFunctionError(callId, module, propertyKey, error as Error);
        throw error;
      }
    };
...
export function traceFunction<T extends(...args: any[]) => any>(
  ...
  return ((...args: Parameters<T>) => {
    ...
    try {
      ...
      debugLogger.logFunctionError(callId, module, functionName, error as Error);
      throw error;
    }
  }) as T;
}
