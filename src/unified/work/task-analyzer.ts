...
import type { TaskInput, TaskAnalysis, TaskComplexity, AgentTopology, ExecutionStrategy } from './types.js';
...
export class TaskAnalyzer {
  ...
  async analyze(input: TaskInput): Promise<TaskAnalysis> {
    ...
    // Analyze complexity
    const complexity = this.analyzeComplexity(normalizedTask, keywords, context, input.params);
    ...
    this.logger.info(`✅ Task analysis complete (confidence: ${suggestions.confidence}%)`);
    return analysis;
  }
  ...
  private async analyzeContext(context: any): Promise<any> {
    ...
    try {
      ...
      this.logger.warn(`Context analysis warning: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    try {
      ...
      for (const file of files) {
        if (file.isDirectory()) {
          ...
          if (['src', 'lib', 'tests', 'test', 'docs', '.git', 'node_modules'].includes(file.name)) {
   ...
   */
  private determineTaskType(normalizedTask: string, keywords: string[], context: any): string {
    ...
    if (keywords.some(k => ['build', 'create', 'implement', 'develop', 'code'].includes(k))) {
      return 'development';
    }
    if (keywords.some(k => ['research', 'analyze', 'study', 'investigate'].includes(k))) {
      return 'research';
    }
    if (keywords.some(k => ['deploy', 'release', 'publish', 'ship'].includes(k))) {
      return 'deployment';
    }
    if (keywords.some(k => ['optimize', 'improve', 'fix', 'debug'].includes(k))) {
      return 'optimization';
    }
    if (keywords.some(k => ['test', 'validate', 'verify', 'check'].includes(k))) {
      return 'testing';
    }
  ...
  private generateSuggestions(
    ...
    const agents = Math.min(Math.max(baseConfig.agents * complexityMultiplier, 2), 12);
    ...
    return {
      ...
      recommendations: this.generateRecommendations(taskType, complexity, context)
    };
  }
   ...
   */
  private getBaseConfiguration(taskType: string): { agents: number; topology: AgentTopology; strategy: ExecutionStrategy } {
    const configs = {
      development: { agents: 4, topology: 'hierarchical' as AgentTopology, strategy: 'parallel' as ExecutionStrategy },
      research: { agents: 3, topology: 'mesh' as AgentTopology, strategy: 'adaptive' as ExecutionStrategy },
      deployment: { agents: 3, topology: 'sequential' as AgentTopology, strategy: 'sequential' as ExecutionStrategy },
      optimization: { agents: 2, topology: 'mesh' as AgentTopology, strategy: 'adaptive' as ExecutionStrategy },
      testing: { agents: 3, topology: 'parallel' as AgentTopology, strategy: 'parallel' as ExecutionStrategy },
      general: { agents: 3, topology: 'mesh' as AgentTopology, strategy: 'adaptive' as ExecutionStrategy }
    };
   ...
   */
  private selectOptimalTopology(taskType: string, complexity: TaskComplexity, agents: number): AgentTopology {
   ...
   */
  private selectOptimalStrategy(taskType: string, complexity: TaskComplexity): ExecutionStrategy {
   ...
   */
  private estimateDuration(taskType: string, complexity: TaskComplexity): string {
    const baseDurations = {
    ...
    const minutes = baseDurations[taskType]?.[complexity] || baseDurations.general[complexity];
   ...
   */
  private calculateConfidence(taskType: string, complexity: TaskComplexity, context: any): number {
   ...
   */
  private generateRecommendations(taskType: string, complexity: TaskComplexity, context: any): string[] {
    ...
    if (context.fileCount > 100) {
      recommendations.push('Large project detected - consider using hierarchical topology');
    }
  ...
  private initializeTaskPatterns(): Map<string, TaskPattern> {
    ...
    patterns.set('development', {
      keywords: ['build', 'create', 'implement', 'develop', 'code', 'api', 'app', 'system', 'feature'],
      matcher: (task, keywords, context) => {
        return keywords.some(k => ['build', 'create', 'implement', 'develop', 'code'].includes(k)) ||
               task.includes('api') || task.includes('app') || task.includes('system');
      }
    });

    patterns.set('research', {
      keywords: ['research', 'analyze', 'study', 'investigate', 'explore', 'find', 'discover'],
      matcher: (task, keywords, context) => {
        return keywords.some(k => ['research', 'analyze', 'study', 'investigate'].includes(k));
      }
    });

    patterns.set('deployment', {
      keywords: ['deploy', 'release', 'publish', 'ship', 'production', 'server', 'host'],
      matcher: (task, keywords, context) => {
        return keywords.some(k => ['deploy', 'release', 'publish', 'ship'].includes(k));
      }
    });

    patterns.set('optimization', {
      keywords: ['optimize', 'improve', 'fix', 'debug', 'performance', 'speed', 'memory'],
      matcher: (task, keywords, context) => {
        return keywords.some(k => ['optimize', 'improve', 'fix', 'debug', 'performance'].includes(k));
      }
    });
    ...
    patterns.set('testing', {
      ...
      matcher: (task, keywords, context) => {
        return keywords.some(k => ['test', 'validate', 'verify', 'check'].includes(k));
      }
    });
    ...
    return [
      {
        ...
        condition: (task, keywords, context, params) => {
          return keywords.some(k => ['multiple', 'several', 'many', 'complex'].includes(k));
        },
        weight: 8
      },
      {
      ...
      },
      {
        ...
        condition: (task, keywords, context, params) => {
          const techKeywords = keywords.filter(k => 
            ['api', 'database', 'frontend', 'backend', 'auth', 'deployment'].includes(k)
          );
      ...
      },
      {
        ...
        condition: (task, keywords, context, params) => {
          return keywords.some(k => ['integrate', 'connect', 'sync', 'api'].includes(k));
        },
        weight: 4
      },
      {
        ...
        condition: (task, keywords, context, params) => {
          return keywords.some(k => ['performance', 'speed', 'optimize', 'scale'].includes(k));
        },
        weight: 6
      },
      {
        ...
        condition: (task, keywords, context, params) => {
          return keywords.some(k => ['auth', 'security', 'login', 'permission'].includes(k));
        },
        weight: 5
      },
      {
  ...
  private detectLanguage(files: string[]): string {
    ...
    if (extensions.some(ext => ['.js', '.ts', '.jsx', '.tsx'].includes(ext))) return 'javascript/typescript';
    ...
    if (extensions.some(ext => ['.java', '.kt'].includes(ext))) return 'java/kotlin';
  ...
  private hasTestFiles(files: string[]): boolean {
    return files.some(f => 
      f.includes('test') || 
      f.includes('spec') || 
      ['.test.js', '.test.ts', '.spec.js', '.spec.ts'].some(ext => f.endsWith(ext))
    );
  }

  private hasDocumentationFiles(files: string[]): boolean {
    return files.some(f => 
      ['README.md', 'DOCUMENTATION.md', 'docs', 'documentation'].includes(f) ||
...
interface ComplexityRule {
  name: string;
  condition: (task: string, keywords: string[], context: any, params: string[]) => boolean;
  weight: number;
}