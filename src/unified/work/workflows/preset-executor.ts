...
export class PresetExecutor {
	...
	private analyzeResourceUsage(preset: WorkPreset): {
		...
		const requiredResources = preset.overrides.requiredResources || [];
		const stepResources = (preset.steps || []).flatMap(
			(step) => step.params?.resources || []
		);
		...
		const underutilized = requiredResources.filter(
			(resource) => (resourceCounts.get(resource) || 0) < 2
		);
	...
	private estimateExecutionDuration(
		...
		return parallelGroups.reduce((total, group) => {
			...
			const groupDuration = Math.max(
				...groupSteps.map((step) => step.params?.estimatedDuration || 5000)
			);
