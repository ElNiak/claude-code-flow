...
import type { WorkPreset, PresetDefinition, PresetMetadata, CustomPreset } from './types.js';
...
export class PresetManager {
  ...
  async initialize(): Promise<void> {
    ...
    this.logger.info(`Preset manager initialized with ${this.presets.size} built-in and ${this.customPresets.size} custom presets`);
  }
  ...
  listPresets(): PresetMetadata[] {
    ...
    for (const [name, definition] of this.presets) {
      metadata.push({
        name,
        type: 'builtin',
    ...
    // Add custom presets
    for (const [name, custom] of this.customPresets) {
      metadata.push({
        name,
        type: 'custom',
   ...
   */
  async createPreset(name: string, preset: WorkPreset, metadata: Partial<PresetMetadata>): Promise<void> {
   ...
   */
  async updatePreset(name: string, updates: Partial<WorkPreset>, metadataUpdates?: Partial<PresetMetadata>): Promise<void> {
    if (!this.customPresets.has(name)) {
  ...
  async importPreset(filePath: string, overwrite = false): Promise<string> {
    try {
      ...
      if (this.customPresets.has(name) && !overwrite) {
        throw new Error(`Preset '${name}' already exists. Use overwrite=true to replace it.`);
      }

      await this.createPreset(name, importData.preset, importData.metadata || {});
      
      this.logger.info(`Preset '${name}' imported successfully from ${filePath}`);
      return name;
    } catch (error) {
      this.logger.error(`Failed to import preset: ${error instanceof Error ? error.message : String(error)}`);
      throw error;
    }
  }
  ...
  validatePreset(preset: WorkPreset): { valid: boolean; errors: string[] } {
    ...
    if (preset.overrides.suggestedAgents !== undefined) {
      if (typeof preset.overrides.suggestedAgents !== 'number' || preset.overrides.suggestedAgents < 1 || preset.overrides.suggestedAgents > 20) {
  ...
  private initializeBuiltinPresets(): void {
    ...
    this.presets.set('development', {
      ...
      overrides: {
        ...
        requiredResources: ['code_generation', 'testing', 'file_operations', 'git_operations'],
        estimatedDuration: '45 minutes'
      },
      steps: [
        { type: 'analysis', description: 'Analyze requirements and design architecture' },
        { type: 'implementation', description: 'Implement core functionality' },
        { type: 'testing', description: 'Write and run tests' },
        { type: 'documentation', description: 'Generate documentation' }
      ]
    });
    ...
    this.presets.set('research', {
      ...
      steps: [
        { type: 'search', description: 'Gather information from multiple sources' },
        { type: 'analysis', description: 'Analyze and synthesize findings' },
        { type: 'documentation', description: 'Create comprehensive report' }
      ]
    });
    ...
    this.presets.set('deployment', {
      ...
      steps: [
        { type: 'preparation', description: 'Prepare deployment environment' },
        { type: 'deployment', description: 'Execute deployment safely' },
        { type: 'validation', description: 'Validate deployment success' },
        { type: 'monitoring', description: 'Setup monitoring and alerts' }
      ]
    });
    ...
    this.presets.set('optimization', {
      ...
      steps: [
        { type: 'profiling', description: 'Profile current performance' },
        { type: 'analysis', description: 'Identify bottlenecks and issues' },
        { type: 'optimization', description: 'Apply targeted optimizations' },
        { type: 'validation', description: 'Validate performance improvements' }
      ]
    });
    ...
    this.presets.set('testing', {
      ...
      steps: [
        { type: 'planning', description: 'Plan comprehensive test strategy' },
        { type: 'execution', description: 'Execute all test suites' },
        { type: 'analysis', description: 'Analyze test results' },
        { type: 'reporting', description: 'Generate quality report' }
      ]
    });
    ...
    this.presets.set('api', {
      ...
      overrides: {
        ...
        suggestedTopology: 'hierarchical',
        suggestedStrategy: 'parallel',
        requiredResources: ['code_generation', 'testing', 'validation', 'documentation'],
        estimatedDuration: '60 minutes'
      },
      steps: [
        { type: 'design', description: 'Design API architecture and endpoints' },
        { type: 'authentication', description: 'Implement authentication system' },
        { type: 'endpoints', description: 'Build REST endpoints' },
        { type: 'testing', description: 'Write API tests' },
    ...
    this.presets.set('data-analysis', {
      ...
      steps: [
        { type: 'exploration', description: 'Explore and understand the data' },
        { type: 'cleaning', description: 'Clean and prepare data' },
        { type: 'analysis', description: 'Perform statistical analysis' },
        { type: 'visualization', description: 'Create data visualizations' },
        { type: 'insights', description: 'Extract and document insights' }
      ]
    });
    ...
    this.presets.set('ml', {
      ...
      overrides: {
        ...
        suggestedTopology: 'hierarchical',
        suggestedStrategy: 'sequential',
        requiredResources: ['analysis', 'modeling', 'validation', 'documentation'],
        estimatedDuration: '90 minutes'
      },
      steps: [
        { type: 'data-prep', description: 'Prepare and preprocess data' },
        { type: 'feature-engineering', description: 'Engineer and select features' },
        { type: 'modeling', description: 'Train and tune models' },
        { type: 'evaluation', description: 'Evaluate model performance' },
        { type: 'deployment', description: 'Prepare model for deployment' }
      ]
    });
  }
  ...
  private async loadCustomPresets(): Promise<void> {
    for (const presetPath of this.presetPaths) {
      try {
        ...
        this.logger.warn(`Failed to load presets from ${presetPath}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  }
    ...
    try {
      ...
      for (const file of presetFiles) {
        ...
        try {
          ...
          this.logger.warn(`Failed to load preset ${file}: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
    } catch (error) {
   ...
   */
  private async tryLoadPresetFromFile(name: string): Promise<WorkPreset | null> {
    for (const presetPath of this.presetPaths) {
   ...
   */
  private async saveCustomPreset(name: string, preset: CustomPreset): Promise<void> {
