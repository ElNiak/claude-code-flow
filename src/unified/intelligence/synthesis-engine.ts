...
import { StructuredThinkingEngine, type SparcContext, type SparcState } from './structured-thinking.js';
import { ParallelCoordinationEngine, type CoordinationContext } from './parallel-coordination.js';
import { StrategicOversightEngine, type StrategicContext, type StrategicResult } from './strategic-oversight.js';
...
export class IntelligenceSynthesisEngine extends EventEmitter {
  ...
  async synthesizeIntelligence(
    ...
    try {
      ...
      const context = await this.buildIntelligenceContext(task, agents, swarmObjective, config);
      
      // Determine synthesis strategy
      const strategy = this.determineSynthesisStrategy(context);
      
      // Execute intelligence engines based on strategy
      const results = await this.executeIntelligenceEngines(task, agents, swarmObjective, config, strategy);
      
      // Synthesize results into unified output
      const synthesized = await this.synthesizeResults(results, context, strategy);
      
      // Generate execution plan
      const executionPlan = await this.generateExecutionPlan(synthesized, context);
      
      // Finalize synthesis
      const final = await this.finalizeSynthesis(synthesized, executionPlan, context, startTime);
  ...
  private async buildIntelligenceContext(
    ...
    const coordinationContext = this.buildCoordinationContext(task, agents, config);
    
    // Build strategic context
    const strategicContext = this.buildStrategicContext(swarmObjective, config);
    
    // Build synthesis context
    const synthesisContext = this.buildSynthesisContext(task, swarmObjective, config);
   ...
   */
  private buildSparcContext(task: TaskDefinition, primaryAgent: AgentState): SparcContext {
    ...
    return {
      ...
      domain,
      complexity,
      constraints: task.constraints?.dependencies?.map(d => d.toString()) || [],
   ...
   */
  private buildCoordinationContext(task: TaskDefinition, agents: AgentState[], config: Partial<SwarmConfig>): any {
   ...
   */
  private buildStrategicContext(swarmObjective: SwarmObjective, config: Partial<SwarmConfig>): StrategicContext {
    return {
      ...
      riskTolerance: config.qualityThreshold && config.qualityThreshold > 0.9 ? 'low' : 'medium',
      successMetrics: []
    };
  }
   ...
   */
  private buildSynthesisContext(task: TaskDefinition, swarmObjective: SwarmObjective, config: Partial<SwarmConfig>): any {
    const objectives: SynthesisObjective[] = [
      { dimension: 'quality', weight: 0.4, target: config.qualityThreshold || 0.8, current: 0 },
  ...
  private determineSynthesisStrategy(context: IntelligenceContext): string {
    ...
    if (strategic.riskTolerance === 'low' || strategic.stakeholders.length > 2) {
  ...
  private async executeIntelligenceEngines(
    ...
    switch (strategy) {
      case 'parallel_dominant':
        ...
        results.coordination = await this.parallelCoordination.applyParallelCoordination(task, agents, config);
        results.sparc = await this.structuredThinking.applyStructuredThinking(task, agents[0]);
        results.strategic = await this.strategicOversight.applyStrategicOversight(swarmObjective, agents, config);
        break;
      ...
      default: // balanced
        ...
        const [sparcResult, coordinationResult, strategicResult] = await Promise.all([
          this.structuredThinking.applyStructuredThinking(task, agents[0]),
          this.parallelCoordination.applyParallelCoordination(task, agents, config),
          this.strategicOversight.applyStrategicOversight(swarmObjective, agents, config)
        ]);
   ...
   */
  private generateAlternativeApproaches(results: any, context: IntelligenceContext): any[] {
    const alternatives = [];
    ...
    if (results.sparc) {
      alternatives.push({
        ...
        strengths: ['Thorough analysis', 'Quality focus', 'Systematic approach'],
    ...
    if (results.strategic) {
      alternatives.push({
        ...
        strengths: ['Strategic alignment', 'Risk management', 'Stakeholder focus'],
   ...
   */
  private synthesizeRecommendations(results: any, context: IntelligenceContext): SynthesizedRecommendation[] {
    ...
    if (results.sparc?.metadata?.thinkingPattern) {
      recommendations.push({
        ...
        rationale: 'SPARC analysis indicates this pattern best fits the task complexity',
        impact: 0.8,
    ...
    recommendations.push(...this.generateCrossEngineRecommendations(results, context));
   ...
   */
  private generateCrossEngineRecommendations(results: any, context: IntelligenceContext): SynthesizedRecommendation[] {
    const recommendations: SynthesizedRecommendation[] = [];
    
    // Quality-focused synthesis recommendation
    if (results.sparc?.quality > 0.9 && results.coordination?.metadata?.parallelizability > 0.6) {
      recommendations.push({
        id: 'quality_parallel_synthesis',
        source: 'synthesis',
        category: 'optimization',
        title: 'Combine High-Quality Thinking with Parallel Execution',
        description: 'Leverage both structured thinking for quality and parallel execution for efficiency',
        rationale: 'Both engines show strong performance, synthesis can capture benefits of both',
        impact: 0.9,
        confidence: 0.85,
        effort: 0.7,
        dependencies: [],
    ...
    if (results.strategic?.objectives?.length > 0) {
      recommendations.push({
        ...
        description: 'Align execution with strategic objectives for optimal outcomes',
        rationale: 'Strategic analysis identified specific objectives that should guide execution',
        impact: 0.8,
        confidence: 0.8,
        effort: 0.3,
        dependencies: [],
   ...
   */
  private synthesizeInsights(results: any, context: IntelligenceContext): SynthesizedInsight[] {
   ...
   */
  private identifyPatternInsights(results: any, context: IntelligenceContext): SynthesizedInsight[] {
    const insights: SynthesizedInsight[] = [];
    ...
    if (qualities.length > 1) {
      ...
      if (variance < 0.01) { // Low variance indicates consistency
        insights.push({
          ...
          type: 'pattern',
          description: 'All intelligence engines predict consistent quality outcomes',
          sources: ['sparc', 'coordination', 'strategic'],
          confidence: 0.9,
          impact: 'high',
          actionable: false,
   ...
   */
  private assessSynthesizedQuality(results: any, context: IntelligenceContext): QualityAssessment {
    const dimensions = {
    ...
    const improvements = this.identifyQualityImprovements(results, context, dimensions);
   ...
   */
  private async generateExecutionPlan(synthesized: SynthesizedOutput, context: IntelligenceContext): Promise<ExecutionPlan> {
  ...
  private async finalizeSynthesis(
    ...
    const metadata: SynthesisMetadata = {
      ...
      convergenceScore: this.calculateConvergenceScore({ sparc: {}, coordination: {}, strategic: {} }),
      adaptations: [],
  ...
  // Helper methods
  private assessTaskComplexity(task: TaskDefinition): 'low' | 'medium' | 'high' | 'critical' {
    let score = 0;
  ...
  private assessParallelizability(task: TaskDefinition): number {
    let score = 0.5;
    const description = task.description.toLowerCase();
    if (description.includes('parallel') || description.includes('batch')) score += 0.3;
    if (description.includes('sequential') || description.includes('dependent')) score -= 0.3;
    return Math.max(0, Math.min(1, score));
  }

  private determineSynthesisMode(task: TaskDefinition, swarmObjective: SwarmObjective): 'sequential' | 'parallel' | 'adaptive' {
    if (this.assessParallelizability(task) > 0.7) return 'parallel';
    if (task.constraints?.dependencies.length > 2) return 'sequential';
    return 'adaptive';
  }

  private determinePriority(task: TaskDefinition, config: Partial<SwarmConfig>): 'quality' | 'speed' | 'balanced' {
    if (config.qualityThreshold && config.qualityThreshold > 0.9) return 'quality';
    if (task.priority === 'critical' && task.constraints?.deadline) return 'speed';
    return 'balanced';
  }

  private combineOutputs(results: any): any {
    return {
      summary: 'Unified intelligence synthesis combining structured thinking, parallel coordination, and strategic oversight',
      components: {
        sparc: results.sparc?.output,
        coordination: results.coordination?.output,
        strategic: results.strategic?.guidance
  ...
  private assessFeasibility(results: any, context: IntelligenceContext): number {
  ...
  private identifyQualityRisks(results: any, context: IntelligenceContext, dimensions: any): QualityRisk[] {
    const risks: QualityRisk[] = [];
    
    if (dimensions.feasibility < 0.7) {
      risks.push({
        type: 'feasibility',
        description: 'Low feasibility score indicates implementation challenges',
        probability: 0.6,
        impact: 0.8,
  ...
  private identifyQualityImprovements(results: any, context: IntelligenceContext, dimensions: any): QualityImprovement[] {
  ...
  private planExecutionPhases(synthesized: SynthesizedOutput, context: IntelligenceContext): ExecutionPhase[] {
    return [
      {
        id: 'analysis',
        name: 'Analysis Phase',
  ...
  private calculateResourceRequirements(phases: ExecutionPhase[], context: IntelligenceContext): ResourceRequirement[] {
    return [
      {
        type: 'agent',
  ...
  private calculateTimeline(phases: ExecutionPhase[], dependencies: PlanDependency[]): PlanTimeline {
    const total = phases.reduce((sum, phase) => sum + phase.estimatedDuration, 0);
    const phaseTimings: { [phaseId: string]: { start: number; duration: number } } = {};
    ...
    return {
      ...
      phases: phaseTimings,
      milestones: [
        { name: 'Analysis Complete', time: phaseTimings['analysis']?.start + phaseTimings['analysis']?.duration || 0, criteria: ['Analysis approved'] },
        { name: 'Implementation Complete', time: phaseTimings['implementation']?.start + phaseTimings['implementation']?.duration || 0, criteria: ['Code complete'] },
        { name: 'Project Complete', time: total, criteria: ['All phases complete'] }
      ],
      buffer: total * 0.2 // 20% buffer
    };
  }

  private planContingencies(phases: ExecutionPhase[], context: IntelligenceContext): Contingency[] {
    return [
      {
  ...
  private calculateCognitiveLoad(synthesized: SynthesizedOutput): number {
    return synthesized.recommendations.length * 0.1 + synthesized.insights.length * 0.05;
  }

  private extractAssumptions(synthesized: SynthesizedOutput, context: IntelligenceContext): string[] {
    return [
