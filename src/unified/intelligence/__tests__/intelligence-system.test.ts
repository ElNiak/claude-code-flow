...
function createMockAgent(overrides: Partial<AgentState> = {}): AgentState {
  const baseAgent: AgentState = {
    id: { id: 'agent-1', swarmId: 'swarm-1', type: 'analyst', instance: 1 } as AgentId,
    name: 'Test Agent',
    type: 'analyst',
    status: 'idle',
    capabilities: {
...
function createMockSwarmObjective(overrides: Partial<SwarmObjective> = {}): SwarmObjective {
...
describe('Intelligence System', () => {
  ...
  beforeEach(() => {
    ...
    mockAgents = [createMockAgent(), createMockAgent({ id: { ...createMockAgent().id, id: 'agent-2' } })];
  ...
  describe('UnifiedIntelligenceSystem', () => {
    ...
    test('should handle errors gracefully', async () => {
      ...
      await expect(
        intelligenceSystem.applyIntelligence(invalidTask, [], mockObjective, mockConfig)
      ).rejects.toThrow();
    });

    test('should provide access to individual engines', async () => {
      const sparcResult = await intelligenceSystem.applyStructuredThinking(mockTask, mockAgents[0]);
      expect(sparcResult).toBeDefined();
      expect(sparcResult.quality).toBeGreaterThan(0);

      const swarmResult = await intelligenceSystem.applyParallelCoordination(mockTask, mockAgents, mockConfig);
      expect(swarmResult).toBeDefined();
      expect(swarmResult.validated).toBe(true);

      const hiveResult = await intelligenceSystem.applyStrategicOversight(mockObjective, mockAgents, mockConfig);
  ...
  describe('StructuredThinkingEngine', () => {
    ...
    test('should apply structured thinking', async () => {
      const result = await structuredThinking.applyStructuredThinking(mockTask, mockAgents[0]);
    ...
    test('should handle different complexity levels', async () => {
      ...
      const complexTask = createMockTask({ 
        description: 'Complex advanced system integration with multiple dependencies',
        priority: 'critical'
      });

      const simpleResult = await structuredThinking.applyStructuredThinking(simpleTask, mockAgents[0]);
      const complexResult = await structuredThinking.applyStructuredThinking(complexTask, mockAgents[0]);
  ...
  describe('ParallelCoordinationEngine', () => {
    ...
    test('should apply parallel coordination', async () => {
      const result = await parallelCoordination.applyParallelCoordination(mockTask, mockAgents, mockConfig);
    ...
    test('should handle different parallelizability levels', async () => {
      const sequentialTask = createMockTask({ description: 'Sequential dependent task' });
      const parallelTask = createMockTask({ description: 'Parallel batch processing task' });

      const sequentialResult = await parallelCoordination.applyParallelCoordination(sequentialTask, mockAgents, mockConfig);
      const parallelResult = await parallelCoordination.applyParallelCoordination(parallelTask, mockAgents, mockConfig);
  ...
  describe('StrategicOversightEngine', () => {
    ...
    test('should apply strategic oversight', async () => {
      const result = await strategicOversight.applyStrategicOversight(mockObjective, mockAgents, mockConfig);
    ...
    test('should handle consensus decisions', async () => {
      const decision = await strategicOversight.proposeConsensusDecision('tactical', {
  ...
  describe('IntelligenceSynthesisEngine', () => {
    ...
    test('should synthesize intelligence from all engines', async () => {
      ...
      expect(result.synthesized).toBeDefined();
      expect(result.metadata).toBeDefined();
      expect(result.metadata.intelligenceUsed).toEqual(['sparc', 'coordination', 'strategic']);
    ...
    test('should handle different synthesis strategies', async () => {
      ...
      const highParallelTask = createMockTask({ description: 'Parallel batch processing multiple data sources' });
      ...
      expect(qualityResult.synthesized.primaryResult.type).toContain('thinking');
      expect(parallelResult.synthesized.primaryResult.type).toContain('coordination');
      expect(strategicResult.synthesized.primaryResult.type).toContain('strategic');
    });
  });
  ...
  describe('Intelligence Selection', () => {
    test('should select SPARC for high complexity tasks', () => {
      ...
      const selection = INTELLIGENCE_SELECTION.selectOptimalApproach(complexTask, mockAgents, qualityConfig);
    ...
    test('should select SWARM for parallelizable tasks', () => {
      ...
      const selection = INTELLIGENCE_SELECTION.selectOptimalApproach(parallelTask, manyAgents, mockConfig);
    ...
    test('should select HIVE for strategic tasks', () => {
      ...
      const selection = INTELLIGENCE_SELECTION.selectOptimalApproach(strategicTask, mockAgents, mockConfig);
    ...
    test('should default to SYNTHESIS for balanced tasks', () => {
      ...
      const selection = INTELLIGENCE_SELECTION.selectOptimalApproach(balancedTask, mockAgents, mockConfig);
  ...
  describe('Intelligence Capabilities', () => {
    test('should provide comprehensive capability information', () => {
      expect(INTELLIGENCE_CAPABILITIES[INTELLIGENCE_ENGINES.SPARC]).toBeDefined();
      expect(INTELLIGENCE_CAPABILITIES[INTELLIGENCE_ENGINES.SWARM]).toBeDefined();
      expect(INTELLIGENCE_CAPABILITIES[INTELLIGENCE_ENGINES.HIVE]).toBeDefined();
      expect(INTELLIGENCE_CAPABILITIES[INTELLIGENCE_ENGINES.SYNTHESIS]).toBeDefined();
  ...
  describe('Error Handling', () => {
    test('should handle missing task gracefully', async () => {
      ...
      await expect(
        intelligenceSystem.applyIntelligence(invalidTask, mockAgents, mockObjective, mockConfig)
      ).rejects.toThrow();
    });

    test('should handle empty agent list', async () => {
      const result = await intelligenceSystem.applyIntelligence(mockTask, [], mockObjective, mockConfig);
      expect(result).toBeDefined();
    ...
    test('should handle invalid configuration', async () => {
      ...
      const result = await intelligenceSystem.applyIntelligence(mockTask, mockAgents, mockObjective, invalidConfig);
      expect(result).toBeDefined();
  ...
  describe('Integration', () => {
    test('should integrate all intelligence types coherently', async () => {
      ...
      expect(result.synthesized.qualityAssessment.overall).toBeLessThanOrEqual(1);
