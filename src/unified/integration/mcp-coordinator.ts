...
import { LoadBalancer } from '../../mcp/load-balancer.js';
...
export interface MCPCoordinatorConfig {
  ...
  loadBalancing: {
    enabled: boolean;
    strategy: 'round-robin' | 'least-connections' | 'capability-based' | 'performance-based';
...
export class MCPCoordinator {
  ...
  async executeTool(
    ...
    try {
      ...
      const executionTime = Date.now() - startTime;
      this.logger.error('Tool execution failed', { toolName, error, executionTime });
   ...
   */
  async executeBatch(
    requests: Array<{ toolName: string; input: unknown; context: ToolSelectionContext }>,
  ): Promise<ToolExecutionResult[]> {
  ...
  async discoverTools(
    capabilities: string[],
    category?: string,
  ): Promise<Array<{ name: string; category: string; capabilities: string[] }>> {
    const discovered: Array<{ name: string; category: string; capabilities: string[] }> = [];
    ...
    discovered.sort((a, b) => {
      const aScore = this.calculateCapabilityMatchScore(a.capabilities, capabilities);
      const bScore = this.calculateCapabilityMatchScore(b.capabilities, capabilities);
  ...
  private async selectOptimalTool(
    ...
    if (context.preferredCategory && this.toolRegistries.has(context.preferredCategory)) {
  ...
  private selectByContext(
    ...
    const preferredCategories = taskTypeMapping[context.taskType] || 
  ...
  private selectByCapabilities(
    ...
    let bestMatch: { tool: MCPTool; category: string; score: number } | null = null;
    ...
    return bestMatch ? { tool: bestMatch.tool, category: bestMatch.category } : null;
  }
  ...
  private findBestPerformingTool(
    ...
    let bestPerforming: { tool: MCPTool; category: string; avgTime: number } | null = null;
    ...
    return bestPerforming ? { tool: bestPerforming.tool, category: bestPerforming.category } : null;
  }
  ...
  private initializeCategoryPreferences(): void {
    ...
    this.categoryPreferences.set('coordination', ['claude-flow', 'swarm', 'ruv-swarm']);
    this.categoryPreferences.set('memory', ['claude-flow', 'ruv-swarm', 'swarm']);
    this.categoryPreferences.set('monitoring', ['swarm', 'ruv-swarm', 'claude-flow']);
    this.categoryPreferences.set('analysis', ['ruv-swarm', 'claude-flow', 'swarm']);
    this.categoryPreferences.set('execution', ['ruv-swarm', 'swarm', 'claude-flow']);
  }
  ...
  private async initializeServers(): Promise<void> {
    for (const [category, registry] of this.toolRegistries.entries()) {
      const server = new MCPServer(registry, this.logger);
  ...
  private calculatePerformanceScore(
    ...
    const avgTime = metrics.reduce((a, b) => a + b, 0) / metrics.length;
    const categoryInfo = this.TOOL_CATEGORIES[category as keyof typeof this.TOOL_CATEGORIES];
