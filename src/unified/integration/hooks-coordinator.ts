...
import type { IEventBus, ILogger } from "../../utils/types.js";
...
export class HooksCoordinator {
	...
	private async executeHookWithIntelligence(
		...
		try {
			...
			if (this.config.automation.enabled && result.success) {
				const postAutomationResult = await this.executeAutomation(
					hookType,
					"post",
					options,
					context,
	...
	private async enrichContext(
		...
		// Add memory context
		if (
			this.config.contextEnrichment.includeMemoryContext &&
			context.sessionId
		) {
			try {
				const memoryContext = await this.getMemoryContext(context.sessionId);
				enriched.memoryContext = memoryContext;
				dataPointsAdded += memoryContext
					? Object.keys(memoryContext).length
					: 0;
				relevanceScore += 0.3;
			} catch (error) {
	...
	private async executeAutomation(
		...
		const automation =
			this.HOOK_AUTOMATION[hookType as keyof typeof this.HOOK_AUTOMATION];
		if (!automation) {
			return result;
		}
	...
	private async updateFailurePattern(
		...
		const existing = this.neuralPatterns.get(`${hookType}:failures`) || {
			patterns: [],
		};
		existing.patterns.push(pattern);
		...
		this.automationRules.set("format-code", [
			{
				phase: "post",
				priority: "all",
				enabled: this.config.automation.autoFormatCode,
			},
		]);
	}
