...
import type { Config, IEventBus, ILogger } from "../../utils/types.js";
...
export class UnifiedIntegrationManager {
	...
	async initialize(): Promise<void> {
		...
		try {
			...
			this.logger.error(
				"Failed to initialize Unified Integration Manager",
				error
			);
			throw error;
		}
	...
	async shutdown(): Promise<void> {
		...
		try {
			...
			this.logger.error(
				"Error during Unified Integration Manager shutdown",
				error
			);
			throw error;
		}
	...
	async getHealthStatus(): Promise<{
		...
		const [memoryHealth, mcpHealth, hooksHealth, githubHealth] =
			await Promise.all([
