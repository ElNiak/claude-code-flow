...
export function createUnifiedTools(logger: ILogger): MCPTool[] {
  return [
    // Unified work coordination tool
    {
      ...
      description: 'Coordinate unified work execution with intrinsic agents and ruv-swarm integration',
      inputSchema: {
        type: 'object',
        properties: {
          task: {
          ...
          agents: {
            type: 'number',
            description: 'Number of agents to coordinate (auto-determined if not specified)',
            minimum: 1,
            maximum: 20
          },
          topology: {
        ...
        try {
          ...
          if (ruvSwarm) {
            try {
              ...
              logger.info('ruv-swarm not available, using intrinsic coordination only');
            }
          }
          ...
          throw new Error(`Work coordination failed: ${getErrorMessage(error)}`);
        }
      }
    },
    ...
    // Intrinsic agent spawning tool
    {
      ...
      inputSchema: {
        ...
        properties: {
          type: {
            type: 'string',
            enum: ['coordinator', 'analyst', 'coder', 'tester', 'researcher', 'architect', 'security'],
    ...
    // Memory coordination tool
    {
      ...
      description: 'Coordinate agents through persistent memory with automatic hooks',
      inputSchema: {
        type: 'object',
        properties: {
          sessionId: {
      ...
      handler: async (input: any, context?: UnifiedToolContext) => {
        try {
          ...
          throw new Error(`Memory coordination failed: ${getErrorMessage(error)}`);
        }
      }
    },
...
async function executeUnifiedCoordination(plan: any, context?: UnifiedToolContext): Promise<any> {
  const results = {
    success: true,
...
async function performSystemHealthCheck(config: any): Promise<any> {
  const health = {
    overall: 'healthy',
    components: {
      intrinsic: { status: 'healthy', details: 'Intrinsic coordination operational' },
  ...
  if (config.checkRuvSwarm) {
    try {
      const { execSync } = await import('child_process');
      execSync('npx ruv-swarm --version', { stdio: 'pipe' });
      health.components.ruvSwarm = { status: 'healthy', details: 'ruv-swarm available' };
    } catch {
      health.components.ruvSwarm = { status: 'unavailable', details: 'ruv-swarm not installed' };
    }
  }
