...
import { LifecycleState, MCPLifecycleManager } from "./lifecycle-manager.js";
import { MCPPerformanceMonitor } from "./performance-monitor.js";
import { MCPProtocolManager } from "./protocol-manager.js";
import { type IMCPServer, MCPServer } from "./server.js";
...
export class MCPOrchestrationIntegration extends EventEmitter {
		...
		this.lifecycleManager.on("stateChange", (event) => {
			...
			if (this.components.eventBus) {
				this.components.eventBus.emit(SystemEvents.SYSTEM_HEALTHCHECK, {
					status:
						event.state === LifecycleState.RUNNING ? "healthy" : "unhealthy",
	...
	private setupPerformanceMonitoring(): void {
		...
		this.performanceMonitor.on("metricsCollected", (metrics) => {
			...
			if (
				this.components.monitor &&
				typeof this.components.monitor.recordMetrics === "function"
			) {
	...
	private registerOrchestrationTools(): void {
		...
		if (
			this.orchestrationConfig.enabledIntegrations.orchestrator &&
			this.components.orchestrator
		) {
		...
		if (
			this.orchestrationConfig.enabledIntegrations.swarm &&
			this.components.swarmCoordinator
		) {
		...
		if (
			this.orchestrationConfig.enabledIntegrations.agents &&
			this.components.agentManager
		) {
		...
		if (
			this.orchestrationConfig.enabledIntegrations.resources &&
			this.components.resourceManager
		) {
		...
		if (
			this.orchestrationConfig.enabledIntegrations.memory &&
		...
		if (
			this.orchestrationConfig.enabledIntegrations.monitoring &&
			this.components.monitor
		) {
		...
		if (
			this.orchestrationConfig.enabledIntegrations.terminals &&
			this.components.terminalManager
		) {
		...
		const tools: MCPTool[] = [
			{
				...
				handler: async () => {
					if (
						typeof this.components.swarmCoordinator?.getStatus === "function"
					) {
			...
			},
			{
				...
				handler: async () => {
					if (
						typeof this.components.swarmCoordinator?.listAgents === "function"
					) {
		...
		const tools: MCPTool[] = [
			{
				...
				handler: async () => {
					if (
						typeof this.components.resourceManager?.listResources === "function"
					) {
			...
			},
			{
				...
				handler: async () => {
					if (
						typeof this.components.resourceManager?.getStatus === "function"
					) {
		...
		const tools: MCPTool[] = [
			{
				...
				handler: async () => {
					if (
						typeof this.components.terminalManager?.listSessions === "function"
					) {
	...
	private async checkComponentHealth(component: string): Promise<boolean> {
		...
		if (typeof componentInstance.healthCheck === "function") {
			...
			return (
				result === true ||
				(typeof result === "object" && result.healthy === true)
			);
		}
