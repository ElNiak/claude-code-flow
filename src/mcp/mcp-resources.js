...
export class MCPResourcesManager {
	...
	async getMemoryNamespaces() {
		const entries = Array.from(this.server.memoryStore.entries());
		const namespaces = {};
	...
	countActiveCoordinations() {
		return Array.from(this.server.swarms.values()).reduce(
	...
	getRecentMemoryActivity(entries) {
		return entries.slice(-10).map(([key, value]) => ({
			key: key.split(":")[1],
