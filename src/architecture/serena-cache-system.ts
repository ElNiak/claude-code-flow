...
export class SerenaMultiTierCache
	extends EventEmitter
	implements SerenaOptimizationEventEmitter
{
	...
	private processSmartInvalidation(task: InvalidationTask): void {
		...
		const directlyAffected = this.dependencyGraph.getDirectlyAffected(
			task.symbol
		);
	...
	private async getHotSymbols(projectPath: string): Promise<string[]> {
		...
		for (const [key, entry] of this.l1Cache.entries()) {
			if (key.startsWith("symbol:") && key.includes(projectPath)) {
				...
				symbolAccess.set(
					symbol,
					(symbolAccess.get(symbol) || 0) + entry.accessCount
				);
			}
		}
	...
	private updateMetrics(): void {
		...
		this.cacheMetrics.l1Metrics.maxSize =
			this.config.l1Cache.maxSize * 1024 * 1024;

		this.cacheMetrics.l2Metrics.size = this.getCurrentL2Size();
		this.cacheMetrics.l2Metrics.maxSize =
			this.config.l2Cache.maxSize * 1024 * 1024;

		this.cacheMetrics.l3Metrics.size = this.getCurrentL3Size();
		this.cacheMetrics.l3Metrics.maxSize =
			this.config.l3Cache.maxSize * 1024 * 1024;
