...
async function testConsensusIntegration() {
  try {
    ...
    const workers = await hiveMindCore.spawnWorkers(['researcher', 'coder', 'architect', 'analyst', 'tester']);
    ...
    console.log('📊 Confidence:', Math.round(consensus1.confidence * 100) + '%');
    ...
    if (highPriorityTask.consensusDecision) {
      ...
      console.log('📊 Consensus confidence:', Math.round(highPriorityTask.consensusConfidence * 100) + '%');
    }
    ...
    const analysis = await queen.analyzeObjective('Build a comprehensive API with authentication and database');
    ...
    const consensusPhases = executionPlan.phases.filter(phase => phase.requiresConsensus);
    ...
    if (consensusPhases.length > 0) {
      ...
      console.log('📊 Phase consensus confidence:', Math.round(phaseConsensus.confidence * 100) + '%');
    }
...
async function testConsensusScenarios() {
  ...
  const scenarios = [
    ...
    },
    {
      ...
      options: ['comprehensive testing', 'efficient validation', 'quick checks']
    }
  ];
  ...
  for (let i = 0; i < scenarios.length; i++) {
    ...
    await hiveMindCore.initialize();
    await hiveMindCore.spawnWorkers(['researcher', 'coder', 'analyst', 'tester']);
    
    const consensus = await hiveMindCore.buildConsensus(scenario.topic, scenario.options);
