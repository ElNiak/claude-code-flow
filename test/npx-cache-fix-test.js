...
async function testBasicIsolation() {
	...
	try {
		...
		if (
			stdout.includes("Environment configured successfully") &&
...
async function testConcurrentNpx() {
	...
	try {
		...
		if (successful === concurrentCount) {
			recordSuccess(
				testName,
				`All ${concurrentCount} concurrent runs succeeded`,
			);
		} else {
			recordFailure(
				testName,
				`Only ${successful}/${concurrentCount} runs succeeded`,
			);
		}
	} catch (error) {
		recordFailure(testName, error.message);
...
async function testCacheCleanup() {
	...
	try {
		...
		try {
			...
			cacheCountBefore = files.filter((f) =>
				f.startsWith("claude-flow-"),
			).length;
		} catch (e) {
		...
		try {
			const files = await fs.readdir(npmCacheDir);
			cacheCountAfter = files.filter((f) =>
				f.startsWith("claude-flow-"),
			).length;
		} catch (e) {
		...
		if (cacheCountAfter <= cacheCountBefore) {
			...
			recordFailure(
				testName,
				`Cache directories not cleaned: before=${cacheCountBefore}, after=${cacheCountAfter}`,
			);
		}
	} catch (error) {
		recordFailure(testName, error.message);
