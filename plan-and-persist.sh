...
# OBJECTIVE="Add OAuth login (PKCE) to Service X" ACTION=persist ROADMAP_SRC=.claude/roadmaps/plan-output.txt bash plan-and-persist.sh

# # 3) Dry-run what would be written
# OBJECTIVE="Add OAuth login (PKCE) to Service X" ACTION=check ROADMAP_SRC=.claude/roadmaps/plan-output.txt bash plan-and-persist.sh
...
SPEC SAVE_STYLE=multi ACTION=plan+persist bash plan-and-persist.sh" >&2
  echo "[INFO] Example: OBJECTIVE='Add OAuth login (PKCE) to Service X' ACTION=persist ROADMAP_SRC=.claude/roadmaps/plan-output.txt bash plan-and-persist.sh" >&2
  echo "[INFO] Example: OBJECTIVE='Add OAuth login (PKCE) to Service X' ACTION=check ROADMAP_SRC=.claude/roadmaps/plan-output.txt bash plan-and-persist.sh" >&2
  ...
  echo "[INFO] Set ROADMAP_PATH to specify where to save the roadmap (default: .claude/roadmaps/<PHASE>_<DATE_STR>_<OBJ_SLUG>.md)." >&2
  echo "[INFO] Set INDEX_SAVE_PATH to specify where to save the index (default: .claude/roadmaps/STACKDOCS_<REPO_SLUG>_<DATE_STR>.md)." >&2
...
: "${ROADMAP_PATH:=.claude/roadmaps/${PHASE}_${DATE_STR}_${OBJ_SLUG}.md}"
: "${INDEX_SAVE_PATH:=.claude/roadmaps/STACKDOCS_${REPO_SLUG}_${DATE_STR}.md}"
...
SYSTEM / GUARDED MODE:
- ULTRATHINKING
- You are a planning agent, not a coding agent.
...
SUB-AGENTS (define roles & least-privilege scopes):
  ...
  - Subagents also READ: CLAUDE.md
  - ultrathink mode also applies to sub-agents.
...
- If any .claude/roadmaps/REVIEW_<DATE_STR>_<OBJ_SLUG>.md exists, read it and summarize the last review and its findings. You must not repeat the same mistakes.
  - In the end of the current phase, if the review plan improvements points are implemented, you can move the .claude/roadmaps/REVIEW_<DATE_STR>_<OBJ_SLUG>.md file to .claude/roadmaps/REVIEW_<DATE_STR>_<OBJ_SLUG>_DONE.md.
...
- NEVER use --no-verify or --skip-hooks in commits nor --force in pushes.
...
- The plan should be a detailed specification of the objective adapted to the current codebase and its context. It must contain:
  - Acceptance criteria: what the objective must achieve
  - Non-functional requirements (NFRs): performance, security, usability, etc.
  - Constraints: technical, business, legal, etc.
  - Edge cases: special scenarios that must be handled.
With the reasonings and assumptions made and the tasks broken down into a task DAG.
If the objective is too vague, ask for clarifications.
If the objective require external knowledge for some package or library, research for it and include it in the plan.
Use sub-agents to gather information, but do not write code or files.
Dont overthink, always think MVP.
You must update your plan as you work on the specification.

EOF
  elif [[ "${PHASE}" = "DESIGN" ]]; then
    read -r -d '' phaserules <<EOF

<!> Read on .claude/roadmaps/SPEC_<DATE_STR>_<OBJ_SLUG>.md.
Produce pseudocode/architecture (modules, interfaces, data/dataflow), reference existing code to MAXIMIZE REUSE and simplificity.
If the roadmap is not present, stop and ask for it to be created.
- The design should be a detailed architecture of the objective adapted to the current codebase and its context. It must contain:
  - Pseudocode: high-level code structure, algorithms, and data structures.
  - Architecture: modules, interfaces, data/dataflow, and how they interact.
  - Reference existing code: reuse existing code as much as possible, simplify the design.
- If the design is too complex, ask for simplifications.
- If the design requires external knowledge for some package or library, research for it and include it in the design.
- Use sub-agents to gather information, but do not write code or files.
- Don't overthink, always think MVP.
- Always prioritize simplicity and clarity in design and reuse existing code. (AVOID LEGACY CODE MIXING)
- You must update your plan as you work on the design.

EOF
  elif [[ "${PHASE}" = "IMPL" ]]; then
    read -r -d '' phaserules <<EOF

<!> Read on .claude/roadmaps/SPEC_<DATE_STR>_<OBJ_SLUG>.md and .claude/roadmaps/DESIGN_<DATE_STR>_<OBJ_SLUG>.md.
Research for the most adapted development approach. (TDD-first, BDD-first, DDD-first, etc.)
Produce detailed implementation steps, migration steps if needed, and risk/rollback plan. (only PLAN, no code changes).
- Always prioritize simplicity and clarity in design and reuse existing code. (AVOID LEGACY CODE MIXING)
- The implementation plan should be a detailed plan of the implementation steps adapted to the current codebase and its context. It must contain:
  - Implementation steps: how to implement the objective, step by step.
  - Migration steps: how to migrate existing code if needed.
  - Risk/rollback plan: how to handle risks and rollbacks if something goes wrong.
  - Verification steps: how to verify the implementation is correct and meets the acceptance criteria.
- If the implementation plan is too complex, ask for simplifications.
- If the implementation plan requires external knowledge for some package or library, research for it and include it in the plan.
- Use sub-agents to gather information, but do not write code or files.
- Don't overthink, always think MVP.
- Always prioritize simplicity and clarity in design and reuse existing code. (AVOID LEGACY CODE MIXING)
You must update your plan as you work on the implementation.

EOF
  elif [[ "${PHASE}" = "DO_IMPL" ]]; then
    read -r -d '' phaserules <<EOF

<!>  Read on .claude/roadmaps/SPEC_<DATE_STR>_<OBJ_SLUG>.md and .claude/roadmaps/DESIGN_<DATE_STR>_<OBJ_SLUG>.md and .claude/roadmaps/IMPL_<DATE_STR>_<OBJ_SLUG>.md
IMPLEMENT the .claude/roadmaps/ in code, write tests, and ensure code quality (linting, type-checking, security checks).
Add documentation updates, release notes, and migration steps if needed.
- The implementation should be a detailed implementation of the objective adapted to the current codebase and its context. It must contain:
  - Code changes: the actual code changes to implement the objective.
  - Tests: unit tests, integration tests, and end-to-end tests to verify the implementation.
  - Documentation updates: how to update the documentation to reflect the changes.
  - Release notes: how to update the release notes to reflect the changes.
- The code must be well-structured, readable, and maintainable and
  follow the existing code style conventions.
- If the implementation is too complex, ask for simplifications.
- If the implementation requires external knowledge for some package or library, research for it and include it in the implementation.
- Use sub-agents to gather information, but do not write code or files
- Don't overthink, always think MVP.
- Always prioritize simplicity and clarity in design and reuse existing code. (AVOID LEGACY CODE MIXING)
- You must update your plan as you work on the implementation.
...
<!> Read on .claude/roadmaps/SPEC_<DATE_STR>_<OBJ_SLUG>.md and .claude/roadmaps/DESIGN_<DATE_STR>_<OBJ_SLUG>.md and .claude/roadmaps/IMPL_<DATE_STR>_<OBJ_SLUG>.md and previous modified files in the commit, plan verification: coverage, static/security analysis, performance checks, doc updates, release notes.
Analyse the previous implementation and produce a verification plan.
- Start by researching the codebase to understand the implementation and its context.
- In parallel, research for the best practices, bad practices, and tools to verify the implementation.
- The verification plan should be a detailed plan of the verification steps adapted to the current codebase and its context. It must contain:
  - Coverage: how to measure the code coverage of the implementation.
  - Static/security analysis: how to perform static and security analysis on the implementation.
  - Performance checks: how to measure the performance of the implementation.
  - Documentation updates: how to update the documentation to reflect the changes.
  - Release notes: how to update the release notes to reflect the changes.
  - Code quality checks: how to ensure the code quality (linting, type-checking, security checks).
  - Placeholders, oversimplifications, and TODOs, creation of duplicate files, bad practices, and legacy code mixing are not allowed.
    -> if any, they must be clearly marked and documented in the verification plan.
You must not repeat the same mistakes.
- If the verification plan is too complex, ask for simplifications.
- If the verification plan requires external knowledge for some package or library, research for it and include it in the plan.
- Use sub-agents to gather information, but do not write code or files.
- Don't overthink, always think MVP.
- Always prioritize simplicity and clarity in design and reuse existing code. (AVOID LEGACY CODE MIXING)
- You must update your plan as you work on the verification.

EOF
  else
  read -r -d '' phaserules <<'EOF'

- AUTO  : infer appropriate planning depth from .claude/roadmaps/
  - SPEC  : produce a detailed task breakdown with acceptance criteria.
  - DESIGN: produce a design overview with modules/interfaces.
  - IMPL  : produce a TDD-first plan with test cases and migration steps.
  - DO_IMPL: implement the inputed .claude/roadmaps/ in code, write tests, and ensure code quality (linting, type-checking, security checks).
  - REVIEW: produce a verification plan (coverage, static/security analysis, performance checks, doc updates).
- If the objective is too vague, ask for clarifications.
- If the objective requires external knowledge for some package or library, research for it and include it in the plan.
- Use sub-agents to gather information, but do not write code or files.
- Don't overthink, always think MVP.
- Always prioritize simplicity and clarity in design and reuse existing code. (AVOID LEGACY CODE MIXING)
- You must update your plan as you work on the phase.
...
- <!> implement the inputed .claude/roadmaps/ plan in code, write tests, and ensure code quality (linting, type-checking, security checks).
EOF
  else
    read -r -d '' concurrency <<'EOF'
...
- Analyse the implementation details .claude/roadmaps/, then spawn adapted agents to implement the plan. Make sure to follow the TDD-first approach.
...
- Required base sections (Schema v1):
  ...
  9) Gate/Exit Criteria  10) Open Questions (≤5)
EOF
  else
    read -r -d '' BLOCK_SCHEMA <<EOF
ROADMAP SCHEMA: ${ROADMAP_SCHEMA}
